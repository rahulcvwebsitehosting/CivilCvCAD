...
void rectangle(double height, double width, const char* name)
{
    std::vector<std::string> rectstring {
        ...
        boost::str(boost::format("V2 = CivilCvCAD.Vector(%d, 0, 0)") % height),
        boost::str(boost::format("V3 = CivilCvCAD.Vector(%d, %d, 0)") % height % width),
        boost::str(boost::format("V4 = CivilCvCAD.Vector(0, %d, 0)") % width),
