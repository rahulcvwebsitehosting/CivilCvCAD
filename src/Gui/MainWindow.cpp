    ...
    : QMainWindow(parent, f /*WDestructiveClose*/)
{
    ...
    // Starts checked because CivilCvCAD shows bottom panels by default on first launch. On subsequent
...
void MainWindow::setActiveWindow(MDIView* view)
{
    ...
    // to take focus.
    // Fixes https://github.com/CivilCvCAD/CivilCvCAD/issues/23798
...
void MainWindow::closeEvent(QCloseEvent* e)
{
    ...
    if (e->isAccepted()) {
        ...
        // https://forum.civilcvcad.org/viewtopic.php?f=8&t=67748
...
void MainWindow::delayedStartup()
{
    ...
    if (Gui::isInternalGuiTestRun()) {
        try {
            ...
            // delayedStartup().
            Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("CivilCvCADGuiTest"));
    ...
    if (SafeMode::SafeModeEnabled()) {
        auto safeModePopup = QMessageBox(
            ...
            tr("CivilCvCAD is now running in safe mode."),
            QMessageBox::Ok
        );
...
void MainWindow::registerQuickLookExtensions()
{
    ...
    // Get the path to CivilCvCAD.app/Contents/PlugIns
    QString appPath = QApplication::applicationDirPath();
    QString plugInsPath = appPath + "/../PlugIns";

    QString thumbnailExt = plugInsPath + "/CivilCvCADThumbnailExtension.appex";
    QString previewExt = plugInsPath + "/CivilCvCADPreviewExtension.appex";
    ...
    const QString thumbnailId = QStringLiteral("org.civilcvcad.CivilCvCAD.quicklook.thumbnail");
    const QString previewId = QStringLiteral("org.civilcvcad.CivilCvCAD.quicklook.preview");
...
void MainWindow::loadUrls(App::Document* doc, const QList<QUrl>& urls)
{
    ...
    for (const auto& it : urls) {
        ...
        if (info.exists() && info.isFile()) {
            ...
            it.scheme().toLower() == QLatin1String("http")
            || it.scheme().toLower() == QLatin1String("https")
            || it.scheme().toLower() == QLatin1String("ftp")
        ) {
            Base::Console().warning("Remote drag-and-drop content is disabled in offline mode.\n");
        }
    }
...
void MainWindow::setWindowTitle(const QString& string)
{
    ...
    if (showVersion) {
        // set main window title with CivilCvCAD Version
