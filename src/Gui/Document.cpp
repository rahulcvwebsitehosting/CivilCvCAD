...
struct DocumentP
{
    ...
    {
        if (auto vp = civilcvcad_cast<ViewProviderDocumentObject*>(p)) {
            return vp;
        }
    ...
    ) const
    {
        auto svp = vp;
        if (sobj != obj) {
            svp = civilcvcad_cast<ViewProviderDocumentObject*>(
    ...
    void signalEditMode()
    {
        if (auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(_editViewProvider)) {
...
private:
    ...
    )
    {
        ...
        auto vp = civilcvcad_cast<ViewProviderDocumentObject*>(
...
{
    // Fix regression: https://forum.civilcvcad.org/viewtopic.php?f=19&t=43629&p=371972#p371972
...
{
    if (auto vp
        = civilcvcad_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj))) {
...
void Document::beforeDelete()
{
    Application::Instance->unsetEditDocumentIf([this](Gui::Document* editDoc) {
        auto vp = civilcvcad_cast<ViewProviderDocumentObject*>(editDoc->d->_editViewProvider);
        auto vpp = civilcvcad_cast<ViewProviderDocumentObject*>(editDoc->d->_editViewProviderParent);
...
void Document::slotSkipRecompute(const App::Document& doc, const std::vector<App::DocumentObject*>& objs)
{
    ...
    if (Gui::Application::Instance->isInEdit(this)) {
        auto vp = civilcvcad_cast<ViewProviderDocumentObject*>(getInEdit());
        if (vp) {
            obj = vp->getObject();
        }
    }
    ...
    if (editDoc) {
        auto vp = civilcvcad_cast<ViewProviderDocumentObject*>(editDoc->getInEdit());
        if (vp) {
            obj = vp->getObject();
        }
    }
...
bool Document::warnIfOlderVersion()
{
    ...
    // Get current CivilCvCAD version
    ...
    if (!hasVersion || (docMajor < currentMajor)
        ...
        msgBox.setWindowTitle(QObject::tr("File Created with Older CivilCvCAD Version"));
        ...
        msgBox.setText(
            QObject::tr(
                ...
                "by older versions of CivilCvCAD after saving.\n\n"
                "Use 'Save As…' to preserve the original file."
                "\n"
            )
                .arg(
                    !hasVersionString
                        ? QObject::tr("an unknown older version of CivilCvCAD")
                        : QObject::tr("CivilCvCAD version %1").arg(QString::fromUtf8(docVersion))
                )
                .arg(currentMajor)
                .arg(currentMinor)
        );
...
bool Document::save()
{
    if (d->_pcDocument->isSaved()) {
        // Warn if this document was created with an older CivilCvCAD version
...
void Document::RestoreDocFile(Base::Reader& reader)
{
    ...
    if (scheme == 1) {
        ...
        for (int i = 0; i < Cnt; i++) {
            ...
            auto pObj = civilcvcad_cast<ViewProviderDocumentObject*>(getViewProviderByName(name.c_str()));
        ...
        if (!Base::Tools::isNullOrEmpty(ppReturn)) {
            ...
            try {
                for (const auto& it : getMDIViews()) {
                    if (auto* viewCamera = civilcvcad_cast<MDIViewWithCamera*>(it)) {
...
{
    auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(getViewProvider(&obj));
    if (vpd) {
...
void Document::SaveDocFile(Base::Writer& writer) const
{
    writer.Stream() << "<?xml version='1.0' encoding='utf-8'?>" << std::endl
                    << "<!--" << std::endl
                    << " CivilCvCAD Document, see  for more information…"
                    << std::endl
                    << "-->" << std::endl;
    ...
    if (scheme == 1) {
        ...
        for (int i = 0; i < Cnt && it != obj.end(); ++i, ++it) {
            ...
            if (pObj) {
                ...
                auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(pObj);
                if (vpd) {
                    vpd->startRestoring();
                }
                pObj->Restore(*localreader);
...
void Document::slotFinishImportObjects(const std::vector<App::DocumentObject*>& objs)
{
    ...
    //     auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(vp);
    ...
    if (!vp) {
        ...
        if (!view) {
            ...
            if (!obj) {
                ...
                if (linked != obj) {
                    auto vpLinked = civilcvcad_cast<ViewProviderDocumentObject*>(
