...
#include "Macro.h"
#include "CivilCvCADGuiModulePy.h"
...
public:
    ...
    {
        auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(const_cast<ViewProvider*>(&vp));
...
Application::Application(bool GUIenabled)
{
    ...
    if (GUIenabled) {
#if CIVILCVCAD_OFFLINE_ONLY
        // Defense in depth for Qt code paths that are not part of the normal
        // UI: route application-wide Qt networking to an unused loopback port.
        ...
        // if this returns a valid pointer then the 'CivilCvCADGui' Python module was loaded,
        // otherwise the executable was launched
        PyObject* modules = PyImport_GetModuleDict();
        PyObject* module = PyDict_GetItemString(modules, "CivilCvCADGui");
        if (!module) {
            static struct PyModuleDef CivilCvCADGuiModuleDef = {PyModuleDef_HEAD_INIT,
                                                             "CivilCvCADGui",
                                                             Gui::CivilCvCADGuiModulePy::moduleDocumentation(),
                                                             -1,
                                                             Gui::CivilCvCADGuiModulePy::Methods,
                                                             nullptr,
                                                             nullptr,
                                                             nullptr,
                                                             nullptr};
            module = PyModule_Create(&CivilCvCADGuiModuleDef);

            PyDict_SetItemString(modules, "CivilCvCADGui", module);
        }
        else if (Gui::CivilCvCADGuiModulePy::addModuleMethods(module) != 0) {
            // CivilCvCADCmd can import a bootstrap-only CivilCvCADGui module before the GUI app exists;
    ...
    Base::PyGILStateLocker lock;
    PyObject* module = PyImport_AddModule("CivilCvCADGui");
        ...
        try {
            if (File.hasExtension("FCStd")) {
                ...
                for (auto& v : d->documents) {
                    ...
                    if (filepath == fi) {
                        handled = true;
                        Command::doCommand(Command::App, "CivilCvCADGui.reload('%s')", doc->getName());
                        break;
                    }
                }

                if (!handled) {
                    Command::doCommand(Command::App, "CivilCvCAD.openDocument('%s')", unicodepath.c_str());
                    Gui::Application::checkForRecomputes();
                }
            }
            else {
                std::string code = fmt::format(
                    "from civilcvcad import module_io\n"
        ...
        try {
            ...
            for (auto it : sel) {
                if (unique_objs.insert(it).second) {
                    str << "__objs__.append(CivilCvCAD.getDocument(\"" << DocName << "\").getObject(\""
...
void Application::checkForRecomputes()
{
    ...
    // this allows them to 'CivilCvCAD.ConfigSet("SuppressRecomputeRequiredDialog", "True")`
    ...
    if (doc != d->documents.end()) {
        ...
        if (d->activeDocument != doc->second) {
            ...
            if (d->activeDocument) {
                ...
                Py::Module("CivilCvCADGui").setAttr(std::string("ActiveDocument"), active);
                ...
                Base::PyGILStateLocker lock;
                Py::Module("CivilCvCADGui").setAttr(std::string("ActiveDocument"), Py::None());
            }
        }
...
#ifdef FC_DEBUG  // redirect Coin messages to CivilCvCAD
...
static void init_resources()
{
    ...
    Q_INIT_RESOURCE(CivilCvCAD_translation);
}
...
void Application::initApplication()
{
    ...
    try {
        initTypes();
        new Base::ScriptProducer("CivilCvCADGuiInit", CivilCvCADGuiInit);
        new Base::ScriptProducer("CivilCvCADGuiTest", CivilCvCADGuiTest);
        init_resources();
...
{
    Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("CivilCvCADGuiInit"));
}
...
void Application::runApplication()
{
    ...
    // https://forum.civilcvcad.org/viewtopic.php?f=3&t=15540
    ...
    // https://forum.civilcvcad.org/viewtopic.php?f=10&t=21665
...
void Application::setStyleSheet(const QString& qssFile, bool tiledBackground)
{
    ...
    // and https://forum.civilcvcad.org/viewtopic.php?f=34&t=50744
    ...
    // See https://forum.civilcvcad.org/viewtopic.php?f=17&t=50783
    if (!d->startingUp) {
        if (mdi->style()) {
...
void Application::setStyle(const QString& name)
{
    const auto createStyleFromName = [](const QString& name) -> QStyle* {
        if (name == QStringLiteral("CivilCvCAD")) {
            return new CivilCvCADStyle();
        }
    ...
    const auto requiresEventFilter = [](QStyle* style) {
        // for now only CivilCvCAD style requires additional event processing
        return qobject_cast<CivilCvCADStyle*>(style) != nullptr;
    };
