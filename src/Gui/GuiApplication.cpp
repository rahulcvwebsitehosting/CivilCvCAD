...
bool GUIApplication::notify(QObject* receiver, QEvent* event)
{
    ...
    // https://github.com/CivilCvCAD/CivilCvCAD/issues/16905
...
bool GUIApplication::event(QEvent* ev)
{
    if (ev->type() == QEvent::FileOpen) {
        // (macOS workaround when opening CivilCvCAD by opening a .FCStd file in 1.0)
        // With the current implementation of the splash screen boot procedure, Qt will
        // start an event loop before CivilCvCAD is fully initialized. This event loop will
        ...
        if (!Gui::getMainWindow()->property("eventLoop").toBool()) {
            // If we never reach this point when opening CivilCvCAD by double clicking an
        ...
        if (fi.suffix().toLower() == QLatin1String("fcstd")) {
            ...
            Application::Instance->open(fn, "CivilCvCAD");
            return true;
        }
    }
