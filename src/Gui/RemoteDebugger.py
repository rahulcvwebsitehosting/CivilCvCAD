# CivilCvCAD offline-only enforcement.
# Remote debugger would connect to an external debugpy server (a network call).
# Disabled by default; set CIVILCVCAD_ALLOW_REMOTE_DEBUG=1 to opt back in.
import os as _cc_rd_os
if not _cc_rd_os.environ.get("CIVILCVCAD_ALLOW_REMOTE_DEBUG"):
    import sys
    print("RemoteDebugger disabled (CIVILCVCAD_OFFLINE_ONLY). "
          "Run with CIVILCVCAD_ALLOW_REMOTE_DEBUG=1 to enable.", file=sys.stderr)
    ...
    def stop(*args, **kwargs):
        return None
    attach = staticmethod(_disabled)
    start = staticmethod(_disabled)
    stop = staticmethod(_disabled)
    raise SystemExit(0)
...
from civilcvcad.utils import get_python_exe


class RemoteDebugger:
    def __init__(self, parent=None):
        ui = App.getHomePath() + "Ext/civilcvcad/gui/RemoteDebugger.ui"
    ...
    def accept(self):
        try:
            ...
            if index == 0:  # winpdb
                ...
                # By default it attempts to use CivilCvCAD's PID mistaking it for python.
