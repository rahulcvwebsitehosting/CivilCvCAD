...
/** \defgroup workbench Workbench Framework
    ...
    CivilCvCAD provides the possibility to have one or more workbenches for a module.
 ...
 * Once you have implemented your workbench class you have to register it to make it known to the
 CivilCvCAD core system. You must make sure that the step
 ...
 * your workbench, if not, the default CivilCvCAD icon is taken, and finally the most important method
 ...
 * \note You must make sure to choose a unique name for your workbench (in this example "My
 workbench"). Since CivilCvCAD doesn't provide a mechanism for
 ...
 * CivilCvCAD and saves memory.
 *
 * At startup CivilCvCAD scans all module directories and invokes InitGui.py. So an item for a
 ...
 *
 * CivilCvCAD provides also the possibility to define pure Python workbenches. Such workbenches are
 temporarily only and are lost after exiting
 * the CivilCvCAD session. But if you want to keep your Python workbench you can write a macro and
 attach it with a user defined button or just
 * perform the macro during the next CivilCvCAD session.
 ...
 * dir(w)                                                     # lists all available function of the
 object
 * CivilCvCADGui.addWorkbench(w)                                 # Creates an item for our workbench
 now
