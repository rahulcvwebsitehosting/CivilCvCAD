...
class TreeWidgetItemDelegate: public QStyledItemDelegate
{
    ...
    //
    // More information: https://github.com/CivilCvCAD/CivilCvCAD/pull/13807
        ...
        if (!info.topObj.empty()) {
            ...
            if (doc) {
                ...
                if (topObj) {
                    ...
                    if (sobj == obj) {
                        propPlacement = civilcvcad_cast<App::PropertyPlacement*>(
            ...
            propPlacement = civilcvcad_cast<App::PropertyPlacement*>(obj->getPropertyByName("Placement"));
            if (propPlacement) {
...
)
{
    ...
    try {
        ...
        for (auto& info : infos) {
            ...
            auto vpc = civilcvcad_cast<ViewProviderDocumentObject*>(
            ...
            if (da == Qt::LinkAction) {
                ...
                auto vpp = civilcvcad_cast<ViewProviderDocumentObject*>(
...
)
{
    ...
    try {
        ...
        for (auto& info : infos) {
            ...
            vp = civilcvcad_cast<ViewProviderDocumentObject*>(
            ...
            if (da != Qt::LinkAction && !info.parentDoc.empty()) {
                ...
                if (parentDoc) {
                    auto parent = parentDoc->getObject(info.parent.c_str());
                    vpp = civilcvcad_cast<ViewProviderDocumentObject*>(
                        Application::Instance->getViewProvider(parent)
                    );
                }
                if (!vpp) {
...
)
{
    ...
    if (targetInfo.targetItem->type() == TreeWidget::ObjectType) {
        ...
        auto propGroup = civilcvcad_cast<App::PropertyLinkList*>(targetObj->getPropertyByName("Group"));
        if (!propGroup) {
            return;
        }
...
void TreeWidget::onUpdateStatus()
{
    ...
    for (auto& v : localNewObjects) {
        ...
        for (auto id : v.second) {
            ...
            auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(gdoc->getViewProvider(obj));
            if (vpd) {
...
void TreeWidget::selectLinkedObject(App::DocumentObject* linked)
{
    ...
    auto linkedVp = civilcvcad_cast<ViewProviderDocumentObject*>(
...
void DocumentItem::slotInEdit(const Gui::ViewProviderDocumentObject& v)
{
    ...
    if (!getTree()->editingItem) {
        ...
        if (!parentVp) {
            parentVp = civilcvcad_cast<ViewProviderDocumentObject*>(vp);
        }
        if (parentVp) {
...
{
    return civilcvcad_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj));
}
...
void DocumentItem::updateItemSelection(DocumentObjectItem* item)
{
    ...
    // selected ie. see https://github.com/civilcvcad/civilcvcad/issues/30161
...
void DocumentObjectItem::setData(int column, int role, const QVariant& value)
{
    ...
    if (role == Qt::EditRole && column <= 1) {
        ...
        // For more details: https://forum.civilcvcad.org/viewtopic.php?f=3&t=72351
        App::Document* doc = obj->getDocument();
