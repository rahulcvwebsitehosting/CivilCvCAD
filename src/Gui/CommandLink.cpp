...
static void linkConvert(bool unlink)
{
    ...
    try {
        ...
        for (auto& v : infos) {
            ...
            auto parentVp = civilcvcad_cast<ViewProviderDocumentObject*>(
            ...
            if (unlink) {
                ...
                auto pla = civilcvcad_cast<App::PropertyPlacement*>(obj->getPropertyByName("Placement"));
                if (pla) {
...
static App::DocumentObject* getSelectedLink(bool finalLink, std::string* subname = nullptr)
{
    ...
    auto vp = civilcvcad_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(sobj));
    if (!vp) {
        return nullptr;
    }
