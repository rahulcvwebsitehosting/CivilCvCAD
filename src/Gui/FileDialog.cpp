...
void FileDialogInternal::normalizeSavePath(QString& path, const FileDialog::Filter& selectedFilter)
{
    ...
    // As it stands, much of CivilCvCAD relies on an extension always being present, so
...
QIcon FileIconProvider::icon(const QFileInfo& info) const
{
    ...
    auto iconFromFile = [](const QString& filename) {
        ...
        return QIcon(QStringLiteral(":/icons/civilcvcad-doc.png"));
    };
    ...
    if (info.suffix().toLower() == QLatin1String("fcstd")) {
        ...
        QIcon icon(QStringLiteral(":/icons/civilcvcad-doc.png"));
        QIcon darkIcon;
