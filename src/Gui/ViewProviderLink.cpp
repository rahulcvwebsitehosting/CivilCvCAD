...
{
    if (linkInfo) {
        linkInfo->pcLinked = civilcvcad_cast<ViewProviderDocumentObject*>(getExtendedContainer());
        linkInfo->update();
    }
}
...
{
    auto owner = civilcvcad_cast<ViewProviderDocumentObject*>(getExtendedContainer());
    if (owner && linkInfo) {
...
void ViewProviderLink::onChanged(const App::Property* prop)
{
    ...
    if (prop == &ChildViewProvider) {
        childVp = civilcvcad_cast<ViewProviderDocumentObject*>(ChildViewProvider.getObject().get());
...
void ViewProviderLink::updateDataPrivate(App::LinkBaseExtension* ext, const App::Property* prop)
{
    ...
    if (prop == &ext->_ChildCache) {
        ...
        if (!ext->_getShowElementValue()) {
            ...
            auto linked = civilcvcad_cast<ViewProviderDocumentObject*>(getLinkedView(true, ext));
            ...
            if (!elements.empty()) {
                ...
                for (size_t i = 0; i < elements.size(); ++i) {
                    auto element = civilcvcad_cast<App::LinkElement*>(elements[i]);
                    if (!element) {
                        continue;
                    }
                    auto vp = civilcvcad_cast<ViewProviderLink*>(
...
void ViewProviderLink::updateElementList(App::LinkBaseExtension* ext)
{
    ...
    if (OverrideMaterialList.getSize() || MaterialList.getSize()) {
        ...
        for (auto obj : elements) {
            ++i;
            auto vp = civilcvcad_cast<ViewProviderLink*>(Application::Instance->getViewProvider(obj));
            if (!vp) {
                continue;
            }
...
) const
{
    ...
    if (!hasSubName && linkView->isLinked()) {
        auto linked = getLinkedView(false, ext);
        if (linked) {
            auto linkedVdp = civilcvcad_cast<ViewProviderDocumentObject*>(linked);
            if (linkedVdp) {
    ...
    if (obj->getDocument() != getObject()->getDocument()
        && !civilcvcad_cast<App::PropertyXLink*>(ext->getLinkedObjectProperty())) {
        return false;
    }
...
{
    if (linkEdit(ext)) {
        if (auto linkvp = civilcvcad_cast<ViewProviderLink*>(linkView->getLinkedView())) {
...
bool ViewProviderLink::initDraggingPlacement()
{
    ...
    if (ext->getLinkTransformValue() && ext->getLinkedObjectValue()) {
        propPla = civilcvcad_cast<App::PropertyPlacement*>(
...
ViewProvider* ViewProviderLink::startEditing(int mode)
{
    ...
    auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(
...
)
{
    ...
    if (!wildcard.empty()) {
        ...
        while (true) {
            ...
            auto next = civilcvcad_cast<ViewProviderLink*>(Application::Instance->getViewProvider(link));
            if (!next) {
                break;
            }
        ...
        if (wildcard != ViewProvider::hiddenMarker()) {
            ...
            auto vpLink = civilcvcad_cast<ViewProviderLink*>(vp);
            if (vpLink) {
...
) const
{
    ...
    auto res = civilcvcad_cast<ViewProviderDocumentObject*>(
        Application::Instance->getViewProvider(linked)
    );
    if (res) {
        return res;
