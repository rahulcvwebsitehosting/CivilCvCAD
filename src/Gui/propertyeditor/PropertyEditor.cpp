...
#ifdef Q_OS_MACOS
        // Brute-force workaround for https://github.com/CivilCvCAD/CivilCvCAD/issues/14350
        int currentIndex = 0;
...
void PropertyEditor::openEditor(const QModelIndex& index)
{
    ...
    auto obj = civilcvcad_cast<App::DocumentObject*>(parent);
    if (!obj) {
        auto view = civilcvcad_cast<ViewProviderDocumentObject*>(parent);
        if (view) {
            obj = view->getObject();
        }
    }
...
QString PropertyEditor::getPropUses(App::Property* prop) const
{
    ...
    auto* obj = civilcvcad_cast<App::DocumentObject*>(prop->getContainer());
    if (obj == nullptr) {
        return content;
    }
