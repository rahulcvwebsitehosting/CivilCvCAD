...
# CivilCvCAD init module - Gui
#
# Gathering all the information to start CivilCvCAD Gui.
...
App.Console.PrintLog("Init: Running CivilCvCADGuiInit.py start script...\n")
...
class Workbench:
    ...
    __Workbench__: "Workbench"  # Injected by CivilCvCAD, see: Application::activateWorkbench
...
class StandardWorkbench(Workbench):
    ...
    def Initialize(self):
        ...
        # load the module
        Log("Init: Loading CivilCvCAD GUI\n")
...
class DirModGui(ModGui):
    ...
    def process_metadata(self) -> bool:
        ...
        if "workbench" in content:
            CivilCvCAD.Gui.addIconPath(str(self.mod.path))
            workbenches = content["workbench"]
            for workbench_metadata in workbenches:
                if not workbench_metadata.supportsCurrentCivilCvCAD():
                    continue
                ...
                if self.run_init_gui(subdirectory):
                    ...
                    if classname:
                        try:
                            wb_handle = CivilCvCAD.Gui.getWorkbench(classname)
                        except Exception:
                            Log(
...
def InitApplications():
    ...
    # Patch civilcvcad module with gui alias of CivilCvCADGui
    import civilcvcad

    civilcvcad.gui = CivilCvCADGui
    ...
    Log("CivilCvCADGuiInit Mod summary:\n")
    for line in output:
        Log(line)
    Log(output[0])
...
def GeneratePackageIcon(
    subdirectory: str, workbench_metadata: CivilCvCAD.Metadata, wb_handle: Workbench
) -> None:
...
App.GuiUp = 1
App.Gui = CivilCvCADGui
CivilCvCADGui.Workbench = Workbench
...
CivilCvCAD.addImportType("Inventor V2.1 (*.iv *.IV)", "CivilCvCADGui")
CivilCvCAD.addImportType(
    "VRML V2.0 (*.wrl *.WRL *.vrml *.VRML *.wrz *.WRZ *.wrl.gz *.WRL.GZ)", "CivilCvCADGui"
)
CivilCvCAD.addImportType("Python (*.py *.FCMacro *.fcmacro *.FCScript *.fcscript)", "CivilCvCADGui")
CivilCvCAD.addExportType("Inventor V2.1 (*.iv)", "CivilCvCADGui")
CivilCvCAD.addExportType("VRML V2.0 (*.wrl *.vrml *.wrz *.wrl.gz)", "CivilCvCADGui")
CivilCvCAD.addExportType("X3D Extensible 3D (*.x3d *.x3dz)", "CivilCvCADGui")
CivilCvCAD.addExportType("WebGL/X3D (*.xhtml)", "CivilCvCADGui")
CivilCvCAD.addTranslatableExportType(
    translate("FileFormat", "Portable Document Format"), ["pdf"], "CivilCvCADGui"
)
# CivilCvCAD.addExportType("IDTF (for 3D PDF) (*.idtf)","CivilCvCADGui")
# CivilCvCAD.addExportType("3D View (*.svg)","CivilCvCADGui")

Log("Init: Running CivilCvCADGuiInit.py start script... done\n")
