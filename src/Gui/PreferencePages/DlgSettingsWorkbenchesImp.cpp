    ...
    : QWidget(parent)
{
    ...
    autoloadCheckBox->setToolTip(tr("Loads %1 automatically when CivilCvCAD starts").arg(wbDisplayName));
    ...
    loadButton->setToolTip(
        tr("To preserve resources, CivilCvCAD does not load workbenches until they are used. Loading "
...
void DlgSettingsWorkbenchesImp::loadSettings()
{
    ...
    // There are two different "autoload" settings: the first, in CivilCvCAD since 2004,
    // controls the module the user sees first when starting CivilCvCAD, and defaults to the Start workbench
