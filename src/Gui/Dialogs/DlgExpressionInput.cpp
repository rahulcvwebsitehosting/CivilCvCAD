...
void DlgExpressionInput::checkExpression(const QString& text)
{
    ...
    if (expr) {
        ...
        auto* n = civilcvcad_cast<NumberExpression*>(result.get());
        if (n) {
...
{
    return civilcvcad_cast<const App::NumberExpression*>(expr);
}

static const App::StringExpression* toStringExpr(const App::Expression* expr)
{
    return civilcvcad_cast<const App::StringExpression*>(expr);
}

static const App::OperatorExpression* toUnitNumberExpr(const App::Expression* expr)
{
    auto* opExpr = civilcvcad_cast<const App::OperatorExpression*>(expr);
...
)
{
    for (auto* varSet : varSets) {
        auto* vp = civilcvcad_cast<Gui::ViewProviderDocumentObject*>(
