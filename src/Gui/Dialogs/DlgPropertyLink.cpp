...
void DlgPropertyLink::init(const App::DocumentObjectT& prop, bool tryFilter)
{
    ...
    auto propLink = civilcvcad_cast<App::PropertyLinkBase*>(objProp.getProperty());
    if (!propLink) {
        return;
    }
...
void DlgPropertyLink::onItemSelectionChanged()
{
    ...
    if (!TreeParams::getSyncView()) {
        ...
        if (doc) {
            auto vp = civilcvcad_cast<Gui::ViewProviderDocumentObject*>(doc->getViewProvider(obj));
            if (vp) {
...
QTreeWidgetItem* DlgPropertyLink::createItem(App::DocumentObject* obj, QTreeWidgetItem* parent)
{
    ...
    auto vp = civilcvcad_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj));
    if (!vp) {
        return nullptr;
    }
    ...
    auto prop = civilcvcad_cast<App::PropertyPythonObject*>(obj->getPropertyByName("Proxy"));
    if (prop) {
