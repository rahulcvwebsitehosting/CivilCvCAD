...
void StdCmdOpen::activated(int iMsg)
{
    ...
    // Cram all formats CivilCvCAD can import under one label
    ...
    for (const QString& file : fileList) {
        ...
        getGuiApplication()->open(file.toUtf8(), "CivilCvCAD");
    ...
    : Command("Std_MergeProjects")
{
    ...
    sToolTipText = QT_TR_NOOP("Merges another CivilCvCAD document into the active one");
...
void StdCmdDelete::activated(int iMsg)
{
    ...
    try {
        ...
        for (auto& editDoc : editDocs) {
            auto vpedit = civilcvcad_cast<ViewProviderDocumentObject*>(editDoc->getInEdit());
