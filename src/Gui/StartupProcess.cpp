...
void StartupProcess::setThemePaths()
{
    ...
    QIcon::setThemeSearchPaths(QIcon::themeSearchPaths() << QStringLiteral(":/icons/CivilCvCAD-default"));
#endif
...
void StartupPostProcess::migrateOldTheme(const std::string& style)
{
    ...
    if (style == "CivilCvCAD Light.qss") {
        prefPackManager->apply("CivilCvCAD Light");
    }
    else if (style == "CivilCvCAD Dark.qss") {
    ...
    if (context.create()) {
        ...
        if (major < 2) {
            auto message = QObject::tr(
                               ...
                               "CivilCvCAD requires OpenGL 2.0 or above. "
...
void StartupPostProcess::setImportImageFormats()
{
    ...
    App::GetApplication().addImportType(filter.c_str(), "CivilCvCADGui");
}
...
void StartupPostProcess::showMainWindow()
{
    ...
    try {
        ...
        Base::Console().error("Error in CivilCvCADGuiInit.py: %s\n", e.what());
