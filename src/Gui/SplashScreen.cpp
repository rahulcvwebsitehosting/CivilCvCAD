...
QPixmap SplashScreen::splashImage()
{
    ...
    if (tc != App::Application::Config().end() && wc != App::Application::Config().end()) {
        ...
        if (title == QLatin1String("CivilCvCAD")) {
            l = 0.0;  // "CivilCvCAD" text is already part of the splashscreen, version goes below it
        }
        ...
        if (color.isValid()) {
            ...
            if (title != QLatin1String("CivilCvCAD")) {
                // CivilCvCAD's Splashscreen already contains the EXE name, no need to draw it
