...
void StdCmdFreezeViews::activated(int iMsg)
{
    ...
    if (iMsg == 0) {
        ...
        // Create a new view
        auto* view = civilcvcad_cast<MDIViewWithCamera*>(getGuiApplication()->activeView());
        if (!view) {
            return;
        }
...
{
    sGroup = "Standard-View";
    sMenuText = QT_TR_NOOP("CivilCvCAD VR");
    sToolTipText = QT_TR_NOOP("Extends the CivilCvCAD 3D Window to a VR device");
...
{
    return civilcvcad_cast<MDIViewWithCamera*>(getGuiApplication()->activeView());
}
...
void StdCmdTreeSelectAllInstances::activated(int iMsg)
{
    ...
    auto vpd = civilcvcad_cast<ViewProviderDocumentObject*>(Application::Instance->getViewProvider(obj));
    if (!vpd) {
        return;
    }
...
void StdCmdClarifySelection::activated(int iMsg)
{
    ...
    // Get the active view
    auto view3d = civilcvcad_cast<View3DInventor*>(Application::Instance->activeView());
    if (!view3d) {
        return;
    }
    ...
    for (int i = 0; i < pplist.getLength(); ++i) {
        ...
        auto vpDoc = civilcvcad_cast<Gui::ViewProviderDocumentObject*>(vp);
        if (!vpDoc) {
            continue;
        }
