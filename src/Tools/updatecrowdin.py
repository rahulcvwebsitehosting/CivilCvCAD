...
"""
This utility offers several commands to interact with the CivilCvCAD project on
crowdin. For it to work, you need a ~/.crowdin-civilcvcad-token file in your
user's folder, that contains the API access token that gives access to the
crowdin CivilCvCAD project. The API token can also be specified in the
...
# See: https://github.com/CivilCvCAD/CivilCvCAD-translations/issues/359
LANGUAGE_CODE_MAP = {
    "sv-SE": "sv",
}
...
locations = [
    ...
    ["CivilCvCAD", "../Gui/Language", "../Gui/Language/translation.qrc"],
...
THRESHOLD = 25  # how many % must be translated for the translation to be included in CivilCvCAD
...
class CrowdinUpdater:
    ...
    def _make_api_req(self, url, extra_headers={}, method="GET", data=None):
        del url, extra_headers, method, data
        raise RuntimeError("Translation service access is disabled in offline-only CivilCvCAD.")
    ...
    def download(self, build_id):
        raise RuntimeError("Translation downloads are disabled in offline-only CivilCvCAD")
...
def load_token():
    # try to read token from ~/.crowdin-civilcvcad-token
    config_file = os.path.join(os.path.expanduser("~"), ".crowdin-civilcvcad-token")
...
def applyTranslations(languages):
    ...
    src = os.path.join(currentfolder, "civilcvcad.zip")
    dst = os.path.join(tempfolder, "civilcvcad.zip")
    if not os.path.exists(src):
        print('civilcvcad.zip file not found! Aborting. Run "download" command before this one.')
        sys.exit()
    shutil.copyfile(src, dst)
    os.chdir(tempfolder)
    zfile = zipfile.ZipFile("civilcvcad.zip")
    print("extracting civilcvcad.zip...")
    zfile.extractall()
