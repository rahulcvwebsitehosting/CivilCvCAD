...
CivilCvCAD -c make_snapshot_from_file.py

or

CivilCvCADCmd make_snapshot_from_file.py
"""
...
def make_snapshot(input_file, output_file, size=48):
    ...
    mod = CivilCvCAD.getImportType(ext)
    if len(mod) == 0:
    ...
    init_gui()
    nodes = [CivilCvCADGui.subgraphFromObject(obj) for obj in doc.Objects]
    ...
    off = CivilCvCADGui.SoQtOffscreenRenderer(width, height)
