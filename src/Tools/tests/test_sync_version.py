...
class TestVersionInfo(unittest.TestCase):
    ...
    def test_lowercase_name(self):
        version = make_version(name="CivilCvCAD")
        self.assertEqual(version.lowercase_name, "civilcvcad")
...
class TestReplaceInTomlSection(unittest.TestCase):
    ...
    def test_replaces_field_in_correct_section(self):
        ...
        self.assertIn('name = "CivilCvCAD"', result)
...
name = "CivilCvCAD"
version = "1.0.0"
description = "pixi instructions for CivilCvCAD"
...
homepage = ""
repository = ""
description = "CivilCvCAD"

[feature.civilcvcad.dependencies]
civilcvcad = { path = "." }
"""
...
class TestSyncRattlerBuildPixiToml(unittest.TestCase):
    def test_updates_version_name_description(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('name = "civilcvcad"', result)
            self.assertIn('description = "CivilCvCAD"', result)
...
class TestSyncFedoraSpec(unittest.TestCase):
    def test_updates_name_and_version(self):
        with tempfile.TemporaryDirectory() as tmp:
            filepath = write_temp_file(Path(tmp), "civilcvcad.spec", FEDORA_SPEC)
            version = make_version()
...
class TestRun(unittest.TestCase):
    def _create_repo(self, tmp: str, version: VersionInfo) -> Path:
        ...
        write_temp_file(root, "package/fedora/civilcvcad.spec", FEDORA_SPEC)
        return root
