...
This directory contains the generated-stub workflow for CivilCvCAD's Python API:
...
`src/Gui/CivilCvCADGui._MainWindow.pyi` when curated type signatures should live
...
`src/App/CivilCvCAD.module.pyi` or `src/Base/CivilCvCAD.Console.module.pyi`.
...
symbols re-export aliases. `CivilCvCAD.Base` is canonical for classes sourced from
`src/Base/`, which preserves type identity for APIs that use paths such as
`CivilCvCAD.Vector`, `CivilCvCAD.Base.Vector`, or `Part.Precision`.
