...
void DocumentObject::getOutList(int options, std::vector<DocumentObject*>& res) const
{
    ...
    std::size_t size = res.size();
    for (auto prop : props) {
        auto link = civilcvcad_cast<PropertyLinkBase*>(prop);
        if (link) {
...
{
    return civilcvcad_cast<DocumentObjectGroup*>(GroupExtension::getGroupOfObject(this));
}
                                             ...
                                             int depth) const
{
    ...
    // should change that.
    if (transform && mat) {
        auto pla = civilcvcad_cast<PropertyPlacement*>(getPropertyByName("Placement"));
        if (pla) {
...
{
    auto prop = civilcvcad_cast<const PropertyComplexGeoData*>(_prop);
    if (!prop) {
        return std::string();
    }
...
{
    Base::Placement plc;
    auto* propPlacement = civilcvcad_cast<App::PropertyPlacement*>(getPropertyByName("Placement"));
    if (propPlacement) {
...
bool DocumentObject::canPropBeReferenced(const App::Property* prop)
{
    ...
    auto* obj = civilcvcad_cast<App::DocumentObject*>(container);
...
std::set<ObjectIdentifier> DocumentObject::getPropertyUses(const App::Property *prop)
{
    ...
    auto* objProp = civilcvcad_cast<DocumentObject*>(prop->getContainer());
