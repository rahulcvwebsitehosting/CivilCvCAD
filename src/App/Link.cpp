...
void LinkBaseExtension::setProperty(int idx, Property* prop)
{
    ...
    switch (idx) {
        ...
        case PropLinkCopyOnChangeGroup:
            if (auto linkProp = civilcvcad_cast<PropertyLinkBase*>(prop)) {
...
App::DocumentObjectExecReturn* LinkBaseExtension::extensionExecute()
{
    ...
    if (getLinkedObjectProperty()) {
        ...
        if (!linked) {
            ...
            ss << "Link broken!";
            auto xlink = civilcvcad_cast<PropertyXLink*>(getLinkedObjectProperty());
            if (xlink) {
        ...
        if (getLinkExecuteProperty() && !boost::iequals(getLinkExecuteValue(), "none")
            ...
            proxy = civilcvcad_cast<PropertyPythonObject*>(
                                          ...
                                          App::DocumentObject* src)
{
    ...
    for (auto it = res.begin(); it != res.end();) {
        ...
        auto prop =
            civilcvcad_cast<PropertyMap*>(obj->getPropertyByName("_CopyOnChangeControl"));
...
void LinkBaseExtension::syncCopyOnChange()
{
    ...
    if (auto prop = getLinkCopyOnChangeGroupProperty()) {
        copyOnChangeGroup = civilcvcad_cast<LinkGroup*>(prop->getValue());
        if (!copyOnChangeGroup) {
            ...
            for (auto obj : objs) {
                ...
                auto prop =
                    civilcvcad_cast<PropertyUUID*>(obj->getPropertyByName("_SourceUUID"));
    ...
    for (auto obj : copiedObjs) {
        auto prop = civilcvcad_cast<PropertyUUID*>(obj->getPropertyByName("_SourceUUID"));
        if (prop) {
    ...
    for (const auto& objT : oldObjs) {
        auto prop = civilcvcad_cast<PropertyUUID*>(objT.getProperty());
        if (!prop) {
            continue;
        }
        ...
        for (auto doc : App::GetApplication().getDocuments()) {
            for (auto o : doc->getObjects()) {
                ...
                for (auto prop : propList) {
                    ...
                    auto linkProp = civilcvcad_cast<App::PropertyLinkBase*>(prop);
                    if (!linkProp) {
                        continue;
                    }
...
void LinkBaseExtension::parseSubName() const
{
    ...
    mySubElements.clear();
    mySubName.clear();
    auto xlink = civilcvcad_cast<const PropertyXLink*>(getLinkedObjectProperty());
...
void LinkBaseExtension::update(App::DocumentObject* parent, const Property* prop)
{
    ...
    if (prop == getLinkPlacementProperty() || prop == getPlacementProperty()) {
        ...
        if (_getShowElementValue()) {
            ...
            for (auto obj : objs) {
                auto element = civilcvcad_cast<LinkElement*>(obj);
                if (element) {
        ...
        if (!_getShowElementValue()) {
            ...
            if (elementCount > objs.size()) {
                ...
                for (size_t i = objs.size(); i < elementCount; ++i) {
                    ...
                    auto obj = civilcvcad_cast<LinkElement*>(doc->getObject(name.c_str()));
                    if (obj
                ...
                while (objs.size() > elementCount) {
                    auto element = civilcvcad_cast<LinkElement*>(objs.back());
...
void LinkBaseExtension::syncElementList()
{
    ...
    auto xlink = civilcvcad_cast<const PropertyXLink*>(link);

    auto owner = getContainer();
    auto ownerID = owner ? owner->getID() : 0;
    auto elements = getElementListValue();
    for (auto i : elements) {
        auto element = civilcvcad_cast<LinkElement*>(i);
        if (!element
                                ...
                                const std::vector<std::string>& subElements)
{
    ...
    if (index >= 0) {
        ...
        if (!obj) {
            ...
            if (getLinkModeValue() >= LinkModeAutoLink || !Base::Tools::isNullOrEmpty(subname)
                ...
                auto pla =
                    civilcvcad_cast<PropertyPlacement*>(obj->getPropertyByName("Placement"));
                if (pla) {
    ...
    auto xlink = civilcvcad_cast<PropertyXLink*>(linkProp);
    if (obj) {
        if (!obj->isAttachedToDocument()) {
