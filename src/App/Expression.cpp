...
void NumberExpression::_toString(std::ostream &ss, bool,int) const
{
    ...
    // https://forum.civilcvcad.org/viewtopic.php?f=3&t=44057&p=375882#p375882
    // See also:
...
ExpressionPtr OperatorExpression::simplify() const
{
    ...
    if (civilcvcad_cast<NumberExpression*>(v1.get()) && civilcvcad_cast<NumberExpression*>(v2.get())) {
        return eval();
    }
    else
...
void OperatorExpression::_toString(std::ostream &s, bool persistent,int) const
{
    ...
    needsParens = false;
    if (civilcvcad_cast<OperatorExpression*>(left))
    ...
    needsParens = false;
    if (civilcvcad_cast<OperatorExpression*>(right))
    ...
    for (auto &arg : args) {
        if (arg->isDerivedFrom<RangeExpression>()) {
            ...
            do {
                ...
                if ((qp = civilcvcad_cast<PropertyQuantity*>(p)))
                    c->collect(qp->getQuantityValue());
                else if ((fp = civilcvcad_cast<PropertyFloat*>(p)))
                    c->collect(Quantity(fp->getValue()));
                else if ((ip = civilcvcad_cast<PropertyInteger*>(p)))
...
ExpressionPtr FunctionExpression::simplify() const
{
    ...
    for (auto it : args) {
        ...
        if (civilcvcad_cast<NumberExpression*>(v.get())) {
            ++numerics;
        }
...
void VariableExpression::addComponent(Component *c) {
    do {
        ...
        long l1=0,l2=0,l3=1;
        if(c->e3) {
            auto n3 = civilcvcad_cast<NumberExpression*>(c->e3.get());
            if(!n3 || !essentiallyEqual(n3->getValue(),(double)l3))
                break;
        }
        if(c->e1) {
            auto n1 = civilcvcad_cast<NumberExpression*>(c->e1.get());
            if(!n1) {
                if(c->e2 || c->e3)
                    break;
                auto s = civilcvcad_cast<StringExpression*>(c->e1.get());
                if(!s)
                    break;
        ...
        auto n2 = civilcvcad_cast<NumberExpression*>(c->e2.get());
...
ExpressionPtr ConditionalExpression::simplify() const
{
    ...
    NumberExpression * v = civilcvcad_cast<NumberExpression*>(e.get());
...
)
{
    ...
    if (!unitExpression) {
        auto* fraction = civilcvcad_cast<OperatorExpression*>(ScanResult.get());

        if (fraction && fraction->getOperator() == OperatorExpression::DIV) {
            NumberExpression * nom = civilcvcad_cast<NumberExpression*>(fraction->getLeft());
            UnitExpression * denom = civilcvcad_cast<UnitExpression*>(fraction->getRight());
    ...
    if (auto num = civilcvcad_cast<NumberExpression*>(simplified.get()); num) {
        return std::make_unique<UnitExpression>(num->getOwner(), num->getQuantity());
    }
    return std::unique_ptr<UnitExpression>(civilcvcad_cast<UnitExpression*>(simplified.release()));
}
