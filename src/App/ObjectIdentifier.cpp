...
{
    if (_owner) {
        const DocumentObject* docObj = civilcvcad_cast<const DocumentObject*>(_owner);
        if (!docObj) {
...
ObjectIdentifier::ObjectIdentifier(const Property& prop, int index)
    ...
    , _hash(0)
{
    DocumentObject* docObj = civilcvcad_cast<DocumentObject*>(prop.getContainer());
...
ObjectIdentifier ObjectIdentifier::parse(const DocumentObject* docObj, const std::string& str)
{
    ...
    VariableExpression* v = civilcvcad_cast<VariableExpression*>(expr.get());
...
#define GET_MODULE(_name)                                                                          \
    ...
    switch (ptype) {
        case PseudoApp:
            GET_MODULE(CivilCvCAD);
            break;
        case PseudoGui:
            GET_MODULE(CivilCvCADGui);
            break;
        case PseudoPart:
            GET_MODULE(Part);
            break;
        case PseudoCadquery:
            GET_MODULE(civilcvcad.fc_cadquery);
            break;
        case PseudoRegex:
            GET_MODULE(re);
            break;
    ...
    auto setPropDep = [deps](DocumentObject* obj, Property* prop, const char* propName) {
        ...
        if (prop && prop->getContainer() != obj) {
            auto linkTouched =
                civilcvcad_cast<PropertyBool*>(obj->getPropertyByName("_LinkTouched"));
            if (linkTouched) {
                propName = linkTouched->getName();
            }
            else {
                auto propOwner = civilcvcad_cast<DocumentObject*>(prop->getContainer());
                if (propOwner) {
                    obj = propOwner;
                }
                else {
