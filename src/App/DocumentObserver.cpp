...
DocumentObjectT& DocumentObjectT::operator=(const Property* prop)
{
    ...
    else if (auto obj = civilcvcad_cast<App::DocumentObject*>(prop->getContainer())) {
        object = obj->getNameInDocument();
        label = obj->Label.getValue();
        document = obj->getDocument()->getName();
...
{
    std::stringstream str;
    str << "CivilCvCAD.getDocument(\"" << document << "\")";
    return str.str();
}
...
{
    std::stringstream str;
    str << "CivilCvCAD.getDocument('" << document << "').getObject('" << object << "')";
    return str.str();
}
