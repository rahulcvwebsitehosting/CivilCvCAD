...
import socket as _civilcvcad_socket
import sys as _civilcvcad_sys

import CivilCvCAD


def _civilcvcad_offline_audit(
    event,
    args,
    _internet_families=frozenset(
        (int(_civilcvcad_socket.AF_INET), int(_civilcvcad_socket.AF_INET6))
    ),
):
    """Deny internet socket use from the embedded Python runtime."""

    if event == "socket.__new__" and len(args) > 1:
        family = int(args[1])
        if family in _internet_families:
            raise PermissionError("CivilCvCAD offline policy blocks internet sockets")
    elif event == "socket.connect" and len(args) > 1 and isinstance(args[1], tuple):
        raise PermissionError("CivilCvCAD offline policy blocks network connections")
    elif event in {
        "socket.getaddrinfo",
        ...
        raise PermissionError("CivilCvCAD offline policy blocks network name resolution")


_civilcvcad_sys.addaudithook(_civilcvcad_offline_audit)

App = CivilCvCAD

App.Console.PrintLog("Init: starting App::CivilCvCADInit.py\n")
...
class FCADLogger:
    ...
    Example usage:
        >>> logger = CivilCvCAD.Logger('MyModule')
    ...
    To set/get logger level:
        >>> CivilCvCAD.setLogLevel('MyModule','Trace')
        >>> CivilCvCAD.getLogLevel('MyModule')
        4
    ...
    CivilCvCAD.setLogLevel() supports both text and integer value, which allows
    ...
    def __init__(self, tag: str, **kwargs) -> None:
        ...
        * tag: a string tag for this logger. The log level of this logger can be
               accessed using CivilCvCAD.getLogLevel(tag)/setLogLevel(tag,level).
        ...
        * title ('CivilCvCAD'): message box title used by report()
        """
        self.tag = tag
...
App.addImportType("CivilCvCAD document (*.FCStd)", "CivilCvCAD")
...
class Transient:
    ...
    def cleanup(cls) -> None:
        ...
        keep = set(("__builtins__", "CivilCvCAD", "App", "os", "sys", "traceback", "inspect"))
...
class WindowsPlatform:
    ...
    def __init__(self) -> None:
        ...
        if lib_pack := utils.env_to_path("CIVILCVCAD_LIBPACK_BIN"):
...
class DarwinPlatform:
    ...
    def post(self) -> None:
        ...
        sys.path.append(os.path.expanduser("~/Library/Application Support/CivilCvCAD/Mod"))
...
class Mod:
    ...
    def supports_civilcvcad_version(self) -> bool:
        """
        Check if the Mod supports the current CivilCvCAD version.
        """
        if meta := self.metadata:
            return meta.supportsCurrentCivilCvCAD()
        return True
...
class ExtMod(Mod):
    ...
    extension modules must be defined in namespace civilcvcad.*, i.e. civilcvcad.MyAddon.
    """

    name: str  # full module name, i.e.: civilcvcad.MyAddon
    ...
    def process_metadata(self, _search_paths: SearchPaths) -> None:
        ...
        if not self.supports_civilcvcad_version():
            self.state = ModState.Unsupported
            Msg(f"NOTICE: {self.name} does not support this version of CivilCvCAD, so is being skipped")
...
class DirMod(Mod):
    ...
    def process_metadata(self, search_paths: SearchPaths):
        ...
        if not self.supports_civilcvcad_version():
            self.state = ModState.Unsupported
            Msg(f"NOTICE: {meta.Name} does not support this version of CivilCvCAD, so is being skipped")
            return
        ...
        if "workbench" in content:
            ...
            for workbench in workbenches:
                if not workbench.supportsCurrentCivilCvCAD():
                    Msg(f"NOTICE: {meta.Name} content item {workbench.Name} does not support this version of CivilCvCAD, so is being skipped")
                    continue
...
class ExtModScanner:
    ...
    def scan(self):
        import civilcvcad
        modules = (m[1] for m in pkgutil.iter_modules(civilcvcad.__path__, "civilcvcad.") if m[2])
...
class InitPipeline:
    ...
    def scan(self) -> None:
        ...
        legacy_user_mod = Path.home() / ".CivilCvCAD" / "Mod"
        ...
        # to have all the module-paths available in CivilCvCADGuiInit.py:
        App.__ModDirs__ = [str(d) for d in mods.dirs()]

        # this allows importing with:
        # from CivilCvCAD.Module import package
    ...
    def load_mods(self) -> None:
        ...
        # Save to use in CivilCvCADGuiInit.py
    ...
    def report(self) -> None:
        ...
        Log("CivilCvCADInit Mod summary:")
        output = []
...
def init_applications() -> None:
    try:
        InitPipeline().run()
        Log('Init: App::CivilCvCADInit.py done')
    except Exception as ex:
