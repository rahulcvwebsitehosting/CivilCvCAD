...
{
    auto owner = civilcvcad_cast<DocumentObject*>(getContainer());
    if (owner) {
...
static std::string propertyName(const Property* prop)
{
    ...
    if (!prop->getContainer() || !prop->hasName()) {
        auto xlink = civilcvcad_cast<const PropertyXLink*>(prop);
        if (xlink) {
...
void PropertyLinkList::setValues(const std::vector<DocumentObject*>& value)
{
    ...
    auto parent = civilcvcad_cast<App::DocumentObject*>(getContainer());
    for (auto obj : value) {
        if (!obj || !obj->isAttachedToDocument()) {
                                ...
                                std::vector<PropertyLinkBase::ShadowSub>& shadows)
{
    ...
    auto owner = civilcvcad_cast<DocumentObject*>(prop->getContainer());
...
void PropertyLinkSubList::Restore(Base::XMLReader& reader)
{
    ...
    DocumentObject* father = civilcvcad_cast<DocumentObject*>(getContainer());
...
public:
    ...
    get(const char* filename, App::Document* pDoc, PropertyXLink* l, const char* objName)
    {
        ...
        if (info->pcDoc) {
            ...
            auto owner = civilcvcad_cast<DocumentObject*>(l->getContainer());
    ...
    void slotDeleteDocument(const App::Document& doc)
    {
        for (auto it = links.begin(), itNext = it; it != links.end(); it = itNext) {
            ++itNext;
            auto link = *it;
            auto obj = civilcvcad_cast<DocumentObject*>(link->getContainer());
                                  ...
                                  bool clear)
{
    ...
    for (auto obj : objs) {
        ...
        for (auto prop : props) {
            auto linkProp = civilcvcad_cast<PropertyLinkBase*>(prop);
            if (linkProp) {
...
void PropertyXLink::Restore(Base::XMLReader& reader)
{
    ...
    if (!name.empty() && file.empty()) {
        if (auto parent = civilcvcad_cast<DocumentObject*>(getContainer())) {
...
{
    auto owner = civilcvcad_cast<const DocumentObject*>(getContainer());
...
void PropertyXLinkContainer::Save(Base::Writer& writer) const
{
    ...
    auto owner = civilcvcad_cast<App::DocumentObject*>(getContainer());
