        ...
        switch (format) {
            ...
            default:
                throw Base::XMLBaseException(
                    "package.xml format version is not supported by this version of CivilCvCAD");
        }
    }
}
...
Meta::Version Metadata::civilcvcadmin() const
{
    return _civilcvcadmin;
}

Meta::Version Metadata::civilcvcadmax() const
{
    return _civilcvcadmax;
}
...
void Metadata::setCivilCvCADMin(const Meta::Version& version)
{
    _civilcvcadmin = version;
}
...
void Metadata::setCivilCvCADMax(const Meta::Version& version)
{
    _civilcvcadmax = version;
}
...
void Metadata::write(const fs::path& file) const
{
    ...
    root->setAttribute(XUTF8StrLiteral("xmlns").unicodeForm(),
                       XUTF8StrLiteral("urn:civilcvcad:package-metadata:1").unicodeForm());
...
bool Metadata::satisfies(const Meta::Dependency& dep)
{
    ...
    // must contain a valid CivilCvCAD Expression. If it evaluates to false, this dependency is
...
bool Metadata::supportsCurrentCivilCvCAD() const
{
    ...
    if (_civilcvcadmin != Meta::Version() && _civilcvcadmin > fcVersion) {
        return false;
    }
    if (_civilcvcadmax != Meta::Version() && _civilcvcadmax < fcVersion) {
        return false;
    }
    return true;
}
...
void Metadata::appendToElement(DOMElement* root) const
{
    ...
    if (_civilcvcadmin != Meta::Version()) {
        appendSimpleXMLNode(root, "civilcvcadmin", _civilcvcadmin.str());
    }

    if (_civilcvcadmax != Meta::Version()) {
        appendSimpleXMLNode(root, "civilcvcadmax", _civilcvcadmax.str());
    }
...
void Metadata::parseVersion1(const DOMNode* startNode)
{
    ...
    for (XMLSize_t i = 0; i < children->getLength(); ++i) {
        ...
        else if (tagString == "civilcvcadmin") {
            _civilcvcadmin = Meta::Version(StrXUTF8(element->getTextContent()).str);
        }
        else if (tagString == "civilcvcadmax") {
            _civilcvcadmax = Meta::Version(StrXUTF8(element->getTextContent()).str);
        }
