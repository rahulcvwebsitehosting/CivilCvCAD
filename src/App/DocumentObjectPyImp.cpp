...
Py::Object DocumentObjectPy::getViewObject() const
{
    try {
        ...
        // check if the CivilCvCADGui module is already loaded, if not then don't try to load it
        Py::Dict sysmod(dict);
        if (!sysmod.hasKey("CivilCvCADGui")) {
            return Py::None();
        }

        // double-check that the module doesn't have a null pointer
        Py::Module module(PyImport_ImportModule("CivilCvCADGui"), true);
        ...
        // CivilCvCADGui is loaded, so there must be wrong something else
        throw;  // re-throw
    }
}
...
PyObject* DocumentObjectPy::evalExpression(PyObject* self, PyObject* args)
{
    ...
    // For CivilCvCAD-specific types the behaviour is a bit different:
