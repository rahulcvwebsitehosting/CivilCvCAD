...
void Document::onChanged(const Property* prop)
{
    ...
    if (prop == &Label) {
        ...
        for (auto obj : d->objectArray) {
            auto geofeature = civilcvcad_cast<GeoFeature*>(obj);
...
void Document::Save(Base::Writer& writer) const
{
    ...
    //       see: https://github.com/CivilCvCAD/CivilCvCAD/issues/27489
    //
    ...
    for (const auto o : objs) {
        if (o && o->isAttachedToDocument()) {
            ...
            if (const auto propUUID =
                    civilcvcad_cast<PropertyUUID*>(o->getPropertyByName("_ObjectUUID"))) {
                auto propSource =
                    civilcvcad_cast<PropertyUUID*>(o->getPropertyByName("_SourceUUID"));
                if (!propSource) {
...
bool Document::saveToFile(const char* filename) const
{
    ...
    // open extra scope to close ZipWriter properly
    {
        ...
        writer.setComment("CivilCvCAD Document");
...
bool Document::afterRestore(const std::vector<DocumentObject*>& objArray, bool checkPartial)
{
    ...
    for (auto obj : objs) {
        ...
        try {
            ...
            // Segfaults later when CivilCvCAD interacts with Python.
            if (PyErr_Occurred()) {
