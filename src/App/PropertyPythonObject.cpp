 ...
 *
 * Modules already in @c sys.modules are permitted -- they were loaded by CivilCvCAD core or addons
 ...
 * a CivilCvCAD module directory.  This prevents a crafted FCStd from importing arbitrary modules
 * (whose <tt>__init__.py</tt> could run malicious code on import) while still allowing
 * legitimate lazy-loaded CivilCvCAD workbench modules to restore.
 *
...
bool isAllowedModule(const std::string& moduleName)
{
    ...
    // CivilCvCAD adds each workbench directory to sys.path individually (e.g. .../Mod/Assembly/),
    ...
    // Use CivilCvCAD.__ModDirs__ as the authoritative list of allowed module directories.
    // This is populated during startup by CivilCvCADInit.py and includes built-in workbenches,
    // user addons, and any additional configured module paths.
    Py::Module civilcvcad(PyImport_ImportModule("CivilCvCAD"), true);
    if (!civilcvcad.hasAttr("__ModDirs__") or !civilcvcad.hasAttr("__MacroDirs__")) {
        throw Py::RuntimeError("CivilCvCAD.__ModDirs__ or CivilCvCAD.__MacroDirs__ not set -- CivilCvCADInit.py has not run yet");
    }
    Py::List allowedDirs(civilcvcad.getAttr("__ModDirs__"));
    allowedDirs.extend(civilcvcad.getAttr("__MacroDirs__"));
    const int allowedDirsSize = static_cast<int>(allowedDirs.size());

    auto isUnderCivilCvCAD = [&](const std::string& path) {
    ...
    return isUnderCivilCvCAD(Py::String(loaderMod.getAttr("__file__")).as_std_string());
}
            ...
            if (reader.hasAttribute("module") && reader.hasAttribute("class")) {
                ...
                if (!isAllowedModule(moduleName)) {
                    Base::Console().warning(
                        ...
                        " document restore. Only modules from CivilCvCAD or installed addons"
                        " are permitted.\n",
                        moduleName.c_str());
