...
// CivilCvCAD offline-only: no startup version checks
// Define CIVILCVCAD_OFFLINE_ONLY to skip every network-touching startup probe.

#include "ApplicationDirectories.h"
#include "ApplicationDirectoriesPy.h"
#include "ApplicationPy.h"
#include "CivilCvCADModulePy.h"
...
// during initialization, to make the built-in __CivilCvCADBase__
// module known to Python.
PyMODINIT_FUNC
init_civilcvcad_base_module(void)
{
    static struct PyModuleDef BaseModuleDef = {
        PyModuleDef_HEAD_INIT,
        "__CivilCvCADBase__", Base_doc, -1,
...
PyMODINIT_FUNC
init_civilcvcad_module(void)
{
    static struct PyModuleDef CivilCvCADModuleDef = {
        PyModuleDef_HEAD_INIT,
        "CivilCvCAD", App::CivilCvCADModulePy::moduleDocumentation(), -1,
        nullptr,
        nullptr, nullptr, nullptr, nullptr
    };
    PyObject* module = PyModule_Create(&CivilCvCADModuleDef);
    App::CivilCvCADModulePy::addModuleMethods(module);
    return module;
}
...
void Application::setupPythonTypes()
{
    ...
    PyObject* pAppModule = PyImport_ImportModule ("CivilCvCAD");
    if (!pAppModule) {
        PyErr_Clear();
        pAppModule = init_civilcvcad_module();
        PyDict_SetItemString(modules, "CivilCvCAD", pAppModule);
    }
    ...
    static struct PyModuleDef ConsoleModuleDef = {
        PyModuleDef_HEAD_INIT,
        "__CivilCvCADConsole__", Base::ConsoleModulePy::moduleDocumentation(), -1,
        nullptr,
        nullptr, nullptr, nullptr, nullptr
    };
    ...
    // remove these types from the CivilCvCAD module.

    PyObject* pBaseModule = PyImport_ImportModule ("__CivilCvCADBase__");
    if (!pBaseModule) {
        PyErr_Clear();
        pBaseModule = init_civilcvcad_base_module();
        PyDict_SetItemString(modules, "__CivilCvCADBase__", pBaseModule);
    }
...
void Application::setupPythonException(PyObject* module)
{
    ...
    Base::PyExc_FC_GeneralError = setup("CivilCvCADError", PyExc_RuntimeError);
    Base::PyExc_FC_CivilCvCADAbort = setup("CivilCvCADAbort", PyExc_BaseException);
...
Document* Application::newDocument(const char * proposedName, const char * proposedLabel, DocumentInitFlags CreateFlags)
{
    ...
    // create the CivilCvCAD document
...
void Application::setActiveDocumentNoSignal(Document* pDoc)
{
    ...
    if (pDoc) {
        ...
        Py::Module("CivilCvCAD").setAttr(std::string("ActiveDocument"), active);
    }
    else {
        Base::PyGILStateLocker lock;
        Py::Module("CivilCvCAD").setAttr(std::string("ActiveDocument"), Py::None());
    }
}
...
void Application::addImportType(const char* filter, const char* moduleName)
{
    ...
    // Due to branding stuff replace "CivilCvCAD" with the branded application name
    if (strncmp(filter, "CivilCvCAD", 7) == 0) {
                                            ...
                                            const std::string &moduleName)
{
    ...
    // Branding: replace "CivilCvCAD" in a file type description with the branded application name
    auto replaceCivilCvCAD =
    [](std::string& s)
    {
        constexpr std::string_view civilcvcad = "CivilCvCAD";
        if (auto pos = s.find(civilcvcad); pos != std::string::npos) {
            s.replace(pos, civilcvcad.size(), getExecutableName());
    ...
    bool containsAppName = replaceCivilCvCAD(translatedDescription);  // Run *AFTER* translation
...
void Application::cleanupUnits()
{
    try {
        Base::PyGILStateLocker lock;
        Py::Module mod (Py::Module("CivilCvCAD").getAttr("Units").ptr());
...
void parseProgramOptions(int ac, char ** av, const std::string& exe, boost::program_options::variables_map& vm)
{
    ...
    try {
        ...
        std::ifstream ifs("CivilCvCAD.cfg");
        if (ifs)
    ...
    if (vm.contains("help")) {
        std::stringstream str;
        str << exe << '\n' << '\n';
        str << "For a detailed description, see the installed offline documentation."
            << '\n' << '\n';
...
void processProgramOptions(const boost::program_options::variables_map& vm, std::map<std::string,std::string>& mConfig)
{
    ...
    if (vm.contains("run-test") || vm.contains("run-open")) {
        ...
        mConfig["ScriptFileName"] = "CivilCvCADTest";
...
#   endif
    ...
    if (!Py_IsInitialized()) {
        // init python
        PyImport_AppendInittab ("CivilCvCAD", init_civilcvcad_module);
        PyImport_AppendInittab ("__CivilCvCADBase__", init_civilcvcad_base_module);
    }
    else {
        // "import CivilCvCAD" in a normal Python 3.12 interpreter would raise
        ...
        auto moduleName = "CivilCvCAD";
        PyImport_AddModule(moduleName);
        PyObject *pyModule = init_civilcvcad_module();
        PyDict_SetItemString(sysModules, moduleName, pyModule);
        Py_DECREF(pyModule);

        moduleName = "__CivilCvCADBase__";
        PyImport_AddModule(moduleName);
        pyModule = init_civilcvcad_base_module();
        PyDict_SetItemString(sysModules, moduleName, pyModule);
        Py_DECREF(pyModule);
    }
    ...
    if (mConfig["RunMode"] != "Cmd" && !(vm.contains("verbose") && vm.contains("version"))) {
        // Remove banner if CivilCvCAD is invoked via the -c command as regular
        // Python interpreter
        ...
        if (SafeMode::SafeModeEnabled()) {
            Base::Console().message("CivilCvCAD is running in _SAFE_MODE_.\n"
...
void Application::initApplication()
{
    ...
    new Base::ScriptProducer( "CivilCvCADInit",    CivilCvCADInit    );
    new Base::ScriptProducer( "CivilCvCADTest",    CivilCvCADTest    );
    ...
    try {
        ...
        Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("CivilCvCADInit"));
    }
    catch (const Base::Exception& e) {
        e.reportException();
    }
...
void Application::runApplication()
{
    ...
    if (mConfig["RunMode"] == "Cmd") {
        ...
        Base::Interpreter().runCommandLine("CivilCvCAD Console mode");
    }
...
void Application::LoadParameters()
{
    ...
    try {
        if (_pcSysParamMngr->LoadOrCreateDocument() && mConfig["Verbose"] != "Strict") {
            ...
            if (!Py_IsInitialized()) {
                ...
                Base::Console().message("   This warning normally means that CivilCvCAD is running for the first time\n"
                                  "   or the configuration was deleted or moved. CivilCvCAD is generating the standard\n"
                                  "   configuration.\n");
            }
        }
    }
    ...
    // VS2010: https://forum.civilcvcad.org/viewtopic.php?f=4&t=1886&p=12553&hilit=boost%3A%3Afilesystem%3A%3Aget#p12553
 ...
 * \list
 * \li CIVILCVCAD_USER_HOME
 * \li CIVILCVCAD_USER_DATA
 * \li CIVILCVCAD_USER_TEMP
 * \endlist
 */
std::tuple<QString, QString, QString> getCustomPaths()
{
    const QProcessEnvironment env(QProcessEnvironment::systemEnvironment());
    QString userHome = env.value(QStringLiteral("CIVILCVCAD_USER_HOME"));
    QString userData = env.value(QStringLiteral("CIVILCVCAD_USER_DATA"));
    QString userTemp = env.value(QStringLiteral("CIVILCVCAD_USER_TEMP"));
    ...
    // if CIVILCVCAD_USER_HOME is set but not CIVILCVCAD_USER_DATA
    if (!userHome.isEmpty() && userData.isEmpty()) {
        userData = userHome;
    }

    // if CIVILCVCAD_USER_HOME is set but not CIVILCVCAD_USER_TEMP
...
void Application::ExtractUserPath()
{
    ...
    // this is to support a portable version of CivilCvCAD
...
std::string Application::FindHomePath(const char* sCall)
{
    ...
    // import the CivilCvCAD.so module from a running Python session. In the latter case the
...
std::string Application::FindHomePath(const char* sCall)
{
    ...
    if (Py_IsInitialized()) {
        ...
        // path. In the worst case we simply get q wrong path and CivilCvCAD is not
...
std::string Application::FindHomePath(const char* sCall)
{
    ...
    // * import the CivilCvCAD.pyd module from a running Python session. In this case the
    //   Python interpreter is already initialized.
    // * use a custom dll that links CivilCvCAD core dlls and that is loaded by its host application
    //   In this case the calling name should be set to CivilCvCADBase.dll or CivilCvCADApp.dll in order
    ...
    // fixes #0001638 to avoid to load DLLs from Windows' system directories before CivilCvCAD's bin folder
