...
Py::String MetadataPy::getCivilCvCADMin() const
{
    return Py::String(getMetadataPtr()->civilcvcadmin().str());
}

void MetadataPy::setCivilCvCADMin(Py::String args)
{
    ...
    if (version) {
        getMetadataPtr()->setCivilCvCADMin(App::Meta::Version(version));
    }
    else {
        getMetadataPtr()->setCivilCvCADMin(App::Meta::Version());
    }
}

Py::String MetadataPy::getCivilCvCADMax() const
{
    return Py::String(getMetadataPtr()->civilcvcadmax().str());
}

void MetadataPy::setCivilCvCADMax(Py::String args)
{
    char* version = nullptr;
    PyObject* p = args.ptr();
    if (!PyArg_Parse(p, "z", &version)) {
    ...
    if (version) {
        getMetadataPtr()->setCivilCvCADMax(App::Meta::Version(version));
    }
    else {
        getMetadataPtr()->setCivilCvCADMax(App::Meta::Version());
    }
}
...
PyObject* MetadataPy::getFirstSupportedCivilCvCADVersion(PyObject* p)
{
    ...
    if (getMetadataPtr()->civilcvcadmin() != App::Meta::Version()) {
        return Py::new_reference_to(Py::String(getMetadataPtr()->civilcvcadmin().str()));
    }

    auto content = getMetadataPtr()->content();
    auto result = App::Meta::Version();
    for (const auto& item : content) {
        auto minVersion = item.second.civilcvcadmin();
...
PyObject* MetadataPy::getLastSupportedCivilCvCADVersion(PyObject* p)
{
    if (!PyArg_ParseTuple(p, "")) {
        return nullptr;
    }
    ...
    // Short-circuit: if the toplevel sets a version, then the lower-levels are overridden
    if (getMetadataPtr()->civilcvcadmax() != App::Meta::Version()) {
        return Py::new_reference_to(Py::String(getMetadataPtr()->civilcvcadmax().str()));
    }

    auto content = getMetadataPtr()->content();
    auto result = App::Meta::Version();
    for (const auto& item : content) {
        auto maxVersion = item.second.civilcvcadmax();
...
PyObject* MetadataPy::supportsCurrentCivilCvCAD(PyObject* p)
{
    if (!PyArg_ParseTuple(p, "")) {
        return nullptr;
    }

    bool result = getMetadataPtr()->supportsCurrentCivilCvCAD();
