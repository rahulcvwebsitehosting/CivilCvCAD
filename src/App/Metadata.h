 ...
 *
 * The metadata format is based on https://ros.org/reps/rep-0149.html, modified for CivilCvCAD
 * use. Full format documentation is available at the CivilCvCAD Wiki:
 * The local package metadata format is documented with the source distribution.
 */
...
public:
    ...
    Meta::Version civilcvcadmin() const;  //< The minimum CivilCvCAD version.
    Meta::Version civilcvcadmax() const;  //< The maximum CivilCvCAD version.
    ...
    void setCivilCvCADMin(const Meta::Version& version);
    void setCivilCvCADMax(const Meta::Version& version);
     ...
     * CivilCvCAD. Does not interrogate content items, which must be queried individually.
     */
    bool supportsCurrentCivilCvCAD() const;
...
private:
    ...
    Meta::Version _civilcvcadmin;
    Meta::Version _civilcvcadmax;
