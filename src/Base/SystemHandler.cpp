...
/** civilcvcadNewHandler()
 * prints an error message and throws an exception
 */
#ifdef _MSC_VER  // New handler for Microsoft Visual C++ compiler
int __cdecl civilcvcadNewHandler(size_t size)
{
    // throw an exception
...
#else  // Ansi C/C++ new handler
static void civilcvcadNewHandler()
{
    // throw an exception
    throw Base::MemoryException();
}
...
void SystemHandler::installNewHandler()
{
    ...
    _set_new_handler(civilcvcadNewHandler);  // Setup new handler
    _set_new_mode(1);                     // Re-route malloc failures to new handler !
#else                                     // Ansi compiler
    std::set_new_handler(civilcvcadNewHandler);  // ANSI new handler
#endif
}
