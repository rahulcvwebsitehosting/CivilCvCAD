 ...
 * \section Motivation
 *
 * CivilCvCAD Base::Console() is capable of outputting to different targets, and has
 ...
 *
 * \code{.py}
 * CivilCvCAD.setLogLevel('Default',level)
 * CivilCvCAD.setLogLevel('DebugDefault',level)
 * \endcode
 *
 ...
 *
 * Python code can call \c CivilCvCAD.setLogLevel(tag,level) to configure any other
 * tag log levels, and \c CivilCvCAD.getLogLevel(tag), which outputs only integer
 * log level.
 *
 ...
 * CivilCvCAD.setLogLevel/getLogLevel. Any change to the log level is reflected
 ...
 *  This class describes an Interface for logging within CivilCvCAD. If you want to add a new
 *  "sink" to CivilCvCAD's logging mechanism, then inherit this class. You'll also need to
...
public:
    ...
    */
    ...
    enum CivilCvCAD_ConsoleMsgType
    {
        MsgType_Txt = 1,
    ...
    public:
        ...
        virtual void postEvent(
            CivilCvCAD_ConsoleMsgType type,
            IntendedRecipient recipient,
            ContentType content,
            const std::string& notifiername,
            const std::string& msg
    ...
    using PostEventHandler = std::function<
        void(CivilCvCAD_ConsoleMsgType, IntendedRecipient, ContentType, const std::string&, const std::string&)>;
    ...
    bool isMsgTypeEnabled(const char* sObs, CivilCvCAD_ConsoleMsgType type) const;
    ...
    constexpr CivilCvCAD_ConsoleMsgType getConsoleMsg(LogStyle style);
...
constexpr ConsoleSingleton::CivilCvCAD_ConsoleMsgType ConsoleSingleton::getConsoleMsg(LogStyle style)
{
 ...
 *  CivilCvCAD is running in GUI mode a Message appears on the status bar.
