...
T civilcvcad_cast(Base::BaseClass* type)
{
...
template<typename T, typename U = std::remove_pointer_t<T>>
    requires(std::is_pointer_v<T>)
const U* civilcvcad_cast(const Base::BaseClass* type)
{
    static_assert(std::is_base_of_v<Base::BaseClass, U>, "T must be derived from Base::BaseClass");
...
// We define global alias for civilcvcad_cast to be used by all CivilCvCAD files that include
// BaseClass.h. While doing using on header level is non-ideal it allows for much easier use
// of the important civilcvcad_cast. In that case the name is prefixed with civilcvcad so there is no
// chance of symbols collision.
using Base::civilcvcad_cast;
