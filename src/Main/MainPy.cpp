...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD ul_reason_for_call, LPVOID /*lpReserved*/)
{
    switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH: {
            // This name is preliminary, we pass it to Application::init() in initCivilCvCAD()
            // which does the rest.
...
PyMOD_INIT_FUNC(CivilCvCAD)
{
    // Init phase ===========================================================
    App::Application::Config()["ExeName"] = "CivilCvCAD";
    App::Application::Config()["ExeVendor"] = "CivilCvCAD";
...
#elif defined(FC_OS_CYGWIN)
    HMODULE hModule = GetModuleHandle("CivilCvCAD.dll");
    char szFileName[MAX_PATH];
    ...
    Dl_info info;
    int ret = dladdr((void*)PyInit_CivilCvCAD, &info);
    if ((ret == 0) || (!info.dli_fname)) {
        PyErr_SetString(PyExc_ImportError, "Cannot get path of the CivilCvCAD module!");
        return nullptr;
    }
    ...
    // to CivilCvCAD.so - this should be OS-agnostic, except these two
    // strings, and the call to access().
    const static char libName[] = "/CivilCvCAD.so";
    ...
    if (PyList_Check(pySysPath)) {
        ...
        // backwards since the CivilCvCAD path was likely appended just before
        // we were imported.
        for (i = PyList_Size(pySysPath) - 1; i >= 0; --i) {
            ...
            if (access(path + libName, R_OK | X_OK) == 0) {
                ...
                // The CivilCvCAD "home" path is one level up from
...
# error "Implement: Retrieve the path of the module for your platform."
    ...
    PyObject* module = PyDict_GetItemString(modules, "CivilCvCAD");
    if (!module) {
        PyErr_SetString(PyExc_ImportError, "Failed to load CivilCvCAD module!");
    }
    return module;
}
