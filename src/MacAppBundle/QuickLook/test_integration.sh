...
# CivilCvCAD QuickLook Extensions - Integration Test Script
# This script tests the QuickLook extension integration in CivilCvCAD.app
...
# Look for CivilCvCAD.app in common install locations
if [[ -d "${SCRIPT_DIR}/../../../CivilCvCAD.app" ]]; then
    CIVILCVCAD_APP="${SCRIPT_DIR}/../../../CivilCvCAD.app"
elif [[ -d "${SCRIPT_DIR}/../../../../CivilCvCAD.app" ]]; then
    CIVILCVCAD_APP="${SCRIPT_DIR}/../../../../CivilCvCAD.app"
else
...
EXTENSIONS_DIR="${CIVILCVCAD_APP}/Contents/PlugIns"
THUMBNAIL_EXT="${EXTENSIONS_DIR}/CivilCvCADThumbnailExtension.appex"
PREVIEW_EXT="${EXTENSIONS_DIR}/CivilCvCADPreviewExtension.appex"

THUMBNAIL_BUNDLE_ID="org.civilcvcad.CivilCvCAD.quicklook.thumbnail"
PREVIEW_BUNDLE_ID="org.civilcvcad.CivilCvCAD.quicklook.preview"
...
# Main test function
main() {
    echo "CivilCvCAD QuickLook Extensions - Integration Test"
    echo "=============================================="
    echo "Testing app at: $CIVILCVCAD_APP"
    echo

    local total_tests=0
    local passed_tests=0

    # Test 1: Check if CivilCvCAD.app exists
    ((total_tests++))
    if [[ -d "$CIVILCVCAD_APP" ]]; then
        print_status "OK" "CivilCvCAD.app exists at: $CIVILCVCAD_APP"
        ((passed_tests++))
    else
        print_status "FAIL" "CivilCvCAD.app not found at: $CIVILCVCAD_APP"
        print_status "INFO" "Please build and install CivilCvCAD first with: make install"
        exit 1
    fi

    # Test 2: Check if main CivilCvCAD executable exists
    ((total_tests++))
    if [[ -f "$CIVILCVCAD_APP/Contents/MacOS/CivilCvCAD" ]]; then
        print_status "OK" "CivilCvCAD executable exists"
        ((passed_tests++))
    else
        print_status "FAIL" "CivilCvCAD executable not found"
    fi
    ...
    ((total_tests++))
    if [[ -f "$THUMBNAIL_EXT/Contents/MacOS/CivilCvCADThumbnailExtension" ]]; then
    ...
    ((total_tests++))
    if [[ -f "$PREVIEW_EXT/Contents/MacOS/CivilCvCADPreviewExtension" ]]; then
    ...
    # Test 12: Basic CivilCvCAD launch test
    ((total_tests++))
    print_status "INFO" "Testing CivilCvCAD launch (--version)..."
    if timeout 10 "$CIVILCVCAD_APP/Contents/MacOS/CivilCvCAD" --version >/dev/null 2>&1; then
        print_status "OK" "CivilCvCAD launches successfully"
        ((passed_tests++))
    else
        print_status "FAIL" "CivilCvCAD failed to launch or crashed"
    ...
    # App signing status
    if codesign -v "$CIVILCVCAD_APP" >/dev/null 2>&1; then
        print_status "OK" "CivilCvCAD.app is signed"
    else
        print_status "WARN" "CivilCvCAD.app is unsigned (normal for development builds)"
    fi
    ...
    if command -v codesign >/dev/null 2>&1; then
        ...
        echo "  CivilCvCAD.app:"
        codesign -dv "$CIVILCVCAD_APP" 2>&1 | grep -E "(Identifier|Authority|Signature)" | head -3 | sed 's/^/    /' || echo "    No signature information"
    ...
    if command -v pluginkit >/dev/null 2>&1; then
        ...
        if pluginkit -m -v -i "$THUMBNAIL_BUNDLE_ID" >/dev/null 2>&1; then
            ...
            print_status "WARN" "Thumbnail extension not registered (normal before first successful CivilCvCAD launch)"
        fi

        if pluginkit -m -v -i "$PREVIEW_BUNDLE_ID" >/dev/null 2>&1; then
            print_status "OK" "Preview extension is registered with system"
        else
            print_status "WARN" "Preview extension not registered (normal before first successful CivilCvCAD launch)"
        fi
    fi
    ...
    if command -v spctl >/dev/null 2>&1; then
        if spctl -a -v "$CIVILCVCAD_APP" >/dev/null 2>&1; then
            print_status "OK" "CivilCvCAD.app passes Gatekeeper checks"
        else
            print_status "WARN" "CivilCvCAD.app rejected by Gatekeeper (normal for unsigned development builds)"
            print_status "INFO" "You may need to: sudo xattr -rd com.apple.quarantine '$CIVILCVCAD_APP'"
        fi
    fi

    # Check for quarantine attributes
    if xattr "$CIVILCVCAD_APP" 2>/dev/null | grep -q quarantine; then
        print_status "WARN" "CivilCvCAD.app has quarantine attributes"
        print_status "INFO" "Remove with: sudo xattr -rd com.apple.quarantine '$CIVILCVCAD_APP'"
    else
    ...
    if [[ $passed_tests -eq $total_tests ]]; then
        ...
        echo
        echo "Next Steps:"
        echo "  1. Ensure CivilCvCAD launches successfully to trigger extension registration"
        ...
        echo "  2. Check that CIVILCVCAD_CREATE_MAC_APP=ON in cmake configuration"
        echo "  3. Run 'make install' to build and install CivilCvCAD with QuickLook extensions"
        echo "  4. If CivilCvCAD crashes, try removing quarantine attributes or ad-hoc signing"
        return 1
    fi
}
...
show_usage() {
    ...
    echo
    echo "This script tests the QuickLook extension integration in CivilCvCAD.app."
    echo "Run this after building and installing CivilCvCAD: 'make install'"
    echo
    echo "The script will look for CivilCvCAD.app in common install locations relative to the script."
}
