...
The test is intended to be executed via CivilCvCAD's test runner:

  CivilCvCAD -t TestCoinNodeSnapshots
...
# CivilCvCAD/Coin/Pivy environments; it is a test harness rather than library code.
...
try:
    import CivilCvCADGui  # type: ignore
except Exception as exc:  # pragma: no cover
    CivilCvCADGui = None
...
def _configure_fontconfig_for_visual_fonts(font_dir: Path):
    ...
    tmp = Path(tempfile.mkdtemp(prefix="civilcvcad-visual-fontconfig-"))
    conf = tmp / "fonts.conf"
...
def _register_visual_fonts():
    ...
    # Register for native font backends (important on Windows, where CivilCvCAD forces Coin
...
def _require_gui():
    if CivilCvCAD is None or CivilCvCADGui is None or coin is None:
        message = "Coin node snapshot tests require the CivilCvCAD GUI and Pivy bindings"
    ...
    # `CivilCvCADCmd -t 0` runs the base test suite without X11/Wayland. Creating a Qt application
    ...
    setup_without_gui = getattr(CivilCvCADGui, "setupWithoutGUI", None)
    ...
    home = Path(CivilCvCAD.getHomePath())
    mod_dir = home / "Mod"
    if mod_dir.is_dir():
...
class _ViewerSnapshotHarness:
    def __init__(self, width: int, height: int):
        self.width = width
        self.height = height
        self.previous_document = CivilCvCAD.activeDocument()
        self.doc = None
        self.view = None
        self.viewer = None

        try:
            self.doc = CivilCvCAD.newDocument("CoinNodeSnapshotHarness")
            CivilCvCADGui.ActiveDocument = CivilCvCADGui.getDocument(self.doc.Name)
            CivilCvCADGui.updateGui()
            gui_doc = CivilCvCADGui.getDocument(self.doc.Name)
            if gui_doc is None:
    ...
    def close(self):
        ...
        if self.doc is not None and CivilCvCAD.getDocument(self.doc.Name):
            document_name = self.doc.Name
            CivilCvCAD.closeDocument(document_name)
            self._wait_until(
                lambda: document_name not in CivilCvCAD.listDocuments(),
                f"document {document_name} did not close",
            )
        self.doc = None

        if self.previous_document is not None and CivilCvCAD.getDocument(self.previous_document.Name):
            CivilCvCAD.setActiveDocument(self.previous_document.Name)
            CivilCvCADGui.ActiveDocument = CivilCvCADGui.getDocument(self.previous_document.Name)
...
class CoinNodeSnapshotTestCase(unittest.TestCase):
    ...
    def test_so_datum_label_ignores_parent_cull_face(self):
        ...
        out_dir = Path(
            os.environ.get(
                "FC_VISUAL_OUT_DIR",
                os.path.join(tempfile.gettempdir(), "CivilCvCADTesting", "CoinNodeSnapshots"),
            )
        )
    ...
    def test_so_brep_face_set_partial_render_transparency_regression(self):
        ...
        doc = CivilCvCAD.newDocument("SoBrepFaceSetPartialRenderTransparencyRegression")
        try:
            box = doc.addObject("Part::Box", "Box")
            doc.recompute()
            CivilCvCADGui.setActiveDocument(doc.Name)
            gui_doc = CivilCvCADGui.getDocument(doc.Name) or CivilCvCADGui.activeDocument()
            if gui_doc is None:
            ...
            if getattr(CivilCvCADGui, "Selection", None) is not None:
                CivilCvCADGui.Selection.clearSelection()
            if CivilCvCAD.getDocument(doc.Name):
