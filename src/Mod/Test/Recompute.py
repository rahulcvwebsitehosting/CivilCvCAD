...
def createPartDesignSketch(testClass):
    ...
    def lsegment(x1, y1, x2, y2):
        return Part.LineSegment(CivilCvCAD.Vector(x1, y1, 0), CivilCvCAD.Vector(x2, y2, 0))
    ...
    sketch.setDatum(10, CivilCvCAD.Units.Quantity("32.0 mm"))
...
class FineGrainedRecomputeCases(unittest.TestCase):
    ...
    def setUp(self):
        self.Doc = CivilCvCAD.newDocument(f"RecomputeTests_{self._testMethodName}")
    ...
    def testOutputInternal(self):
        ...
        self.assertEqual(cube1.DoubleLength.Unit, CivilCvCAD.Units.Length)
        self.assertEqual(cube2.Length.Value, 40.0)
        self.assertEqual(cube2.Length.Unit, CivilCvCAD.Units.Length)
