...
class TestMetadata(unittest.TestCase):
    ...
    def test_copy_constructor(self):
        filename = os.path.join(self.test_dir, "basic_metadata.xml")
        md = CivilCvCAD.Metadata(filename)
        copy_of_md = CivilCvCAD.Metadata(md)
...
<package format="1" xmlns="https://wiki.civilcvcad.org/Package_Metadata">
  <name>test</name>
    ...
    def test_content_item_tags(self):
        ...
        for workbench in workbenches:
            ...
            if workbench.Classname == "TestWorkbenchA":
                found[0] = True
                self.assertEqual(workbench.CivilCvCADMin, "0.20.0")
                tags = workbench.Tag
                self.assertTrue("Tag A" in tags)
            elif workbench.Classname == "TestWorkbenchB":
                found[1] = True
                self.assertEqual(workbench.CivilCvCADMin, "0.1.0")
                self.assertEqual(workbench.CivilCvCADMax, "0.19.0")
                tags = workbench.Tag
