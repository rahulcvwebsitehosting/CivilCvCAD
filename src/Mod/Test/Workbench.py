...
import CivilCvCAD, CivilCvCADGui, os, unittest
import tempfile
...
class WorkbenchTestCase(unittest.TestCase):
    def setUp(self):
        self.Active = CivilCvCADGui.activeWorkbench()
        CivilCvCAD.Console.PrintLog(CivilCvCADGui.activeWorkbench().name())

    def testActivate(self):
        wbs = CivilCvCADGui.listWorkbenches()
        # this gives workbenches a possibility to detect that we're under test environment
        CivilCvCAD.TestEnvironment = True
        for i in wbs:
            try:
                print("Activate workbench '{}'".format(i))
                cobj = CallableCheckWarning()
                QtCore.QTimer.singleShot(500, cobj)
                if CivilCvCADGui.activeWorkbench().name() != i:
                    success = CivilCvCADGui.activateWorkbench(i)
                else:
                    # Cannot test activation of an already-active workbench
                    success = True
                CivilCvCAD.Console.PrintLog(
                    "Active: " + CivilCvCADGui.activeWorkbench().name() + " Expected: " + i + "\n"
                )
        ...
        del CivilCvCAD.TestEnvironment
    ...
    def testInvalidType(self):
        ...
        CivilCvCADGui.addWorkbench(MyExtWorkbench())
        with self.assertRaises(TypeError):
            CivilCvCADGui.activateWorkbench("MyExtWorkbench")
        CivilCvCADGui.removeWorkbench("MyExtWorkbench")

    def tearDown(self):
        CivilCvCADGui.activateWorkbench(self.Active.name())
        CivilCvCAD.Console.PrintLog(self.Active.name())
...
class CommandTestCase(unittest.TestCase):
    def testPR6889(self):
        ...
        name = CivilCvCADGui.Command.createCustomCommand(macroName)
        cmd = CivilCvCADGui.Command.get(name)
        cmd.run()
...
class TestNavigationStyle(unittest.TestCase):
    def setUp(self):
        ...
        self.View = CivilCvCADGui.getDocument(self.Doc).ActiveView
    ...
    def testOrientationLockAllowsTranslationOnlyViewPosition(self):
        ...
        target = CivilCvCAD.Placement(
            CivilCvCAD.Vector(initial.Base.x + 10.0, initial.Base.y + 20.0, initial.Base.z + 30.0),
            initial.Rotation,
        )
