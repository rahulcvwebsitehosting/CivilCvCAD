...
This module is intentionally not registered in CivilCvCAD.__unit_test__, so it is
only run when requested explicitly.

Examples:
    CivilCvCAD src/Mod/Test/RunAutoSaverStress.py
    CivilCvCAD src/Mod/Test/RunAutoSaverStress.py documents=3 objects=150
    CivilCvCAD src/Mod/Test/RunAutoSaverStress.py iterations=25 burst=10 timeout=5.0
"""
...
class TestAutoSaverStress(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for index in range(self.document_count):
            document = CivilCvCAD.newDocument(f"AutoSaverStressDoc{index}")
            self.assertIsNotNone(CivilCvCADGui.getDocument(document.Name))
            objects = [
    ...
    def tearDown(self):
        for document, _objects in reversed(self.documents):
            if CivilCvCAD.getDocument(document.Name):
                CivilCvCAD.closeDocument(document.Name)
