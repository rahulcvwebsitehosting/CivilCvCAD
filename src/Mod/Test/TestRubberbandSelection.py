...
To run tests:
    CivilCvCAD -t TestRubberbandSelection.TestRubberbandSelection
"""
...
class TestRubberbandSelection(unittest.TestCase):
    ...
    def setUp(self):
        self.doc = CivilCvCAD.newDocument("TestRubberbandSelection")
        ...
        self.view_preferences = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View")
    ...
    def test_ctrl_drag_adds_in_supported_styles(self):
        ...
        for label, style in self.ADDITIVE_DRAG_STYLES:
            with self.subTest(style=label):
                CivilCvCADGui.Selection.clearSelection()
                CivilCvCADGui.Selection.addSelection(self.doc.Name, self.right_box.Name)
                self.view.setNavigationType(style)
                self._refresh_view()
    ...
    def test_annotation_label_corner_drag_reaches_dragger(self):
        ...
        label.BasePosition = CivilCvCAD.Vector(0, 0, 0)
        label.TextPosition = CivilCvCAD.Vector(0, 0, 0)
        self.doc.recompute()
    ...
    def test_cubic_bezier_drag_release_is_not_stolen_when_selection_is_disabled(self):
        ...
        CivilCvCADGui.activateWorkbench("DraftWorkbench")
        CivilCvCADGui.ActiveDocument = CivilCvCADGui.getDocument(self.doc.Name)
        self.view = CivilCvCADGui.ActiveDocument.ActiveView
        self.viewer = self.view.getViewer()
    ...
    def _ensure_selection_objects(self):
        ...
        left_shape = Part.makeBox(6, 6, 6, CivilCvCAD.Vector(-20, -3, 0))
        right_shape = Part.makeBox(6, 6, 6, CivilCvCAD.Vector(20, -3, 0))
    ...
    def _selected_names(self):
        return {obj.Name for obj in CivilCvCADGui.Selection.getSelection()}
