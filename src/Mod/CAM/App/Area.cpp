...
static std::vector<gp_Pnt> discretize(const TopoDS_Edge& edge, double deflection)
{
    ...
    // https://forum.civilcvcad.org/viewtopic.php?f=15&t=42628
    //
...
// See https://github.com/realthunder/CivilCvCAD/blob/LinkStable/src/Mod/Part/App/WireJoiner.h for the
// original implementation of the class and https://github.com/CivilCvCAD/CivilCvCAD/pull/12535 for the
// import conversation.
struct WireJoiner
{
            ...
            for (auto it = myShapes.begin(); it != myShapes.end(); ++it) {
                ...
                for (TopExp_Explorer xp(s.shape.Moved(loc), TopAbs_SOLID); xp.More(); xp.Next()) {
                    ...
                    Part::FuzzyHelper::withBooleanFuzzy(.0, [&]() {
                        // Workaround for https://github.com/CivilCvCAD/CivilCvCAD/issues/17748
