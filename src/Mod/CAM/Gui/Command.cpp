...
void CmdPathArea::activated(int iMsg)
{
    ...
    for (const Gui::SelectionObject& selObj :
        ...
        if (subnames.empty()) {
            ...
            sources << "CivilCvCAD.activeDocument()." << pcObj->getNameInDocument() << ",";
            continue;
        }
        for (const std::string& name : subnames) {
            ...
            cmd << "CivilCvCAD.activeDocument().addObject('Part::Feature','" << sub_fname
                << "').Shape = PathCommands.findShape(CivilCvCAD.activeDocument()."
            ...
            sources << "CivilCvCAD.activeDocument()." << sub_fname << ",";
        }
    }
    if (addView && !areaName.empty()) {
        ...
        doCommand(
            Doc,
            "CivilCvCAD.activeDocument().addObject('Path::FeatureAreaView','%s')",
            FeatName.c_str()
        );
        doCommand(
            Doc,
            "CivilCvCAD.activeDocument().%s.Source = CivilCvCAD.activeDocument().%s",
            FeatName.c_str(),
            areaName.c_str()
        );
        commitCommand();
    ...
    doCommand(Doc, "CivilCvCAD.activeDocument().addObject('Path::FeatureArea','%s')", FeatName.c_str());
    doCommand(
        Doc,
        "CivilCvCAD.activeDocument().%s.Sources = [ %s ]",
        FeatName.c_str(),
        sources.str().c_str()
    );
    commitCommand();
...
void CmdPathAreaWorkplane::activated(int iMsg)
{
    ...
    doCommand(
        Doc,
        "CivilCvCAD.activeDocument().%s.WorkPlane = PathCommands.findShape("
        "CivilCvCAD.activeDocument().%s)",
        areaName.c_str(),
        planeSubname.c_str()
    );
    doCommand(Doc, "CivilCvCAD.activeDocument().%s.ViewObject.Visibility = True", areaName.c_str());
    commitCommand();
    updateActive();
}
    ...
    if (!Sel.empty()) {
        ...
        for (std::vector<Gui::SelectionSingleton::SelObj>::const_iterator it = Sel.begin();
            ...
            if ((*it).pObject->isDerivedFrom<Path::Feature>()) {
                ...
                cmd << "CivilCvCAD.activeDocument()." << pcPathObject->getNameInDocument() << ",";
            }
            else {
                Base::Console().error(
        ...
        doCommand(
            Doc,
            "CivilCvCAD.activeDocument().addObject('Path::FeatureCompound','%s')",
            FeatName.c_str()
        );
        doCommand(Doc, "CivilCvCAD.activeDocument().%s.Group = %s", FeatName.c_str(), cmd.str().c_str());
        commitCommand();
        updateActive();
    }
    else {
...
void CmdPathShape::activated(int iMsg)
{
    ...
    doCommand(Doc, "CivilCvCAD.activeDocument().addObject('Path::FeatureShape','%s')", FeatName.c_str());
    doCommand(
        Doc,
        "CivilCvCAD.activeDocument().%s.Sources = [ %s ]",
        FeatName.c_str(),
