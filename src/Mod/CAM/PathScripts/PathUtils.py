...
def getEnvelope(partshape, subshape=None, depthparams=None):
    ...
    newPlace = CivilCvCAD.Placement(Vector(0, 0, zShift), sec.Placement.Rotation)
    ...
    if Path.Log.getLevel(Path.Log.thisModule()) == Path.Log.Level.DEBUG:
        removalshape = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "Envelope")
...
def getOffsetArea(
    ...
    https://github.com/sliptonic/CivilCvCAD/wiki/PathArea-notes."""
...
def applyPlacementToPath(placement, path):
    ...
    for cmd in path.Commands:
        if cmd.Name in Path.Geom.CmdMoveAll:
            ...
            x, y, z = placement.Rotation.multVec(CivilCvCAD.Vector(x, y, z))
            ...
            if cmd.Name in Path.Geom.CmdMoveArc:
                ...
                i, j, k = placement.Rotation.multVec(CivilCvCAD.Vector(i, j, 0))
