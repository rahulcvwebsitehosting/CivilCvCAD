...
def find_shape_object(doc: "CivilCvCAD.Document") -> Optional["CivilCvCAD.DocumentObject"]:
    """
    ...
    Args:
        doc (CivilCvCAD.Document): The document to search within.

    Returns:
        Optional[CivilCvCAD.DocumentObject]: The found object or None.
    """
    obj = None
    # Prioritize Body
...
def get_unset_value_for(attribute_type: str):
    if attribute_type == "App::PropertyLength":
        return CivilCvCAD.Units.Quantity(0)
...
def get_object_properties(
    ...
    """
    Extract properties from a CivilCvCAD PropertyBag, including their types.
    ...
    Returns:
        Dict[str, Tuple[Any, str]]: A dictionary mapping property names to a tuple
                                    (value, type_id). Values are CivilCvCAD native types.
...
def update_shape_object_properties(
    obj: "CivilCvCAD.DocumentObject", parameters: Dict[str, Any]
) -> None:
    """
    Update properties of a CivilCvCAD PropertyBag based on a dictionary of parameters.

    Args:
        obj (CivilCvCAD.DocumentObject): The PropertyBag to update properties on.
...
def get_doc_state() -> Any:
    """
    Used to make a "snapshot" of the current state of CivilCvCAD, to allow
    for restoring the ActiveDocument and selection state later.
    """
    doc_name = CivilCvCAD.ActiveDocument.Name if CivilCvCAD.ActiveDocument else None
    if CivilCvCAD.GuiUp:
...
def restore_doc_state(state):
    ...
    if CivilCvCAD.GuiUp:
        ...
        for sel in selection:
            CivilCvCADGui.Selection.addSelection(doc_name, sel.Name)


class ShapeDocFromBytes:
    """
    Context manager to create and manage a temporary CivilCvCAD document,
    ...
    def __enter__(self) -> "CivilCvCAD.Document":
        """Creates a new temporary CivilCvCAD document or loads cache if provided."""
        ...
        # When we open a new document, CivilCvCAD loses the state, of the active
