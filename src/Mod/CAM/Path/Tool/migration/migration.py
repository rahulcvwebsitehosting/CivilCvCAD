...
Handles migration of CAM assets during CivilCvCAD version upgrades.
"""
...
class CAMAssetMigrator:
    ...
    def check_asset_location(self):
        ...
        not been offered for the current CivilCvCAD version.
        """
        Path.Log.debug("Starting CAM asset migration check")

        try:
            # Get current directories
            user_app_data_dir = CivilCvCAD.getUserAppDataDir()
            ...
            if CivilCvCAD.ApplicationDirectories.isVersionedPath(str(current_asset_path)):
                # Check if we're already using the current version
                if CivilCvCAD.ApplicationDirectories.usingCurrentVersionConfig(
    ...
    def _offer_asset_relocation(self):
        ...
        major = int(CivilCvCAD.ConfigGet("BuildVersionMajor"))
        minor = int(CivilCvCAD.ConfigGet("BuildVersionMinor"))
        current_version = CivilCvCAD.ApplicationDirectories.versionStringForPath(major, minor)
        ...
        msg = (
            f"CivilCvCAD has been upgraded to version {current_version}.\n\n"
        ...
        pref_group = CivilCvCAD.ParamGet(self.pref_group_path)
    ...
    def _migrate_assets(self, source_path):
        ...
        try:
            CivilCvCAD.ApplicationDirectories.migrateAllPaths([source_path])
            Path.Log.info(
    ...
    def has_migration_been_offered(self):
        ...
        current_version_string = CivilCvCAD.ApplicationDirectories.versionStringForPath(major, minor)
        offered_versions = pref_group.GetString("OfferedToMigrateCAMAssets", "")
        known_versions = set(offered_versions.split(",")) if offered_versions else set()
