...
def units_from_json(params):
    ...
    Args:
        params: Dictionary of parameters from JSON (before conversion to CivilCvCAD.Units.Quantity)
...
def format_value(
    value: CivilCvCAD.Units.Quantity | int | float | None,
    ...
    This function uses the ToolBitSchema (via setToolBitSchema) to ensure that units are formatted according to the correct schema (Metric or Imperial) when a CivilCvCAD.Units.Quantity is provided. The schema is temporarily set for formatting and then restored.
    ...
    if value is None:
        return None
    elif isinstance(value, CivilCvCAD.Units.Quantity):
...
def setToolBitSchema(schema=None):
    """
    Set the CivilCvCAD units schema. If passed 'Metric' or 'Imperial', set accordingly (case-insensitive).
    Otherwise, if a document is open, set to its schema. If no document, fallback to user preference or provided schema.
    """
    units_schema_map = {
        "metric": 6,  # 6 = Metric schema in CivilCvCAD
        "imperial": 3,  # 3 = Imperial schema in CivilCvCAD
    }
    if isinstance(schema, str) and schema.lower() in units_schema_map:
        CivilCvCAD.Units.setSchema(units_schema_map[schema.lower()])
        return
    if CivilCvCAD.ActiveDocument is not None:
        try:
            doc_schema = CivilCvCAD.ActiveDocument.getSchema()
            CivilCvCAD.Units.setSchema(doc_schema)
            return
        except Exception:
            pass
    # Fallback to user preference or provided schema
    if schema is None:
        schema = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt(
            "UserSchema", 6
        )
    CivilCvCAD.Units.setSchema(schema)
