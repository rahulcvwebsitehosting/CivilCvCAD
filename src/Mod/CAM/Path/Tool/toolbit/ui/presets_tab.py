...
# expresses it in ft/min (imperial) or m/min (metric) — not what CivilCvCAD's
...
def _is_imperial_length() -> bool:
    ...
    try:
        unit = CivilCvCAD.Units.Quantity(1.0, "mm").getUserPreferred()[2]
    except Exception:
        return False
...
def _preferred_unit(unit_expr: str) -> str:
    ...
    """
    try:
        pref = CivilCvCAD.Units.Quantity(1.0, unit_expr).getUserPreferred()
...
def _fmt_quantity(value: Optional[float], unit_expr: str) -> str:
    ...
    if value is None:
        return ""
    try:
        return CivilCvCAD.Units.Quantity(float(value), unit_expr).UserString
    except Exception:
        return _fmt(value)
...
class _EditPresetDialog:
    ...
    def __init__(
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/FeedsSpeedsPresetEdit.ui")
        ...
        # Horiz feed (velocity) and chipload (length) map onto CivilCvCAD's
