...
class ParameterAccessor:
    """
    Unified accessor for dicts and CivilCvCAD objects for migration logic.
    """
    ...
    def get_shape_type(self):
        if self.is_dict:
            ...
            # For CivilCvCAD objects, use ShapeType attribute
...
def migrate_parameters(accessor: ParameterAccessor) -> bool:
    ...
    Args:
        accessor: ParameterAccessor instance wrapping dict or CivilCvCAD object
    ...
    if shape_type and str(shape_type).lower() == "bullnose":
        ...
        if has_flat and has_diam and not has_corner and not has_torus:
            try:
                ...
                diameter = CivilCvCAD.Units.Quantity(diam_raw)
                flat_radius = CivilCvCAD.Units.Quantity(flat_raw)
                corner_radius = (float(diameter) / 2.0) - float(flat_radius)

                # Convert to correct unit
                if isinstance(diam_raw, str) and diam_raw.strip().endswith("in"):
                    cr_in = CivilCvCAD.Units.Quantity(f"{corner_radius} mm").getValueAs("in")
                    value = f"{float(cr_in):.4f} in"
                else:
                    if isinstance(diam_raw, str) and not diam_raw.strip().endswith("mm"):
                        cr_mm = CivilCvCAD.Units.Quantity(f"{corner_radius} in").getValueAs("mm")
