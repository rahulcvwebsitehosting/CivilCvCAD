...
class ToolBitCustom(ToolBit):
    ...
    def summary(self) -> str:
        return CivilCvCAD.Qt.translate("CAM", "Unknown custom toolbit type")

    # Connor: Adding in getters and setters for diameter and length
    def get_diameter(self) -> CivilCvCAD.Units.Quantity:
        """
        Get the diameter of the rotary tool bit from the shape.
        """
        return self.obj.Diameter

    def set_diameter(self, diameter: CivilCvCAD.Units.Quantity):
        """
        Set the diameter of the rotary tool bit on the shape.
        """
        if not isinstance(diameter, CivilCvCAD.Units.Quantity):
            raise ValueError("Diameter must be a CivilCvCAD Units.Quantity")
        self.obj.Diameter = diameter

    def get_length(self) -> CivilCvCAD.Units.Quantity:
        """
        Get the length of the rotary tool bit from the shape.
        """
        return self.obj.Length

    def set_length(self, length: CivilCvCAD.Units.Quantity):
        """
        Set the length of the rotary tool bit on the shape.
        """
        if not isinstance(length, CivilCvCAD.Units.Quantity):
            raise ValueError("Length must be a CivilCvCAD Units.Quantity")
