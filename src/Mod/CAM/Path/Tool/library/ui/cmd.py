...
class CommandLibraryEditorOpen:
    ...
    def Activated(self):
        library = LibraryEditor(parent=CivilCvCADGui.getMainWindow())
        library.open()


if CivilCvCAD.GuiUp:
    CivilCvCADGui.addCommand("CAM_ToolBitLibraryOpen", CommandLibraryEditorOpen())
    CivilCvCADGui.addCommand("CAM_ToolBitDock", CommandToolBitLibraryDockOpen())

BarList = ["CAM_ToolBitDock"]
MenuList = ["CAM_ToolBitLibraryOpen", "CAM_ToolBitDock"]

CivilCvCAD.Console.PrintLog("Loading PathToolBitLibraryCmd… done\n")
