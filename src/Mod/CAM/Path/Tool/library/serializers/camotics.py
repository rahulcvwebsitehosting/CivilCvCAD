...
class CamoticsLibrarySerializer(AssetSerializer):
    ...
    def get_label(cls) -> str:
        return CivilCvCAD.Qt.translate("CAM", "Camotics Tool Library")
    ...
    def serialize(cls, asset: Asset) -> bytes:
        ...
        for tool_no, tool in asset._bit_nos.items():
            ...
            if isinstance(diameter_value, CivilCvCAD.Units.Quantity):
                try:
            ...
            if isinstance(length_value, CivilCvCAD.Units.Quantity):
                try:
    ...
    def deserialize(
        ...
        for tool_no_str, toolitem in data_dict.items():
            ...
            # Translate parameters to CivilCvCAD types
            params = {}
            try:
                diameter = float(toolitem.get("diameter", 2))
                params["Diameter"] = CivilCvCAD.Units.Quantity(f"{diameter} mm")
            except (ValueError, TypeError):
                print(f"Warning: Invalid diameter for tool {tool_no_str}, skipping.")

            try:
                length = float(toolitem.get("length", 10))
                params["Length"] = CivilCvCAD.Units.Quantity(f"{length} mm")
            except (ValueError, TypeError):
