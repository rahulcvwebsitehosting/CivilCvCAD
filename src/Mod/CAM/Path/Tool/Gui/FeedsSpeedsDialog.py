...
Adapters convert CivilCvCAD doc objects into the resolver's frozen dataclass
...
def _surface_speed_m_per_min(props, key: str) -> Optional[float]:
    ...
    if not raw:
        return None
    try:
        return float(CivilCvCAD.Units.Quantity(raw).getValueAs("m/min"))
...
def doc_unit_schema(obj) -> int:
    ...
    try:
        ...
        pass
    try:
        return CivilCvCAD.Units.getSchema()
    except Exception:
        return 0
...
def _format_feed(value: Optional[float], schema: int) -> str:
    ...
    if value is None:
        return "—"
    try:
        quantity = CivilCvCAD.Units.Quantity(float(value), "mm/min")
        return CivilCvCAD.Units.schemaTranslate(quantity, schema)[0]
    except Exception:
