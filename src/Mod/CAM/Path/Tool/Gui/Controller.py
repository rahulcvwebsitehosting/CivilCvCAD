    ...
    def selectedJob(self):
        if CivilCvCAD.ActiveDocument:
            ...
            if sel and sel[0].Object.Name[:3] == "Job":
                return sel[0].Object
        jobs = [o for o in CivilCvCAD.ActiveDocument.Objects if o.Name[:3] == "Job"]
        if 1 == len(jobs):
            return jobs[0]
        return None
...
class ToolControllerEditor(object):
    ...
    def __init__(
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/DlgToolControllerEdit.ui")
        self.controller = CivilCvCADGui.PySideUic.loadUi(":/panels/ToolControllerEdit.ui")
...
class TaskPanel:
    ...
    def accept(self):
        ...
        CivilCvCADGui.Control.closeDialog()
        if self.toolrep:
            CivilCvCAD.ActiveDocument.removeObject(self.toolrep.Name)
        CivilCvCAD.ActiveDocument.recompute()
    ...
    def setupUi(self):
        if self.editor.editor:
            ...
            self.toolrep = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tool")
            self.toolrep.Shape = t
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_ToolController", CommandPathToolController())

CivilCvCAD.Console.PrintLog("Loading PathToolControllerGui… done\n")
