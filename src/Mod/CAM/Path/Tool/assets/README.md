...
updating, deleting, and receiving assets for the CivilCvCAD CAM workbench.
...
Assets are arbitrary data, such as CivilCvCAD models, Tools, and many more.
...
and a ToolBit requires a ToolBitShape (which is a CivilCvCAD model).
...
- **Manages storage** while existing CivilCvCAD tool library file structures retained
- **Manages dependencies** including detection of cyclic dependencies, deep vs. shallow fetching
- **Manages threading** for asynchronous storage, while CivilCvCAD objects are assembled in the main UI thread
...
- Adding a GitStore, to connect to things like the [CivilCvCAD library](https://github.com/CivilCvCAD/CivilCvCAD-library).
