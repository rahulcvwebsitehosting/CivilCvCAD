...
__url__ = ""
__doc__ = "Helper class to use CivilCvCADGUi.Snapper to let the user enter arbitrary points while the task panel is active."
...
class TaskPanel:
    ...
    def __init__(self, form, onPath=False):
        ...
        self.formOrig = form
        self.formPoint = CivilCvCADGui.PySideUic.loadUi(":/panels/PointEdit.ui")
    ...
    def setupUi(self):
        ...
        self.formPoint.XGlobal.setProperty("unit", CivilCvCAD.Units.MilliMetre.getUserPreferred()[2])
        self.formPoint.YGlobal.setProperty("unit", CivilCvCAD.Units.MilliMetre.getUserPreferred()[2])
        self.formPoint.ZGlobal.setProperty("unit", CivilCvCAD.Units.MilliMetre.getUserPreferred()[2])
    ...
    def getPoint(self, whenDone, start=None):
        ...
        def mouseMove(cb):
            ...
            if self.onPath:
                ...
                if snapInfo:
                    obj = CivilCvCAD.ActiveDocument.getObject(snapInfo["Object"])
                    if hasattr(obj, "Path"):
                        self.obj = obj
                        p = CivilCvCAD.Vector(snapInfo["x"], snapInfo["y"], snapInfo["z"])
                        self.pt = p
                    else:
                        self.obj = None
            else:
                ...
                self.pt = CivilCvCADGui.Snapper.snap(
                    pos, lastpoint=start, active=cntrl, constrain=shift
                )
                plane = CivilCvCAD.DraftWorkingPlane
                p = plane.getLocalCoords(self.pt)
                self.obj = CivilCvCADGui.Snapper.lastSnappedObject
        ...
        if start:
            ...
            displayPoint(CivilCvCAD.Vector(0, 0, 0))
        ...
        CivilCvCADGui.Snapper.forceGridOff = True
    ...
    def updatePoint(self, usePoint=True):
        ...
        if usePoint and self.point:
            ...
            x = CivilCvCAD.Units.Quantity(self.formPoint.XGlobal.text()).Value
            y = CivilCvCAD.Units.Quantity(self.formPoint.YGlobal.text()).Value
            z = CivilCvCAD.Units.Quantity(self.formPoint.ZGlobal.text()).Value
            self.pt = CivilCvCAD.Vector(x, y, z)
