        ...
        if hasattr(obj, "ExpressionEngine"):
            ...
            for prp, expr in obj.ExpressionEngine:
                if prp == prop:
                    ...
                    value = CivilCvCAD.Units.Quantity(obj.evalExpression(expr)).Value
                    if not Path.Geom.isRoughly(attrValue, value):
                        isDiff = True
                    break
            if exprSet:
...
class QuantitySpinBox(QtCore.QObject):
    ...
    def updateWidget(self, quantity=None):
        ...
        if self.valid:
            ...
            if quantity is None:
                if expr:
                    quantity = CivilCvCAD.Units.Quantity(self.obj.evalExpression(expr))
                else:
...
def getDocNode():
    doc = CivilCvCADGui.ActiveDocument.Document.Name
    tws = CivilCvCADGui.getMainWindow().findChildren(QtGui.QTreeWidget)
