...
class _PropertyAngleEditor(_PropertyEditor):
    ...
    def setEditorData(self, widget):
        ...
        if quantity is None:
            quantity = CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Angle)
        widget.setText(quantity.getUserPreferred()[0])

    def setModelData(self, widget):
        self.prop.setValue(CivilCvCAD.Units.Quantity(widget.text()))
...
class _PropertyLengthEditor(_PropertyEditor):
    ...
    def setEditorData(self, widget):
        quantity = self.prop.getValue()
        if quantity is None:
            quantity = CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length)
        widget.setText(quantity.getUserPreferred()[0])
