...
def ensure_polygon(wire, num_segments=64):
    ...
    if points[0].distanceToPoint(points[-1]) > 1e-6:
        points.append(CivilCvCAD.Vector(points[0]))
...
def select_starting_corner(corners, primary_vec, step_vec, milling_direction):
    ...
    Args:
        ...
        primary_vec (CivilCvCAD.Vector): Primary direction vector (normalized)
        step_vec (CivilCvCAD.Vector): Step direction vector (normalized)
        milling_direction (str): "climb" or "conventional"

    Returns:
        CivilCvCAD.Vector: The selected starting corner point
    """
    if len(corners) < 4:
        return corners[0]
...
def get_angled_polygon(wire, angle):
    ...
    rotation_axis = CivilCvCAD.Vector(0, 0, 1)  # Z-axis
    ...
    corners = [
        CivilCvCAD.Vector(bounding_box.XMin, bounding_box.YMin, bounding_box.ZMin),
        CivilCvCAD.Vector(bounding_box.XMax, bounding_box.YMin, bounding_box.ZMin),
        CivilCvCAD.Vector(bounding_box.XMax, bounding_box.YMax, bounding_box.ZMin),
        CivilCvCAD.Vector(bounding_box.XMin, bounding_box.YMax, bounding_box.ZMin),
    ]
...
def validate_inputs(
    ...
    if start_point is not None:
        if not hasattr(start_point, "x") or not hasattr(start_point, "y"):
            raise ValueError("Start point must be a CivilCvCAD.Vector with x and y coordinates")
...
def align_edges_to_angle(primary_vec, step_vec, primary_length, step_length, angle_degrees):
    ...
    dir_vec = CivilCvCAD.Vector(math.cos(rad), math.sin(rad), 0)
...
def unit_vectors_from_angle(angle_degrees):
    ...
    rad = math.radians(angle_degrees)
    p = CivilCvCAD.Vector(math.cos(rad), math.sin(rad), 0)
    # +90° rotation
    s = CivilCvCAD.Vector(-math.sin(rad), math.cos(rad), 0)
    return p, s
...
def slice_wire_segments(wire, primary_vec, step_vec, t, origin):
    ...
    # Iterate edges
    for edge in wire.Edges:
        A = CivilCvCAD.Vector(edge.Vertexes[0].Point)
        B = CivilCvCAD.Vector(edge.Vertexes[1].Point)
