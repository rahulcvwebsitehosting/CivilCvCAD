...
def generate(
    ...
    startPoint = CivilCvCAD.Vector(cmd.x, cmd.y, cmd.z)
        or
    startPoint = cmd.Placement.Base
    """

    if not isinstance(center, CivilCvCAD.Vector):
    ...
    def createInitMove(radius, cmdName="G1"):
        ...
        p = CivilCvCAD.Vector(center.x + dx, center.y + dy, center.z)
    ...
    def createCircle(radius, arcCmdName):
        ...
        dx = radius * math.cos(dir_angle_rad)
        dy = radius * math.sin(dir_angle_rad)
        p1 = CivilCvCAD.Vector(center.x - dx, center.y - dy, center.z)
        p2 = CivilCvCAD.Vector(center.x + dx, center.y + dy, center.z)
    ...
    while count <= iters:
        ...
        z = center.z
        arcPoints.append(CivilCvCAD.Vector(x, y, z))  # Get all points of arcs
        if count == iters:
