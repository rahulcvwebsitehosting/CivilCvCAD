...
def generate(
    ...
    def helix_cone(bottomRadius):
        ...
        while count <= iters:
            ...
            arcPoints.append(CivilCvCAD.Vector(arcX, arcY, arcZ))  # Get all points of arcs
            if count == iters:
                break
            angle += stepRotate
            arcR -= stepRadius
        ...
        if finish_circle:
            ...
            p1 = CivilCvCAD.Vector(topCenterPoint.x - dx, topCenterPoint.y - dy, 0)
            commandlist.append(Path.Command(arcCmdName, {"X": p1.x, "Y": p1.y, "I": -dx, "J": -dy}))
            p2 = CivilCvCAD.Vector(topCenterPoint.x + dx, topCenterPoint.y + dy, 0)
