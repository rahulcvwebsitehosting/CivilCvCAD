...
def directional(
    ...
    if kept_segments == 0:
        ...
        for t in t_candidates:
            ...
            sp = (
                CivilCvCAD.Vector(origin)
                .add(CivilCvCAD.Vector(primary_vec).multiply(p_start))
                .add(CivilCvCAD.Vector(step_vec).multiply(t))
            )
            ep = (
                CivilCvCAD.Vector(origin)
                .add(CivilCvCAD.Vector(primary_vec).multiply(p_end))
                .add(CivilCvCAD.Vector(step_vec).multiply(t))
            )
            sp.z = z
            ep.z = z
