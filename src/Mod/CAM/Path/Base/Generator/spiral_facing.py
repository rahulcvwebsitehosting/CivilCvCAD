...
def generate_spiral_corners(
    ...
    inward_primary = CivilCvCAD.Vector(primary_vec).multiply(inward_offset)
    inward_step = CivilCvCAD.Vector(step_vec).multiply(inward_offset)

    # The actual starting corner for this layer is offset inward
    layer_start_corner = CivilCvCAD.Vector(start_corner).add(inward_primary).add(inward_step)

    # Build rectangle from the offset starting corner with reduced dimensions
    corner1 = CivilCvCAD.Vector(layer_start_corner)
    corner2 = CivilCvCAD.Vector(corner1).add(
        CivilCvCAD.Vector(primary_vec).multiply(adjusted_primary_length)
    )
    corner3 = CivilCvCAD.Vector(corner2).add(CivilCvCAD.Vector(step_vec).multiply(adjusted_step_length))
    corner4 = CivilCvCAD.Vector(corner3).add(
        CivilCvCAD.Vector(primary_vec).multiply(-adjusted_primary_length)
    )
...
def spiral(
    ...
    primary_vec = CivilCvCAD.Vector(primary_vec).normalize()
    step_vec = CivilCvCAD.Vector(step_vec).normalize()
