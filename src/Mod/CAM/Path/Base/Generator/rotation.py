...
def compute_rotation_matrix(chain: List[RotaryAxis], angles: Dict[str, float]) -> CivilCvCAD.Rotation:
    ...
    Returns:
        CivilCvCAD.Rotation representing cumulative transformation
    """
    if not chain:
    ...
    total_rotation = CivilCvCAD.Rotation()

    for axis in ordered:
        if axis.name in angles:
            rot = CivilCvCAD.Rotation(axis.rotation_vector, angles[axis.name])
            total_rotation = rot.multiply(total_rotation)

    return total_rotation


def _relAngle(vec: CivilCvCAD.Vector, ref_axis: CivilCvCAD.Vector, plane_normal: CivilCvCAD.Vector) -> float:
    ...
    """
    norm = CivilCvCAD.Vector(vec)  # copy
    norm.projectToPlane(CivilCvCAD.Vector(0, 0, 0), plane_normal)

    if norm.Length < 1e-12:
        return 0.0

    rot = CivilCvCAD.Rotation(norm, ref_axis)
...
def _decompose_2axis(
    first_axis: RotaryAxis, second_axis: RotaryAxis, desired_axis: CivilCvCAD.Vector
) -> List[Dict[str, float]]:
    ...
    z_target = CivilCvCAD.Vector(0, 0, 1)
    ...
    for first_val in first_valid:
        ...
        first_rot = CivilCvCAD.Rotation(first_axis.rotation_vector, first_val)
...
def _solve_analytical_2axis(
    chain: List[RotaryAxis], desired_axis: CivilCvCAD.Vector, current_state: Dict[str, float]
) -> List[Dict[str, float]]:
...
def _get_relangle_params(rotation_vector: CivilCvCAD.Vector) -> Tuple[CivilCvCAD.Vector, CivilCvCAD.Vector]:
    ...
    if abs(rotation_vector.x) > 0.9:
        return CivilCvCAD.Vector(0, 0, 1), CivilCvCAD.Vector(1, 0, 0)
    elif abs(rotation_vector.y) > 0.9:
        return CivilCvCAD.Vector(0, 0, 1), CivilCvCAD.Vector(0, 1, 0)
    elif abs(rotation_vector.z) > 0.9:
        return CivilCvCAD.Vector(0, 1, 0), CivilCvCAD.Vector(0, 0, 1)
    else:
        # Non-standard axis - best effort
        return CivilCvCAD.Vector(0, 0, 1), rotation_vector.normalize()
...
def solve_orientation(
    machine: Machine,
    desired_tool_axis: CivilCvCAD.Vector,
    ...
    for candidate in candidates:
        ...
        if len(chain) >= 2:
            ...
            if all_table or all_head:
                ...
                if all_head:
                    achieved = rot.multVec(CivilCvCAD.Vector(0, 0, 1))
                    target = desired_tool_axis
                else:
                    achieved = rot.multVec(desired_tool_axis)
                    target = CivilCvCAD.Vector(0, 0, 1)
                ...
                for first_ax, second_ax in [(chain[0], chain[1]), (chain[1], chain[0])]:
                    ...
                    first_rot = CivilCvCAD.Rotation(first_ax.rotation_vector, candidate[first_ax.name])
