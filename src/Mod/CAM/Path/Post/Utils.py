...
class FilenameGenerator:
    ...
    def get_path_and_filename_default(self):
        ...
        if not filename:
            filename = CivilCvCAD.ActiveDocument.Label

        if not outputpath:
            outputpath, _ = os.path.split(CivilCvCAD.ActiveDocument.getFileName())

        if not outputpath:
            outputpath = (
                os.getcwd()
            )  ## TODO: This should be avoided as it gives the CivilCvCAD executable's path in some systems (e.g. Windows)
    ...
    def _apply_path_substitutions(self, file_path):
        ...
        substitutions = {
            ...
            "%j": self.job.Label,
            "%M": os.path.dirname(CivilCvCAD.getUserMacroDir()),
        }
...
class GCodeEditorDialog(QtGui.QDialog):
    def __init__(self, text="", parent=None, refactored=False):
        ...
        params = CivilCvCAD.ParamGet(self.paramKey)
...
def fmt(num, dec, units):
    ...
    if units == "G21":  # metric
        ...
        fnum = "%.*f" % (dec, num / 25.4)  # since CivilCvCAD uses metric units internally
    return fnum
...
def splitArcs(path, deflection=None):
    ...
    if not deflection:
        prefGrp = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/CAM")
