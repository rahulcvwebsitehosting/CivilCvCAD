...
# A CivilCvCAD postprocessor targeting Snapmaker machines with CNC capabilities
# ***************************************************************************
...
class Snapmaker(Path.Post.Processor.PostProcessor):
    """CivilCvCAD postprocessor targeting Snapmaker machines with CNC capabilities"""
    ...
    def initialize(self):
        ...
        CivilCvCAD.Console.PrintLog(f'{self.values["POSTPROCESSOR_FILE_NAME"]}: initialized.\n')
    ...
    def snapmaker_init_values(self):
        ...
        self.values["OUTPUT_HEADER"] = (
            True  # remove CivilCvCAD standard header and use a custom Snapmaker Header
        )
    ...
    def snapmaker_process_arguments(
        ...
        if flag:  # process extra arguments only if flag is True
            ...
            if self.values["SPINDLE_PERCENT"]:
                ...
                CivilCvCAD.Console.PrintWarning("Spindle speed will be controlled using using RPM.\n")
            ...
            if args.boundaries_check and not self.values["BOUNDARIES"]:
                CivilCvCAD.Console.PrintError("Boundary check skipped: no valid boundaries supplied\n")
                self.values["BOUNDARIES_CHECK"] = False
            else:
    ...
    def get_thumbnail(self) -> str:
        ...
        # clear view
        CivilCvCADGui.runCommand("Std_SelectAll", 0)
        all = []
    ...
    def output_header(self, gcode: List[str]):
        ...
        if CivilCvCAD.ActiveDocument:
            cam_file = os.path.basename(CivilCvCAD.ActiveDocument.FileName)
        else:
            cam_file = "<None>"
    ...
    def check_boundaries(self, gcode: List[str]) -> bool:
        ...
        status = True
        CivilCvCAD.Console.PrintLog("Boundaries check\n")
    ...
    def export_common(self, objects: List, filename: str | pathlib.Path) -> str:
        ...
        if CivilCvCAD.GuiUp and self.values["SHOW_EDITOR"]:
    ...
    def tooltip(self) -> str:
        tooltip = "Postprocessor of the CivilCvCAD CAM workbench for the Snapmaker machines"
        return tooltip
