...
TOOLTIP = """
This is a post processor file for the CivilCvCAD Path workbench. It is used to
take a pseudo-G-code fragment outputted by a Path object, and output
real G-code suitable for Dynapath Delta 40,50, & 60 Controls. It has been written
and tested on CivilCvCAD Path workbench bundled with CivilCvCAD v21.
This post processor, once placed in the appropriate PathScripts folder, can be
used directly from inside CivilCvCAD, via the GUI importer or via python scripts with:
...
def export(objectslist, filename, argstring):
    ...
    if OUTPUT_HEADER:
        ...
        gcode += linenumber() + "(T)" + "EXPORTED BY CIVILCVCAD$\n"
            ...
            for param in params:
                if param in c.Parameters:
                    ...
                    # required XYZ move after Tool change.
                        ...
                        pos = Units.Quantity(c.Parameters["X"], CivilCvCAD.Units.Length)
                        outstring.append(
                            param + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string)
                        )
                    elif param == "Y" and (command in QCYCLE_RANGE):
                        pos = Units.Quantity(c.Parameters["Y"], CivilCvCAD.Units.Length)
                        outstring.append(
                            param + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string)
                        )
                    ...
                    # Converts "Q" to "K" as needed by Dynapath.
                        ...
                        pos = Units.Quantity(c.Parameters["Q"], CivilCvCAD.Units.Length)
                        outstring.append(
                    ...
                    # don't need the 2nd reference plane.
                        ...
                        pos = Units.Quantity(pathobj.ClearanceHeight.Value, CivilCvCAD.Units.Length)
                        outstring.insert(
                            6,
                            "O" + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string),
                        )  # Insert "O" param for 2nd reference plane (Clearance Height)
                        pos = Units.Quantity(c.Parameters["R"], CivilCvCAD.Units.Length)
                        outstring.append(
                            param + format(float(pos.getValueAs(UNIT_FORMAT)), precision_string)
