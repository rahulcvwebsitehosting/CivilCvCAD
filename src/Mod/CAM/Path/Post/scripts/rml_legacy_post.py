...
TOOLTIP = """
CivilCvCAD Path post-processor to output code for the Roland Modela MDX-## machines.
...
def xyarc(args, state):
    ...
    lastPoint = CivilCvCAD.Vector(state["X"], state["Y"])
    newPoint = CivilCvCAD.Vector(float(args["X"]), float(args["Y"]))
    centerOffset = CivilCvCAD.Vector(float(args["I"]), float(args["J"]))
    center = lastPoint + centerOffset
    radius = (center - lastPoint).Length
    xyNormal = CivilCvCAD.Vector(0, 0, 1)
    ...
    # consider direction?
    # print('p = Part.ArcOfCircle(Part.Circle(CivilCvCAD.Vector(%f, %f), CivilCvCAD.Vector(0, 0, 1), %f), %f, %f)' % (center.x, center.y, radius, p0, p1))
    for p in points:
