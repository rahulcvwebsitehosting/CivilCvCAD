...
used directly from inside CivilCvCAD, via the GUI importer or via python scripts
with:
                ...
                if param in c.Parameters:
                    if param == "F":
                        if command not in RAPID_MOVES:
                            speed = Units.Quantity(c.Parameters["F"], CivilCvCAD.Units.Velocity)
...
def drill_translate(outstring, cmd, params):
    ...
    RETRACT_Z = Units.Quantity(params["R"], CivilCvCAD.Units.Length)
    ...
    drill_feedrate = Units.Quantity(params["F"], CivilCvCAD.Units.Velocity)
    if cmd == "G83":
