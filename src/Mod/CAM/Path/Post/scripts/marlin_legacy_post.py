...
# *****************************************************************************
# *   Due to the fundamentals of the CivilCvCAD pre-processor,                   *
...
# * Therefore, check CivilCvCAD gcodes for conflicts with Marlin.                *
...
def export(objectslist, filename, argstring):
    ...
    # Write header:
    if OUTPUT_HEADER:
        gcode += linenumber() + "(Exported by CivilCvCAD)\n"
    ...
    if CivilCvCAD.GuiUp and SHOW_EDITOR:
        ...
        for c in PathUtils.getPathWithPlacement(pathobj).Commands:
                ...
                if param in c.Parameters:
                    if param == "F":
                        if command not in RAPID_MOVES:
                            feedRate = Units.Quantity(c.Parameters["F"], CivilCvCAD.Units.Velocity)
                    ...
                    # [X, Y, Z, U, V, W, I, J, K, R, Q]
                        ...
                        pos = Units.Quantity(c.Parameters[param], CivilCvCAD.Units.Length)
                        outlist.append(
            ...
            if command in MOTION_COMMANDS:
                if "X" in c.Parameters:
                    CURRENT_X = Units.Quantity(c.Parameters["X"], CivilCvCAD.Units.Length)
                if "Y" in c.Parameters:
                    CURRENT_Y = Units.Quantity(c.Parameters["Y"], CivilCvCAD.Units.Length)
                if "Z" in c.Parameters:
                    CURRENT_Z = Units.Quantity(c.Parameters["Z"], CivilCvCAD.Units.Length)
...
# * The following code converts CivilCvCAD's canned drill cycles into            *
...
def drill_translate(outlist, cmd, params):
    ...
    drill_X = Units.Quantity(params["X"], CivilCvCAD.Units.Length)
    drill_Y = Units.Quantity(params["Y"], CivilCvCAD.Units.Length)
    drill_Z = Units.Quantity(params["Z"], CivilCvCAD.Units.Length)
    drill_R = Units.Quantity(params["R"], CivilCvCAD.Units.Length)
    drill_F = Units.Quantity(params["F"], CivilCvCAD.Units.Velocity)
    if cmd == "G82":
        drill_DwellTime = params["P"]
    elif cmd == "G83":
        drill_Step = Units.Quantity(params["Q"], CivilCvCAD.Units.Length)
