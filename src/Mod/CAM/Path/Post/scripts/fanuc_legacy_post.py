...
can be used directly from inside CivilCvCAD, via the GUI importer or via
python scripts with:
...
def export(objectslist, filename, argstring):
    ...
    if OUTPUT_HEADER:
        ...
        if job:
            ...
            body, job = ("CIVILCVCAD-FILENAME-GOES-HERE", "JOB-NAME-GOES-HERE")
        gcode += "(" + body.upper() + ", " + job.upper() + ")\n"
        gcode += (
            linenumber() + "(POST PROCESSOR: FANUC USING CIVILCVCAD %d.%d" % (major, minor) + ")\n"
        )
    ...
    for obj in objectslist:
        ...
        # to stay compatible with CivilCvCAD 1.0
        def activeForOp(obj):
    ...
    if hasattr(pathobj, "Group"):  # We have a compound or project.
            ...
            if hasattr(pathobj, "ToolController"):
                if (
                    ...
                    opHorizRapid = Units.Quantity(
                        pathobj.ToolController.HorizRapid, CivilCvCAD.Units.Velocity
                    )
                else:
                ...
                if (
                    ...
                    opVertRapid = Units.Quantity(
                        pathobj.ToolController.VertRapid, CivilCvCAD.Units.Velocity
                    )
                else:
                    CivilCvCAD.Console.PrintWarning(
        ...
        for index, c in enumerate(commands):
            ...
            if command == "G74" or command == "G84":
                ...
                for param in ["P", "Q"]:
                    if param in c.Parameters:
                        value = Units.Quantity(c.Parameters[param], CivilCvCAD.Units.Length)
                        paramstring += (
                            " "
                            + param
                            + format(
