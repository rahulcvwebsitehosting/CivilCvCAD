...
can be used directly from inside CivilCvCAD, via the GUI importer or
    ...
    for obj in objectslist:
        ...
        if hasattr(job, "SetupSheet"):
            if hasattr(job.SetupSheet, "HorizRapid"):
                HORIZRAPID = Units.Quantity(job.SetupSheet.HorizRapid, CivilCvCAD.Units.Velocity)
            if hasattr(job.SetupSheet, "VertRapid"):
                VERTRAPID = Units.Quantity(job.SetupSheet.HorizRapid, CivilCvCAD.Units.Velocity)
