...
class DlgSelectPostProcessor:
    ...
    def __init__(self):
        self.dialog = CivilCvCADGui.PySideUic.loadUi(":/panels/DlgSelectPostProcessor.ui")
        firstItem = None
...
class CommandPathPost:
    ...
    def _write_file(self, filename, gcode, policy):
        ...
        CivilCvCAD.Console.PrintMessage(f"File written to {filename}\n")
    ...
    def Activated(self):
        ...
        if use_new_flow and CivilCvCAD.GuiUp:
            ...
            CivilCvCAD.ActiveDocument.openTransaction("Post Process the Selected Job")
        ...
        if use_new_flow:
            ...
            try:
                ...
                CivilCvCAD.Console.PrintError(f"Machine configuration error: {e}\n")
                CivilCvCAD.ActiveDocument.abortTransaction()
                return
        else:
        ...
        for item in post_data:
            ...
            # the system where CivilCvCAD is running.  In the old-style postprocessors the
            ...
            if gcode is not None:
                ...
                if use_new_flow and CivilCvCAD.GuiUp and Path.Preferences.showEditorOnPostProcess():
                    if len(gcode) > 100000:
                        ...
                        if editor_result == 1:  # User clicked OK
                            ...
                            CivilCvCAD.Console.PrintMessage(f"Post-processing cancelled for {fname}\n")
                            continue
...
class CommandPathPostSelected(CommandPathPost):
    ...
    def Activated(self):
        ...
        on user selection and document context.
        """
        CivilCvCAD.ActiveDocument.openTransaction("Post Process the Selected operations")
        ...
        if (
            ...
            baseOp = CivilCvCAD.ActiveDocument.getObject(selection[0].Base[0])
        ...
        for item in post_data:
            ...
            if gcode is not None:
                ...
                final_gcode = gcode
                if CivilCvCAD.GuiUp and Path.Preferences.showEditorOnPostProcess():
                    if len(gcode) > 100000:
                        CivilCvCAD.Console.PrintWarning(
                            "Skipping editor since output is greater than 100kb\n"
                        )
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_Post", CommandPathPost())
    CivilCvCADGui.addCommand("CAM_PostSelected", CommandPathPostSelected())

CivilCvCAD.Console.PrintLog("Loading PathPost… done\n")
