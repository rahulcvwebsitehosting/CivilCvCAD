...
def debugWire(label, w):
    ...
    if not PrintWireDebug:
        return
    print("#%s wire >>>>>>>>>>>>>>>>>>>>>>>>" % label)
    print("grp = CivilCvCAD.ActiveDocument.addObject('App::DocumentObjectGroup', '%s')" % label)
    for i, e in enumerate(w.Edges):
        edge = "%s_e%d" % (label, i)
        debugEdge("%s = " % edge, e)
        print("Part.show(%s, '%s')" % (edge, edge))
        print("grp.addObject(CivilCvCAD.ActiveDocument.ActiveObject)")
    print("#%s wire <<<<<<<<<<<<<<<<<<<<<<<<" % label)
...
def getCoincideTolerance(edges):
    ...
    """
    tolerance = CivilCvCAD.Base.Precision.confusion()
...
def offsetWire(wire, base, offset, forward, Side=None, tolerance=0.01):
    ...
    if len(wire.Edges) == 1:
        ...
        if isinstance(curve, Part.Circle) and wire.isClosed():
            ...
            center = curve.Center
            radius = curve.Radius
            axis = CivilCvCAD.Vector(0, 0, -1) if forward else CivilCvCAD.Vector(0, 0, 1)
            checkSidePoint = CivilCvCAD.Vector(center.x + radius + tolerance * 2, center.y, center.z)
        ...
        if isinstance(curve, (Part.Line, Part.LineSegment)):
            ...
            n = v0.cross(CivilCvCAD.Vector(0, 0, 1))
    ...
    for e in owire.Edges + owire.Edges:
        if isCircleAt(e, start):
            if Path.Geom.pointsCoincide(e.Curve.Axis, CivilCvCAD.Vector(0, 0, 1)):
