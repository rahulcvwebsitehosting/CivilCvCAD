...
class ObjectProfile(PathAreaOp.ObjectOp):
    ...
    def areaOpShapes(self, obj):
        ...
        if self.isDebug:
            for grpNm in ("tmpDebugGrp", "tmpDebugGrp001"):
                if hasattr(CivilCvCAD.ActiveDocument, grpNm):
                    for go in CivilCvCAD.ActiveDocument.getObject(grpNm).Group:
                        CivilCvCAD.ActiveDocument.removeObject(go.Name)
                    CivilCvCAD.ActiveDocument.removeObject(grpNm)
        ...
        if self.isDebug:
            if CivilCvCAD.GuiUp:
                ...
                CivilCvCADGui.ActiveDocument.getObject(tmpGrpNm).Visibility = False
    ...
    def _processWire(self, obj, base, edges):
        ...
        if wire.isClosed():
            ...
            if self.JOB.GeometryTolerance.Value == 0.0:
                ...
                flatWire.translate(CivilCvCAD.Vector(0, 0, diffDepth))
    ...
    def _getOpenProfileDiffDepth(self, base, edges):
        ...
        diffDepth = 2 * tol
        offset = CivilCvCAD.Vector(0, 0, diffDepth)
    ...
    def _flattenWire(self, obj, wire, trgtDep):
        ...
        if not Path.Geom.isRoughly(wBB.ZLength, 0):
            ...
            mbbEXT = wire.extrude(CivilCvCAD.Vector(0, 0, extFwdLen))
            ...
            srtWire.translate(CivilCvCAD.Vector(0, 0, trgtDep - srtWire.BoundBox.ZMin))
    ...
    def _getCutAreaCrossSection(self, obj, base, origWire, flatWire):
        ...
        pb = CivilCvCAD.Vector(Vb.X, Vb.Y, fdv)
        pe = CivilCvCAD.Vector(Ve.X, Ve.Y, fdv)
        ...
        if blen > 0.1:
            ...
            bcp = CivilCvCAD.Vector(begE.Vertexes[1].X, begE.Vertexes[1].Y, fdv)
        if elen > 0.1:
            ecp = endE.valueAt(
                endE.getParameterByLength(elen - 0.1)
            )  # point returned 0.1 mm along edge
        else:
            ecp = CivilCvCAD.Vector(endE.Vertexes[1].X, endE.Vertexes[1].Y, fdv)
        ...
        extBndboxEXT = extBndbox.extrude(CivilCvCAD.Vector(0, 0, 1))
        ...
        topComp.translate(
            CivilCvCAD.Vector(0, 0, fdv - topComp.BoundBox.ZMin)
        ...
        botComp.translate(
            CivilCvCAD.Vector(0, 0, fdv - botComp.BoundBox.ZMin)
        )  # Translate face to final depth
        ...
        pathStops.translate(CivilCvCAD.Vector(0, 0, fdv - pathStops.BoundBox.ZMin))
        ...
        if lenWS < 3:
            ...
            fcShp.translate(CivilCvCAD.Vector(0, 0, fdv - workShp.BoundBox.ZMin))
        # Eif
        ...
        if wi > 0:  # and isInterior is False:
            ...
            if isReady is False:
                ...
                pfcShp.translate(CivilCvCAD.Vector(0, 0, fdv - workShp.BoundBox.ZMin))
    ...
    def _extractPathWire(self, obj, base, flatWire, cutShp):
        ...
        cent0 = CivilCvCAD.Vector(frstVrt.X, frstVrt.Y, fdv)
        cent1 = CivilCvCAD.Vector(lstVrt.X, lstVrt.Y, fdv)
        ...
        if numOSWires > 1:
            ...
            for T in self.cutSideTags.Faces:
                tcom = T.CenterOfMass
                tv = CivilCvCAD.Vector(tcom.x, tcom.y, 0.0)
            ...
            for w in osWrIdxs:
                if w != w0:
                    ...
                    for V in VTXS:
                        v = CivilCvCAD.Vector(V.X, V.Y, 0.0)
    ...
    def _findNearestVertex(self, shape, point):
        ...
        PT = CivilCvCAD.Vector(point.x, point.y, 0.0)
        ...
        for w in range(0, len(shape.Wires)):
            WR = shape.Wires[w]
            V = WR.Vertexes[0]
            P = CivilCvCAD.Vector(V.X, V.Y, 0.0)
    ...
    def _separateWireAtVertexes(self, wire, VV1, VV2):
        ...
        grps = [[], []]
        wireIdxs = [[], []]
        V1 = CivilCvCAD.Vector(VV1.X, VV1.Y, VV1.Z)
        V2 = CivilCvCAD.Vector(VV2.X, VV2.Y, VV2.Z)
        ...
        for e in range(0, edgeCount):
            v = 0
            E = wire.Edges[e]
            fv0 = CivilCvCAD.Vector(E.Vertexes[0].X, E.Vertexes[0].Y, E.Vertexes[0].Z)
            fv1 = CivilCvCAD.Vector(E.Vertexes[1].X, E.Vertexes[1].Y, E.Vertexes[1].Z)
    ...
    def _makeCrossSection(self, shape, sliceZ, zHghtTrgt=False):
        ...
        # Create cross-section of shape and translate
        wires = []
        slcs = shape.slice(CivilCvCAD.Vector(0, 0, 1), sliceZ)
        if len(slcs) > 0:
            for i in slcs:
                wires.append(i)
            comp = Part.Compound(wires)
            if zHghtTrgt is not False:
                comp.translate(CivilCvCAD.Vector(0, 0, zHghtTrgt - comp.BoundBox.ZMin))
            return comp
    ...
    def _makeOffsetCircleTag(self, p1, p2, cutterRad, depth, lbl, reverse=False):
        ...
        pb = CivilCvCAD.Vector(p1.x, p1.y, 0.0)
        pe = CivilCvCAD.Vector(p2.x, p2.y, 0.0)
        ...
        perpE = CivilCvCAD.Vector(-1 * toMid.y, toMid.x, 0.0).multiply(-1 * cutFactor)  # exterior tag
        extPnt = pb.add(toMid.add(perpE))

        # make exterior tag
        eCntr = extPnt.add(CivilCvCAD.Vector(0, 0, depth))
        ecw = Part.Wire(Part.makeCircle((cutterRad / 2), eCntr).Edges[0])
        extTag = Part.Face(ecw)

        # make interior tag
        perpI = CivilCvCAD.Vector(-1 * toMid.y, toMid.x, 0.0).multiply(cutFactor)  # interior tag
        intPnt = pb.add(toMid.add(perpI))
        iCntr = intPnt.add(CivilCvCAD.Vector(0, 0, depth))
    ...
    def _makeStop(self, sType, pA, pB, lbl):
        ...
        E = CivilCvCAD.Vector(pB.x, pB.y, 0)  # endpoint
        C = CivilCvCAD.Vector(pA.x, pA.y, 0)  # checkpoint
    ...
    def _makePerp2DVector(self, v1, v2, dist):
        p1 = CivilCvCAD.Vector(v1.x, v1.y, 0.0)
        p2 = CivilCvCAD.Vector(v2.x, v2.y, 0.0)
        toEnd = p2.sub(p1)
        factor = dist / toEnd.Length
        perp = CivilCvCAD.Vector(-1 * toEnd.y, toEnd.x, 0.0).multiply(factor)
        return p1.add(toEnd.add(perp))

    # Method to add temporary debug object
    def _addDebugObject(self, objName, objShape):
        if self.isDebug:
            newDocObj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmp_" + objName)
