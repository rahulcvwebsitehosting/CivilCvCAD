...
class ObjectOp(PathOp.ObjectOp):
    ...
    def holePosition(self, base, sub):
        ...
        try:
            shape = base.Shape.getElement(sub)
            if isinstance(shape, Part.Vertex):
                return CivilCvCAD.Vector(shape.X, shape.Y, 0)

            if isinstance(shape, Part.Edge) and isinstance(shape.Curve, Part.Circle):
                return CivilCvCAD.Vector(shape.Curve.Center.x, shape.Curve.Center.y, 0)

            if isinstance(shape, Part.Face):
                if isinstance(shape.Surface, Part.Cylinder):
                    return CivilCvCAD.Vector(shape.Surface.Center.x, shape.Surface.Center.y, 0)
                if all(isinstance(e.Curve, Part.Circle) for e in shape.Edges):
                    ...
                    if all(Path.Geom.pointsCoincide(center, e.Curve.Center) for e in shape.Edges):
                        return CivilCvCAD.Vector(center.x, center.y, 0)
            return CivilCvCAD.Vector(shape.CenterOfMass.x, shape.CenterOfMass.y, 0)
        except Exception as e:
            Path.Log.error(e)
    ...
    def findAllHoles(self, obj, selection=[]):
        ...
        matchvector = None if job.JobType == "Multiaxis" else CivilCvCAD.Vector(0, 0, 1)
