...
class ObjectSurface(PathOp.ObjectOp):
    ...
    def opExecute(self, obj):
        ...
        CivilCvCAD.Console.PrintMessage("3D Surface " + msg + " {}\n".format(exTime))
    ...
    def _processPlanarOp(self, JOB, obj, mdlIdx, cmpdShp, fsi):
        ...
        def getTransition(two):
            ...
            trans = [[CivilCvCAD.Vector(first.x, first.y, safe)]]
            return trans
    ...
    def _offsetFacesToPointData(self, obj, subShp, profile=True):
        ...
        if not profile:
            ...
            # Reference https://forum.civilcvcad.org/viewtopic.php?t=28861#p234939
                ...
                for a in range(0, len(ARCS)):
                    ...
                    if Arc == "BRK":
                        ...
                        if scan is False:
                            ...
                            if aTyp == "L":
                                scan.append(CivilCvCAD.Vector(scan[0].x, scan[0].y, scan[0].z))
                            stpOvr.append(scan)
                if erFlg is False:
                    SCANS.append(stpOvr)
        # Eif
    ...
    def _planarDropCutScan(self, pdc, A, B):
        ...
        CLP = pdc.getCLPoints()
        PNTS = [CivilCvCAD.Vector(p.x, p.y, p.z) for p in CLP]
    ...
    def _planarCircularDropCutScan(self, pdc, Arc, cMode):
        ...
        # Convert OCL object data to CivilCvCAD vectors
        return [CivilCvCAD.Vector(p.x, p.y, p.z) for p in CLP]
                    ...
                    for i in range(0, lenAdjPrts):
                        ...
                        if prt == "BRK" and prtsHasCmds:
                            if i + 1 < lenAdjPrts:
                                ...
                                nxtStart = CivilCvCAD.Vector(last.x, last.y, obj.SafeHeight.Value)
                            prtsCmds.extend(
    ...
    def _planarMultipassPreProcess(self, obj, LN, prvDep, layDep):
        ...
        if optLinTrans is True:
            for P in LN:
                ...
                if P.z <= layDep:
                    PTS.append(CivilCvCAD.Vector(P.x, P.y, layDep))
                elif P.z > prvDep:
                    PTS.append(CivilCvCAD.Vector(P.x, P.y, safe))
                else:
                    PTS.append(CivilCvCAD.Vector(P.x, P.y, P.z))
            # Efor
        else:
            for P in LN:
                ALL.append(P)
    ...
    def _planarMultipassProcess(self, obj, PNTS, lMax):
        ...
        nxt = None
        pnt = PNTS[0]
        prev = CivilCvCAD.Vector(-442064564.6, 258539656553.27, 3538553425.847)

        #  Add temp end point
        PNTS.append(CivilCvCAD.Vector(-4895747464.6, -25855763553.2, 35865763425))
    ...
    def _processRotationalOp(self, JOB, obj, mdlIdx, compoundFaces=None):
        ...
        if initIdx != 0.0:
            self.basePlacement = CivilCvCAD.ActiveDocument.getObject(base.Name).Placement
            if obj.RotationAxis == "X":
                base.Placement = CivilCvCAD.Placement(
                    CivilCvCAD.Vector(0.0, 0.0, 0.0),
                    CivilCvCAD.Rotation(CivilCvCAD.Vector(1.0, 0.0, 0.0), initIdx),
                )
            else:
                base.Placement = CivilCvCAD.Placement(
                    CivilCvCAD.Vector(0.0, 0.0, 0.0),
                    CivilCvCAD.Rotation(CivilCvCAD.Vector(0.0, 1.0, 0.0), initIdx),
                )
    ...
    def _indexedDropCutScan(self, obj, stl, advances, xmin, ymin, xmax, ymax, layDep, sample):
        ...
        for adv in advances:
            ...
            if obj.DepthOffset.Value != 0.0:
                Lines.append(
                    [CivilCvCAD.Vector(p.x, p.y, p.z + obj.DepthOffset.Value) for p in result]
                )
            else:
                Lines.append([CivilCvCAD.Vector(p.x, p.y, p.z) for p in result])
    ...
    def _indexedScanToGcode(self, obj, li, CLP, idxAng, prvDep, layerDepth, numDeps):
        ...
        lenCLP = len(CLP)
        lastCLP = lenCLP - 1
        prev = CivilCvCAD.Vector(0.0, 0.0, 0.0)
