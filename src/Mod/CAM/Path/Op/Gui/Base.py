        ...
        class FaceSelectionObserver:
            ...
            def addSelection(self, doc, obj, sub, pnt):
                ...
                if sub and sub.startswith("Face"):
                    try:
                        # Get the face object
                        selected_obj = CivilCvCAD.ActiveDocument.getObject(obj)
                        ...
                        CivilCvCAD.Console.PrintError(f"Error setting workplane: {e}\n")
                        self.active = False
                        CivilCvCADGui.Selection.removeObserver(self)

        # Create and add the observer
        observer = FaceSelectionObserver(self._workplaneOperation, self)
        CivilCvCADGui.Selection.addObserver(observer)

        # Clear current selection and provide user feedback
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCAD.Console.PrintMessage(f"Click on a face to set workplane for {self.Object.Label}\n")
...
class TaskPanelPage(object):
    ...
    def resetTCCombo(self):
        ...
        labels.append(CivilCvCAD.Qt.translate("CAM_Operation", "Copy {0}…").format(tcName))
        labels.append(CivilCvCAD.Qt.translate("CAM_Operation", "New tool controller…"))
    ...
    def setupToolController(self, obj=None, combo=None):
        ...
        if hasattr(self.form, "editToolController"):
            ...
            labelStr = CivilCvCAD.Qt.translate(
...
class TaskPanelBaseGeometryPage(TaskPanelPage):
    ...
    def getForm(self):
        panel = CivilCvCADGui.PySideUic.loadUi(":/panels/PageBaseGeometryEdit.ui")
    ...
    def itemActivated(self):
        ...
        for item in self.form.baseList.selectedItems():
            ...
            if sub:
                CivilCvCADGui.Selection.addSelection(obj, sub)
            else:
    ...
    def addBase(self):
        Path.Log.track()
        if self.addBaseGeometry(CivilCvCADGui.Selection.getSelectionEx()):
    ...
    def importBaseGeometry(self):
        ...
        ops = CivilCvCAD.ActiveDocument.getObjectsByLabel(opLabel)
        if len(ops) > 1:
        ...
        for base, subList in ops[0].Base:
            CivilCvCADGui.Selection.clearSelection()
            CivilCvCADGui.Selection.addSelection(base, subList)
            self.addBase()
...
class TaskPanelBaseLocationPage(TaskPanelPage):
    ...
    def getForm(self):
        self.formLoc = CivilCvCADGui.PySideUic.loadUi(":/panels/PageBaseLocationEdit.ui")
    ...
    def updateLocations(self):
        ...
        for i in range(self.formLoc.baseList.rowCount()):
            ...
            location = CivilCvCAD.Vector(x, y, 0)
...
class TaskPanelHeightsPage(TaskPanelPage):
    ...
    def getForm(self):
        return CivilCvCADGui.PySideUic.loadUi(":/panels/PageHeightsEdit.ui")
    ...
    def setFields(self, obj):
        ...
        self.updateSelection(obj, CivilCvCADGui.Selection.getSelectionEx())
    ...
    def depthSet(self, obj, spinbox, prop):
        z = self.selectionZLevel(CivilCvCADGui.Selection.getSelectionEx())
        if z is not None:
            Path.Log.debug("depthSet(%s, %s, %.2f)" % (obj.Label, prop, z))
            if spinbox.expression():
                obj.setExpression(prop, None)
                self.setDirty()
            spinbox.updateWidget(CivilCvCAD.Units.Quantity(z, CivilCvCAD.Units.Length))
...
class TaskPanelDiametersPage(TaskPanelPage):
    ...
    def getForm(self):
        return CivilCvCADGui.PySideUic.loadUi(":/panels/PageDiametersEdit.ui")
...
class TaskPanel(object):
    ...
    def __init__(self, obj, deleteOnReject, opPage, selectionFactory):
        ...
        CivilCvCAD.ActiveDocument.openTransaction(translate("PathOp", "AreaOp Operation"))
        self.obj = obj
    ...
    def reject(self, resetEdit=True):
        ...
        if self.deleteOnReject:
            CivilCvCAD.ActiveDocument.openTransaction(translate("PathOp", "Uncreate AreaOp Operation"))
            try:
    ...
    def setupUi(self):
        ...
        for prp, expr in self.obj.ExpressionEngine:
            ...
            if not isinstance(evalExpr, (int, float, CivilCvCAD.Units.Quantity)):
                continue
            val = CivilCvCAD.Units.Quantity(evalExpr)
...
class CommandSetStartPoint:
    ...
    def IsActive(self):
        if CivilCvCAD.ActiveDocument is None:
            return False
    ...
    def setpoint(self, point, o):
        CivilCvCADGui.Snapper.grid.off()
        obj = self.obj
    ...
    def Activated(self):
        self.obj = CivilCvCADGui.Selection.getSelection()[0]
        if not hasattr(CivilCvCADGui, "Snapper"):
            import DraftTools
        CivilCvCADGui.Snapper.getPoint(callback=self.setpoint)
...
def Create(res):
    ...
    CivilCvCAD.ActiveDocument.openTransaction("Create %s" % res.name)
    if res.job is None:
        CivilCvCAD.ActiveDocument.abortTransaction()
...
def SetupOperation(name, objFactory, opPageClass, pixmap, menuText, toolTip, setupProperties=None):
    ...
    Creates an instance of CommandPathOp with the given parameters and registers the command with CivilCvCAD.
    ...
    CivilCvCADGui.addCommand("CAM_%s" % name.replace(" ", "_"), command)
...
CivilCvCADGui.addCommand("CAM_SetStartPoint", CommandSetStartPoint())

CivilCvCAD.Console.PrintLog("Loading PathOpGui… done\n")
