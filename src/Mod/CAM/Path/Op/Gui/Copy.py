...
__doc__ = """CAM Copy object and CivilCvCAD command"""
...
class CommandPathCopy:
    ...
    def Activated(self):
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Create Copy")
        CivilCvCADGui.addModule("Path.Op.Gui.Copy")
...
if len(selection) != 1:
    CivilCvCAD.Console.PrintError(translate("CAM_Copy", "Select one toolpath object")+"\n")
    selGood = False

if not selection[0].isDerivedFrom("Path::Feature"):
    CivilCvCAD.Console.PrintError(translate("CAM_Copy", "The selected object is not a toolpath")+"\n")
    selGood = False

if selGood:
    obj = CivilCvCAD.ActiveDocument.addObject("Path::FeaturePython", str(selection[0].Name)+'_Copy')
    Path.Op.Gui.Copy.ObjectPathCopy(obj)
    Path.Op.Gui.Copy.ViewProviderPathCopy(obj.ViewObject)
    obj.Base = CivilCvCAD.ActiveDocument.getObject(selection[0].Name)
    if hasattr(obj.Base, 'ToolController'):
        obj.ToolController = obj.Base.ToolController
...
"""
        ...
        CivilCvCADGui.doCommand(consolecode)
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_Copy", CommandPathCopy())

CivilCvCAD.Console.PrintLog("Loading PathCopy… done\n")
