...
__doc__ = """CAM Array object and CivilCvCAD command"""
...
class ObjectArray:
    def __init__(self, obj):
        ...
        obj.JitterMagnitude = CivilCvCAD.Vector(10, 10, 0)
        obj.JitterAngle = 10
    ...
    def execute(self, obj):
        ...
        if obj.UseJitter:
            ...
            if obj.JitterMagnitude != CivilCvCAD.Vector():
...
class PathArray:
    ...
    def getPath(self):
        ...
        self.jitterCentre = CivilCvCAD.Vector()
        if self.jitterAngle:
    ...
    def getPathPoint(self, op, reverse=False):
        ...
        for cmd in cmds:
            ...
            if x is not None and y is not None and z is not None:
                return CivilCvCAD.Vector(x, y, z)
    ...
    def getBasePathCenter(self, operations):
        ...
        if xmin is not None and ymin is not None and xmax is not None and ymax is not None:
            return CivilCvCAD.Vector(xmin + (xmax - xmin) / 2, ymin + (ymax - ymin) / 2, 0)
    ...
    def getLinear1DArray(self, commands):
        ...
        for i in reversed(range(self.copies)) if self.reverse else range(self.copies):
            ...
            for b in self.base:
                ...
                pl.move(pos)
                pl.rotate(self.jitterCentre, CivilCvCAD.Vector(0, 0, 1), alpha)
    ...
    def getPolarArray(self, commands):
        ...
        for i in reversed(range(self.copies)) if self.reverse else range(self.copies):
            ...
            ang = stepAng * (i + 1)
            pl = CivilCvCAD.Placement()
            pl.rotate(self.polarCentre, CivilCvCAD.Vector(0, 0, 1), ang)

            # add jitter to placement
            pos, alpha = self.calculateJitter(CivilCvCAD.Vector())
            pl.move(pos)
            pl.rotate(self.jitterCentre, CivilCvCAD.Vector(0, 0, 1), alpha)
    ...
    def getPointsArray(self, commands):
        ...
        if originPoint != CivilCvCAD.Vector() or originAngle:
            for pos in points:
                pos["point"] -= originPoint
                pos["angle"] -= originAngle

        # remove points which similar with origin
        points = [p for p in points if p["point"] != CivilCvCAD.Vector() or p["angle"]]
        ...
        if self.pointsSorting == "Automatic":
            ...
            for i, pos in enumerate(points):
                ...
                dirStartOffset = DraftVecUtils.rotate(
                    dirStart, math.radians(pos["angle"]), CivilCvCAD.Vector(0, 0, 1)
                )
                dirEndOffset = DraftVecUtils.rotate(
                    dirEnd, math.radians(pos["angle"]), CivilCvCAD.Vector(0, 0, 1)
                )
                routes.append(
                    {
        ...
        for pos in reversed(points) if self.reverse else points:
            ...
            for b in self.base:
                ...
                pl.move(point)
                pl.rotate(self.jitterCentre, CivilCvCAD.Vector(0, 0, 1), alpha)
                pl.rotate(originPoint, CivilCvCAD.Vector(0, 0, 1), pos["angle"])
                path = PathUtils.getPathWithPlacement(b)
                path = PathUtils.applyPlacementToPath(pl, path)
                commands.extend(path.Commands)

    def getPointsAngle(self, p1, p2=CivilCvCAD.Vector()):
        """return angle between vector (direction) and Y-axis"""
        direction = p1 - p2
        if Path.Geom.pointsCoincide(direction, CivilCvCAD.Vector()):
            return 0
        angle = math.degrees(direction.getAngle(CivilCvCAD.Vector(0, 1, 0)))
        if direction.x > 0:
            angle = -angle
...
class CommandPathArray:
    ...
    def Activated(self):
        ...
        # undo/redo stack
        CivilCvCAD.ActiveDocument.openTransaction("Create Array")
        CivilCvCADGui.addModule("Path.Op.Gui.Array")
        CivilCvCADGui.addModule("PathScripts.PathUtils")

        CivilCvCADGui.doCommand(
            'obj = CivilCvCAD.ActiveDocument.addObject("Path::FeaturePython","Array")'
        )

        CivilCvCADGui.doCommand("Path.Op.Gui.Array.ObjectArray(obj)")

        baseString = "[%s]" % ",".join(
            ["CivilCvCAD.ActiveDocument.%s" % sel.Name for sel in selection]
        )
        CivilCvCADGui.doCommand("obj.Base = %s" % baseString)

        CivilCvCADGui.doCommand("Path.Op.Gui.Array.ViewProviderArray(obj.ViewObject)")
        CivilCvCADGui.doCommand("job = PathScripts.PathUtils.findParentJob(obj.Base[0])")
        CivilCvCADGui.doCommand("PathScripts.PathUtils.addToJob(obj, job.Name)")
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_Array", CommandPathArray())
