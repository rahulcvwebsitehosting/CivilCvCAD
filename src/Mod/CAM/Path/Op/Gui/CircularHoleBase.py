...
class TaskPanelHoleGeometryPage(PathOpGui.TaskPanelBaseGeometryPage):
    ...
    def getForm(self):
        form = CivilCvCADGui.PySideUic.loadUi(":/panels/PageBaseHoleGeometryEdit.ui")
        ...
        for base, subs in obj.Base:
            for sub in subs:
                ...
                if pos:
                    ...
                    diameterQty = CivilCvCAD.Units.Quantity(dia, CivilCvCAD.Units.Length)
                    diaString = diameterQty.getUserPreferred("Length")[0]
                    posXQty = CivilCvCAD.Units.Quantity(pos[0], CivilCvCAD.Units.Length)
                    posXString = posXQty.getUserPreferred("Length")[0]
                    posYQty = CivilCvCAD.Units.Quantity(pos[1], CivilCvCAD.Units.Length)
        ...
        if selected_rows:
            ...
            for row in selected_rows:
                ...
                if obj_name is not None:
                    obj = CivilCvCAD.ActiveDocument.getObject(obj_name)
                    if obj is not None:
    ...
    def updateChecked(self):
        ...
        for i in range(self.form.baseList.rowCount()):
            ...
            base = CivilCvCAD.ActiveDocument.getObject(base_name)
            sub = str(item.data(self.DataObjectSub))
