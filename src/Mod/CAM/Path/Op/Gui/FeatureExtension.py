...
class TaskPanelExtensionPage(PathOpGui.TaskPanelPage):
    ...
    def getForm(self):
        form = CivilCvCADGui.PySideUic.loadUi(":/panels/PageOpPocketExtEdit.ui")
        return form
    ...
    def restoreSelection(self, selection):
        ...
        if 0 == self.model.rowCount():
            ...
            def setSelectionVisuals(item, ext):
                ...
                if selected:
                    CivilCvCADGui.Selection.addSelection(ext.base, ext.face)
                    ext.select()
                else:
                    ext.deselect()
...
CivilCvCAD.Console.PrintLog("Loading PathFeatureExtensionsGui... done\n")
