...
class ObjectHelix(PathCircularHoleBase.ObjectOp):
    ...
    def getDirAngle(self, obj, holes, i):
        p1 = CivilCvCAD.Vector(holes[i]["x"], holes[i]["y"], 0)
        p2 = CivilCvCAD.Vector()  # by default orient to (0,0)

        if obj.StartAt == "Inside":
            if i < len(holes) - 1:
                # orient each hole (except last) to next hole
                p2 = CivilCvCAD.Vector(holes[i + 1]["x"], holes[i + 1]["y"], 0)
            elif len(holes) > 1:
    ...
    def circularHoleExecute(self, obj, holes):
        ...
        for hole_index, hole in enumerate(holes):
            ...
            if (args["outer_radius"] < 0 and not isRoughly(args["outer_radius"], 0)) or (
                ...
                posX = hole["x"]
                posY = hole["y"]
                posXQty = CivilCvCAD.Units.Quantity(posX, CivilCvCAD.Units.Length)
                posYQty = CivilCvCAD.Units.Quantity(posY, CivilCvCAD.Units.Length)
            ...
            centerTop = CivilCvCAD.Vector(hole["x"], hole["y"], obj.StartDepth.Value)
            centerBottom = CivilCvCAD.Vector(centerTop.x, centerTop.y, centerTop.z)
            ...
            for iter_num in range(iters):
                ...
                if isRoughly(args["inner_radius"], 0) or isRoughly(args["outer_radius"], 0):
                    ...
                    if hole_index == 0 and iter_num == 0:  # move to the first hole without linking
                        ...
                        linkingArgs["target_position"] = CivilCvCAD.Vector(
                    ...
                    cmd = helixCommands[1]
                    linkingArgs["target_position"] = CivilCvCAD.Vector(cmd.x, cmd.y, safeHeight)
                    if hole_index == 0 and iter_num == 0:  # move to the first hole without linking
