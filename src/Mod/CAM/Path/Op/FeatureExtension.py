...
def extendWire(feature, wire, length):
    ...
    except CivilCvCAD.Base.CivilCvCADError as ee:
        Path.Log.debug(ee)
        return None
...
class Extension(object):
    ...
    def _getDirection(self, wire):
        ...
        normal = tangent.cross(CivilCvCAD.Vector(0, 0, 1))
        if Path.Geom.pointsCoincide(normal, CivilCvCAD.Vector(0, 0, 0)):
            return None
    ...
    def _makeCircularExtFace(self, edge, extWire):
        ...
        edgeFace.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - edgeFace.BoundBox.ZMin))
        extWireFace = Part.Face(extWire)
        extWireFace.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - extWireFace.BoundBox.ZMin))

        if extWireFace.Area >= edgeFace.Area:
            extensionFace = extWireFace.cut(edgeFace)
        else:
            extensionFace = edgeFace.cut(extWireFace)
        extensionFace.translate(CivilCvCAD.Vector(0.0, 0.0, edge.BoundBox.ZMin))
...
def getExtendOutlineFace(base_shape, face, extension, remHoles=False, offset_tolerance=1e-4):
    ...
    depth = 0.2
    offset_ext = offset_face.extrude(CivilCvCAD.Vector(0.0, 0.0, depth))
    face_del = offset_face.extrude(CivilCvCAD.Vector(0.0, 0.0, -1.0 * depth))
    ...
    # Debug
    # Part.show(available)
    # CivilCvCAD.ActiveDocument.ActiveObject.Label = "available"
    ...
    if bottom_faces:
        ...
        for bf in bottom_faces:
            ...
            bf.translate(CivilCvCAD.Vector(0.0, 0.0, diff))
            cmn = bf.common(face)
...
def getWaterlineFace(base_shape, face):
    ...
    rawCompExtNeg = rawComp.extrude(CivilCvCAD.Vector(0.0, 0.0, baseBB.ZMin - faceHeight - 1.0))
