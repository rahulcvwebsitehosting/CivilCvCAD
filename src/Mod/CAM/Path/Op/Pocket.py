...
class ObjectPocket(PathPocketBase.ObjectPocket):
            ...
            for base in self.baseShapes(obj):
                ...
                if allSubsFaceType is True and obj.HandleMultipleFeatures == "Collectively":
                    ...
                    if obj.AdaptivePocketStart is True or obj.AdaptivePocketFinish is True:
                        ...
                        faceExtrusions = [f.extrude(CivilCvCAD.Vector(0.0, 0.0, 1.0)) for f in Faces]
                    ...
                    for sub in base[1]:
                        ...
                        rawRemovalShape = env.cut(base[0].Shape)
                        faceExtrusions = [shape.extrude(CivilCvCAD.Vector(0.0, 0.0, 1.0))]
                        obj.removalshape = _identifyRemovalSolids(rawRemovalShape, faceExtrusions)
    ...
    def calculateAdaptivePocket(self, obj, base, subObjTups):
        ...
        if len(high) > 0 and obj.AdaptivePocketStart is True:
            ...
            if makeHighFace > 0:
                CivilCvCAD.ActiveDocument.addObject("Part::Feature", "topEdgeFace")
                highFace = CivilCvCAD.ActiveDocument.ActiveObject
        ...
        if len(low) > 0 and obj.AdaptivePocketFinish is True:
            ...
            try:
                ...
                CivilCvCAD.ActiveDocument.addObject("Part::Feature", "bottomEdgeFace")
                lowFace = CivilCvCAD.ActiveDocument.ActiveObject
        ...
        if CivilCvCAD.GuiUp:
            ...
            for rn in removeList:
                CivilCvCADGui.ActiveDocument.getObject(rn).Visibility = False

        for rn in removeList:
            CivilCvCAD.ActiveDocument.getObject(rn).purgeTouched()
    ...
    def orderFacesAroundCenterOfMass(self, subObjTups):
        ...
        sortList = []
        subCnt = 0
        sumCom = CivilCvCAD.Vector(0.0, 0.0, 0.0)
        avgCom = CivilCvCAD.Vector(0.0, 0.0, 0.0)
        ...
        for sub, face in subObjTups:
            ...
            # face = CivilCvCAD.ActiveDocument.getObject(bsNm).Shape.Faces[fIdx]
            subCnt += 1
        ...
        for sub, face in subObjTups:
            ...
            if len(holding) == 1:
                ...
                if self.hasCommonVertex(edg0, edg1, show=False) < 0:
                    ...
                    com = CivilCvCAD.Vector(0, 0, 0)
                    com.add(edg0.CenterOfMass)
                    com.add(edg1.CenterOfMass)
                    avgCom = CivilCvCAD.Vector(com.x / 2.0, com.y / 2.0, com.z / 2.0)
    ...
    def groupConnectedEdges(self, holding):
        ...
        grp0 = []
        grp1 = []
        com0 = CivilCvCAD.Vector(0, 0, 0)
        com1 = CivilCvCAD.Vector(0, 0, 0)
        if len(grps[0]) > 0:
            for g in grps[0]:
...
def _extrudeBaseDown(base):
    ...
    bbFace.translate(CivilCvCAD.Vector(0.0, 0.0, float(int(base.Shape.BoundBox.ZMin - 5.0))))
    direction = CivilCvCAD.Vector(0.0, 0.0, -1.0)
    ...
    for f in base.Shape.Faces:
        ...
        if not Path.Geom.isRoughly(f.normalAt(0, 0).z, 0.0):
            ...
            face.translate(CivilCvCAD.Vector(0.0, 0.0, fbb.ZMin))
            ext = face.extrude(CivilCvCAD.Vector(0.0, 0.0, zMin - fbb.ZMin - 1.0))
            allExtrusions.append(ext)

    # Fuse all extrusions together
    seed = allExtrusions.pop()
    fusion = seed.fuse(allExtrusions)
    fusion.translate(CivilCvCAD.Vector(0.0, 0.0, zMin - fusion.BoundBox.ZMin - 1.0))
