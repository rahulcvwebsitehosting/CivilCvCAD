...
class ObjectPocket(PathPocketBase.ObjectPocket):
    ...
    def removeHoles(self, solid, face):
        ...
        try:
            ...
            cutting_plane.translate(CivilCvCAD.Vector(0, 0, adaptive_tolerance))
            ...
            if hasattr(section, "Edges") and section.Edges:
                ...
                for edge in section.Edges:
                    ...
                    translated_edge.translate(CivilCvCAD.Vector(0, 0, -adaptive_tolerance))
        ...
        if obj.Base:
            ...
            if len(self.vert) > 0:
                ...
                for shape in self.vertical:
                    try:
                        self.vWires.append(
                            TechDraw.findShapeOutline(shape, 1, CivilCvCAD.Vector(0, 0, 1))
                        )
            ...
            for h in self.horizontal:
                h.translate(
                    CivilCvCAD.Vector(0.0, 0.0, obj.FinalDepth.Value - h.BoundBox.ZMin - buffer)
                )

            # extrude all faces up to StartDepth plus buffer and those are the removal shapes
            extent = CivilCvCAD.Vector(0, 0, obj.StartDepth.Value - obj.FinalDepth.Value + buffer)
            self.removalshapes = [
            ...
            self.outlines = [
                Part.Face(TechDraw.findShapeOutline(base.Shape, 1, CivilCvCAD.Vector(0, 0, 1)))
            ...
            for outline in self.outlines:
                outline.translate(CivilCvCAD.Vector(0, 0, stockBB.ZMin - 1))
                body = outline.extrude(CivilCvCAD.Vector(0, 0, stockBB.ZLength + 2))
    ...
    def classifySubFace(self, bs, sub):
        ...
        if isinstance(face.Surface, Part.Plane):
            ...
            if any(e.isClosed() for e in face.Edges):
                ...
                disk.translate(CivilCvCAD.Vector(0, 0, face.BoundBox.ZMin - disk.BoundBox.ZMin))
