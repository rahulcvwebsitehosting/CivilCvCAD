...
if CivilCvCAD.GuiUp:
    CivilCvCADGui = LazyLoader("CivilCvCADGui", globals(), "CivilCvCADGui")
...
class ObjectSlot(PathOp.ObjectOp):
    ...
    def initOpProperties(self, obj, warn=False):
        ...
        if len(self.addNewProps) > 0:
            ...
            if warn:
                ...
                CivilCvCAD.Console.PrintWarning(newPropMsg + "\n")
    ...
    def opPropertyDefaults(self, obj, job):
        ...
        defaults = {
            "CustomPoint1": CivilCvCAD.Vector(0, 0, 0),
            "ExtendPathStart": 0,
            "Reference1": "Center of Mass",
            "CustomPoint2": CivilCvCAD.Vector(0, 0, 0),
            "ExtendPathEnd": 0,
    ...
    def opExecute(self, obj):
        ...
        if self.showDebugObjects:
            ...
            self.tmpGrp = CivilCvCAD.ActiveDocument.addObject(
        ...
        # Hide debug visuals
        if self.showDebugObjects and CivilCvCAD.GuiUp:
            CivilCvCADGui.ActiveDocument.getObject(self.tmpGrp.Name).Visibility = False
    ...
    def _makeOperation(self, obj):
        ...
        if not hasattr(obj, "Base"):
            ...
            CivilCvCAD.Console.PrintUserWarning(msg + "\n")
            return False
    ...
    def _processSingleVertFace(self, obj, shape):
        ...
        perpVect = CivilCvCAD.Vector(-slope.y, slope.x, slope.z)
    ...
    def _processSingleEdge(self, obj, edge):
        ...
        verts = edge.Vertexes
        V1 = verts[0]
        p1 = CivilCvCAD.Vector(V1.X, V1.Y, 0)
        p2 = p1 if len(verts) == 1 else CivilCvCAD.Vector(verts[1].X, verts[1].Y, 0)
        ...
        if isinstance(edge.Curve, (Part.Line, Part.LineSegment)):
            ...
            if len(verts) == 1:  # Full circle
                ...
                self.arcCenter = CivilCvCAD.Vector(center.x, center.y, 0)
                mid = edge.valueAt(edge.getParameterByLength(edge.Length / 2))
                self.arcMidPnt = CivilCvCAD.Vector(mid.x, mid.y, 0)
                ...
                mid.z = 0
                center = circumCircleFrom3Points(p1, p2, CivilCvCAD.Vector(mid.x, mid.y, 0))
                if not center:
    ...
    def _normalizeVector(self, v):
        ...
        tol = 1e-10
        V = CivilCvCAD.Vector(v).normalize()
        ...
        return CivilCvCAD.Vector(snap(V.x), snap(V.y), snap(V.z))
    ...
    def _getLowestPoint(self, shape):
        ...
        return CivilCvCAD.Vector(avg_x, avg_y, lowest_z)
    ...
    def _getHighestPoint(self, shape):
        ...
        return CivilCvCAD.Vector(avg_x, avg_y, highest_z)
    ...
    def _processFeature(self, obj, shape, sub, pNum):
        ...
        if sub.startswith("Face"):
            ...
            temp = CivilCvCAD.Vector(v1.x - V0.X, v1.y - V0.Y, 0)
            dYdX = self._normalizeVector(temp) if temp.Length != 0 else CivilCvCAD.Vector(0, 0, 0)

            # Face normal must be vertical
            norm = shape.normalAt(0, 0)
            if norm.z != 0:
                msg = translate("CAM_Slot", "The selected face is not oriented vertically:")
                CivilCvCAD.Console.PrintError(f"{msg} {sub}.\n")
                return False

            # Choose working point
            if Ref == "Center of Mass":
                com = shape.CenterOfMass
                p = CivilCvCAD.Vector(com.x, com.y, 0)
            elif Ref == "Center of BoundBox":
                bbox = shape.BoundBox.Center
                p = CivilCvCAD.Vector(bbox.x, bbox.y, 0)
    ...
    def _extendArcSlot(self, p1, p2, cent, begExt, endExt):
        ...
        def makeChord(angle_rad):
            ...
            a = CivilCvCAD.Vector(self.newRadius, 0, 0)
            b = CivilCvCAD.Vector(x, y, 0)
            return Part.makeLine(a, b)

        origin = CivilCvCAD.Vector(0, 0, 0)
        z_axis = CivilCvCAD.Vector(0, 0, 1)
    ...
    def _getOppMidPoints(self, same):
        ...
        p1 = CivilCvCAD.Vector(com1.x, com1.y, 0)
        p2 = CivilCvCAD.Vector(com2.x, com2.y, 0)
        return p1, p2

    def _isParallel(self, dYdX1, dYdX2):
        """Determine if two orientation vectors are parallel."""
        return dYdX1.cross(dYdX2) == CivilCvCAD.Vector(0, 0, 0)
    ...
    def _makePerpendicular(self, p1, p2, length):
        ...
        if getattr(self, "dYdX1", None):
            half = CivilCvCAD.Vector(self.dYdX1.x, self.dYdX1.y, 0).multiply(halfDist)
            n1 = midPnt.add(half)
            n2 = midPnt.sub(half)
            return n1, n2

        elif getattr(self, "dYdX2", None):
            half = CivilCvCAD.Vector(self.dYdX2.x, self.dYdX2.y, 0).multiply(halfDist)
            n1 = midPnt.add(half)
            n2 = midPnt.sub(half)
            return n1, n2
            ...
            toEnd = p2.sub(p1)
            perp = CivilCvCAD.Vector(-toEnd.y, toEnd.x, 0)
    ...
    def _findLowestPointOnEdge(self, E):
        ...
        location = CivilCvCAD.Vector(0, 0, Part.Edge(E.Curve).BoundBox.ZMin)
        plane = Part.Plane(location, CivilCvCAD.Vector(0, 0, 1))
    ...
    def _findHighestPointOnEdge(self, E):
        ...
        # use intersection with plane to find lowest point
        location = CivilCvCAD.Vector(0, 0, Part.Edge(E.Curve).BoundBox.ZMax)
        plane = Part.Plane(location, CivilCvCAD.Vector(0, 0, 1))
        point = plane.intersect(E.Curve)[0][0]
        return point.toShape().Point
    ...
    def _getCutSidePoints(self, obj, v0, v1, a1, a2, b1, b2):
        ...
        cubeA = boxA.extrude(CivilCvCAD.Vector(0, 0, 1))
        ...
        cubeB = boxB.extrude(CivilCvCAD.Vector(0, 0, 1))
    ...
    def _getVertFaceType(self, shape):
        ...
        if (wire := slices[0]).isClosed() and (face := Part.Face(wire)) > 0:
            ...
            face.translate(CivilCvCAD.Vector(0, 0, z_offset))
            return "Face", face
        return "Wire", wire
    ...
    def _lineCollisionCheck(self, obj, p1, p2):
        ...
        extVect = CivilCvCAD.Vector(0, 0, obj.StartDepth.Value - obj.FinalDepth.Value)

        def make_cylinder(point):
            circle = Part.makeCircle(rad, point)
            face = Part.Face(Part.Wire(circle.Edges))
            face.translate(CivilCvCAD.Vector(0, 0, obj.FinalDepth.Value - face.BoundBox.ZMin))
    ...
    def _arcCollisionCheck(self, obj, p1, p2, arcCenter, arcRadius):
        ...
        def make_cylinder_at_point(point, radius, height, final_depth):
            ...
            face = Part.Face(Part.Wire(circle.Edges))
            face.translate(CivilCvCAD.Vector(0, 0, final_depth - face.BoundBox.ZMin))
            return face.extrude(CivilCvCAD.Vector(0, 0, height))
        ...
        def make_arc_face(p1, p2, center, inner_radius, outer_radius):
            ...
            pa = CivilCvCAD.Vector(*arc_inside.Vertexes[0].Point[:2], 0)
            pb = CivilCvCAD.Vector(*arc_inside.Vertexes[1].Point[:2], 0)
            pc = CivilCvCAD.Vector(*arc_outside.Vertexes[1].Point[:2], 0)
            pd = CivilCvCAD.Vector(*arc_outside.Vertexes[0].Point[:2], 0)
        ...
        if self.isArc == 1:
            ...
            path.translate(CivilCvCAD.Vector(0, 0, obj.FinalDepth.Value - path.BoundBox.ZMin))
            ...
            rectFace.translate(CivilCvCAD.Vector(0, 0, obj.FinalDepth.Value - rectFace.BoundBox.ZMin))
    ...
    def _addDebugObject(self, objShape, objName):
        if self.showDebugObjects:
            do = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmp_" + objName)
            do.Shape = objShape
            do.purgeTouched()
...
def Create(name, obj=None, parentJob=None):
    ...
    if obj is None:
        obj = CivilCvCAD.ActiveDocument.addObject("Path::FeaturePython", name)
