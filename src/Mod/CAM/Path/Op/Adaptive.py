...
import area
import Constants
from CivilCvCAD import BoundBox
...
def initSceneGraph(z=10):
    ...
    sceneGraph = CivilCvCADGui.ActiveDocument.ActiveView.getSceneGraph()
    topZ = z
    return True
    ...
    for region in adaptiveResults:
        ...
        for passEndDepth in depthParams.data:
            ...
            if helixRadius > 0.01:
                ...
                centerTop = CivilCvCAD.Vector(p1[0], p1[1], passStartDepth)
                centerBottom = CivilCvCAD.Vector(p1[0], p1[1], passEndDepth)
...
def Execute(op, obj):
    ...
    try:
        ...
        outer_wire = TechDraw.findShapeOutline(op.stock.Shape, 1, CivilCvCAD.Vector(0, 0, 1))
...
def projectFacesToXY(faces, minEdgeLength=1e-10):
    ...
    """
    projdir = CivilCvCAD.Vector(0, 0, 1)
    outfaces = []
    for f in faces:
...
def _getSolidProjection(shp, z):
    ...
    bbCutTop = Part.makeBox(
        ...
        CivilCvCAD.Vector(bb.XMin, bb.YMin, z),
    )
            ...
            if sub.startswith("Face"):
                if sub not in avoidFeatures:
                    if obj.UseOutline:
                        ...
                        # wire_A = TechDraw.findShapeOutline(face, 1, CivilCvCAD.Vector(0.0, 0.0, 1.0))
