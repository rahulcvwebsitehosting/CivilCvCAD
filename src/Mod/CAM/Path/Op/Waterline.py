...
class ObjectWaterline(PathOp.ObjectOp):
    ...
    def opPropertyDefaults(self, obj, job):
        ...
        defaults = {
            ...
            "StartPoint": CivilCvCAD.Vector(0.0, 0.0, obj.ClearanceHeight.Value),
            ...
            "AvoidLastX_Faces": 0,
            "PatternCenterCustom": CivilCvCAD.Vector(0.0, 0.0, 0.0),
            "GapThreshold": 0.005,
    ...
    def opExecute(self, obj):
        ...
        FCAD = CivilCvCAD.ActiveDocument
        ...
        if CivilCvCAD.GuiUp:
            for m in range(0, len(JOB.Model.Group)):
                ...
                modelVisibility.append(CivilCvCADGui.ActiveDocument.getObject(mNm).Visibility)
        ...
        if CivilCvCAD.GuiUp:
            CivilCvCADGui.ActiveDocument.getObject(tempGroupName).Visibility = False
            for m in range(0, len(JOB.Model.Group)):
                M = JOB.Model.Group[m]
            ...
            for STEP in PNTSET:
                for SEG in STEP:
                    if SEG == "BRK":
                        ...
                        P1 = CivilCvCAD.Vector(A[0], A[1], csHght)
                        P2 = CivilCvCAD.Vector(B[0], B[1], csHght)
                        stpOvr.append((P1, P2))
                SCANS.append(stpOvr)
                stpOvr = list()
        elif cutPattern in ["Circular", "CircularZigZag"]:
            ...
            for so in range(0, len(PNTSET)):
                ...
                for a in range(0, len(ARCS)):
                    ...
                    if Arc == "BRK":
                        ...
                        sp, ep, cp = Arc
                        S = CivilCvCAD.Vector(sp[0], sp[1], csHght)
                        E = CivilCvCAD.Vector(ep[0], ep[1], csHght)
                        C = CivilCvCAD.Vector(cp[0], cp[1], csHght)
                        scan = (S, E, C, cMode)
                        if scan is False:
                            erFlg = True
                        else:
    ...
    def _oclWaterlineOp(self, JOB, obj, mdlIdx, subShp=None):
        ...
        if self.holdPoint is None:
            self.holdPoint = CivilCvCAD.Vector(0.0, 0.0, 0.0)
        if self.layerEndPnt is None:
            self.layerEndPnt = CivilCvCAD.Vector(0.0, 0.0, 0.0)
        ...
        if obj.Algorithm == "OCL Adaptive":
            ...
            oclScan = [CivilCvCAD.Vector(P.x, P.y, P.z + depOfst) for P in oclScan]
    ...
    def _waterlineAdaptiveScan(self, stl, smplInt, minSmplInt, zheights, depOfst):
        ...
        for zh in zheights:
            ...
            for loop in temp_loops:
                # Convert OCL Points to CivilCvCAD Vectors and apply Z offset
                fc_loop = [CivilCvCAD.Vector(P.x, P.y, P.z + depOfst) for P in loop]
    ...
    def _loopToGcode(self, obj, layDep, loop):
        ...
        nxt = CivilCvCAD.Vector(0.0, 0.0, 0.0)
        ...
        if obj.Algorithm == "OCL Adaptive":
            if obj.CutMode == "Climb":
                ...
                loop.reverse()
            pnt = pnt1 = CivilCvCAD.Vector(loop[0].x, loop[0].y, loop[0].z)
        else:
            pnt = CivilCvCAD.Vector(loop[0].x, loop[0].y, layDep)
    ...
    def _experimentalWaterlineOp(self, JOB, obj, mdlIdx, subShp=None):
        ...
        for ca in range(0, caLen):
            ...
            if area.Area > 0.0:
                ...
                data = CivilCvCAD.Units.Quantity(csHght, CivilCvCAD.Units.Length).UserString
                Path.Log.debug("Cut area at {} is zero.".format(data))

            # get offset wire(s) based upon cross-section cut area
            if cont:
                area.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - area.BoundBox.ZMin))
            ...
            if cont:
                ...
                clearArea.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - clearArea.BoundBox.ZMin))
        ...
        if clearLastLayer and obj.ClearLastLayer != "Off":
            ...
            lastClearArea.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - lastClearArea.BoundBox.ZMin))
            if clrLyr == "Offset":
        ...
        for dp in range(0, lenDP):
            ...
            if csFaces:
                ...
                if useFaces:
                    ...
                    cutArea.translate(CivilCvCAD.Vector(0.0, 0.0, csHght - cutArea.BoundBox.ZMin))
    ...
    def _wiresToWaterlinePath(self, obj, ofstPlnrShp, csHght):
        ...
        ofstPlnrShp.translate(CivilCvCAD.Vector(0.0, 0.0, csHght - ofstPlnrShp.BoundBox.ZMin))
        ...
        for w in range(start, len(ofstPlnrShp.Wires)):
            ...
            if obj.CutMode == "Climb":
                lv = len(V) - 1
                startVect = CivilCvCAD.Vector(V[lv].X, V[lv].Y, V[lv].Z)
            else:
                startVect = CivilCvCAD.Vector(V[0].X, V[0].Y, V[0].Z)
    ...
    def _makeCutPatternLayerPaths(self, JOB, obj, clrAreaShp, csHght, cutPattern):
        ...
        clrAreaShp.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - clrAreaShp.BoundBox.ZMin))
        ...
        if cutPattern == "Offset":
            ...
            if not pathGeom:
                ...
                return commands
            pathGeom.translate(CivilCvCAD.Vector(0.0, 0.0, csHght - pathGeom.BoundBox.ZMin))
    ...
    def _getModelCrossSection(self, shape, csHght):
        ...
        for i in shape.slice(CivilCvCAD.Vector(0, 0, 1), csHght):
            wires.append(i)
        ...
        if len(wires) > 0:
            ...
            FCS = list()
            for w in wires:
                w.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - w.BoundBox.ZMin))
    ...
    def _makeExtendedBoundBox(self, wBB, bbBfr, zDep):
        ...
        pl.Rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), 0)
        pl.Base = CivilCvCAD.Vector(0, 0, 0)
