...
class PathGeometryGenerator:
    ...
    def __init__(self, obj, shape, pattern):
        ...
        if shape.BoundBox.ZMin != 0.0:
            shape.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - shape.BoundBox.ZMin))
        if shape.BoundBox.ZLength > 1.0e-8:
            ...
            CivilCvCAD.Console.PrintWarning(msg)
        else:
            self.shape = shape
    ...
    def _prepareConstants(self):
        ...
        if self.pattern in ["Circular", "CircularZigZag", "Spiral"]:
            if self.obj.PatternCenterAt == "CenterOfMass":
                fCnt = 0
                totArea = 0.0
                zeroCOM = CivilCvCAD.Vector(0.0, 0.0, 0.0)
                for F in self.shape.Faces:
                    comF = F.CenterOfMass
                    areaF = F.Area
                    totArea += areaF
                    fCnt += 1
                    zeroCOM = zeroCOM.add(CivilCvCAD.Vector(comF.x, comF.y, 0.0).multiply(areaF))
                if fCnt == 0:
                    ...
                    CivilCvCAD.Console.PrintError(msg)
                    bbC = self.shape.BoundBox.Center
                    zeroCOM = CivilCvCAD.Vector(bbC.x, bbC.y, 0.0)
                else:
                    avgArea = totArea / fCnt
                    zeroCOM.multiply(1 / fCnt)
                    zeroCOM.multiply(1 / avgArea)
                self.centerOfMass = CivilCvCAD.Vector(zeroCOM.x, zeroCOM.y, 0.0)
            self.centerOfPattern = self._getPatternCenter()
        else:
            bbC = self.shape.BoundBox.Center
            self.centerOfPattern = CivilCvCAD.Vector(bbC.x, bbC.y, 0.0)
    ...
    def generatePathGeometry(self):
        ...
        if self.pattern in ["Line", "ZigZag"]:
            if self.obj.CutPatternAngle != 0.0:
                geomShape.Placement.Rotation = CivilCvCAD.Rotation(
                    CivilCvCAD.Vector(0, 0, 1), self.obj.CutPatternAngle
                )
            bbC = self.shape.BoundBox.Center
            geomShape.Placement.Base = CivilCvCAD.Vector(bbC.x, bbC.y, 0.0 - geomShape.BoundBox.ZMin)

        if self.debugObjectsGroup:
            F = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmpGeometrySet")
            F.Shape = geomShape
            F.purgeTouched()
        ...
        if self.debugObjectsGroup:
            F = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmpPathGeometry")
            F.Shape = cmnShape
            F.purgeTouched()
            self.debugObjectsGroup.addObject(F)
    ...
    def _Line(self):
        GeoSet = []
        centRot = CivilCvCAD.Vector(0.0, 0.0, 0.0)  # Bottom left corner of face/selection/model
        ...
        for lc in range((-1 * (self.halfPasses - 1)), self.halfPasses + 1):
            ...
            # y2 = y1
            p1 = CivilCvCAD.Vector(x1, y1, 0.0)
            p2 = CivilCvCAD.Vector(x2, y1, 0.0)
    ...
    def _Spiral(self):
        ...
        lastPoint = CivilCvCAD.Vector(0.0, 0.0, 0.0)
    ...
    def _getPatternCenter(self):
        ...
        if centerAt == "CenterOfMass":
            cntrPnt = CivilCvCAD.Vector(self.centerOfMass.x, self.centerOfMass.y, 0.0)
        elif centerAt == "CenterOfBoundBox":
            cent = self.shape.BoundBox.Center
            cntrPnt = CivilCvCAD.Vector(cent.x, cent.y, 0.0)
        elif centerAt == "XminYmin":
            cntrPnt = CivilCvCAD.Vector(self.shape.BoundBox.XMin, self.shape.BoundBox.YMin, 0.0)
        elif centerAt == "Custom":
            cntrPnt = CivilCvCAD.Vector(
    ...
    def _getRadialPasses(self):
        ...
        if self.obj.PatternCenterAt != "CenterOfBoundBox":
            ...
            CORNERS = [
                CivilCvCAD.Vector(EBB.XMin, EBB.YMin, 0.0),
                CivilCvCAD.Vector(EBB.XMin, EBB.YMax, 0.0),
                CivilCvCAD.Vector(EBB.XMax, EBB.YMax, 0.0),
                CivilCvCAD.Vector(EBB.XMax, EBB.YMin, 0.0),
            ]
            dMax = 0.0
            for c in range(0, 4):
    ...
    def _makeRegSpiralPnt(self, move, b, radAng):
        ...
        return CivilCvCAD.Vector(x, y, 0.0).add(move)

    def _makeOppSpiralPnt(self, move, b, radAng):
        x = b * radAng * math.cos(radAng)
        y = b * radAng * math.sin(radAng)
        return CivilCvCAD.Vector(-1 * x, y, 0.0).add(move)
        ...
        if self.obj.Base:
            if len(self.obj.Base) > 0:
                ...
                if self.obj.ScanType == "Rotational":
                    self.checkBase = False
                    CivilCvCAD.Console.PrintWarning(self.msgNoFaces)
    ...
    def _preProcessFacesAndVoids(self, base, FCS, VDS):
            ...
            if self.obj.HandleMultipleFeatures == "Collectively":
                ...
                if cont:
                    if self.showDebugObjects:
                        T = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmpCollectiveShape")
                        T.Shape = cfsL
                        T.purgeTouched()
                ...
                if cont:
                    ...
                    if not self.obj.InternalFeaturesCut:
                        if lenIfL == 0:
                            ...
                            if self.showDebugObjects:
                                C = CivilCvCAD.ActiveDocument.addObject(
        ...
        if VDS:
            ...
            if lenOtFcs == 0:
                ...
                if self.showDebugObjects:
                    P = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmpVoidEnvelope")
                    P.Shape = avoid
                    P.purgeTouched()
            ...
            if cont:
                if self.showDebugObjects:
                    P = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmpVoidCompound")
                    P.Shape = avoid
                    P.purgeTouched()
                    self.tempGroup.addObject(P)
        ...
        if cont:
            ...
            if faceOffsetShape is False:
                ...
                faceOffsetShape.translate(
                    CivilCvCAD.Vector(0.0, 0.0, 0.0 - faceOffsetShape.BoundBox.ZMin)
                )
...
def getExtrudedShape(wire):
    ...
    try:
        shell = wire.extrude(CivilCvCAD.Vector(0.0, 0.0, extFwd))
    except Exception as ee:
...
def getShapeSlice(shape):
    ...
    ymin = bb.YMin - 1.0
    ymax = bb.YMax + 1.0
    p1 = CivilCvCAD.Vector(xmin, ymin, mid)
    p2 = CivilCvCAD.Vector(xmax, ymin, mid)
    p3 = CivilCvCAD.Vector(xmax, ymax, mid)
    p4 = CivilCvCAD.Vector(xmin, ymax, mid)
    ...
    if slcArea < midArea:
        ...
        if len(slcShp.Wires) == 1:
            wire = slcShp.Wires[0]
            slc = Part.Face(wire)
            slc.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - slc.BoundBox.ZMin))
            return slc
        else:
            fL = []
            for W in slcShp.Wires:
...
def getProjectedFace(tempGroup, wire):
    ...
    F = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmpProjectionWire")
    F.Shape = wire
    F.purgeTouched()
    tempGroup.addObject(F)
    try:
        prj = Draft.makeShape2DView(F, CivilCvCAD.Vector(0, 0, 1))
        prj.recompute()
        prj.purgeTouched()
...
def getCrossSection(shape):
    ...
    for i in shape.slice(CivilCvCAD.Vector(0, 0, 1), mid):
        wires.append(i)

    if len(wires) > 0:
        comp = Part.Compound(wires)  # produces correct cross-section wire !
        comp.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - comp.BoundBox.ZMin))
        csWire = comp.Wires[0]
        if csWire.isClosed() is False:
            Path.Log.debug(" -comp.Wires[0] is not closed")
            return False
        CS = Part.Face(csWire)
        CS.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - CS.BoundBox.ZMin))
        return CS
    else:
...
def getShapeEnvelope(shape):
    ...
    try:
        ...
        CivilCvCAD.Console.PrintError("PathUtils.getEnvelope() failed.\n" + str(ee) + "\n")
        return False
    else:
        return env
...
def getSliceFromEnvelope(env):
    ...
    tf.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - tf.BoundBox.ZMin))
...
def _makeSafeSTL(self, JOB, obj, mdlIdx, faceShapes, voidShapes, ocl):
    ...
    if obj.BoundBox == "BaseBoundBox":
        ...
        bH = 1.0
        crnr = CivilCvCAD.Vector(xMin, yMin, zMin - 1.0)
        B = Part.makeBox(bL, bW, bH, crnr, CivilCvCAD.Vector(0, 0, 1))
        fuseShapes.append(B)
    ...
    if self.showDebugObjects:
        T = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "safeSTLShape")
        T.Shape = fused
        T.purgeTouched()
        self.tempGroup.addObject(T)
...
def pathGeomToLinesPointSet(self, obj, compGeoShp):
    ...
    if self.CutClimb is True:
        tup = (p2, p1)
        lst = CivilCvCAD.Vector(p1[0], p1[1], 0.0)
    else:
        tup = (p1, p2)
        lst = CivilCvCAD.Vector(p2[0], p2[1], 0.0)
    inLine.append(tup)
    sp = CivilCvCAD.Vector(p1[0], p1[1], 0.0)  # start point
    ...
    for ei in range(1, ec):
        ...
        ep = CivilCvCAD.Vector(v2[0], v2[1], 0.0)  # end point
        cp = CivilCvCAD.Vector(v1[0], v1[1], 0.0)  # check point (first / middle point)
        ...
        if iC is True:
            ...
            if self.CutClimb is True:
                sp = cp  # CivilCvCAD.Vector(v1[0], v1[1], 0.0)
            else:
                sp = ep
...
def pathGeomToZigzagPointSet(self, obj, compGeoShp):
    ...
    if dirFlg == 1:
        ...
        tup = (p2, p1)
        lst = CivilCvCAD.Vector(p1[0], p1[1], 0.0)
        sp = CivilCvCAD.Vector(p2[0], p2[1], 0.0)  # start point
    inLine.append(tup)
    ...
    for ei in range(1, ec):
        ...
        cp = CivilCvCAD.Vector(v1[0], v1[1], 0.0)  # check point (start point of segment)
        ep = CivilCvCAD.Vector(v2[0], v2[1], 0.0)  # end point
        iC = cp.isOnLineSegment(sp, ep)
        if iC:
            inLine.append("BRK")
...
def pathGeomToCircularPointSet(self, obj, compGeoShp):
    ...
    def dist_to_cent(item):
        ...
        s = item[2][0][0]
        p1 = CivilCvCAD.Vector(s[0], s[1], 0.0)
        e = item[2][0][2]
        p2 = CivilCvCAD.Vector(e[0], e[1], 0.0)
    ...
    for ei in range(0, ec):
        ...
        if edg.Closed is True:
            ...
            if isSame is False:
                segEI.append(ei)
                isSame = True
                pnt = CivilCvCAD.Vector(edg.Vertexes[0].X, edg.Vertexes[0].Y, 0.0)
    ...
    for so in range(0, len(stpOvrEI)):
        ...
        if SO[0] == "L":  # L = Loop/Ring/Circle
            ...
            # p1 = CivilCvCAD.Vector(v1.X, v1.Y, v1.Z)
            p1 = CivilCvCAD.Vector(v1.X, v1.Y, 0.0)  # z=0.0 for waterline; z=v1.Z for 3D Surface
...
def pathGeomToSpiralPointSet(obj, compGeoShp):
    ...
    p1 = CivilCvCAD.Vector(edg1.Vertexes[0].X, edg1.Vertexes[0].Y, 0.0)
    p2 = CivilCvCAD.Vector(edg1.Vertexes[1].X, edg1.Vertexes[1].Y, 0.0)
    tup = ((p1.x, p1.y), (p2.x, p2.y))
    inLine.append(tup)

    for ei in range(start, ec):  # Skipped first edge, started with second edge above as edg1
        edg = compGeoShp.Edges[ei]  # Get edge for vertexes
        sp = CivilCvCAD.Vector(
            edg.Vertexes[0].X, edg.Vertexes[0].Y, 0.0
        )  # check point (first / middle point)
        ep = CivilCvCAD.Vector(edg.Vertexes[1].X, edg.Vertexes[1].Y, 0.0)  # end point
...
def pathGeomToOffsetPointSet(obj, compGeoShp):
    ...
    for ei in range(0, ofstCnt):
        ...
        fp = CivilCvCAD.Vector(OS[0].x, OS[0].y, OS[0].z)
        OS.append(fp)
...
class FindUnifiedRegions:
    ...
    def _showShape(self, shape, name):
        if self.tempGroup:
            S = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "tmp" + name)
            S.Shape = shape
            S.purgeTouched()
    ...
    def _extractTopFaces(self):
        for F, fcIdx in self.FACES:  # format is tuple (faceShape, faceIndex_on_base)
            ...
            ef = F.extrude(CivilCvCAD.Vector(0.0, 0.0, extFwd))
            ...
            cutBox.translate(CivilCvCAD.Vector(efBB.XMin - 1.0, efBB.YMin - 1.0, zHght))
            base = ef.cut(cutBox)
            ...
            if cont:
                topFace.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - zMin))
        ...
        if numLoops > 0:
            for li in range(0, numLoops):
                ...
                if wire.isClosed():
                    ...
                    extWire = wire.extrude(CivilCvCAD.Vector(0.0, 0.0, 2.0))
...
class OCL_Tool:
    """The OCL_Tool class is designed to translate a CivilCvCAD standard ToolBit shape
    ...
    """
        #### CivilCvCAD Legacy tool shape properties per tool type
        shape = EndMill
        Diameter
        CuttingEdgeHeight
        LengthOffset
        ...
        #### CivilCvCAD packaged ToolBit named constraints per shape files
        shape = endmill
    ...
    def getOclTool(self):
        ...
        if self.toolMethod == "None":
            ...
            CivilCvCAD.Console.PrintError(err + "\n")
            return False
        else:
...
def makeExtendedBoundBox(wBB, bbBfr, zDep):
    ...
    p1 = CivilCvCAD.Vector(wBB.XMin - bbBfr, wBB.YMin - bbBfr, zDep)
    p2 = CivilCvCAD.Vector(wBB.XMax + bbBfr, wBB.YMin - bbBfr, zDep)
    p3 = CivilCvCAD.Vector(wBB.XMax + bbBfr, wBB.YMax + bbBfr, zDep)
    p4 = CivilCvCAD.Vector(wBB.XMin - bbBfr, wBB.YMax + bbBfr, zDep)
