...
class ObjectRotarySurface(PathOp.ObjectOp):
    ...
    def _resolve_rotary(self, obj, job):
        ...
        rot_vec = CivilCvCAD.Vector(rv.x, rv.y, rv.z)
    ...
    def _validate_stock(self, stock, axis_label):
        ...
        cyl_axis = rot.multVec(CivilCvCAD.Vector(0, 0, 1))
        cyl_axis.normalize()

        target = CivilCvCAD.Vector(1, 0, 0) if axis_label == "X" else CivilCvCAD.Vector(0, 1, 0)
