...
def _sortVoronoiWires(wires, start=CivilCvCAD.Vector(0, 0, 0)):
...
def canSkipRepositioning(positionHistory, newPosition, tolerance):
    ...
    v0 = CivilCvCAD.Base.Vector(currentPosition.x, currentPosition.y)
    v1 = CivilCvCAD.Base.Vector(newPosition.x, newPosition.y)
...
class ObjectVcarve(PathEngraveBase.ObjectOp):
    ...
    def buildMedialWires(self, obj, faces):
        ...
        def is_exterior(vertex, face):
            vector = CivilCvCAD.Vector(vertex.toPoint(face.BoundBox.ZMin))
        ...
        def insert_many_wires(vd, wires):
            for wire in wires:
                ...
                ptv = [CivilCvCAD.Vector(p.x, p.y) for p in pts]
                ...
                if len(ptv) > 0:
                    ...
                    if dist < CivilCvCAD.Base.Precision.confusion():
                        Path.Log.debug(
    ...
    def debugVoronoiMedial(self, obj):
        ...
        vPart = CivilCvCAD.activeDocument().addObject("App::Part", f"{obj.Name}-VoronoiDebugMedial")
    ...
    def debugVoronoiEdges(self, obj):
        ...
        vPart = CivilCvCAD.activeDocument().addObject("App::Part", f"{obj.Name}-VoronoiDebugEdge")
