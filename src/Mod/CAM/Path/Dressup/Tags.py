...
def debugMarker(vector, label, color=None, radius=0.5):
    if logger.getLevel() == Path.Log.Level.DEBUG:
        obj = CivilCvCAD.ActiveDocument.addObject("Part::Sphere", label)
        obj.Label = label
        obj.Radius = radius
        obj.Placement = CivilCvCAD.Placement(vector, CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), 0))
        if color:
...
def debugCylinder(vector, r, height, label, color=None):
    if logger.getLevel() == Path.Log.Level.DEBUG:
        obj = CivilCvCAD.ActiveDocument.addObject("Part::Cylinder", label)
        obj.Label = label
        obj.Radius = r
        obj.Height = height
        obj.Placement = CivilCvCAD.Placement(vector, CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), 0))
...
def debugCone(vector, r1, r2, height, label, color=None):
    if logger.getLevel() == Path.Log.Level.DEBUG:
        obj = CivilCvCAD.ActiveDocument.addObject("Part::Cone", label)
        obj.Label = label
        obj.Radius1 = r1
        obj.Radius2 = r2
        obj.Height = height
...
class Tag:
    def __init__(self, nr, x, y, width, height, angle, radius, enabled=True):
        ...
        self.radius = getattr(
            radius, "Value", CivilCvCAD.Units.Quantity(radius, CivilCvCAD.Units.Length).Value
        )
    ...
    def originAt(self, z):
        return CivilCvCAD.Vector(self.x, self.y, z)
    ...
    def createSolidsAt(self, z, R):
        ...
        if not Path.Geom.isRoughly(0, R):  # testing is easier if the solid is not rotated
            ...
            self.solid.rotate(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(0, 0, 1), angle)
    ...
    def isPointOnEdge(self, pt, edge):
        ...
        # p1 = edge.Vertexes[0]
        # f1 = edge.Curve.parameter(CivilCvCAD.Vector(p1.X, p1.Y, p1.Z))
        # p2 = edge.Vertexes[1]
        # f2 = edge.Curve.parameter(CivilCvCAD.Vector(p2.X, p2.Y, p2.Z))
        return False
...
class MapWireToTag:
    ...
    def shell(self):
        ...
        shell = wire.extrude(CivilCvCAD.Vector(0, 0, self.tag.height + 1))
...
class PathData:
    ...
    def copyTags(self, obj, fromObj, width, height, angle, radius):
        ...
        for i, pos in enumerate(fromObj.Positions):
            if i in fromObj.Disabled:
                continue
            p = Part.Vertex(CivilCvCAD.Vector(pos.x, pos.y, self.minZ))
    ...
    def pointAtBottom(self, p):
        return CivilCvCAD.Vector(p.x, p.y, self.minZ)
        ...
        if logger.getLevel() == Path.Log.Level.DEBUG:
            for tag in self.tags:
                ...
                if tag.enabled:
                    ...
                    # debugMarker(CivilCvCAD.Vector(tag.x, tag.y, self.pathData.minZ), "tag-{:02d}".format(tagID) , (1.0, 0.0, 1.0), 0.5)
