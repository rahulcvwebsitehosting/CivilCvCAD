...
class TaskPanel(object):
    ...
    def __init__(self, viewProvider, obj):
        ...
        self.obj = obj
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/DogboneEdit.ui")
        self.s = None
        CivilCvCAD.ActiveDocument.openTransaction("Edit Dogbone Dress-up")
        # self.height = 10 ???
        self.markers = []
    ...
    def reject(self):
        ...
        CivilCvCADGui.Selection.removeObserver(self.s)
        self.cleanup()
    ...
    def updateBoneList(self):
        ...
        for item in sorted(itemList, key=lambda item: item.data(self.DataState).boneIDs()[0]):
            ...
            markers.append(
                Marker(
                    CivilCvCAD.Vector(loc.x, loc.y, min(zs)),
                    r,
                    max(1, max(zs) - min(zs)),
                    state.isEnabled(),
                )
    ...
    def open(self):
        ...
        CivilCvCADGui.Selection.addObserver(self.s)
...
class SelObserver(object):
    ...
    def addSelection(self, doc, obj, sub, pnt):
        CivilCvCADGui.doCommand(f"obj = CivilCvCAD.ActiveDocument.getObject('{obj}')")
        CivilCvCADGui.doCommand("Gui.Selection.addSelection(obj)")
...
class CommandDressupDogboneII(object):
    ...
    def Activated(self):
        ...
        # everything ok!
        CivilCvCAD.ActiveDocument.openTransaction("Create Dogbone Dress-up")
        CivilCvCADGui.addModule("Path.Dressup.Gui.DogboneII")
        CivilCvCADGui.doCommand(f"base = CivilCvCAD.ActiveDocument.getObject('{op.Name}')")
        CivilCvCADGui.doCommand("Path.Dressup.Gui.DogboneII.Create(base)")
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_DressupDogbone", CommandDressupDogboneII())

CivilCvCAD.Console.PrintLog("Loading DressupDogboneII… done\n")
