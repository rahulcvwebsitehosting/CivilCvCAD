...
__doc__ = """Dragknife Dressup object and CivilCvCAD command"""
...
class ObjectDressup:
    ...
    def shortcut(self, queue):
        ...
        if queue[1].Name in arccommands:
            ...
            v1 = radvector.cross(CivilCvCAD.Vector(0, 0, 1))
        else:
        ...
        if queue[0].Name in arccommands:
            ...
            v2 = radvector.cross(CivilCvCAD.Vector(0, 0, 1))
        else:
    ...
    def segmentAngleXY(self, prevCommand, currCommand, endpos=False, currentZ=0):
        ...
        myAngle = D.angle(v1, CivilCvCAD.Base.Vector(1, 0, 0), CivilCvCAD.Base.Vector(0, 0, -1))
        return myAngle
    ...
    def arcExtension(self, obj, queue):
        ...
        C = CivilCvCAD.Base.Vector(queue[2].x + queue[1].I, queue[2].y + queue[1].J, currLocation["Z"])
        ...
        # endpoint = CivilCvCAD.Base.Vector(Bx, By, currLocation["Z"])
    ...
    def arcTwist(self, obj, queue, lastXY, twistCW=False):
        ...
        arccenter = CivilCvCAD.Base.Vector(
            queue[1].x + queue[0].I, queue[1].y + queue[0].J, currLocation["Z"]
        )
        ...
        segAngle = D.angle(v1, CivilCvCAD.Base.Vector(1, 0, 0), CivilCvCAD.Base.Vector(0, 0, -1))
        ...
        endpointvector = CivilCvCAD.Base.Vector(Bx, By, currLocation["Z"])
    ...
    def lineTwist(self, obj, queue, lastXY, twistCW=False):
        ...
        segAngle = D.angle(v2, CivilCvCAD.Base.Vector(1, 0, 0), CivilCvCAD.Base.Vector(0, 0, -1))
        xoffset = math.cos(segAngle) * offset
        yoffset = math.sin(segAngle) * offset
                    ...
                    if changedXYFlag and (len(queue) == 3):
                        ...
                        if abs(incident_angle) >= obj.filterAngle:
                            ...
                            if queue[1].Name in ["G01", "G1"]:
                                ...
                                CivilCvCAD.Console.PrintWarning("I don't know what's up")
                            #
                            #  DO THE TWIST
                            #
...
class TaskPanel:
    def __init__(self, obj):
        self.obj = obj
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/DragKnifeEdit.ui")
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Edit Dragknife Dress-up")
...
class ViewProviderDressup:
        ...
        if self.Object and self.Object.Base:
            for i in self.Object.Base.InList:
                if hasattr(i, "Group"):
                    ...
                    i.Group = group
                    print(i.Group)
            # CivilCvCADGui.ActiveDocument.getObject(obj.Base.Name).Visibility = False
    ...
    def onDelete(self, arg1=None, arg2=None):
        if arg1.Object and arg1.Object.Base:
            CivilCvCADGui.ActiveDocument.getObject(arg1.Object.Base.Name).Visibility = True
...
class CommandDressupDragknife:
    ...
    def Activated(self):
        ...
        # everything ok!
        CivilCvCAD.ActiveDocument.openTransaction("Create Dress-up")
        CivilCvCADGui.addModule("Path.Dressup.Gui.Dragknife")
        CivilCvCADGui.addModule("PathScripts.PathUtils")
        CivilCvCADGui.doCommand(
            'obj = CivilCvCAD.ActiveDocument.addObject("Path::FeaturePython","DragknifeDressup")'
        )
        CivilCvCADGui.doCommand("Path.Dressup.Gui.Dragknife.ObjectDressup(obj)")
        CivilCvCADGui.doCommand(f"base = CivilCvCAD.ActiveDocument.getObject('{op.Name}')")
        CivilCvCADGui.doCommand("job = PathScripts.PathUtils.findParentJob(base)")
        CivilCvCADGui.doCommand("obj.Base = base")
        CivilCvCADGui.doCommand("job.Proxy.addOperation(obj, base)")
        CivilCvCADGui.doCommand(
            "obj.ViewObject.Proxy = Path.Dressup.Gui.Dragknife.ViewProviderDressup(obj.ViewObject)"
        )
        CivilCvCADGui.doCommand("base.Visibility = False")
        CivilCvCADGui.doCommand("obj.filterAngle = 20")
        CivilCvCADGui.doCommand("obj.offset = 2")
        CivilCvCADGui.doCommand("obj.pivotheight = 4")
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_DressupDragKnife", CommandDressupDragknife())

CivilCvCAD.Console.PrintLog("Loading CAM_DressupDragKnife… done\n")
