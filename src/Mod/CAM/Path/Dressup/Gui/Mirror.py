...
class CommandPathDressup:
    ...
    def Activated(self):
        ...
        CivilCvCADGui.addModule("Path.Dressup.Gui.Mirror")
        CivilCvCADGui.addModule("PathScripts.PathUtils")
        CivilCvCADGui.doCommand(
            'obj = CivilCvCAD.ActiveDocument.addObject("Path::FeaturePython", "MirrorDressup")'
        )
        CivilCvCADGui.doCommand("Path.Dressup.Gui.Mirror.ObjectDressup(obj)")
        CivilCvCADGui.doCommand(f"baseOp = CivilCvCAD.ActiveDocument.getObject('{op.Name}')")
        CivilCvCADGui.doCommand("job = PathScripts.PathUtils.findParentJob(baseOp)")
        CivilCvCADGui.doCommand("obj.Base = baseOp")
        CivilCvCADGui.doCommand("job.Proxy.addOperation(obj, baseOp)")
        CivilCvCADGui.doCommand("Path.Dressup.Gui.Mirror.ViewProviderDressup(obj.ViewObject)")
        CivilCvCADGui.doCommand("baseOp.Visibility = False")
        CivilCvCAD.ActiveDocument.commitTransaction()  # Final `commitTransaction()`
        CivilCvCAD.ActiveDocument.recompute()


if CivilCvCAD.GuiUp:
    # register the CivilCvCAD command
    CivilCvCADGui.addCommand("CAM_DressupMirror", CommandPathDressup())
