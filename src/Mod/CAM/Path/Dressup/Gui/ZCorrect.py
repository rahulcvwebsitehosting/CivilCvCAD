...
class ObjectDressup:
    ...
    def _bilinearInterpolate(self, surface, x, y):
        p1 = CivilCvCAD.Vector(x, y, 100.0)
        p2 = CivilCvCAD.Vector(x, y, -100.0)
            ...
            for xi in range(len(xlist)):
                for p in pointlist:
                    if xlist[xi] == p[0] and ylist[yi] == p[1]:
                        array[yi][xi] = CivilCvCAD.Vector(p[0], p[1], p[2])
                        break
            if None in array[yi]:  # fill missed probe positions
                ...
                for xi in range(len(xlist)):
                    if array[yi][xi] is None:
                        array[yi][xi] = CivilCvCAD.Vector(xlist[xi], ylist[yi], averageZ)
    ...
    def execute(self, obj):
        ...
        for cmd in path.Commands:
            ...
            if cmd.Name not in Path.Geom.CmdMoveMill:
                ...
                curVec = CivilCvCAD.Vector(currLocation["X"], currLocation["Y"], currLocation["Z"])
                ...
                for point in pointlist:
                    if not bb.isInside(CivilCvCAD.Vector(point.x, point.y, 0)):
                        obj.Path = path
...
class TaskPanel:
    def __init__(self, obj):
        self.obj = obj
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/ZCorrectEdit.ui")
        CivilCvCAD.ActiveDocument.openTransaction("Edit Z Correction Dress-up")
        self.interpshape = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "InterpolationSurface")
    ...
    def accept(self):
        ...
        CivilCvCAD.ActiveDocument.removeObject(self.interpshape.Name)
...
class ViewProviderDressup:
    ...
    def setEdit(self, vobj, mode=0):
        if mode == 1:
            CivilCvCADGui.runCommand("Std_TransformManip")
        elif mode == 0:
            CivilCvCADGui.Control.closeDialog()
...
class CommandPathDressup:
    ...
    def Activated(self):
        ...
        CivilCvCADGui.addModule("Path.Dressup.Gui.ZCorrect")
        CivilCvCADGui.addModule("PathScripts.PathUtils")
        CivilCvCADGui.doCommand(
            'obj = CivilCvCAD.ActiveDocument.addObject("Path::FeaturePython", "ZCorrectDressup")'
        )
        CivilCvCADGui.doCommand("Path.Dressup.Gui.ZCorrect.ObjectDressup(obj)")
        CivilCvCADGui.doCommand(f"obj.Base = CivilCvCAD.ActiveDocument.getObject('{op.Name}')")
        CivilCvCADGui.doCommand("Path.Dressup.Gui.ZCorrect.ViewProviderDressup(obj.ViewObject)")
        CivilCvCADGui.doCommand("PathScripts.PathUtils.addToJob(obj)")
        CivilCvCADGui.doCommand("obj.Base.Visibility = False")
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_DressupZCorrect", CommandPathDressup())

CivilCvCAD.Console.PrintLog("Loading PathDressup… done\n")
