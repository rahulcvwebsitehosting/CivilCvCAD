...
class TaskPanel(object):
    def __init__(self, obj, viewProvider):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/DressupPathBoundary.ui")
        if obj.Stock:
...
def Create(base, name="DressupPathBoundary"):
    CivilCvCAD.ActiveDocument.openTransaction("Create a Boundary dressup")
...
class CommandPathDressupPathBoundary:
    ...
    def Activated(self):
        ...
        # everything ok!
        CivilCvCAD.ActiveDocument.openTransaction("Create Path Boundary Dress-up")
        CivilCvCADGui.addModule("Path.Dressup.Gui.Boundary")
        CivilCvCADGui.doCommand(f"base = CivilCvCAD.ActiveDocument.getObject('{op.Name}')")
        CivilCvCADGui.doCommand("Path.Dressup.Gui.Boundary.Create(base)")
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_DressupPathBoundary", CommandPathDressupPathBoundary())
