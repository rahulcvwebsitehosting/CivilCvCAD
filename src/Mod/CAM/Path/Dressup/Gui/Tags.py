...
class PathDressupTagTaskPanel:
    ...
    def __init__(self, obj, viewProvider, jvoVisibility=None):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/HoldingTagsEdit.ui")
        ...
        if jvoVisibility is None:
            CivilCvCAD.ActiveDocument.openTransaction("Edit HoldingTags Dress-up")
        ...
        self.pt = CivilCvCAD.Vector(0, 0, 0)
    ...
    def getFields(self):
        width = CivilCvCAD.Units.Quantity(self.form.ifWidth.text()).Value
        height = CivilCvCAD.Units.Quantity(self.form.ifHeight.text()).Value
        angle = self.form.dsbAngle.value()
        radius = CivilCvCAD.Units.Quantity(self.form.ifRadius.text()).Value
        ...
        for i, (x, y, enabled) in enumerate(tags):
            positions.append(CivilCvCAD.Vector(x, y, 0))
            if not enabled:
                disabled.append(i)
    ...
    def updateTagsView(self):
        ...
        for i, pos in enumerate(self.Positions):
            lbl = "%d: (%s, %s)" % (
                i,
                CivilCvCAD.Units.Quantity(pos.x, CivilCvCAD.Units.Length).UserString,
                CivilCvCAD.Units.Quantity(pos.y, CivilCvCAD.Units.Length).UserString,
            )
    ...
    def copyNewTags(self):
        ...
        form = CivilCvCADGui.PySideUic.loadUi(":/panels/DlgTCChooser.ui")
    ...
    def updateTagsViewWith(self, tags):
        self.tags = tags
        self.Positions = [CivilCvCAD.Vector(t[0], t[1], 0) for t in tags]
    ...
    def addNewTagAt(self, point, obj):
        if point and obj and self.obj.Proxy.pointIsOnPath(self.obj, point):
            ...
            self.Positions.append(CivilCvCAD.Vector(point.x, point.y, 0))
            self.updateTagsView()
        else:
    ...
    def editTag(self, item):
        if item:
            ...
            self.getPoint.getPoint(self.editTagAt, CivilCvCAD.Vector(x, y, z))
    ...
    def setFields(self):
        ...
        self.form.ifHeight.setText(
            CivilCvCAD.Units.Quantity(self.obj.Height, CivilCvCAD.Units.Length).UserString
        )
        self.form.ifWidth.setText(
            CivilCvCAD.Units.Quantity(self.obj.Width, CivilCvCAD.Units.Length).UserString
        )
        self.form.dsbAngle.setValue(self.obj.Angle)
        self.form.ifRadius.setText(
            CivilCvCAD.Units.Quantity(self.obj.Radius, CivilCvCAD.Units.Length).UserString
        )
...
class PathDressupTagViewProvider:
    ...
    def setupTaskPanel(self, panel):
        ...
        panel.setupUi()
        CivilCvCADGui.Selection.addSelectionGate(self)
...
def Create(baseObject, name="DressupTag"):
    ...
    """
    CivilCvCAD.ActiveDocument.openTransaction("Create a Tag dressup")
...
class CommandPathDressupTag:
    ...
    def Activated(self):
        ...
        # everything ok!
        CivilCvCAD.ActiveDocument.openTransaction("Create Tag Dress-up")
        CivilCvCADGui.addModule("Path.Dressup.Gui.Tags")
        CivilCvCADGui.doCommand(f"base = CivilCvCAD.ActiveDocument.getObject('{op.Name}')")
        CivilCvCADGui.doCommand("Path.Dressup.Gui.Tags.Create(base)")
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_DressupTag", CommandPathDressupTag())
