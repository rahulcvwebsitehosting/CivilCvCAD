...
class JobCreate:
    ...
    def __init__(self, parent=None, sel=None):
        self.dialog = CivilCvCADGui.PySideUic.loadUi(":/panels/DlgJobCreate.ui")
    ...
    def _schemaUsesMinutes(self, schema_id):
        ...
        try:
            q = CivilCvCAD.Units.Quantity(1, CivilCvCAD.Units.Velocity)
            r = CivilCvCAD.Units.schemaTranslate(q, schema_id)
            return "/min" in r[2]
    ...
    def _setupUnitSchema(self):
        ...
        Lists every schema returned by CivilCvCAD.Units.listSchemas(). Minute-based
        ...
        keys = CivilCvCAD.Units.listSchemas()
        labels = []
        for i in range(len(keys)):
            try:
                labels.append(CivilCvCAD.Units.listSchemas(i))
            except (IndexError, TypeError):
                labels.append(keys[i])

        current_label = CivilCvCAD.ActiveDocument.UnitSystem if CivilCvCAD.ActiveDocument else labels[0]
    ...
    def _applySelectedSchema(self):
        ...
        if not label:
            return
        if CivilCvCAD.ActiveDocument.UnitSystem == label:
            return
        try:
            CivilCvCAD.ActiveDocument.UnitSystem = label
    ...
    def setupModel(self, job=None):
        ...
        if job:
            ...
            preSelected = Counter([obj.Label for obj in CivilCvCADGui.Selection.getSelection()])
            jobResources = []
...
class JobTemplateExport:
    ...
    def __init__(self, job, parent=None):
        self.job = job
        self.dialog = CivilCvCADGui.PySideUic.loadUi(":/panels/DlgJobTemplateExport.ui")
        if parent:
