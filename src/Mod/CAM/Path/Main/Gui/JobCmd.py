...
class CommandJobCreate:
    ...
    def Execute(cls, base, template):
        CivilCvCADGui.addModule("Path.Main.Gui.Job")
        if template:
            template = "'%s'" % template
        else:
            template = "None"
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_Job", CommandJobCreate())
    CivilCvCADGui.addCommand("CAM_ExportTemplate", CommandJobTemplateExport())

CivilCvCAD.Console.PrintLog("Loading PathJobCmd… done\n")
