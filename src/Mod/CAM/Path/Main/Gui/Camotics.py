...
class CAMoticsUI:
    def __init__(self, simulation):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/TaskPathCamoticsSim.ui")
    ...
    def reject(self):
        ...
        if self.simulation.simmesh is not None:
            CivilCvCAD.ActiveDocument.removeObject(self.simulation.simmesh.Name)
        CivilCvCADGui.Control.closeDialog()
...
class CamoticsSimulation(QtCore.QObject):
    ...
    def addMesh(self, surface):
        ...
        if self.simmesh is None:
            self.simmesh = CivilCvCAD.ActiveDocument.addObject("Mesh::Feature", "Camotics")
        buffer = io.BytesIO()
    ...
    def Activate(self):
        ...
        self.job = CivilCvCADGui.Selection.getSelectionEx()[0].Object
        self.SIM.set_metric()
    ...
    def buildproject(self):  # , files=[]):
        ...
        unitstring = "imperial" if CivilCvCAD.Units.getSchema() in [2, 3, 5, 7] else "metric"
...
class CommandCamoticsSimulate:
    ...
    def IsActive(self):
        if bool(CivilCvCADGui.Selection.getSelection()) is False:
            return False
        try:
            job = CivilCvCADGui.Selection.getSelectionEx()[0].Object
...
if CivilCvCAD.GuiUp:
    CivilCvCADGui.addCommand("CAM_Camotics", CommandCamoticsSimulate())


CivilCvCAD.Console.PrintLog("Loading PathCamoticsSimulateGui… done\n")
