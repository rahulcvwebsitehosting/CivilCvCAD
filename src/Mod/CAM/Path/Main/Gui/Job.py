    ...
    try:
        ...
        for s in sel:
            if s.SubElementNames:
                CivilCvCADGui.Selection.addSelection(s.Object, s.SubElementNames)
            else:
                CivilCvCADGui.Selection.addSelection(s.Object)
...
class ViewProvider:
    ...
    def attach(self, vobj):
        ...
        parts = CivilCvCADGui.PlacementIndicatorParts
    ...
    def openTaskPanel(self, activate=None):
        ...
        CivilCvCADGui.Control.showDialog(self.taskPanel)
...
class MaterialDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, current_uuid=None):
        ...
        self.materialTree = CivilCvCADGui.UiLoader().createWidget("MatGui::MaterialTreeWidget")
...
class StockEdit(object):
    ...
    def setLengthField(self, widget, prop):
        quantity = CivilCvCAD.Units.Quantity(prop.Value, CivilCvCAD.Units.Length)
...
class StockFromBaseBoundBoxEdit(StockEdit):
    ...
    def getFieldsStock(self, stock, fields=None):
        ...
        try:
            if "xneg" in fields:
                stock.ExtXneg = CivilCvCAD.Units.Quantity(
                    self.form.stockExtXneg.property("rawValue"), CivilCvCAD.Units.Length
                )
            if "xpos" in fields:
                stock.ExtXpos = CivilCvCAD.Units.Quantity(
                    self.form.stockExtXpos.property("rawValue"), CivilCvCAD.Units.Length
                )
            if "yneg" in fields:
                stock.ExtYneg = CivilCvCAD.Units.Quantity(
                    self.form.stockExtYneg.property("rawValue"), CivilCvCAD.Units.Length
                )
            if "ypos" in fields:
                stock.ExtYpos = CivilCvCAD.Units.Quantity(
                    self.form.stockExtYpos.property("rawValue"), CivilCvCAD.Units.Length
                )
            if "zneg" in fields:
                stock.ExtZneg = CivilCvCAD.Units.Quantity(
                    self.form.stockExtZneg.property("rawValue"), CivilCvCAD.Units.Length
                )
            if "zpos" in fields:
                stock.ExtZpos = CivilCvCAD.Units.Quantity(
                    self.form.stockExtZpos.property("rawValue"), CivilCvCAD.Units.Length
                )
        except Exception:
            pass
...
class StockCreateBoxEdit(StockEdit):
    ...
    def getFields(self, obj, fields=None):
        ...
        try:
            if self.IsStock(obj):
                if "length" in fields:
                    obj.Stock.Length = CivilCvCAD.Units.Quantity(
                        self.form.stockBoxLength.property("rawValue"), CivilCvCAD.Units.Length
                    )
                if "width" in fields:
                    obj.Stock.Width = CivilCvCAD.Units.Quantity(
                        self.form.stockBoxWidth.property("rawValue"), CivilCvCAD.Units.Length
                    )
                if "height" in fields:
                    obj.Stock.Height = CivilCvCAD.Units.Quantity(
                        self.form.stockBoxHeight.property("rawValue"), CivilCvCAD.Units.Length
                    )
            else:
...
class StockCreateCylinderEdit(StockEdit):
    ...
    def getFields(self, obj, fields=None):
        ...
        try:
            if self.IsStock(obj):
                if "radius" in fields:
                    obj.Stock.Radius = CivilCvCAD.Units.Quantity(
                        self.form.stockCylinderRadius.property("rawValue"), CivilCvCAD.Units.Length
                    )
                if "height" in fields:
                    obj.Stock.Height = CivilCvCAD.Units.Quantity(
                        self.form.stockCylinderHeight.property("rawValue"), CivilCvCAD.Units.Length
                    )
            else:
...
class TaskPanel:
    ...
    def __init__(self, vobj, deleteOnReject):
        CivilCvCAD.ActiveDocument.openTransaction("Edit Job")
        self.vobj = vobj
        self.vproxy = vobj.Proxy
        self.obj = vobj.Object
        self.deleteOnReject = deleteOnReject
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/PathEdit.ui")
        self.name = self.obj.Name

        vUnit = CivilCvCAD.Units.Quantity(1, CivilCvCAD.Units.Velocity).getUserPreferred()[2]
    ...
    def preCleanup(self):
        Path.Log.track()
        CivilCvCADGui.Selection.removeObserver(self)
    ...
    def reject(self, resetEdit=True):
        ...
        if self.deleteOnReject and CivilCvCAD.ActiveDocument.getObject(self.name):
            Path.Log.info("Uncreate Job")
            CivilCvCAD.ActiveDocument.openTransaction("Uncreate Job")
            ...
            Path.Log.track(
                self.name,
                self.deleteOnReject,
                CivilCvCAD.ActiveDocument.getObject(self.name),
            )
        self.cleanup(resetEdit)
        return True
    ...
    def toolControllerAdd(self):
        ...
        for toolbit in toolbits:
            toolbit.attach_to_doc(CivilCvCAD.ActiveDocument)
    ...
    def toolControllerChanged(self, item):
        ...
        if "Label" == prop:
            ...
            vUnit = CivilCvCAD.Units.Quantity(1, CivilCvCAD.Units.Velocity).getUserPreferred()[2]
            try:
                val = CivilCvCAD.Units.Quantity(item.text())
                if CivilCvCAD.Units.Velocity == val.Unit:
                    setattr(tc, prop, val)
                elif CivilCvCAD.Units.Unit() == val.Unit:
                    val = CivilCvCAD.Units.Quantity(item.text() + vUnit)
                    setattr(tc, prop, val)
            except Exception:
                pass
    ...
    def modelSetAxis(self, axis):
        ...
        def alignSel(sel, normal, flip=False):
            ...
            if Path.Geom.pointsCoincide(abs(v), abs(normal)):
                ...
                r = CivilCvCAD.Vector(v.y, v.z, v.x)
                a = 180
            else:
                ...
                for feature in sel.SubElementNames:
                    ...
                    if "Face" == sub.ShapeType:
                        ...
                        if sub.Orientation == "Reversed":
                            normal = CivilCvCAD.Vector() - normal
                            Path.Log.debug(
                                "(%.2f, %.2f, %.2f) -> reversed (%s)"
                        ...
                        elif Path.Geom.pointsCoincide(axis, CivilCvCAD.Vector() - normal):
                            alignSel(sel, CivilCvCAD.Vector() - normal, True)
                        else:
                            alignSel(sel, normal)
        ...
        if selObject and selFeature:
            CivilCvCADGui.Selection.clearSelection()
            CivilCvCADGui.Selection.addSelection(selObject, selFeature)

    def restoreSelection(self, selection):
        CivilCvCADGui.Selection.clearSelection()
        for sel in selection:
            CivilCvCADGui.Selection.addSelection(sel.Object, sel.SubElementNames)
        ...
        with selectionEx() as selection:
            for sel in selection:
                ...
                for name in sel.SubElementNames:
                    ...
                    bb = feature.BoundBox
                    offset = CivilCvCAD.Vector(axis.x * bb.XMax, axis.y * bb.YMax, axis.z * bb.ZMax)
    ...
    def alignSetOrigin(self):
        ...
        placement = CivilCvCADGui.ActiveDocument.ActiveView.viewPosition()
        placement.Base = placement.Base + by
        CivilCvCADGui.ActiveDocument.ActiveView.viewPosition(placement, 0)
        CivilCvCADGui.Selection.clearSelection()
    ...
    def alignMoveToOrigin(self):
        ...
        selFeature = None
        p = None
        for sel in CivilCvCADGui.Selection.getSelectionEx():
            selObject = sel.Object
            for feature in sel.SubElementNames:
                selFeature = feature
                sub = sel.Object.Shape.getElement(feature)
                if "Vertex" == sub.ShapeType:
                    p = CivilCvCAD.Vector() - sub.Point
                if "Edge" == sub.ShapeType:
                    p = CivilCvCAD.Vector() - sub.Curve.Location
                if "Face" == sub.ShapeType:
                    p = CivilCvCAD.Vector() - sub.BoundBox.Center
    ...
    def updateSelection(self):
        ...
        if self.obj in CivilCvCADGui.Selection.getSelection():
            CivilCvCADGui.Selection.removeSelection(self.obj)
    ...
    def setupUi(self, activate):
        ...
        self.form.modelSetXAxis.clicked.connect(lambda: self.modelSetAxis(CivilCvCAD.Vector(1, 0, 0)))
        self.form.modelSetYAxis.clicked.connect(lambda: self.modelSetAxis(CivilCvCAD.Vector(0, 1, 0)))
        self.form.modelSetZAxis.clicked.connect(lambda: self.modelSetAxis(CivilCvCAD.Vector(0, 0, 1)))
        self.form.modelSetX0.clicked.connect(lambda: self.modelSet0(CivilCvCAD.Vector(-1, 0, 0)))
        self.form.modelSetY0.clicked.connect(lambda: self.modelSet0(CivilCvCAD.Vector(0, -1, 0)))
        self.form.modelSetZ0.clicked.connect(lambda: self.modelSet0(CivilCvCAD.Vector(0, 0, -1)))
        ...
        self.form.modelMoveLeftUp.clicked.connect(lambda: self.modelMove(CivilCvCAD.Vector(-1, 1, 0)))
        self.form.modelMoveLeft.clicked.connect(lambda: self.modelMove(CivilCvCAD.Vector(-1, 0, 0)))
        self.form.modelMoveLeftDown.clicked.connect(
            lambda: self.modelMove(CivilCvCAD.Vector(-1, -1, 0))
        )

        self.form.modelMoveUp.clicked.connect(lambda: self.modelMove(CivilCvCAD.Vector(0, 1, 0)))
        self.form.modelMoveDown.clicked.connect(lambda: self.modelMove(CivilCvCAD.Vector(0, -1, 0)))

        self.form.modelMoveRightUp.clicked.connect(lambda: self.modelMove(CivilCvCAD.Vector(1, 1, 0)))
        self.form.modelMoveRight.clicked.connect(lambda: self.modelMove(CivilCvCAD.Vector(1, 0, 0)))
        self.form.modelMoveRightDown.clicked.connect(
            lambda: self.modelMove(CivilCvCAD.Vector(1, -1, 0))
        )

        self.form.modelRotateLeft.clicked.connect(lambda: self.modelRotate(CivilCvCAD.Vector(0, 0, 1)))
        self.form.modelRotateRight.clicked.connect(
            lambda: self.modelRotate(CivilCvCAD.Vector(0, 0, -1))
        )
    ...
    def open(self):
        CivilCvCADGui.Selection.addObserver(self)
...
def Create(base, template=None, openTaskPanel=True):
    ...
    CivilCvCADGui.addModule("Path.Main.Job")
    CivilCvCAD.ActiveDocument.openTransaction("Create Job")
    try:
