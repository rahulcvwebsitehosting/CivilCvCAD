...
from CivilCvCAD import Vector, Base
...
class CAMSimTaskUi:
    def __init__(self, parent):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/panels/TaskPathSimulator.ui")
        self.parent = parent
...
class PathSimulation:
    def __init__(self):
        ...
        self.stdrot = CivilCvCAD.Rotation(Vector(0, 0, 1), 0)
        self.iprogress = 0
        self.numCommands = 0
        self.simperiod = 20
        self.accuracy = 0.1
    ...
    def Activate(self):
        ...
        self.onJobChange()
        CivilCvCADGui.Control.showDialog(self.taskForm)
    ...
    def _populateJobSelection(self, form):
        ...
        jobList = CivilCvCAD.ActiveDocument.findObjects("Path::FeaturePython", "Job.*")
        jCnt = len(jobList)

        # Check if user has selected a specific job for simulation
        guiSelection = CivilCvCADGui.Selection.getSelectionEx()
    ...
    def SetupOperation(self, itool):
        ...
        self.icmd = 0
        self.curpos = CivilCvCAD.Placement(self.initialPos, self.stdrot)
    ...
    def SimulateMill(self):
        ...
        # Add cut tool
        self.cutTool = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "CutTool")
        self.cutTool.ViewObject.Proxy = 0
        self.cutTool.ViewObject.hide()

        # Add cut material
        if self.isVoxel:
            self.cutMaterial = CivilCvCAD.ActiveDocument.addObject(
                "Mesh::FeaturePython", "CutMaterial"
            )
            self.cutMaterialIn = CivilCvCAD.ActiveDocument.addObject(
        ...
        if self.debug:
            self.cutSolid = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "CutDebug")
    ...
    def PerformCutBoolean(self):
        ...
        if not self.disableAnim:
            self.cutTool.Placement = CivilCvCAD.Placement(self.curpos, self.stdrot)
        self.icmd += 1
        self.iprogress += 1
        self.UpdateProgress()
    ...
    def RemoveTool(self):
        if self.cutTool is None:
            return
        CivilCvCAD.ActiveDocument.removeObject(self.cutTool.Name)
        self.cutTool = None
    ...
    def RemoveInnerMaterial(self):
        if self.cutMaterialIn is not None:
            ...
            CivilCvCAD.ActiveDocument.removeObject(self.cutMaterialIn.Name)
            self.cutMaterialIn = None

    def RemoveMaterial(self):
        if self.cutMaterial is not None:
            CivilCvCAD.ActiveDocument.removeObject(self.cutMaterial.Name)
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("CAM_Simulator", CommandPathSimulate())
    CivilCvCAD.Console.PrintLog("Loading PathSimulator Gui… done\n")
