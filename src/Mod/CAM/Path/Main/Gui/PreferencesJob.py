...
class JobPreferencesPage:
    def __init__(self, parent=None):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":preferences/PathJob.ui")
    ...
    def saveStockSettings(self):
        if self.form.stockGroup.isChecked():
            ...
            if typ == PathStock.StockType.CreateBox:
                attrs["length"] = CivilCvCAD.Units.Quantity(self.form.stockBoxLength.text()).UserString
                attrs["width"] = CivilCvCAD.Units.Quantity(self.form.stockBoxWidth.text()).UserString
                attrs["height"] = CivilCvCAD.Units.Quantity(self.form.stockBoxHeight.text()).UserString
            if typ == PathStock.StockType.CreateCylinder:
                attrs["radius"] = CivilCvCAD.Units.Quantity(
                    self.form.stockCylinderRadius.text()
                ).UserString
                attrs["height"] = CivilCvCAD.Units.Quantity(
                    self.form.stockCylinderHeight.text()
                ).UserString
            if typ == PathStock.StockType.FromBase:
                attrs["xneg"] = CivilCvCAD.Units.Quantity(self.form.stockExtXneg.text()).UserString
                attrs["xpos"] = CivilCvCAD.Units.Quantity(self.form.stockExtXpos.text()).UserString
                attrs["yneg"] = CivilCvCAD.Units.Quantity(self.form.stockExtYneg.text()).UserString
                attrs["ypos"] = CivilCvCAD.Units.Quantity(self.form.stockExtYpos.text()).UserString
                attrs["zneg"] = CivilCvCAD.Units.Quantity(self.form.stockExtZneg.text()).UserString
                attrs["zpos"] = CivilCvCAD.Units.Quantity(self.form.stockExtZpos.text()).UserString
            if self.form.stockPlacementGroup.isChecked():
                angle = CivilCvCAD.Units.Quantity(self.form.stockAngle.text()).Value
                axis = CivilCvCAD.Vector(
                    self.form.stockAxisX.value(),
                    self.form.stockAxisY.value(),
                    self.form.stockAxisZ.value(),
                )
                rot = CivilCvCAD.Rotation(axis, angle)
                attrs["rotX"] = rot.Q[0]
                attrs["rotY"] = rot.Q[1]
                attrs["rotZ"] = rot.Q[2]
                attrs["rotW"] = rot.Q[3]
                attrs["posX"] = CivilCvCAD.Units.Quantity(self.form.stockPositionX.text()).Value
                attrs["posY"] = CivilCvCAD.Units.Quantity(self.form.stockPositionY.text()).Value
                attrs["posZ"] = CivilCvCAD.Units.Quantity(self.form.stockPositionZ.text()).Value
    ...
    def loadStockSettings(self):
        ...
        if (
            ...
            pos = CivilCvCAD.Vector(float(posX), float(posY), float(posZ))
            rot = CivilCvCAD.Rotation(float(rotX), float(rotY), float(rotZ), float(rotW))
            placement = CivilCvCAD.Placement(pos, rot)
        ...
        self.form.stockAngle.setText(
            CivilCvCAD.Units.Quantity("%f rad" % placement.Rotation.Angle).UserString
        )
        ...
        self.form.stockPositionX.setText(
            CivilCvCAD.Units.Quantity(placement.Base.x, CivilCvCAD.Units.Length).UserString
        )
        self.form.stockPositionY.setText(
            CivilCvCAD.Units.Quantity(placement.Base.y, CivilCvCAD.Units.Length).UserString
        )
        self.form.stockPositionZ.setText(
            CivilCvCAD.Units.Quantity(placement.Base.z, CivilCvCAD.Units.Length).UserString
        )
