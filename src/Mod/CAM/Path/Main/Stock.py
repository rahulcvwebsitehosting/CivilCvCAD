...
class StockFromBase(Stock):
    ...
    def __init__(self, obj, base):
        ...
        if bb:
            obj.Placement = CivilCvCAD.Placement(
                CivilCvCAD.Vector(bb.XMin, bb.YMin, bb.ZMin), CivilCvCAD.Rotation()
            )
        else:
    ...
    def execute(self, obj):
        ...
        if bb:
            self.origin = CivilCvCAD.Vector(-obj.ExtXneg.Value, -obj.ExtYneg.Value, -obj.ExtZneg.Value)
...
def SetupStockObject(obj, stockType):
    ...
    if CivilCvCAD.GuiUp and obj.ViewObject:
        obj.addProperty(
            "App::PropertyString",
            "StockType",
            "Stock",
...
def CreateFromBase(job, neg=None, pos=None, placement=None):
    ...
    base = _getBase(job)
    obj = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "Stock")
...
def CreateBox(job, extent=None, placement=None):
    ...
    if placement:
        ...
        bb = shapeBoundBox(base.Group)
        origin = CivilCvCAD.Vector(bb.XMin, bb.YMin, bb.ZMin)
        obj.Placement = CivilCvCAD.Placement(origin, CivilCvCAD.Vector(), 0)
...
def CreateCylinder(job, radius=None, height=None, placement=None):
    ...
    if placement:
        ...
        bb = shapeBoundBox(base.Group)
        origin = CivilCvCAD.Vector((bb.XMin + bb.XMax) / 2, (bb.YMin + bb.YMax) / 2, bb.ZMin)
        obj.Placement = CivilCvCAD.Placement(origin, CivilCvCAD.Vector(), 0)
        ...
        if stockType:
            ...
            if stockType == StockType.FromBase:
                ...
                if (
                    ...
                    neg = CivilCvCAD.Vector(
                        CivilCvCAD.Units.Quantity(xneg).Value,
                        CivilCvCAD.Units.Quantity(yneg).Value,
                        CivilCvCAD.Units.Quantity(zneg).Value,
                    )
                    pos = CivilCvCAD.Vector(
                        CivilCvCAD.Units.Quantity(xpos).Value,
                        CivilCvCAD.Units.Quantity(ypos).Value,
                        CivilCvCAD.Units.Quantity(zpos).Value,
                    )
                elif (
                    xneg is not None
                    or xpos is not None
            ...
            if stockType == StockType.CreateBox:
                ...
                if length is not None and width is not None and height is not None:
                    ...
                    extent = CivilCvCAD.Vector(
                        CivilCvCAD.Units.Quantity(length).Value,
                        CivilCvCAD.Units.Quantity(width).Value,
                        CivilCvCAD.Units.Quantity(height).Value,
                    )
...
CivilCvCAD.Console.PrintLog("Loading PathStock... done\n")
