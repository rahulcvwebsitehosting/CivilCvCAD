...
"""
This file has utilities for checking and catching common errors in CivilCvCAD
...
class CAMSanity:
    ...
    def squawk(self, operator, note, date=datetime.now(), squawkType="NOTE"):
        ...
        path = f"{CivilCvCAD.getHomePath()}Mod/CAM/Path/Main/Sanity/{squawk_icon}.svg"
    ...
    def _runData(self):
        ...
        data["jobMinZ"] = CivilCvCAD.Units.Quantity(
            obj.Path.BoundBox.ZMin, CivilCvCAD.Units.Length
        ).UserString
        data["jobMaxZ"] = CivilCvCAD.Units.Quantity(
            obj.Path.BoundBox.ZMax, CivilCvCAD.Units.Length
        ).UserString
        data["jobDescription"] = obj.Description
        ...
        for op in obj.Operations.Group:
            ...
            if op.Path.BoundBox.isValid():
                zmin = CivilCvCAD.Units.Quantity(
                    op.Path.BoundBox.ZMin, CivilCvCAD.Units.Length
                ).UserString
                zmax = CivilCvCAD.Units.Quantity(
                    op.Path.BoundBox.ZMax, CivilCvCAD.Units.Length
                ).UserString
            else:
                zmin = ""
                zmax = ""
    ...
    def _stockData(self):
        ...
        data["xLen"] = CivilCvCAD.Units.Quantity(bb.XLength, CivilCvCAD.Units.Length).UserString
        data["yLen"] = CivilCvCAD.Units.Quantity(bb.YLength, CivilCvCAD.Units.Length).UserString
        data["zLen"] = CivilCvCAD.Units.Quantity(bb.ZLength, CivilCvCAD.Units.Length).UserString
        ...
        if hasattr(obj.Stock, "ShapeMaterial"):
            ...
            if "SurfaceSpeedCarbide" in props:
                data["surfaceSpeedCarbide"] = CivilCvCAD.Units.Quantity(
                    props["SurfaceSpeedCarbide"]
                ).UserString
            if "SurfaceSpeedHSS" in props:
                data["surfaceSpeedHSS"] = CivilCvCAD.Units.Quantity(
