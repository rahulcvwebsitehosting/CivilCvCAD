...
class GuiImageBuilder(ImageBuilder):
    """
    A class for generating images of 3D objects in a CivilCvCAD GUI environment.
    """
    ...
    def __init__(self, file_path):
        ...
        self.file_path = file_path
        self.currentCamera = CivilCvCADGui.ActiveDocument.ActiveView.getCameraType()

        self.doc = CivilCvCADGui.ActiveDocument
    ...
    def prepare_view(self, obj, view="default"):
        ...
        view_obj = CivilCvCADGui.ActiveDocument.createView("Gui::View3DInventor")
        ...
        CivilCvCADGui.Selection.clearSelection()  # Clear so the selection highlight does not appear in the image

        # Get the active view and fit to selection
        a_view = CivilCvCADGui.activeDocument().activeView()
        try:
    ...
    def capture_image_to_bytes(self):
        ...
        try:
            # Use CivilCvCAD's built-in method for getting a QImage directly
