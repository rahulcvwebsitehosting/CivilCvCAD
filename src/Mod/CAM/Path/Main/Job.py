...
class ObjectJob:
    ...
    def setupOperations(self, obj):
        ...
        # ops = CivilCvCAD.ActiveDocument.addObject(
        #     "Path::FeatureCompoundPython", "Operations"
        # )
        ops = CivilCvCAD.ActiveDocument.addObject("App::DocumentObjectGroup", "Operations")
        if ops.ViewObject:
    ...
    def setupBaseModel(self, obj, models=None):
        ...
        if addModels:
            model = CivilCvCAD.ActiveDocument.addObject("App::DocumentObjectGroup", "Model")
            if model.ViewObject:
    ...
    def setupToolTable(self, obj):
        ...
        if addTable:
            toolTable = CivilCvCAD.ActiveDocument.addObject("App::DocumentObjectGroup", "Tools")
    ...
    def fixupOperations(self, obj):
        if getattr(obj.Operations, "ViewObject", None):
            try:
                ...
                ops = CivilCvCAD.ActiveDocument.addObject("Path::FeatureCompoundPython", "Operations")
                ops.ViewObject.Proxy = 0
    ...
    def baseCandidates(cls):
        ...
        return sorted(
            [obj for obj in CivilCvCAD.ActiveDocument.Objects if cls.isBaseCandidate(obj)],
...
def Instances():
    ...
    if CivilCvCAD.ActiveDocument:
        return [
            job
            for job in CivilCvCAD.ActiveDocument.Objects
...
def Create(name, base, templateFile=None):
    ...
    if isinstance(base[0], str):
        ...
        for baseName in base:
            models.append(CivilCvCAD.ActiveDocument.getObject(baseName))
    else:
        models = base
