...
def isVertical(obj):
    ...
    if isinstance(obj, CivilCvCAD.Vector):
    ...
    if cmd.Name in CmdMoveArc:
        ...
        if isRoughly(startPoint.z, endPoint.z):
            ...
            if pointsCoincide(startPoint, endPoint, 0.001):
                return Part.makeCircle(R, center, CivilCvCAD.Vector(0, 0, 1))
            else:
...
def makeBoundBoxFace(bBox, offset=0.0, zHeight=0.0):
    ...
    p1 = CivilCvCAD.Vector(bBox.XMin - offset, bBox.YMin - offset, zHeight)
    p2 = CivilCvCAD.Vector(bBox.XMax + offset, bBox.YMin - offset, zHeight)
    p3 = CivilCvCAD.Vector(bBox.XMax + offset, bBox.YMax + offset, zHeight)
    p4 = CivilCvCAD.Vector(bBox.XMin - offset, bBox.YMax + offset, zHeight)
...
def combineHorizontalFaces(faces, keepOrder=False):
    ...
    for f in faces:
        if f.BoundBox.ZMin != 0.0:
            f.translate(CivilCvCAD.Vector(0.0, 0.0, 0.0 - f.BoundBox.ZMin))
    ...
    bboxSolid = bboxFace.extrude(CivilCvCAD.Vector(0.0, 0.0, 10.0))
    extrudedFaces = list()
    for f in faces:
        extrudedFaces.append(f.extrude(CivilCvCAD.Vector(0.0, 0.0, 6.0)))
    ...
    # Debug
    # Part.show(cut)
    # CivilCvCAD.ActiveDocument.ActiveObject.Label = "cut"
