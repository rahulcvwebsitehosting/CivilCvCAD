...
def preferences():
    return CivilCvCAD.ParamGet(PreferencesGroup)


def tool_preferences():
    return CivilCvCAD.ParamGet(ToolGroup)
...
def pathPostSourcePath():
    return os.path.join(CivilCvCAD.getHomePath(), "Mod/CAM/Path/Post/")


def getBuiltinAssetPath() -> pathlib.Path:
    home = pathlib.Path(CivilCvCAD.getHomePath())
...
def getDefaultAssetPath() -> Path:
    data_dir = pathlib.Path(CivilCvCAD.getUserAppDataDir())
...
def getAssetPath() -> pathlib.Path:
    ...
    if cam_assets_path:
        ...
        most_recent_path = CivilCvCAD.ApplicationDirectories.mostRecentConfigFromBase(cam_assets_path)
    ...
    if legacy_path:
        ...
        if legacy_path_obj.exists() and legacy_path_obj.is_dir():
            ...
            most_recent_legacy = CivilCvCAD.ApplicationDirectories.mostRecentConfigFromBase(
                str(legacy_path_obj)
            )
    ...
    most_recent_default = CivilCvCAD.ApplicationDirectories.mostRecentConfigFromBase(str(default))
...
def macroFilePath():
    grp = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Macro")
    return grp.GetString("MacroPath", CivilCvCAD.getUserMacroDir())
...
def _scan_addon_post_dirs() -> None:
    """Scan CivilCvCAD Mod directories for post-processor addons via package.xml content.
...
def addAddonPostPath(path: str) -> None:
    ...
    Called by addon Init.py at CivilCvCAD startup. Each call adds one
