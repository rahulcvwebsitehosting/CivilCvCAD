...
refAxis = RefAxes(
    CivilCvCAD.Vector(1, 0, 0),  # x: linear direction
    CivilCvCAD.Vector(0, 1, 0),  # y: linear direction
    CivilCvCAD.Vector(0, 0, 1),  # z: linear direction
)

RefRotAxes = namedtuple("RefRotAxes", ["a", "b", "c"])
refRotAxis = RefRotAxes(
    CivilCvCAD.Vector(1, 0, 0),  # a: rotational direction
    CivilCvCAD.Vector(0, 1, 0),  # b: rotational direction
    CivilCvCAD.Vector(0, 0, 1),  # c: rotational direction
)
...
class LinearAxis:
    ...
    name: str
    direction_vector: CivilCvCAD.Vector
    min_limit: float = 0
    ...
    def __post_init__(self):
        ...
        if self.direction_vector is None or self.direction_vector.Length < 1e-6:
            ...
            self.direction_vector = CivilCvCAD.Vector(1, 0, 0)
        else:
...
class RotaryAxis:
    ...
    name: str
    rotation_vector: CivilCvCAD.Vector
    min_limit: float = -360
    max_limit: float = 360
    ...
    def __post_init__(self):
        ...
        if self.rotation_vector is None or self.rotation_vector.Length < 1e-6:
            ...
            self.rotation_vector = CivilCvCAD.Vector(0, 0, 1)
        else:
...
class Machine:
    ...
    version: int = 1
    civilcvcad_version: str = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.civilcvcad_version = ".".join(CivilCvCAD.Version()[0:3])
    ...
    def create_AC_table_config(cls, a_limits=(-120, 120), c_limits=(-360, 360)):
        ...
        config.add_linear_axis("X", CivilCvCAD.Vector(1, 0, 0))
        config.add_linear_axis("Y", CivilCvCAD.Vector(0, 1, 0))
        config.add_linear_axis("Z", CivilCvCAD.Vector(0, 0, 1))
        config.linear_axes["Z"].role = AxisRole.HEAD_LINEAR
        config.add_rotary_axis("C", CivilCvCAD.Vector(0, 0, 1), c_limits[0], c_limits[1])
        config.rotary_axes["C"].role = AxisRole.TABLE_ROTARY
        config.rotary_axes["C"].sequence = 0
        config.add_rotary_axis("A", CivilCvCAD.Vector(1, 0, 0), a_limits[0], a_limits[1])
    ...
    def create_BC_head_config(cls, b_limits=(-120, 120), c_limits=(-360, 360)):
        ...
        config.add_linear_axis("Z", CivilCvCAD.Vector(0, 0, 1))
        config.linear_axes["Z"].role = AxisRole.HEAD_LINEAR
        config.add_rotary_axis("B", CivilCvCAD.Vector(0, 1, 0), b_limits[0], b_limits[1])
    ...
    def to_dict(self):
        ...
        data = {
            "civilcvcad_version": self.civilcvcad_version,
            "machine": {
                "name": self.name,
    ...
    def _initialize_3axis_config(self) -> None:
        ...
        self.linear_axes = {
            "X": LinearAxis("X", CivilCvCAD.Vector(1, 0, 0), role=AxisRole.TABLE_LINEAR),
            "Y": LinearAxis("Y", CivilCvCAD.Vector(0, 1, 0), role=AxisRole.TABLE_LINEAR),
            "Z": LinearAxis("Z", CivilCvCAD.Vector(0, 0, 1), role=AxisRole.HEAD_LINEAR),
        }
        self.rotary_axes = {}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Machine":
        ...
        for axis_name, axis_data in axes.items():
            ...
            if isinstance(joint, dict) and "origin" in joint and "axis" in joint:
                ...
                vec0 = (
                    CivilCvCAD.Vector(joint[0][0], joint[0][1], joint[0][2])
                    if len(joint[0]) >= 3
                    else CivilCvCAD.Vector(0, 0, 0)
                )
                vec1 = (
                    CivilCvCAD.Vector(joint[1][0], joint[1][1], joint[1][2])
                    if len(joint[1]) >= 3
                    else CivilCvCAD.Vector(0, 0, 0)
                )
            ...
            if axis_type == "linear":
                ...
                direction_vec = CivilCvCAD.Vector(axis_vector[0], axis_vector[1], axis_vector[2])
                ...
                rotation_vec = CivilCvCAD.Vector(axis_vector[0], axis_vector[1], axis_vector[2])
...
class MachineFactory:
    ...
    def get_config_directory(cls):
        ...
        if cls._config_dir is None:
            # Use CivilCvCAD user data directory + CAM/Machines
            try:
    ...
    def list_builtin_templates(cls) -> list[tuple[str, str]]:
        ...
        templates = []
        try:
            # Get the built-in machines directory from CivilCvCAD installation
            machines_dir = (
                pathlib.Path(CivilCvCAD.getHomePath()) / "Mod" / "CAM" / "Machine" / "machines"
            )
    ...
    def _scan_installed_addon_dirs(cls) -> None:
        """Scan CivilCvCAD Mod directories for machine addons via package.xml content.
        ...
        """
        for get_dir in (CivilCvCAD.getUserAppDataDir, CivilCvCAD.getHomePath):
            try:
                ...
                for entry in sorted(mod_root.iterdir(), key=lambda e: e.name.lower()):
                    ...
                    if not pkg_xml.exists():
                        continue
                    try:
                        meta = CivilCvCAD.Metadata(str(pkg_xml))
                    except Exception:
