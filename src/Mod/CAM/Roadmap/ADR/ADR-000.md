...
A bit-flag (`FeatureTool`, `FeatureDepths`, `FeatureHeights`, …) that an **Operation** ORs together to declare which standard properties it supports. Distinct from CivilCvCAD's generic notion of "feature" (a document object).
...
**Resolver**:
The pure function `resolve(tool, material, op) → FeedSpeedResult` that walks a chain of **Providers** in priority order and merges their contributions. GUI- and CivilCvCAD-Document-independent at the boundary; testable from fixtures without a running CivilCvCAD process.
