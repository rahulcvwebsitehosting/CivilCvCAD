...
- Either ignore CivilCvCAD's existing Material `Machinability` model (the data
...
- **CivilCvCAD Material workbench** defines a `Machinability` model used by
...
Extends CivilCvCAD's `Machinability` material model with per-tool-material `Vc`
and `Fz` curves, as proposed in #16801. Operations compute RPM from SFM and
feed from chipload × flutes × RPM.

| Pros | Cons |
| --- | --- |
| Leverages existing CivilCvCAD Material workbench | Tool-specific overrides (vendor-published coated carbide that runs faster than generic carbide) are awkward to express |
...
- `Path/Tool/FeedsSpeeds/Resolver.py`: pure function
  ...
  The resolver is GUI-independent and CivilCvCAD-Document-independent at the
...
- Unit test harness for the resolver with fixture tools, fixture materials,
  fixture rules; no CivilCvCAD-Gui dependency.
...
| Material `Machinability` data | CivilCvCAD Material file (`.FCMat`) | Job's Stock material |
