...
class TestMachineDataclass(PathTestUtils.PathTestBase):
    ...
    def test_default_initialization(self):
        ...
        machine.add_linear_axis("X", CivilCvCAD.Vector(1, 0, 0))
        machine.add_linear_axis("Y", CivilCvCAD.Vector(0, 1, 0))
        self.assertEqual(machine.machine_type, "custom")  # Still missing Z axis

        machine.add_linear_axis("Z", CivilCvCAD.Vector(0, 0, 1))
        self.assertEqual(machine.machine_type, "xyz")  # Now has XYZ axes

        # Add rotary axes and verify machine type updates
        machine.add_rotary_axis("A", CivilCvCAD.Vector(1, 0, 0), -120, 120)
        self.assertEqual(machine.machine_type, "xyza")

        machine.add_rotary_axis("C", CivilCvCAD.Vector(0, 0, 1), -360, 360)
        ...
        self.assertIsNotNone(machine.civilcvcad_version)
