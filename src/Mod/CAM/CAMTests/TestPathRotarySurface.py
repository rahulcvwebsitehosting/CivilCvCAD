...
def _make_tapered_part(doc, length=40.0, r0=8.0, r1=12.0, axis="X"):
    ...
    if axis == "X":
        cone.Placement = CivilCvCAD.Placement(
            CivilCvCAD.Vector(-length / 2.0, 0, 0),
            CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 1, 0), 90.0),
        )
    elif axis == "Y":
        cone.Placement = CivilCvCAD.Placement(
            CivilCvCAD.Vector(0, -length / 2.0, 0),
            CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), -90.0),
        )
    else:
...
def _setup_cyl_stock_along_axis(job, radius, length, axis):
    ...
    if job.Stock and job.Stock is not new_stock:
        try:
            CivilCvCAD.ActiveDocument.removeObject(job.Stock.Name)
        except Exception:
            pass
    job.Stock = new_stock
    return new_stock
...
def _install_stub_machine(job, axis, min_limit=-360.0, max_limit=360.0):
    ...
    if axis == "X":
        name = "A"
        rot_vec = CivilCvCAD.Vector(1, 0, 0)
    elif axis == "Y":
        name = "B"
        rot_vec = CivilCvCAD.Vector(0, 1, 0)
    else:
        raise ValueError("axis must be 'X' or 'Y'")
...
class TestPathRotarySurface(PathTestBase):
    ...
    def setUp(self):
        self.doc = CivilCvCAD.newDocument("TestRotarySurface")
    ...
    def test05_validation_rejects_misaligned_stock(self):
        ...
        job.Stock.Placement = CivilCvCAD.Placement(
            job.Stock.Placement.Base,
            CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), 5.0).multiply(job.Stock.Placement.Rotation),
        )
        op = self._build_op(job, axis="X")
        self.doc.recompute()
        a_commands = [c for c in op.Path.Commands if c.Name == "G1" and "A" in c.Parameters]
    ...
    def test08_offcenter_part_warns(self):
        ...
        part.Placement = CivilCvCAD.Placement(
            CivilCvCAD.Vector(part.Placement.Base.x, 5.0, part.Placement.Base.z),
