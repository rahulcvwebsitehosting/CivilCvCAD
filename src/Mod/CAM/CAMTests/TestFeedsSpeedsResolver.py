...
"""
Pure-function tests for the FeedsSpeeds resolver. No CivilCvCAD doc objects;
...
class TestFeedsSpeedsResolver(unittest.TestCase):
    ...
    def test_resolver_does_not_import_civilcvcad(self):
        # Sanity check: the resolver module should be importable without
        # CivilCvCAD. We can't fully prove that here (we're inside CivilCvCAD), but
        # we can at least confirm the module's __dict__ has no CivilCvCAD ref.
        from Path.Tool.FeedsSpeeds import resolver, types, providers

        for mod in (resolver, types, providers):
            self.assertNotIn("CivilCvCAD", dir(mod), f"{mod.__name__} leaks CivilCvCAD")
