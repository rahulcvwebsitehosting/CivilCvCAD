...
class TestPathUtil(PathTestBase):
    def setUp(self):
        self.doc = CivilCvCAD.newDocument("TestPathUtils")

    def tearDown(self):
        CivilCvCAD.closeDocument("TestPathUtils")
    ...
    def test02(self):
        ...
        box.Width = 10
        box.Height = 1
        box.Placement = CivilCvCAD.Placement(
            CivilCvCAD.Vector(-5, -5, 0), CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), 0)
        )
        cyl = self.doc.addObject("Part::Cylinder", "Cylinder")
        cyl.Radius = 1
        cyl.Height = 10
        box.Placement = CivilCvCAD.Placement(
            CivilCvCAD.Vector(0, 0, -5), CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), 0)
        )
