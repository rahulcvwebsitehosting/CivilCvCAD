...
class TestGetLinkingMoves(PathTestUtils.PathTestBase):
    def setUp(self):
        self.start = CivilCvCAD.Vector(0, 0, 0)
        self.target = CivilCvCAD.Vector(10, 0, 0)
    ...
    def test_path_blocked_by_solid(self):
        ...
        blocking_box.translate(CivilCvCAD.Vector(-5, -5, 0))
    ...
    def test_plunge_to_zero_depth(self):
        ...
        start = CivilCvCAD.Vector(0, 0, 1)  # Start below clearance
        target = CivilCvCAD.Vector(10, 10, 0)  # Target depth is 0
    ...
    def test_plunge_to_negative_depth(self):
        ...
        start = CivilCvCAD.Vector(0, 0, 1)  # Start below clearance
        target = CivilCvCAD.Vector(10, 10, -2)  # Target depth is negative
    ...
    def test_path_collision_by_tooldiameter(self):
        ...
        blocking_box1.translate(CivilCvCAD.Vector(5, -12, 0))
        blocking_box2.translate(CivilCvCAD.Vector(5, 2, 0))
        ...
        blocking_box1.translate(CivilCvCAD.Vector(0, -1, 0))
        blocking_box2.translate(CivilCvCAD.Vector(0, -1, 0))
        with self.assertRaises(RuntimeError):
            generator.get_linking_moves(
                start_position=self.start,
                target_position=self.target,
    ...
    def test_zero_retract_offset_uses_local_clearance(self):
        cmds = generator.get_linking_moves(
            start_position=self.start,
            target_position=CivilCvCAD.Vector(10, 0, 5),
            heights_clearance=self.heights_clearance,
