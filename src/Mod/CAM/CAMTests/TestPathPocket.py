...
class TestPathPocket(PathTestBase):
    ...
    def setUp(self):
        ...
        self.doc = CivilCvCAD.newDocument("TestPocket")
    ...
    def test_pocket_square_offset(self):
        ...
        inner = Part.makeBox(
            ...
            pocket_size,
            pocket_depth_amount,
            CivilCvCAD.Vector(pocket_offset_xy, pocket_offset_xy, pocket_bottom_z),
        )
        pocket_solid = outer.cut(inner)

        part_obj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "PocketPart")
    ...
    def test_pocket_pointy_triangle_offset(self):
        ...
        v1 = CivilCvCAD.Vector(center_x - triangle_base / 2.0, base_y, pocket_bottom_z)
        v2 = CivilCvCAD.Vector(center_x + triangle_base / 2.0, base_y, pocket_bottom_z)
        v3 = CivilCvCAD.Vector(center_x, apex_y, pocket_bottom_z)

        # Create triangle wire and extrude
        triangle_wire = Part.makePolygon([v1, v2, v3, v1])
        triangle_face = Part.Face(triangle_wire)
        inner = triangle_face.extrude(CivilCvCAD.Vector(0, 0, pocket_depth_amount))

        pocket_solid = outer.cut(inner)

        part_obj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "TrianglePart")
        part_obj.Shape = pocket_solid
    ...
    def test_pocket_square_line(self):
        ...
        inner = Part.makeBox(
            ...
            pocket_height,
            pocket_depth_amount,
            CivilCvCAD.Vector(pocket_offset_x, pocket_offset_y, pocket_bottom_z),
        )
        pocket_solid = outer.cut(inner)

        part_obj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "LinePocketPart")
        part_obj.Shape = pocket_solid
    ...
    def test_pocket_square_grid(self):
        ...
        part_obj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "GridPocketPart")
        part_obj.Shape = pocket_solid
