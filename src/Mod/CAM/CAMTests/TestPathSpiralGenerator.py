...
def _resetArgs():
    return {
        "center": CivilCvCAD.Vector(10, 10, 0),
        "outer_radius": 7.5,
        "step": 2.5,
        "inner_radius": 2.5,
        "direction": "CW",
...
class TestPathSpiralGenerator(PathTestUtils.PathTestBase):
    ...
    def test02(self):
        ...
        step = 5
        turns = 3
        center = CivilCvCAD.Vector(10, 5, 0)
        dir_angle = 30

        # create Part spiral
        doc = CivilCvCAD.newDocument("TestPathSpiralGenerator")
        ...
        pl.rotate(center, CivilCvCAD.Vector(0, 0, 1), dir_angle)
        startPoint = pl.multVec(CivilCvCAD.Vector(center.x + inner_radius, center.y, 0))
        for cmd in cmds:
