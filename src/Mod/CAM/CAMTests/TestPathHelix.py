...
class TestPathHelix(PathTestUtils.PathTestBase):
    ...
    def setUp(self):
        self.clone = None
        self.doc = CivilCvCAD.open(CivilCvCAD.getHomePath() + "Mod/CAM/CAMTests/test_holes00.fcstd")
    ...
    def test02(self):
        ...
        for deg in range(self.RotateBy, 360, self.RotateBy):
            model.Placement.Rotation = CivilCvCAD.Rotation(deg, 0, 0)
            for base in op.Base:
                model = base[0]
                for sub in base[1]:
                    pos = proxy.holePosition(model, sub)
    ...
    def test03(self):
        ...
        for deg in range(self.RotateBy, 360, self.RotateBy):
            self.tearDown()
            self.doc = CivilCvCAD.open(CivilCvCAD.getHomePath() + "Mod/CAM/CAMTests/test_holes00.fcstd")
            self.doc.Body.Placement.Rotation = CivilCvCAD.Rotation(deg, 0, 0)
    ...
    def test04(self):
        ...
        for deg in range(self.RotateBy, 360, self.RotateBy):
            ...
            self.clone.Placement.Rotation = CivilCvCAD.Rotation(deg, 0, 0)
    ...
    def testRecomputeHelixFromV021(self):
        """Verify that we can still open and recompute a Helix created with older CivilCvCAD"""
        self.tearDown()
        self.doc = CivilCvCAD.openDocument(str(FIXTURE_PATH / "OpHelix_v0-21.FCStd"))
