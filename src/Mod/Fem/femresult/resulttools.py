...
def reset_mesh_deformation(resultobj):
    ...
    resultobj : Fem::ResultMechanical
        CivilCvCAD FEM mechanical result object
    """
...
def show_result(resultobj, result_type="Sabs", limit=None):
    ...
    if resultobj:
        ...
        CivilCvCAD.Console.PrintError("Error, No result object given.\n")
...
def get_all_stats(res_obj):
    ...
    the methods _getCivilCvCADMechResultVectorProperties()
    and _getCivilCvCADMechResultScalarProperties()
    as well as forum topic
    https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&start=30#p277434
...
def fill_femresult_stats(res_obj):
    """Fills a CivilCvCAD FEM mechanical result object with stats data.
    ...
    CivilCvCAD.Console.PrintLog("Calculate stats list for result obj: " + res_obj.Name + "\n")
    ...
    CivilCvCAD.Console.PrintLog("Stats list for result obj: " + res_obj.Name + " calculated\n")
    return res_obj


def add_disp_apps(res_obj):
    res_obj.DisplacementLengths = calculate_disp_abs(res_obj.DisplacementVectors)
    CivilCvCAD.Console.PrintLog("Added DisplacementLengths.\n")
    return res_obj
...
def add_von_mises(res_obj):
    ...
    CivilCvCAD.Console.PrintLog("Added von Mises stress.\n")
    return res_obj
...
def add_principal_stress_std(res_obj):
    ...
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&p=416006#p416006
    ...
    CivilCvCAD.Console.PrintLog("Added standard principal stresses and max shear values.\n")

    #
    # Add critical strain ratio using the Stress Modified Critical Strain (SMCS) criterion
    #   Forum Discussion: https://forum.civilcvcad.org/viewtopic.php?f=18&t=35893#p303392
    ...
    MatMechNon = CivilCvCAD.ActiveDocument.getObject("MaterialMechanicalNonlinear")
    if MatMechNon:
...
def get_concrete_nodes(res_obj):
    ...
    for obj in res_obj.getParentGroup().Group:
        if obj.isDerivedFrom("App::MaterialObjectPython") and is_of_type(
            ...
            CivilCvCAD.Console.PrintMessage("ReinforcedMaterial\n")
            if obj.References == []:
            ...
            CivilCvCAD.Console.PrintMessage("No ReinforcedMaterial\n")
            if obj.References == []:
                for iic in range(nsr):
                    if ic[iic] == 0:
                        ic[iic] = 2
...
def add_principal_stress_reinforced(res_obj):
    ...
    for obj in res_obj.getParentGroup().Group:
        if is_of_type(obj, "Fem::MaterialReinforced"):
            matrix_af = float(
                CivilCvCAD.Units.Quantity(obj.Material["AngleOfFriction"]).getValueAs("rad")
            )
            matrix_cs = float(
                CivilCvCAD.Units.Quantity(obj.Material["CompressiveStrength"]).getValueAs("MPa")
            )
            reinforce_yield = float(
                CivilCvCAD.Units.Quantity(obj.Reinforcement["YieldStrength"]).getValueAs("MPa")
            )
    # print(matrix_af)
    # print(matrix_cs)
...
def compact_result(res_obj):
    ...
    # CivilCvCAD result obj does not support elem results ATM
...
def calculate_von_mises(stress_tensor):
    ...
    Simplification: https://forum.civilcvcad.org/viewtopic.php?f=18&t=33974&p=296542#p296542
...
def calculate_principal_stress_std(stress_tensor):
    ...
    # https://forum.civilcvcad.org/viewtopic.php?f=22&t=33911&start=10#p284229
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=32649#p274291
    ...
    )  # https://forum.civilcvcad.org/viewtopic.php?f=18&t=24637&start=10#p240408
...
def calculate_principal_stress_reinforced(stress_tensor):
    ...
    https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&start=90#p296539
    """
...
def calculate_rho(stress_tensor, fy):
    ...
    See post:
    https://forum.civilcvcad.org/viewtopic.php?f=18&t=28821
...
def calculate_disp_abs(displacements):
    # see https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&start=100#p296657
