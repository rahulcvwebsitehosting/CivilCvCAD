...
__title__ = "CivilCvCAD FEM import tools"
__author__ = "Bernd Hahnebach"
__url__ = ""
## @package importToolsFem
#  \ingroup FEM
#  \brief CivilCvCAD FEM import tools
...
def get_FemMeshObjectElementTypes(fem_mesh_obj, remove_zero_element_entries=True):
    ...
    elements_list_with_zero = [
        ...
        for (s, d) in CivilCvCAD_element_names_dims.items()
    ]
    # ugly but necessary
...
def make_femmesh(mesh_data):
    """makes an CivilCvCAD FEM Mesh object from FEM Mesh data"""
    import Fem

    mesh = Fem.FemMesh()
    m = mesh_data
    if ("Nodes" in m) and (len(m["Nodes"]) > 0):
        CivilCvCAD.Console.PrintLog("Found: nodes\n")
        if (
            ("Seg2Elem" in m)
            or ("Seg3Elem" in m)
            or ("Tria3Elem" in m)
            ...
            nds = m["Nodes"]
            CivilCvCAD.Console.PrintLog("Found: elements\n")
            for i in nds:
                n = nds[i]
...
def fill_femresult_mechanical(res_obj, result_set):
    """fills a CivilCvCAD FEM mechanical result object with result data"""
