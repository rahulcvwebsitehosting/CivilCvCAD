...
#  \ingroup FEM
#  \brief CivilCvCAD Calculix FRD Reader for FEM workbench

import os
import math

import CivilCvCAD
from CivilCvCAD import Console
from builtins import open as pyopen

# ********* generic CivilCvCAD import and export methods *********
...
def setupPipeline(doc, analysis, results_name, result_data):
    ...
    if not "BUILD_FEM_VTK" in CivilCvCAD.__cmake__:
        return
        ...
        if len(m["Results"]) > 0:
            for result_set in m["Results"]:
                ...
                if not res_obj.MassFlowRate:
                    ...
                    # example frd file: https://forum.civilcvcad.org/viewtopic.php?t=32649#p274291
            ...
            if number_of_increments > 1:
                ...
                match result_analysis_type:
                    case "frequency":
                        unit = CivilCvCAD.Units.Frequency
                        description = "Eigenmode"
                    case "buckling":
                        unit = CivilCvCAD.Units.Unit()
                        description = "Buckling factor"
                    case "thermomech":
                        unit = CivilCvCAD.Units.TimeSpan
...
def read_frd_result(frd_input):
    ...
    for line in frd_file:
        ...
        if nodes_found and (line[1:3] == "-1"):
            ...
            nodes[elem] = CivilCvCAD.Vector(nodes_x, nodes_y, nodes_z)
        ...
        if elements_found and (line[1:3] == "-2"):
            ...
            if elemType == 1:
                # C3D8 CalculiX --> hexa8 CivilCvCAD
                ...
                # C3D6 Calculix --> penta6 CivilCvCAD
                # N5, N6, N4, N2, N3, N1
                nd1 = int(line[3:13])
                nd2 = int(line[13:23])
                nd3 = int(line[23:33])
                ...
                # C3D4 Calculix --> tetra4 CivilCvCAD
                # N2, N1, N3, N4
                nd1 = int(line[3:13])
                nd2 = int(line[13:23])
                nd3 = int(line[23:33])
                ...
                # first line
                # C3D20 Calculix --> hexa20 CivilCvCAD
                ...
                # first line
                # C3D15 Calculix --> penta15 CivilCvCAD
                ...
                # C3D10 Calculix --> tetra10 CivilCvCAD
                ...
                # B31 CalculiX --> seg2 CivilCvCAD
                # N1, N2
                nd1 = int(line[3:13])
                nd2 = int(line[13:23])
                elements_seg2[elem] = (nd1, nd2)
            elif elemType == 12:
                # B32 CalculiX --> seg3 CivilCvCAD
        ...
        if mode_disp_found and (line[1:3] == "-1"):
            ...
            mode_disp[elem] = CivilCvCAD.Vector(mode_disp_x, mode_disp_y, mode_disp_z)
        ...
        if mode_stress_found and (line[1:3] == "-1"):
            ...
            # CivilCvCAD:            (Sxx, Syy, Szz, Sxy, Sxz, Syz)
        ...
        if mode_strain_found and (line[1:3] == "-1"):
            ...
            # CivilCvCAD:            (Exx, Eyy, Ezz, Exy, Exz, Eyz)
            # thus exchange the last two entries
        ...
        if mode_heatflux_found and (line[1:3] == "-1"):
            ...
            mode_heatflux[elem] = CivilCvCAD.Vector(mode_heatflux_x, mode_heatflux_y, mode_heatflux_z)
        ...
        if (
            ...
            mode_results = {}
            # https://forum.civilcvcad.org/viewtopic.php?f=18&t=32649&start=10#p274686
            mode_results["number"] = float("NaN")
            mode_results["time"] = float("NaN")
            end_of_section_found = False
