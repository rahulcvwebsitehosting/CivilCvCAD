...
__title__ = "CivilCvCAD Fenics XML mesh reader"
__author__ = "Johannes Hartung"
__url__ = ""
## @package importFenicsXML
#  \ingroup FEM
#  \brief CivilCvCAD Fenics Mesh XML reader for FEM workbench
...
def read_fenics_mesh_xml(xmlfilename):
        ...
        if find_vertices is None:
            ...
            for vertex in find_vertices:
                ...
                if vertex.tag.lower() == "vertex":
                    ...
                    nodes_dict[ind + 1] = CivilCvCAD.Vector(node_x, node_y, node_z)
                    # increase node index by one, since fenics starts at 0, CivilCvCAD at 1
    ...
    def generate_lower_dimensional_structures(nodes, cell_dict, cell_type, dim):
        ...
        def correct_volume_det(element_dict):
            ...
            if dim == 2:
                nz = CivilCvCAD.Vector(0.0, 0.0, 1.0)
        ...
        for key, val in list(Fenics_to_CivilCvCAD_dict.items()):
            element_dict[val] = {}
        ...
        for cell_index, cell in list(cell_dict.items()):
            ...
            element_dict[Fenics_to_CivilCvCAD_dict[cell_type]][cell] = element_counter[cell_type]
            for ld in cell_lower_dims:
                for vertextuple in itertools.combinations(cell, num_vert_dict[ld]):
                    element_counter[ld] = addtupletodict(
                        element_dict[Fenics_to_CivilCvCAD_dict[ld]], vertextuple, element_counter[ld]
                    )
        ...
        for key, val_dict in list(element_dict.items()):
            # to ensure distinct indices for CivilCvCAD
            # print("key: ", key)
    ...
    for val in list(Fenics_to_CivilCvCAD_dict.values()):
        element_dict[val] = {}
