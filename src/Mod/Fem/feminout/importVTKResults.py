...
#  \ingroup FEM
#  \brief CivilCvCAD Result import and export VTK file library
...
def export(objectslist, filename):
    "called when civilcvcad exports an object to vtk"
    if len(objectslist) > 1:  # the case of no selected obj is caught by CivilCvCAD already
...
def importVtk(filename, object_name=None, object_type=None):
    if not object_type:
        vtkinout_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/InOutVtk")
...
def importVtkVtkResult(filename, resultname):
    vtk_result_obj = CivilCvCAD.ActiveDocument.addObject("Fem::FemPostPipeline", resultname)
...
def importVtkFemMesh(filename, meshname):
    meshobj = CivilCvCAD.ActiveDocument.addObject("Fem::FemMeshObject", meshname)
...
def importVtkFCResult(filename, resultname, analysis=None, result_name_prefix=None):
    # only fields from vtk are imported if they exactly named as the CivilCvCAD result properties
    # See _getCivilCvCADMechResultProperties() in FemVTKTools.cpp for the supported names
    ...
    result_obj = ObjectsFem.makeResultMechanical(CivilCvCAD.ActiveDocument, results_name)
