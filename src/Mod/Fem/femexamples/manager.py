...
# CivilCvCAD Gui update between the examples would makes sense too
...
def run_analysis(doc, base_name, analysis=None, filepath="", run_solver=False,  blocking=True):
    ...
    CivilCvCAD.Console.PrintMessage(f"Save CivilCvCAD file for {base_name} analysis to {save_fc_file}\n.")
...
def run_example(example, solver=None, base_name=None, run_solver=False, blocking=True, doc=None):
    ...
    if not hasattr(module, "setup"):
        CivilCvCAD.Console.PrintError(f"Setup method not found in {example}\n")
        return None
