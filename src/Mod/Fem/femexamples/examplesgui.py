...
"""
https://forum.civilcvcad.org/viewtopic.php?f=10&t=48427
"""
...
class FemExamples(QtGui.QWidget):
    ...
    def __del__(
        ...
        # --> see https://forum.civilcvcad.org/viewtopic.php?f=18&t=10732&start=10#p86493
        return
    ...
    def accept(self):
        ...
        CivilCvCADGui.doCommand(f"from femexamples.{str(example)}  import setup")
        if solver is not None:
            CivilCvCADGui.doCommand(f'setup(solvertype="{str(solver)}")')
        else:
            CivilCvCADGui.doCommand("setup()")
        ...
        if parent is not None:
            ...
            if grand_parent is not None:
                ...
                if grand_parent_name == "Solvers":
                    solver = parent.text(0)
        # if done this way the Python commands are printed in Python console
        CivilCvCADGui.doCommand("from femexamples.manager import run_example")
        if solver is not None:
            CivilCvCADGui.doCommand(
                f'run_example("{str(example)}", solver="{str(solver)}", run_solver=True, blocking=False)'
            )
        else:
            CivilCvCADGui.doCommand(f'run_example("{str(example)}", run_solver=True)')
        QtGui.QApplication.restoreOverrideCursor()
