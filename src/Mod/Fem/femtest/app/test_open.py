...
class TestObjectOpen(unittest.TestCase):
    ...
    def test_femobjects_open_head(self):
        ...
        CivilCvCAD.closeDocument(self.document.Name)
        self.document = CivilCvCAD.open(file_path)
    ...
    def test_femobjects_open_de9b3fb438(self):
        ...
        # https://bugs.launchpad.net/ubuntu/+source/civilcvcad/+bug/1918474.
        if platform.machine() == "s390x":
            return

        # the number in method name is the CivilCvCAD commit the document was created with
        # https://github.com/CivilCvCAD/CivilCvCAD/commit/de9b3fb438
        # the document was created by running the object create unit test
        # CivilCvCAD --run-test "femtest.app.test_object.TestObjectCreate.test_femobjects_make"
        fcc_print("load old document objects")
        CivilCvCAD.closeDocument(self.document.Name)  # close the empty document from setUp first
        self.document = CivilCvCAD.open(join(self.test_file_dir, "all_objects_de9b3fb438.FCStd"))
