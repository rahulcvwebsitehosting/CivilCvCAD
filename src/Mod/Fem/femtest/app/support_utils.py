...
def get_fem_test_home_dir():
    return join(CivilCvCAD.getHomePath(), "Mod", "Fem", "femtest", "data")
...
def fcc_print(message):
    CivilCvCAD.Console.PrintMessage(f"{message} \n")
...
def get_defmake_count(fem_vtk_post=True):
    ...
    """
    name_modfile = join(CivilCvCAD.getHomePath(), "Mod", "Fem", "ObjectsFem.py")
...
def get_fem_test_defs():
    ...
    test_path = join(CivilCvCAD.getHomePath(), "Mod", "Fem", "femtest", "app")
    ...
    cf.write("make -j 4 && ./bin/CivilCvCAD --run-test 'TestFemApp'\n")
    cf.write("\n")
    cf.write("make -j 4 && ./bin/CivilCvCADCmd --run-test 'TestFemApp'\n")
    cf.write("\n")
    cf.write("\n")
    cf.write("'''\n")
    cf.write("\n")
    ...
    for m in collected_test_modules:
        cf.write(f"make -j 4 && ./bin/CivilCvCADCmd -t {m}\n")
    cf.write("\n")
    cf.write("\n")
    cf.write("# classes\n")
    ...
    cf.write("\n")
    cf.write("\n")
    cf.write("# methods in CivilCvCAD\n")
    for m in collected_test_methods:
        cf.write(
            "\nimport unittest\n"
...
def collect_python_modules(femsubdir=None):
    if not femsubdir:
        pydir = join(CivilCvCAD.ConfigGet("AppHomePath"), "Mod", "Fem")
    else:
        pydir = join(CivilCvCAD.ConfigGet("AppHomePath"), "Mod", "Fem", femsubdir)
    collected_modules = []
    fcc_print(pydir)
...
# open the file in CivilCvCAD GUI and return the doc identifier
def cube_frequency():
    ...
    doc = CivilCvCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_frequency", "cube_frequency.FCStd"))
    return doc
...
def cube_static():
    ...
    unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(testname))
    doc = CivilCvCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_static", "cube_static.FCStd"))
    return doc
...
def multimat():
    ...
    unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(testname))
    doc = CivilCvCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_multimat", "multimat.FCStd"))
    return doc
...
def spine_thermomech():
    ...
    unittest.TextTestRunner().run(unittest.TestLoader().loadTestsFromName(testname))
    doc = CivilCvCAD.open(join(get_fem_test_tmp_dir(), "FEM_ccx_thermomech", "spine_thermomech.FCStd"))
    return doc
