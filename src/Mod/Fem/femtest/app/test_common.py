...
class TestFemCommon(unittest.TestCase):
    ...
    def test_adding_refshaps(self):
        ...
        ref_eles = []
        # CivilCvCAD list property doesn't seem to support append,
    ...
    def test_pyimport_all_FEM_modules(self):
        # we're going to try to import all python modules from CivilCvCAD FEM
        pymodules = []
        ...
        for mod in pymodules:
            ...
            try:
                ...
                if "vtkmodules" in e.name and not "BUILD_FEM_VTK_PYTHON" in CivilCvCAD.__cmake__:
                    im = True
                else:
                    im = False
            if not im:
