...
)
{
    ...
    while (aVertexIter->more()) {
        ...
        if (aVertex->GetEntityType() == SMDSEntity_0D) {
            ...
            throw Base::TypeError("Vertex not yet supported by CivilCvCAD's VTK mesh builder\n");
        }
    }
...
)
{
    ...
    while (aEdgeIter->more()) {
        ...
        // quadratic edge
            ...
            throw Base::TypeError("Edge not yet supported by CivilCvCAD's VTK mesh builder\n");
        }
    }
...
)
{
    ...
    while (aFaceIter->more()) {
        ...
        // quadratic quad
            ...
            throw Base::TypeError("Face not yet supported by CivilCvCAD's VTK mesh builder\n");
        }
    }
...
)
{
    ...
    while (aVolIter->more()) {
        ...
        if (aVol->GetEntityType() == SMDSEntity_Tetra) {  // tetra4
            ...
            throw Base::TypeError("Volume not yet supported by CivilCvCAD's VTK mesh builder\n");
        }
    }
...
)
{
    ...
    while (aNodeIter->more()) {
        ...
        // this needs to be taken into account on node mapping when CivilCvCAD FEM results
        // are exported to vtk
    }
...
App::DocumentObject* FemVTKTools::readResult(const char* filename, App::DocumentObject* res)
{
    ...
    if (result) {
        ...
        importCivilCvCADResult(dataset, result);
    }
...
void FemVTKTools::writeResult(const char* filename, const App::DocumentObject* res)
{
    ...
    // result
    FemVTKTools::exportCivilCvCADResult(res, grid);
...
std::map<std::string, std::string> _getCivilCvCADMechResultVectorProperties()
{
    ...
    // https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&start=70#p296317
    // https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&p=416006#p412800
...
// see https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&start=30#p277434 for further
...
// in CivilCvCAD VTK pipeline
std::map<std::string, std::string> _getCivilCvCADMechResultScalarProperties()
{
    ...
    // but I do not know how (Bernd), for some help see paraview tutorial on CivilCvCAD wiki
    // thus TODO they might not be exported to external file format (first I need to know
    // how to generate them in paraview)
    // but there are needed anyway because the pipeline in CivilCvCAD needs the principal stress values
    // https://forum.civilcvcad.org/viewtopic.php?f=18&t=33106&p=416006#p412800
...
void FemVTKTools::importCivilCvCADResult(vtkSmartPointer<vtkDataSet> dataset, App::DocumentObject* result)
{
    Base::Console().log("Start: import vtk result file data into a CivilCvCAD result object.\n");

    std::map<std::string, std::string> vectors = _getCivilCvCADMechResultVectorProperties();
    std::map<std::string, std::string> scalars = _getCivilCvCADMechResultScalarProperties();
    ...
    for (const auto& it : vectors) {
        int dim = 3;  // Fixme: currently 3D only, here we could run into trouble,
                      //        CivilCvCAD only supports dim 3D, I do not know about VTK
    ...
    Base::Console().log("End: import vtk result file data into a CivilCvCAD result object.\n");
}


void FemVTKTools::exportCivilCvCADResult(const App::DocumentObject* result, vtkSmartPointer<vtkDataSet> grid)
{
    Base::Console().log("Start: Create VTK result data from CivilCvCAD result data.\n");
    ...
    // (when the civilcvcad smesh mesh has gaps in the points
    ...
    // vectors
    for (const auto& it : vectors) {
        const int dim = 3;  // Fixme, detect dim, but CivilCvCAD PropertyVectorList ATM only has DIM of 3
    ...
    Base::Console().log("End: Create VTK result data from CivilCvCAD result data.\n");
}
