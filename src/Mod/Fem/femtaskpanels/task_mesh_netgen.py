...
__title__ = "CivilCvCAD FEM mesh netgen task panel for mesh netgen object"
...
class _TaskPanel(base_femlogtaskpanel._BaseWorkerTaskPanel):
    ...
    def __init__(self, obj):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(
            CivilCvCAD.getHomePath() + "Mod/Fem/Resources/ui/MeshNetgen.ui"
        )
    ...
    def set_widgets(self):
        ...
        CivilCvCADGui.ExpressionBinding(self.form.qsb_max_size).bind(self.obj, "MaxSize")

        self.form.qsb_min_size.setProperty("value", self.min_size)
        CivilCvCADGui.ExpressionBinding(self.form.qsb_min_size).bind(self.obj, "MinSize")

        self.form.dsb_growth_rate.setProperty("value", self.growth_rate)
        CivilCvCADGui.ExpressionBinding(self.form.dsb_growth_rate).bind(self.obj, "GrowthRate")

        self.form.dsb_curvature_safety.setProperty("value", self.curvature_safety)
        CivilCvCADGui.ExpressionBinding(self.form.dsb_curvature_safety).bind(
            self.obj, "CurvatureSafety"
        )

        self.form.dsb_seg_per_edge.setProperty("value", self.seg_per_edge)
        CivilCvCADGui.ExpressionBinding(self.form.dsb_seg_per_edge).bind(self.obj, "SegmentsPerEdge")
