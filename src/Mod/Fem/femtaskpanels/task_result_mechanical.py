...
__title__ = "CivilCvCAD result mechanical task panel"
...
class _TaskPanel:
    ...
    def __init__(self, obj):
        ...
        ui_path = CivilCvCAD.getHomePath() + "Mod/Fem/Resources/ui/"
        self.result_widget = CivilCvCADGui.PySideUic.loadUi(ui_path + "ResultShow.ui")
        self.info_widget = CivilCvCADGui.PySideUic.loadUi(ui_path + "ResultHints.ui")
        ...
        self.fem_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/General")
    ...
    def restore_result_dialog(self):
        try:
            rt = CivilCvCAD.FEM_dialog["results_type"]
            if rt == "None":
            ...
            sd = CivilCvCAD.FEM_dialog["show_disp"]
            self.result_widget.cb_show_displacement.setChecked(sd)
            self.show_displacement(sd)

            df = CivilCvCAD.FEM_dialog["disp_factor"]
            dfm = CivilCvCAD.FEM_dialog["disp_factor_max"]
            ...
            # animate
            self.startAnimate = False
            if CivilCvCAD.FEM_dialog["animate"][0] != -1:
                self.result_widget.steps.setValue(CivilCvCAD.FEM_dialog["animate"][0])
                self.result_widget.loops.setValue(CivilCvCAD.FEM_dialog["animate"][1])
                self.result_widget.framerate.setValue(CivilCvCAD.FEM_dialog["animate"][2])
                if CivilCvCAD.FEM_dialog["animate"][3]:
                    self.result_widget.rb_full_cycle.setChecked(True)
                else:
                    self.result_widget.rb_half_cycle.setChecked(True)
                self.result_widget.sb_displacement_factor.setValue(CivilCvCAD.FEM_dialog["animate"][4])
        except Exception:
            self.restore_initial_result_dialog()

    def restore_initial_result_dialog(self):
        # initialize CivilCvCAD.FEM_dialog and set standard values
        # the FEM result mechanical task panel restore values
        # are saved in a dictionary which is an attribute of CivilCvCAD
        # the name is FEM_dialog
        # in python console after result task panel has been opened once
        # CivilCvCAD.FEM_dialog or CivilCvCAD.__dir__()
        # This is not smart at all IMHO (Bernd)
        # It was added with commit 3a7772d
        # https://github.com/CivilCvCAD/CivilCvCAD/commit/3a7772d
        CivilCvCAD.FEM_dialog = {
            "results_type": "None",
    ...
    def none_selected(self, state):
        ...
        CivilCvCAD.FEM_dialog["results_type"] = "None"
    ...
    # see https://forum.civilcvcad.org/viewtopic.php?f=18&t=39162
    ...
    def show_histogram_clicked(self):
        if len(plt.get_fignums()) > 0:
            ...
            # see https://github.com/CivilCvCAD/CivilCvCAD/issues/6975
            if CivilCvCAD.FEM_dialog["results_type"] != "None":
                self.restore_result_dialog()
            if len(plt.get_fignums()) > 0:
                plt.show()
            else:
    ...
    def user_defined_text(self, equation):
        CivilCvCAD.FEM_dialog["results_type"] = "user"
    ...
    def calculate(self):
        ...
        # https://forum.civilcvcad.org/viewtopic.php?f=18&t=42425&start=10#p368774 ff
        # https://github.com/CivilCvCAD/CivilCvCAD/pull/3020
        from ply import lex
    ...
    def result_selected(self, res_type, res_values, res_unit, res_title):
        ...
        CivilCvCAD.FEM_dialog["results_type"] = res_type
        ...
        fig_manager.window.setParent(CivilCvCADGui.getMainWindow())
    ...
    def update_displacement(self, factor=None):
        if factor is None:
            if CivilCvCAD.FEM_dialog["show_disp"]:
    ...
    def show_displacement(self, checked):
        ...
        QApplication.setOverrideCursor(Qt.WaitCursor)
        CivilCvCAD.FEM_dialog["show_disp"] = checked
        if "result_obj" in CivilCvCAD.FEM_dialog:
            if CivilCvCAD.FEM_dialog["result_obj"] != self.result_obj:
                self.update_displacement()
        CivilCvCAD.FEM_dialog["result_obj"] = self.result_obj
        if self.suitable_results:
    ...
    def hsb_disp_factor_changed(self, value):
        ...
        CivilCvCAD.FEM_dialog["disp_factor"] = scaled_val
        self.update_displacement()
    ...
    def sb_disp_factor_max_changed(self, value):
        CivilCvCAD.FEM_dialog["disp_factor_max"] = value
        inputbox = self.result_widget.sb_displacement_factor
    ...
    def sb_disp_factor_changed(self, value):
        CivilCvCAD.FEM_dialog["disp_factor"] = value
        self.update_displacement()
    ...
    def update(self):
        ...
        if self.mesh_obj.FemMesh.NodeCount == 0:
            ...
            CivilCvCAD.Console.PrintError(error_message)
    ...
    def reject(self):
        ...
        if len(self.animateText) > 0:
            ...
            self.animateText = []
        self.startAnimate = False
        CivilCvCAD.FEM_dialog["animate"][0] = self.result_widget.steps.value()
        CivilCvCAD.FEM_dialog["animate"][1] = self.result_widget.loops.value()
        CivilCvCAD.FEM_dialog["animate"][2] = self.result_widget.framerate.value()
        CivilCvCAD.FEM_dialog["animate"][3] = self.result_widget.rb_full_cycle.isChecked()
        try:
            CivilCvCAD.FEM_dialog["animate"][4] = self.result_widget.sb_displacement_factor.value()
        except:
            CivilCvCAD.FEM_dialog["animate"][4] = 1
...
def hide_parts_constraints():
    ...
    fem_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/General")
