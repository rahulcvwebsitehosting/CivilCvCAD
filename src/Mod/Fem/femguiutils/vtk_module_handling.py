...
CivilCvCAD is linked with VTK libraries during its build process. To use the VTK
python module and pass objects between python and c++ the compiled module library
needs to be linked to the exact same vtk library as CivilCvCAD is. This is ensured by
...
set of VTK libs, and hence object passing in CivilCvCAD fails. (Note: import and
...
__url__ = ""
# Note: This file is imported from CivilCvCAD App files. Do not import any CivilCvCADGui
...
def vtk_module_compatible():
    # checks if the VTK library CivilCvCAD is build against is the one used by
    # the python module
...
# If CivilCvCAD is build with VTK python support this function checks if the
...
def vtk_module_handling():
    ...
    if not "BUILD_FEM_VTK_PYTHON" in CivilCvCAD.__cmake__:
        # no VTK python api support in CivilCvCAD
        return
    ...
    if not vtk_module_compatible():
        ...
        message = translate(
            "FEM",
            (
                "CivilCvCAD is linked to a different VTK library than the imported "
            ...
            if not loaded:
                ...
                message += translate(
                    ...
                    (
                        ...
                        "Restart CivilCvCAD to get the option for loading this module."
                    ),
                )
