...
__title__ = "CivilCvCAD FEM postprocessing view for summarizing extractor links"
...
def build_new_visualization_tree_model():
    ...
    for vis_name in visualizations:
        vis_icon = CivilCvCADGui.getIcon(visualizations[vis_name].icon)
        ...
        for ext in visualizations[vis_name].extractions:
            icon = CivilCvCADGui.getIcon(ext.icon)
...
class _SummaryWidget(QtGui.QWidget):
    ...
    def __init__(self, st_object, extractor, post_dialog):
        ...
        self.rmButton.setIcon(CivilCvCADGui.getIcon("delete.svg"))
    ...
    def showVisualization(self):
        if vis.is_visualization_object(self._st_object):
            ...
            CivilCvCADGui.Selection.addSelection(self._st_object)
...
class ExtractLinkView(QtGui.QWidget):
    ...
    def newVisualization(self, vis_data, ext_data):
        ...
        CivilCvCADGui.addModule(vis_data.module)
        CivilCvCADGui.addModule(ext_data.module)
        CivilCvCADGui.addModule("FemGui")

        # create visualization
        CivilCvCADGui.doCommand(
            f"visualization = {vis_data.module}.{vis_data.factory}(CivilCvCAD.ActiveDocument)"
        )

        analysis = self._find_parent_analysis(self._object)
        if analysis:
            CivilCvCADGui.doCommand(f"CivilCvCAD.ActiveDocument.{analysis.Name}.addObject(visualization)")

        # create extraction and add it
        CivilCvCADGui.doCommand(
            f"extraction = {ext_data.module}.{ext_data.factory}(CivilCvCAD.ActiveDocument)"
        )
        CivilCvCADGui.doCommand(f"extraction.Source = CivilCvCAD.ActiveDocument.{self._object.Name}")
        # default values: color
        color_prop = CivilCvCADGui.ActiveDocument.ActiveObject.Proxy.get_default_color_property()
        if color_prop:
            CivilCvCADGui.doCommand(
                f"extraction.ViewObject.{color_prop} = visualization.ViewObject.Proxy.get_next_default_color()"
            )

        CivilCvCADGui.doCommand(f"visualization.addObject(extraction)")
    ...
    def addExtractionToVisualization(self, vis_obj, ext_data):
        ...
        CivilCvCADGui.doCommand(f"App.ActiveDocument.{vis_obj.Name}.addObject(extraction)")
    ...
    def addExtractionToPostObject(self, post_obj, ext_data):
        ...
        CivilCvCADGui.doCommand(f"extraction.Source = CivilCvCAD.ActiveDocument.{post_obj.Name}")
        ...
        CivilCvCADGui.doCommand(f"App.ActiveDocument.{self._object.Name}.addObject(extraction)")
