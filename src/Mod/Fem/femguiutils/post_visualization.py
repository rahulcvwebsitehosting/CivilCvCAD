...
__title__ = "CivilCvCAD visualization registry"
...
class _VisualizationCommand:
    ...
    def Activated(self):
        ...
        vis = _registry[self._visualization_type]
        CivilCvCAD.ActiveDocument.openTransaction(f"Create {vis.name}")

        CivilCvCADGui.addModule(vis.module)
        CivilCvCADGui.addModule("FemGui")

        CivilCvCADGui.doCommand(f"obj = {vis.module}.{vis.factory}(CivilCvCAD.ActiveDocument)")
        CivilCvCADGui.doCommand(f"FemGui.getActiveAnalysis().addObject(obj)")

        CivilCvCADGui.Selection.clearSelection()
        CivilCvCADGui.doCommand("CivilCvCADGui.ActiveDocument.setEdit(obj)")
...
def setup_commands(toplevel_name):
    ...
    for vis in _registry:
        CivilCvCADGui.addCommand(_to_command_name(vis), _VisualizationCommand(vis))

    # build the group command!
    CivilCvCADGui.addCommand("FEM_PostVisualization", _VisualizationGroupCommand())
