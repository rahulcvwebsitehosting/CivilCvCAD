...
__title__ = "CivilCvCAD FEM select widget"
__author__ = "Markus Hovorka, Bernd Hahnebach"
__url__ = ""
## @package FemSelectWidget
#  \ingroup FEM
#  \brief CivilCvCAD FEM FemSelectWidget
...
class SolidSelector(_Selector):
    ...
    def _getObjects(self, obj, names):
        ...
        if not hasattr(obj, "Shape"):
            CivilCvCAD.Console.PrintMessage("Selected object has no Shape.\n")
            return objects
        shape = obj.Shape
        for n in names:
...
class GeometryElementsSelection(QtGui.QWidget):
    ...
    def initElemTypes(self, eltypes):
        ...
        # CivilCvCAD.Console.PrintMessage("Selection of: " + self.sel_elem_text + " is allowed.\n")
    ...
    def select_clicked_reference_shape(self):
        ...
        if self.sel_server:
            CivilCvCADGui.Selection.removeObserver(self.sel_server)
            self.sel_server = None
            ...
            for ref in self.references:
                if self.get_item_text(ref) == currentItemName:
                    ...
                    if ref[1].startswith("Solid") and (
                        ...
                        for f in faces:
                            CivilCvCADGui.Selection.addSelection(ref[0], f)
                    else:
                        # Selection of all other element types is supported
                        CivilCvCADGui.Selection.addSelection(ref[0], ref[1])
    ...
    def choose_selection_mode_standard(self, state):
        ...
        if self.sel_server and not self.selection_mode_solid:
            CivilCvCAD.Console.PrintMessage(self.selection_mode_std_print_message + "\n")
    ...
    def choose_selection_mode_solid(self, state):
        ...
        if self.sel_server and self.selection_mode_solid:
            CivilCvCAD.Console.PrintMessage(self.selection_mode_solid_print_message + "\n")
    ...
    def attachSelection(self):
        if self.sel_server:
            CivilCvCADGui.Selection.addObserver(self.sel_server)
    ...
    def has_equal_references_shape_types(self, ref_shty=""):
        for ref in self.references:
            ...
            if not r:
                CivilCvCAD.Console.PrintError(f"Problem in retrieving element: {ref[1]} \n")
                continue
...
class FemSelectionObserver:
    ...
    def __init__(self, parseSelectionFunction, print_message=""):
        ...
        # CivilCvCAD.Console.PrintMessage(print_message + "!\n")

    def addSelection(self, docName, objName, sub, pos):
        selected_object = CivilCvCAD.getDocument(docName).getObject(objName)  # get the obj objName
        edit_doc = CivilCvCADGui.editDocument()
