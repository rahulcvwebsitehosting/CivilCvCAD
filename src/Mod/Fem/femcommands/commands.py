...
__title__ = "CivilCvCAD FEM command definitions"
__author__ = "Bernd Hahnebach"
__url__ = ""
## @package commands
#  \ingroup FEM
#  \brief CivilCvCAD FEM command definitions

import CivilCvCAD
import CivilCvCADGui
from CivilCvCAD import Qt
...
# https://forum.civilcvcad.org/viewtopic.php?f=18&t=62449&p=543845#p543593
...
class _Analysis(CommandManager):
    ...
    def Activated(self):
        CivilCvCAD.ActiveDocument.openTransaction("Create Analysis")
        CivilCvCADGui.addModule("FemGui")
        CivilCvCADGui.addModule("ObjectsFem")
        CivilCvCADGui.doCommand("ObjectsFem.makeAnalysis(CivilCvCAD.ActiveDocument, 'Analysis')")
        CivilCvCADGui.doCommand("FemGui.setActiveAnalysis(CivilCvCAD.ActiveDocument.ActiveObject)")
        CivilCvCAD.ActiveDocument.commitTransaction()
        def_solver = get_default_solver()
        if def_solver:
            CivilCvCAD.ActiveDocument.openTransaction("Create default solver")
            cmd = ""
            match def_solver:
                case "CalculiX":
            ...
            if cmd:
                CivilCvCADGui.doCommand(f'CivilCvCADGui.runCommand("{cmd}")')

            CivilCvCADGui.doCommand(
                "CivilCvCADGui.ActiveDocument.toggleTreeItem(FemGui.getActiveAnalysis(), 2)"
            )
            CivilCvCAD.ActiveDocument.commitTransaction()
...
class _ClippingPlaneAdd(CommandManager):
    ...
    def Activated(self):
        ...
        overallboundbox = getBoundBoxOfAllDocumentShapes(CivilCvCAD.ActiveDocument)
        ...
        aFace = getSelectedFace(CivilCvCADGui.Selection.getSelectionEx())
        if aFace:
            ...
            f_CoM = CivilCvCAD.Vector(0, 0, 0)
            f_normal = CivilCvCAD.Vector(0, 0, 1)
        ...
        CivilCvCADGui.ActiveDocument.ActiveView.getSceneGraph().insertChild(clip_plane, 1)
...
class _ClippingPlaneRemoveAll(CommandManager):
    ...
    def Activated(self):
        ...
        CivilCvCADGui.doCommand("from pivy import coin")
        CivilCvCADGui.doCommand("sg = Gui.ActiveDocument.ActiveView.getSceneGraph()")
        CivilCvCADGui.doCommand("nodes = sg.getChildren()")
        CivilCvCADGui.doCommand(line1 + line2 + line3)
...
class _Examples(CommandManager):
    ...
    def Activated(self):
        CivilCvCADGui.addModule("femexamples.examplesgui")
        CivilCvCADGui.doCommand("femexamples.examplesgui.show_examplegui()")
...
class _MaterialEditor(CommandManager):
    ...
    def __init__(self):
        ...
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_MaterialEditor", "Opens the CivilCvCAD material editor"
        )
        self.is_active = "always"

    def Activated(self):
        CivilCvCADGui.addModule("MaterialEditor")
        CivilCvCADGui.doCommand("MaterialEditor.openEditor()")
...
class _MaterialMechanicalNonlinear(CommandManager):
    ...
    def Activated(self):
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Create FemMaterialMechanicalNonlinear")
        CivilCvCADGui.addModule("ObjectsFem")
        lin_mat_obj = f"CivilCvCAD.ActiveDocument.getObject('{self.selobj.Name}')"
        command_to_run = (
            f"ObjectsFem.makeMaterialMechanicalNonlinear(CivilCvCAD.ActiveDocument, {lin_mat_obj})"
        )
        CivilCvCADGui.doCommand(command_to_run)
...
class _FEMMesh2Mesh(CommandManager):
    ...
    def Activated(self):
        CivilCvCAD.ActiveDocument.openTransaction("Create Mesh from FEMMesh")
        if self.selobj and not self.selobj2:  # no result object selected
            CivilCvCADGui.addModule("femmesh.femmesh2mesh")
            CivilCvCADGui.doCommand(
                "out_mesh = femmesh.femmesh2mesh.femmesh_2_mesh("
                "CivilCvCAD.ActiveDocument.{}.FemMesh)".format(self.selobj.Name)
            )
            CivilCvCADGui.addModule("Mesh")
            CivilCvCADGui.doCommand("Mesh.show(Mesh.Mesh(out_mesh))")
            CivilCvCADGui.doCommand(
                "CivilCvCAD.ActiveDocument." + self.selobj.Name + ".ViewObject.hide()"
            )
        if self.selobj and self.selobj2:
            ...
            CivilCvCADGui.doCommand(
                "out_mesh = femmesh.femmesh2mesh.femmesh_2_mesh("
                "CivilCvCAD.ActiveDocument.{}.FemMesh, CivilCvCAD.ActiveDocument.{})".format(
                    femmesh.Name, res.Name
                )
            )
            CivilCvCADGui.addModule("Mesh")
            CivilCvCADGui.doCommand("Mesh.show(Mesh.Mesh(out_mesh))")
            CivilCvCADGui.doCommand("CivilCvCAD.ActiveDocument." + femmesh.Name + ".ViewObject.hide()")
        CivilCvCAD.ActiveDocument.commitTransaction()
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCAD.ActiveDocument.recompute()
...
class _MeshClear(CommandManager):
    ...
    def Activated(self):
        CivilCvCAD.ActiveDocument.openTransaction("Clear FEM mesh")
        CivilCvCADGui.addModule("Fem")
        CivilCvCADGui.doCommand(
            "CivilCvCAD.ActiveDocument." + self.selobj.Name + ".FemMesh = Fem.FemMesh()"
        )
        CivilCvCAD.ActiveDocument.commitTransaction()
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCAD.ActiveDocument.recompute()
...
class _MeshClearGroups(CommandManager):
    ...
    def Activated(self):
        CivilCvCAD.ActiveDocument.openTransaction("ClearGroups FEM mesh")
        CivilCvCADGui.addModule("Fem")
        grps = "CivilCvCAD.ActiveDocument." + self.selobj.Name + ".FemMesh.Groups"
        remove_func = "CivilCvCAD.ActiveDocument." + self.selobj.Name + ".FemMesh.removeGroup"
        CivilCvCADGui.doCommand(f"tuple(map({remove_func}, {grps}))")
...
class _MeshDisplayInfo(CommandManager):
    ...
    def Activated(self):
        CivilCvCAD.ActiveDocument.openTransaction("Display FEM mesh info")
        CivilCvCADGui.doCommand("print(CivilCvCAD.ActiveDocument." + self.selobj.Name + ".FemMesh)")
        CivilCvCADGui.addModule("PySide")
        CivilCvCADGui.doCommand(
            "mesh_info = str(CivilCvCAD.ActiveDocument." + self.selobj.Name + ".FemMesh)"
        )
        CivilCvCADGui.doCommand(
...
class _MeshGmshFromShape(CommandManager):
    ...
    def Activated(self):
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Create FEM mesh by Gmsh")
        mesh_obj_name = "FEMMeshGmsh"
        # if requested by some people add Preference for this
        # mesh_obj_name = self.selobj.Name + "_Mesh"
        CivilCvCADGui.addModule("ObjectsFem")
        CivilCvCADGui.doCommand(
            "ObjectsFem.makeMeshGmsh(CivilCvCAD.ActiveDocument, '" + mesh_obj_name + "')"
        )
        CivilCvCADGui.doCommand(
            "CivilCvCAD.ActiveDocument.ActiveObject.Shape = CivilCvCAD.ActiveDocument.{}".format(
                self.selobj.Name
            )
        )
        CivilCvCADGui.doCommand("CivilCvCAD.ActiveDocument.ActiveObject.ElementOrder = '2nd'")
        ...
        # https://forum.civilcvcad.org/viewtopic.php?t=41738
        # https://forum.civilcvcad.org/viewtopic.php?f=18&t=45260&start=20#p389494
        CivilCvCADGui.doCommand("CivilCvCAD.ActiveDocument.ActiveObject.SecondOrderLinear = False")
        ...
        if FemGui.getActiveAnalysis():
            CivilCvCADGui.addModule("FemGui")
            CivilCvCADGui.doCommand(
                "FemGui.getActiveAnalysis().addObject(CivilCvCAD.ActiveDocument.ActiveObject)"
            )
        CivilCvCADGui.doCommand(
            "CivilCvCADGui.ActiveDocument.setEdit(CivilCvCAD.ActiveDocument.ActiveObject.Name)"
        )
        CivilCvCADGui.Selection.clearSelection()
...
class _MeshNetgenFromShape(CommandManager):
    ...
    def Activated(self):
        # a mesh could be made with and without an analysis,
        # we're going to check not for an analysis in command manager module
        netgen_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Netgen")
        CivilCvCAD.ActiveDocument.openTransaction("Create FEM mesh Netgen")
        ...
        if netgen_prefs.GetBool("UseLegacyNetgen", 1):
            CivilCvCADGui.doCommand(
                "ObjectsFem.makeMeshNetgenLegacy(CivilCvCAD.ActiveDocument, '" + mesh_obj_name + "')"
            )
        else:
            CivilCvCADGui.doCommand(
                "ObjectsFem.makeMeshNetgen(CivilCvCAD.ActiveDocument, '" + mesh_obj_name + "')"
            )
            CivilCvCADGui.doCommand("CivilCvCAD.ActiveDocument.ActiveObject.EndStep = 'OptimizeVolume'")
        ...
        CivilCvCADGui.doCommand("CivilCvCAD.ActiveDocument.ActiveObject.Fineness = 'Moderate'")
...
class _GMSHRefine:
    ...
    def IsActive(self):
        if not CivilCvCADGui.ActiveDocument:
            return False
...
class _ResultsPurge(CommandManager):
    ...
    def Activated(self):
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Purge FEM results")
...
class _SolverCalculixContextManager:
    ...
    def __enter__(self):
        ccx_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Ccx")
        CivilCvCAD.ActiveDocument.openTransaction("Create SolverCalculiX")
        CivilCvCADGui.addModule("ObjectsFem")
        CivilCvCADGui.addModule("FemGui")
        CivilCvCADGui.doCommand(
            f"{self.cli_name} = ObjectsFem.{self.make_name}(CivilCvCAD.ActiveDocument)"
        )
        CivilCvCADGui.doCommand(
    ...
    def __exit__(self, exc_type, exc_value, trace):
        CivilCvCADGui.doCommand(f"FemGui.getActiveAnalysis().addObject({self.cli_name})")
        CivilCvCAD.ActiveDocument.commitTransaction()
...
class _SolverCcxTools(CommandManager):
    ...
    def Activated(self):
        with _SolverCalculixContextManager("makeSolverCalculiXCcxTools", "solver") as cm:
            CivilCvCADGui.doCommand(f"{cm.cli_name}.MaterialNonlinearity = True")
...
class _SolverControl(CommandManager):
    ...
    def Activated(self):
        CivilCvCADGui.ActiveDocument.setEdit(self.selobj, 0)
...
class _SolverElmer(CommandManager):
    ...
    def Activated(self):
        CivilCvCAD.ActiveDocument.openTransaction(f"Create Fem SolverElmer")
        CivilCvCADGui.addModule("ObjectsFem")
        CivilCvCADGui.addModule("FemGui")
        # expand parent obj in tree view if selected
        expandParentObject()
        # add the object
        CivilCvCADGui.doCommand("ObjectsFem.makeSolverElmer(CivilCvCAD.ActiveDocument)")
        # select only added object
        CivilCvCADGui.doCommand(
            "FemGui.getActiveAnalysis().addObject(CivilCvCAD.ActiveDocument.ActiveObject)"
        )
        elmer_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Elmer")
        bin_out = elmer_prefs.GetBool("BinaryOutput", False)
        save_id = elmer_prefs.GetBool("SaveGeometryIndex", False)
        CivilCvCADGui.doCommand(
            "CivilCvCAD.ActiveDocument.ActiveObject.BinaryOutput = {}".format(bin_out)
        )
        CivilCvCADGui.doCommand(
            "CivilCvCAD.ActiveDocument.ActiveObject.SaveGeometryIndex = {}".format(save_id)
        )

        CivilCvCADGui.Selection.clearSelection()
        CivilCvCADGui.doCommand(
            "CivilCvCADGui.Selection.addSelection(CivilCvCAD.ActiveDocument.ActiveObject)"
        )
...
class _SolverZ88(CommandManager):
    ...
    def Activated(self):
        CivilCvCAD.ActiveDocument.openTransaction(f"Create Fem SolverZ88")
        CivilCvCADGui.addModule("ObjectsFem")
        CivilCvCADGui.addModule("FemGui")
        # expand parent obj in tree view if selected
        expandParentObject()
        # add the object
        CivilCvCADGui.doCommand("ObjectsFem.makeSolverZ88(CivilCvCAD.ActiveDocument)")
        # select only added object
        CivilCvCADGui.doCommand(
            "FemGui.getActiveAnalysis().addObject(CivilCvCAD.ActiveDocument.ActiveObject)"
        )
        z88_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Z88")
        solver_type = z88_prefs.GetString("Solver", "sorcg")
        maxgs = z88_prefs.GetInt("MaxGS", 100000000)
        maxkoi = z88_prefs.GetInt("MaxKOI", 2800000)

        CivilCvCADGui.doCommand(f"CivilCvCAD.ActiveDocument.ActiveObject.SolverType = '{solver_type}'")
        CivilCvCADGui.doCommand(f"CivilCvCAD.ActiveDocument.ActiveObject.MatrixMaximum = {maxgs}")
        CivilCvCADGui.doCommand(f"CivilCvCAD.ActiveDocument.ActiveObject.VectorMaximum = {maxkoi}")
...
class _CompSolvers(CommandManager):
    ...
    def Activated(self, i):
        CivilCvCADGui.runCommand(self.commands[i])

    def GetCommands(self):
        return self.commands

    def GetDefaultCommand(self):
        gen_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/General")
        # DefaultSolver == 0 is "None"
        index = gen_prefs.GetInt("DefaultSolver", 0)
        return (index - 1) if index > 0 else 0


# the string in add command will be the page name on CivilCvCAD wiki
CivilCvCADGui.addCommand("FEM_Analysis", _Analysis())
CivilCvCADGui.addCommand("FEM_ClippingPlaneAdd", _ClippingPlaneAdd())
CivilCvCADGui.addCommand("FEM_ClippingPlaneRemoveAll", _ClippingPlaneRemoveAll())
CivilCvCADGui.addCommand("FEM_ConstantVacuumPermittivity", _ConstantVacuumPermittivity())
CivilCvCADGui.addCommand("FEM_ConstraintBodyHeatSource", _ConstraintBodyHeatSource())
CivilCvCADGui.addCommand("FEM_ConstraintCentrif", _ConstraintCentrif())
CivilCvCADGui.addCommand("FEM_ConstraintCurrentDensity", _ConstraintCurrentDensity())
CivilCvCADGui.addCommand("FEM_ConstraintElectricChargeDensity", _ConstraintElectricChargeDensity())
CivilCvCADGui.addCommand("FEM_ConstraintElectromagnetic", _ConstraintElectromagnetic())
CivilCvCADGui.addCommand("FEM_ConstraintFlowVelocity", _ConstraintFlowVelocity())
CivilCvCADGui.addCommand("FEM_ConstraintInitialFlowVelocity", _ConstraintInitialFlowVelocity())
CivilCvCADGui.addCommand("FEM_ConstraintInitialPressure", _ConstraintInitialPressure())
CivilCvCADGui.addCommand("FEM_ConstraintMagnetization", _ConstraintMagnetization())
CivilCvCADGui.addCommand("FEM_ConstraintSectionPrint", _ConstraintSectionPrint())
CivilCvCADGui.addCommand("FEM_ConstraintSelfWeight", _ConstraintSelfWeight())
CivilCvCADGui.addCommand("FEM_ConstraintTie", _ConstraintTie())
CivilCvCADGui.addCommand("FEM_ElementFluid1D", _ElementFluid1D())
CivilCvCADGui.addCommand("FEM_ElementGeometry1D", _ElementGeometry1D())
CivilCvCADGui.addCommand("FEM_ElementGeometry2D", _ElementGeometry2D())
CivilCvCADGui.addCommand("FEM_ElementRotation1D", _ElementRotation1D())
CivilCvCADGui.addCommand("FEM_EquationDeformation", _EquationDeformation())
CivilCvCADGui.addCommand("FEM_EquationElasticity", _EquationElasticity())
CivilCvCADGui.addCommand("FEM_EquationElectricforce", _EquationElectricforce())
CivilCvCADGui.addCommand("FEM_EquationElectrostatic", _EquationElectrostatic())
CivilCvCADGui.addCommand("FEM_EquationFlow", _EquationFlow())
CivilCvCADGui.addCommand("FEM_EquationFlux", _EquationFlux())
CivilCvCADGui.addCommand("FEM_EquationHeat", _EquationHeat())
CivilCvCADGui.addCommand("FEM_EquationMagnetodynamic", _EquationMagnetodynamic())
CivilCvCADGui.addCommand("FEM_EquationMagnetodynamic2D", _EquationMagnetodynamic2D())
CivilCvCADGui.addCommand("FEM_EquationStaticCurrent", _EquationStaticCurrent())
CivilCvCADGui.addCommand("FEM_Examples", _Examples())
CivilCvCADGui.addCommand("FEM_MaterialEditor", _MaterialEditor())
CivilCvCADGui.addCommand("FEM_MaterialFluid", _MaterialFluid())
CivilCvCADGui.addCommand("FEM_MaterialMechanicalNonlinear", _MaterialMechanicalNonlinear())
CivilCvCADGui.addCommand("FEM_MaterialReinforced", _MaterialReinforced())
CivilCvCADGui.addCommand("FEM_MaterialSolid", _MaterialSolid())
CivilCvCADGui.addCommand("FEM_FEMMesh2Mesh", _FEMMesh2Mesh())
CivilCvCADGui.addCommand("FEM_MeshBoundaryLayer", _MeshBoundaryLayer())
CivilCvCADGui.addCommand("FEM_MeshClear", _MeshClear())
CivilCvCADGui.addCommand("FEM_MeshClearGroups", _MeshClearGroups())
CivilCvCADGui.addCommand("FEM_MeshDisplayInfo", _MeshDisplayInfo())
CivilCvCADGui.addCommand("FEM_MeshGmshFromShape", _MeshGmshFromShape())
CivilCvCADGui.addCommand("FEM_MeshGroup", _MeshGroup())
CivilCvCADGui.addCommand("FEM_MeshNetgenFromShape", _MeshNetgenFromShape())
CivilCvCADGui.addCommand("FEM_MeshRegion", _MeshRegion())
CivilCvCADGui.addCommand("FEM_MeshDistance", _MeshDistance())
CivilCvCADGui.addCommand("FEM_MeshManipulate", _MeshManipulate())
CivilCvCADGui.addCommand("FEM_MeshAdvanced", _MeshAdvanced())
CivilCvCADGui.addCommand("FEM_MeshShape", _MeshShape())
CivilCvCADGui.addCommand("FEM_MeshTransfiniteCurve", _MeshTransfiniteCurve())
CivilCvCADGui.addCommand("FEM_MeshTransfiniteSurface", _MeshTransfiniteSurface())
CivilCvCADGui.addCommand("FEM_MeshTransfiniteVolume", _MeshTransfiniteVolume())
CivilCvCADGui.addCommand("FEM_MeshGMSHRefinement", _GMSHRefine())
CivilCvCADGui.addCommand("FEM_ResultShow", _ResultShow())
CivilCvCADGui.addCommand("FEM_ResultsPurge", _ResultsPurge())
CivilCvCADGui.addCommand("FEM_SolverCalculiXCcxTools", _SolverCcxTools())
CivilCvCADGui.addCommand("FEM_SolverCalculiX", _SolverCalculiX())
CivilCvCADGui.addCommand("FEM_SolverControl", _SolverControl())
CivilCvCADGui.addCommand("FEM_SolverElmer", _SolverElmer())
CivilCvCADGui.addCommand("FEM_SolverMystran", _SolverMystran())
CivilCvCADGui.addCommand("FEM_SolverRun", _SolverRun())
CivilCvCADGui.addCommand("FEM_SolverZ88", _SolverZ88())
CivilCvCADGui.addCommand("FEM_CompSolvers", _CompSolvers())

if "BUILD_FEM_VTK_PYTHON" in CivilCvCAD.__cmake__:
    CivilCvCADGui.addCommand("FEM_PostFilterGlyph", _PostFilterGlyph())
