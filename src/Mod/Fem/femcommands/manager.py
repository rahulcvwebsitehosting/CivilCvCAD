...
__title__ = "CivilCvCAD FEM command base class"
__author__ = "Przemo Firszt, Bernd Hahnebach"
__url__ = ""
## @package manager
#  \ingroup FEM
#  \brief CivilCvCAD FEM command base class
...
class CommandManager:
    ...
    def IsActive(self):
        if not self.is_active:
            ...
            active = CivilCvCADGui.ActiveDocument is not None
            ...
            active = (
                ...
                CivilCvCADGui.ActiveDocument is not None
                and self.result_selected()
            )
        elif self.is_active == "with_vtk_selresult":
            active = self.vtk_result_selected()
        elif self.is_active == "with_part_feature":
            active = CivilCvCADGui.ActiveDocument is not None and self.part_feature_selected()
        elif self.is_active == "with_femmesh":
            active = CivilCvCADGui.ActiveDocument is not None and self.femmesh_selected()
        elif self.is_active == "with_gmsh_femmesh":
            active = CivilCvCADGui.ActiveDocument is not None and self.gmsh_femmesh_selected()
        elif self.is_active == "with_femmesh_andor_res":
            active = (
                CivilCvCADGui.ActiveDocument is not None and self.with_femmesh_andor_res_selected()
            )
    ...
    def active_analysis_in_active_doc(self):
        ...
        if analysis.Document is CivilCvCAD.ActiveDocument:
    ...
    # ****************************************************************************************
    # methods to add the objects to the document in CivilCvCADGui mode

    def add_obj_on_gui_set_edit(self, objtype):
        CivilCvCAD.ActiveDocument.openTransaction(f"Create Fem{objtype}")
        ...
        CivilCvCADGui.doCommand(
            ...
            "make{}(CivilCvCAD.ActiveDocument))".format(objtype)
        )
    ...
    def add_obj_on_gui_noset_edit(self, objtype):
        ...
        # CivilCvCAD.ActiveDocument.commitTransaction()  # solver command class had this line
    ...
    def add_obj_on_gui_expand_noset_edit(self, objtype):
        ...
        CivilCvCADGui.doCommand(
            ...
            "make{}(CivilCvCAD.ActiveDocument))[0]".format(objtype)
        )
        # select only added object
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCADGui.doCommand("addedObjDocObj = CivilCvCAD.ActiveDocument.getObject(addedObj.Name)")
        CivilCvCADGui.doCommand("CivilCvCADGui.Selection.addSelection(addedObjDocObj)")
    ...
    def add_obj_on_gui_selobj_set_edit(self, objtype):
        ...
        CivilCvCADGui.doCommand(
            "ObjectsFem.make{}("
            "CivilCvCAD.ActiveDocument, CivilCvCAD.ActiveDocument.{})".format(objtype, self.selobj.Name)
        )
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCADGui.doCommand(
            "CivilCvCADGui.ActiveDocument.setEdit(CivilCvCAD.ActiveDocument.ActiveObject.Name)"
    ...
    def add_filter_set_edit(self, filtertype):
        ...
        CivilCvCAD.ActiveDocument.openTransaction(f"Create Fem{filtertype}")
        CivilCvCADGui.addModule("ObjectsFem")
        CivilCvCADGui.doCommand(
            "ObjectsFem.make{}("
            "CivilCvCAD.ActiveDocument, CivilCvCAD.ActiveDocument.{})".format(filtertype, group.Name)
        )
        # set display and selection style to assure the user sees the new object
        CivilCvCADGui.doCommand(
            'CivilCvCAD.ActiveDocument.ActiveObject.ViewObject.DisplayMode = "Surface"'
        )
        CivilCvCADGui.doCommand(
            'CivilCvCAD.ActiveDocument.ActiveObject.ViewObject.SelectionStyle = "BoundBox"'
        )
        CivilCvCADGui.doCommand(
            f"CivilCvCAD.ActiveDocument.ActiveObject.ViewObject.NoneFieldColor = {self.selobj.ViewObject.NoneFieldColor}"
        )

        # hide selected filter
        CivilCvCADGui.doCommand(
            "CivilCvCAD.ActiveDocument.{}.ViewObject.Visibility = False".format(self.selobj.Name)
        )

        # recompute, expand selobj in tree view
        expandParentObject()
        CivilCvCADGui.doCommand("CivilCvCAD.ActiveDocument.ActiveObject.recompute()")
