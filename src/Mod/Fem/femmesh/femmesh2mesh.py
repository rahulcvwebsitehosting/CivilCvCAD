...
the_file = join(CivilCvCAD.getResourceDir(), "examples", "FEMExample.FCStd")
fc_file = CivilCvCAD.openDocument(the_file)
...
# The key is the face number, used internally by CivilCvCAD.
...
def femmesh_2_mesh(myFemMesh, myResults=None, myDispScale=1):
    ...
    while actFaceIdx < allFaces:
        if actFaceIdx < (allFaces - 1):
            ...
            CivilCvCAD.Console.PrintMessage(f"Found a last Face: {faceCodeList[actFaceIdx]}\n")
            singleFaces.append(faceCodeList[actFaceIdx])
            actFaceIdx += 1

    output_mesh = []
    if myResults:
        CivilCvCAD.Console.PrintMessage(f"{myResults.Name}\n")
    ...
    CivilCvCAD.Console.PrintMessage(f"Mesh by surface search method: {end_time - start_time}\n")
...
def mesh_2_femmesh(myFemMesh, singleFaces, faceCodeDict):
    ...
    obj = CivilCvCAD.ActiveDocument.addObject("Fem::FemMeshObject", "Mesh2Fem")
    obj.FemMesh = femmesh
    end_time = time.process_time()
    CivilCvCAD.Console.PrintMessage(f"Convert to FemMesh: {end_time - start_time}\n")
    return obj
