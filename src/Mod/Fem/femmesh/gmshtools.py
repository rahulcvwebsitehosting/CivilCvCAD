...
class GmshTools(ObjectTools):
    ...
    def compute(self):
        log_level = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Gmsh").GetString(
            "LogVerbosity", "3"
        )
    ...
    def get_gmsh_command(self):
        self.gmsh_bin = CivilCvCAD.ParamGet(
    ...
    def postprocess_groups(self):
        ...
        analysis_group_meshing = CivilCvCAD.ParamGet(
    ...
    def get_region_data(self):
        ...
        if not mesh_region_list:
            ...
            # https://forum.civilcvcad.org/viewtopic.php?f=18&t=18780&start=40#p149467
            # https://forum.civilcvcad.org/viewtopic.php?f=18&t=18780&p=149520#p149520
            part = self.part_obj
            if (
                mesh_region_list
    ...
    def get_boundary_layer_data(self):
        ...
        if not mesh_boundary_list:
            ...
            if self.part_obj.Shape.ShapeType == "Compound":
                # see https://forum.civilcvcad.org/viewtopic.php?f=18&t=18780&start=40#p149467 and
                # https://forum.civilcvcad.org/viewtopic.php?f=18&t=18780&p=149520#p149520
                self.outputCompoundWarning()
    ...
    def write_groups(self, geo):
        ...
        # we use the element index of CivilCvCAD which starts with 1 (example: "Face1"),
        ...
        if len(self.bl_setting_list):
            ...
            for item in self.bl_setting_list:
                ...
                for k in item:
                    v = item[k]
                    if k == "CurvesList":
                        # the element name of CivilCvCAD which starts
    ...
    def write_part_file(self):
        ...
        # get partner shape
        geom_trans = geom.transformed(CivilCvCAD.Placement().Matrix)
    ...
    def write_geo(self):
        ...
        geo.write("// geo file for meshing with Gmsh meshing software created by CivilCvCAD\n")
        geo.write("\n")
        ...
        if self.ParallelProcessing and self.algorithm2D != "7":
            cpu_count = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Gmsh").GetInt(
        ...
        # see https://forum.civilcvcad.org/viewtopic.php?f=18&t=21571&start=20#p179443
    ...
    def run_gmsh_with_geo(self):
        ...
        # https://forum.civilcvcad.org/viewtopic.php?f=18&t=33946
        useless_warning = (
...
class GmshPreviewTools(GmshTools):
    ...
    def write_preview_geo(self):
        ...
        factor = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Gmsh").GetInt(
            "previewMeshFactor", 5
        )
