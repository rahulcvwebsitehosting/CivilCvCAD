...
def get_femnodes_by_femobj_with_references(femmesh, femobj):
    ...
    if femmesh.GroupCount:
        ...
        # CivilCvCAD.Console.PrintMessage("node_set_group: {}\n".format(node_set))
        if node_set:
            CivilCvCAD.Console.PrintLog(
    ...
    if not node_set:
        ...
        # CivilCvCAD.Console.PrintMessage("node_set_nogroup: {}\n".format(node_set))
...
def get_femnodes_by_refshape(femmesh, ref):
    ...
    for refelement in ref[1]:
        ...
        if r.ShapeType == "Vertex":
            ...
            CivilCvCAD.Console.PrintMessage("  No Vertice, Edge, Face or Solid as reference shapes!\n")
    return nodes
...
def get_femelement_table(femmesh):
    ...
    if is_solid_femmesh(femmesh):
        ...
        CivilCvCAD.Console.PrintError("Neither solid nor face nor edge femmesh!\n")
...
def get_bit_pattern_dict(femelement_table, femnodes_ele_table, node_set):
    ...
    https://forum.civilcvcad.org/viewtopic.php?f=18&t=17318&start=50#p141108
...
def get_element_faces_from_binary_search(
    ...
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=17318#p137171
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=17318&start=60#p141484
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=17318&start=50#p141108
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=17318&start=40#p140371
    vol_dict = {
        4: mask_tetra4,
        6: mask_penta6,
        8: mask_hexa8,
...
def get_femelements_by_femnodes_bin(femelement_table, femnodes_ele_table, node_list):
    ...
    """
    CivilCvCAD.Console.PrintMessage("binary search: get_femelements_by_femnodes_bin\n")
...
def get_femelements_by_femnodes_std(femelement_table, node_list):
    ...
    e: elementlist
    nodes: nodelist"""
    CivilCvCAD.Console.PrintMessage("std search: get_femelements_by_femnodes_std\n")
    e = []  # elementlist
...
def get_femelement_sets(femmesh, femelement_table, fem_objects, femnodes_ele_table=None):
    # fem_objects = CivilCvCAD FEM document objects
...
def get_femelement_direction1D_set(femmesh, femelement_table, beamrotation_objects, theshape=None):
    ...
    if len(beamrotation_objects) == 0:
        ...
        CivilCvCAD.Console.PrintError("Multiple Rotations not yet supported!\n")
...
def get_femelement_directions_theshape(femmesh, femelement_table, theshape):
    ...
    for e in theshape.Shape.Edges:
        ...
        for rot in rotations_ids:
            # tolerance will be managed by CivilCvCAD
            # see https://forum.civilcvcad.org/viewtopic.php?f=22&t=14179
...
# ************************************************************************************************
def get_beam_main_axis_m(beam_direction: CivilCvCAD.Vector, defined_angle: int) -> CivilCvCAD.Vector:

    # former name was get_beam_normal
    # see forum topic https://forum.civilcvcad.org/viewtopic.php?f=18&t=24878
    # beam_direction ... CivilCvCAD vector
    ...
    #   https://forum.civilcvcad.org/viewtopic.php?f=18&t=24878&start=30#p195567
    #   https://forum.civilcvcad.org/viewtopic.php?f=13&t=59239&start=140#p521999
    # thread with updated analysis, that fixes a found bug:
    #   https://forum.civilcvcad.org/viewtopic.php?t=85951
    #
    ...
    def normalize(vec: CivilCvCAD.Vector) -> CivilCvCAD.Vector:
        return vec / vec.Length

    def get_normal(vec: CivilCvCAD.Vector) -> CivilCvCAD.Vector:
        x, y, z = vec
        ...
        # z < 0 as well
        n = normalize((1 | -(z > 0)) * CivilCvCAD.Vector(n))
    ...
    def rotate_around_vector(axis: CivilCvCAD.Vector, angle: int) -> CivilCvCAD.Vector:
        """
        * - is the axis vector, which is looking at us
        ...
        return CivilCvCAD.Vector([coord * (abs(coord) > 1e-15) for coord in rot])
...
def get_force_obj_vertex_nodeload_table(femmesh, frc_obj):
    ...
    #     ]
    force_obj_node_load_table = []
    force_quantity = CivilCvCAD.Units.Quantity(frc_obj.Force.getValueAs("N"))
...
def get_force_obj_edge_nodeload_table(femmesh, femelement_table, femnodes_mesh, frc_obj):
    ...
    for o, elem_tup in frc_obj.References:
        for elem in elem_tup:
            ...
            if ratio_refedge_lengths < 0.99 or ratio_refedge_lengths > 1.01:
                ...
                CivilCvCAD.Console.PrintMessage(f"  sum_node_lengths: {sum_node_lengths}\n")
                CivilCvCAD.Console.PrintMessage(f"  refedge_length: {ref_edge.Length}\n")
    ...
    if ratio < 0.99 or ratio > 1.01:
        ...
        CivilCvCAD.Console.PrintMessage(f"  sum_ref_edge_node_length: {sum_ref_edge_node_length}\n")
        CivilCvCAD.Console.PrintMessage(f"  sum_ref_edge_length:      {sum_ref_edge_length}\n")
        CivilCvCAD.Console.PrintMessage(f"  sum_node_load:            {sum_node_load}\n")
        CivilCvCAD.Console.PrintMessage(f"  frc_obj.Force:            {force_quantity}\n")
        CivilCvCAD.Console.PrintMessage(
        ...
        CivilCvCAD.Console.PrintMessage("DEBUGGING\n")
        CivilCvCAD.Console.PrintMessage(f"{bad_refedge}\n")

        CivilCvCAD.Console.PrintMessage("bad_refedge_nodes\n")
        bad_refedge_nodes = femmesh.getNodesByEdge(bad_refedge)
        CivilCvCAD.Console.PrintMessage(f"{len(bad_refedge_nodes)}\n")
        CivilCvCAD.Console.PrintMessage(f"{bad_refedge_nodes}\n")
        # import CivilCvCADGui
        # frc_obj.Document.Compound_Mesh.HighlightedNodes = bad_refedge_nodes

        CivilCvCAD.Console.PrintMessage("bad_edge_table\n")
        # bad_edge_table:
        #     { meshedgeID : ( nodeID, ... , nodeID ) }
        bad_edge_table = get_ref_edgenodes_table(femmesh, femelement_table, bad_refedge)
        CivilCvCAD.Console.PrintMessage(f"{len(bad_edge_table)}\n")
        bad_edge_table_nodes = []
        for elem in bad_edge_table:
            CivilCvCAD.Console.PrintMessage(elem, f" --> {bad_edge_table[elem]}\n")
            for node in bad_edge_table[elem]:
                if node not in bad_edge_table_nodes:
                    bad_edge_table_nodes.append(node)
        CivilCvCAD.Console.PrintMessage("sorted(bad_edge_table_nodes)\n")
        # should be == bad_refedge_nodes
        CivilCvCAD.Console.PrintMessage(f"{sorted(bad_edge_table_nodes)}\n")
        # import CivilCvCADGui
...
def get_force_obj_face_nodeload_table(femmesh, femelement_table, femnodes_mesh, frc_obj):
    ...
    for o, elem_tup in frc_obj.References:
        for elem in elem_tup:
            ...
            if ratio_refface_areas < 0.99 or ratio_refface_areas > 1.01:
                ...
                CivilCvCAD.Console.PrintMessage(f"  sum_node_areas: {sum_node_areas}\n")
                CivilCvCAD.Console.PrintMessage(f"  ref_face_area:  {ref_face.Area}\n")
    ...
    if ratio < 0.99 or ratio > 1.01:
        ...
        CivilCvCAD.Console.PrintMessage(f"  sum_ref_face_node_area: {sum_ref_face_node_area}\n")
        CivilCvCAD.Console.PrintMessage(f"  sum_ref_face_area:      {sum_ref_face_area}\n")
        CivilCvCAD.Console.PrintMessage(f"  sum_node_load:          {sum_node_load}\n")
        CivilCvCAD.Console.PrintMessage(f"  frc_obj.Force:          {force_quantity}\n")
        CivilCvCAD.Console.PrintMessage(
    ...
    if is_solid_femmesh(femmesh):
        if has_no_face_data(femmesh):
            CivilCvCAD.Console.PrintMessage(
                ...
                "CivilCvCAD uses getccxVolumesByFace() "
            ...
            if ref_face_volume_elements:  # mesh with tetras
                ...
                CivilCvCAD.Console.PrintLog(
                    ...
                    "CivilCvCAD tries to use get_femvolumeelements_by_femfacenodes().\n"
                )
                # list of integer [mv]
...
def get_ref_facenodes_areas(femnodes_mesh, face_table):
    ...
    # see https://forum.civilcvcad.org/viewtopic.php?f=18&t=10939&start=40#p91355  and ff
    ...
    if (not femnodes_mesh) or (not face_table):
        CivilCvCAD.Console.PrintError("Error: Empty femnodes_mesh or face_table!\n")
        return []
    node_area_table = []
    mesh_face_area = 0
...
def build_mesh_faces_of_volume_elements(face_table, femelement_table):
    ...
    for veID in face_table:
        ...
        CivilCvCAD.Console.PrintLog(f"VolElement: {veID}\n")
        CivilCvCAD.Console.PrintLog(f"  --> {femelement_table[veID]}\n")
        CivilCvCAD.Console.PrintLog(f"  --> {face_table[veID]}\n")
        CivilCvCAD.Console.PrintLog(f"  --> {face_nodenumber_table[veID]}\n")
    ...
    for veID in face_nodenumber_table:
        ...
        node_numbers = ()
        if vol_node_ct == 10:
            CivilCvCAD.Console.PrintLog("  --> tetra10 --> tria6 face\n")
            ...
            CivilCvCAD.Console.PrintLog("  --> tetra4 --> tria3 face\n")
            ...
            CivilCvCAD.Console.PrintLog("  --> hexa20 --> quad8 face\n")
            ...
            CivilCvCAD.Console.PrintLog("  --> hexa8 --> quad4 face\n")
            ...
            CivilCvCAD.Console.PrintLog("  --> penta15 --> tria6 and quad8 faces\n")
            ...
            CivilCvCAD.Console.PrintLog("  --> penta6 --> tria3 and quad4 faces\n")
...
def get_analysis_group_elements(aAnalysis, aPart):
    ...
    for m in aAnalysis.Group:
        if hasattr(m, "References"):
            if len(m.References) > 0:
                ...
                CivilCvCAD.Console.PrintMessage(f"  Empty reference: {m.Name}\n")
    ...
    if empty_references:
        if len(empty_references) == 1:
            ...
            CivilCvCAD.Console.PrintError("Problem: more than one object with empty references.\n")
            CivilCvCAD.Console.PrintMessage(
                "We are going to try to get the empty material references anyway.\n"
            )
            for er in empty_references:
                CivilCvCAD.Console.PrintMessage(f"{er.Name}\n")
            group_elements = get_anlysis_empty_references_group_elements(
                group_elements, aAnalysis, aPart.Shape
            )
...
def get_reference_group_elements(obj, aPart):
    ...
    see https://forum.civilcvcad.org/viewtopic.php?f=18&t=12212&p=175777#p175777
    and following posts
    ...
    for r in obj.References:
        ...
        for child1 in childs:
            ...
            for ref_shape in subchilds:
                CivilCvCAD.Console.PrintLog(f"{ref_shape}\n")
                ...
                if found_element is not None:
                    ...
                    CivilCvCAD.Console.PrintMessage("    " + obj.Name + "\n")
                    CivilCvCAD.Console.PrintMessage("    " + str(obj.References) + "\n")
                    CivilCvCAD.Console.PrintMessage("    " + r[0].Name + "\n")
                    if parent.Name != aPart.Name:
                        ...
                        CivilCvCAD.Console.PrintError("This should not happen, please debug!\n")
...
def get_anlysis_empty_references_group_elements(group_elements, aAnalysis, aShape):
    ...
    # CivilCvCAD.Console.PrintMessage(group_elements)
...
def femelements_count_ok(len_femelement_table, count_femelements):
    ...
    if count_femelements == len_femelement_table:
        ...
        CivilCvCAD.Console.PrintMessage("ERROR: femelement_table != count_femelements\n")
        return False
...
def get_femmesh_eletype(femmesh, femelement_table=None):
    if not femmesh:
        CivilCvCAD.Console.PrintError("Error: No femmesh.\n")
    if not femelement_table:
        CivilCvCAD.Console.PrintWarning("The femelement_table needs to be calculated.\n")
    ...
    for elem in sorted(femelement_table):
        ...
        CivilCvCAD.Console.PrintLog(f"Node count of first element: {elem_length}\n")
    ...
    if is_solid_femmesh(femmesh):
        if femmesh.TetraCount == femmesh.VolumeCount:
            if elem_length == 4:
                ...
                CivilCvCAD.Console.PrintMessage("Tetra with neither 4 nor 10 nodes.\n")
                return "None"
            ...
            if elem_length == 8:
                ...
                CivilCvCAD.Console.PrintError("Hexa with neither 8 nor 20 nodes.\n")
                return "None"
        else:
            CivilCvCAD.Console.PrintError("no tetra, no hexa or Mixed Volume Elements.\n")
    elif is_face_femmesh(femmesh):
        if femmesh.TriangleCount == femmesh.FaceCount:
            if elem_length == 3:
                ...
                CivilCvCAD.Console.PrintError("Tria with neither 3 nor 6 nodes.\n")
                return "None"
            ...
            if elem_length == 4:
                ...
                CivilCvCAD.Console.PrintError("Quad with neither 4 nor 8 nodes.\n")
                return "None"
        else:
            CivilCvCAD.Console.PrintError("no tria, no quad\n")
            return "None"
    elif is_edge_femmesh(femmesh):
        if elem_length == 2:
            return "seg2"
            ...
            CivilCvCAD.Console.PrintError("Seg with neither 2 nor 3 nodes.\n")
            return "None"
    else:
        CivilCvCAD.Console.PrintError("Neither edge nor face nor solid femmesh.\n")
        return "None"
    return "None"
...
def get_three_non_colinear_nodes(nodes_coords):
    ...
    if not nodes_coords:
        CivilCvCAD.Console.PrintMessage(f"{len(nodes_coords)}\n")
        CivilCvCAD.Console.PrintMessage("Error: No nodes in nodes_coords\n")
        return []
    ...
    CivilCvCAD.Console.PrintMessage(f"{[node_1, node_2, node_3]}\n")
...
def use_correct_fluidinout_ele_def(FluidInletoutlet_ele, fileName, fluid_inout_nodes_file):
    ...
    cnt = 0
    elem_counter = 0
    CivilCvCAD.Console.PrintMessage(f"1DFlow inout nodes file: {fluid_inout_nodes_file}\n")
...
def compact_mesh(old_femmesh):
    ...
    # https://forum.civilcvcad.org/viewtopic.php?t=48215
    ele_id = 1
...
def sub_shape_at_global_placement(obj, sub_name):
    ...
    # get partner shape
    partner = sub_sh.transformed(CivilCvCAD.Placement().Matrix)
