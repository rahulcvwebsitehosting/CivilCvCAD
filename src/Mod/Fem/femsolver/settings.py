...
CivilCvCAD related to the FEM module. The collection of independent functions use
...
__title__ = "CivilCvCAD FEM solver settings"
...
class DirSetting:
    ...
    Strings used to indicate the solver directory setting set in CivilCvCADs
    setting system. Returned by :func:`get_dir_setting` for that purpose. There
    are three different possible values:

    :cvar TEMPORARY:
        Let CivilCvCAD manage (create, delete) the working directories for all
...
def get_binary(name, silent=False):
    ...
    Return the specific path set by the user in CivilCvCADs settings/parameter
...
def get_custom_dir():
    ...
    param_group = CivilCvCAD.ParamGet(_GENERAL_PARAM)
    custom_dir = param_group.GetString("CustomDirectoryPath")
    # if no custom directory, use home directory
    return custom_dir if custom_dir else CivilCvCAD.ConfigGet("UserHomePath")
...
def get_dir_setting():
    ...
    depending on the setting set in CivilCvCAD parameter system. Result dependes
...
class _SolverDlg:
    ...
    the CivilCvCAD parameter path is different for each one. A instance of this
    ...
    :ivar param_path:
        Parent param path (CivilCvCADs settings/parameter system) that contains
    ...
    def __init__(self, default, param_path, custom_path):
        ...
        self.param_group = CivilCvCAD.ParamGet(self.param_path)

    def get_binary(self, silent=False):

        # set the binary path to the CivilCvCAD defaults
        # ATM pure unix shell commands without path names are used as standard
        # TODO the binaries provided with the CivilCvCAD distribution should be found
        ...
        if not binary:
            binary = self.default
        CivilCvCAD.Console.PrintLog(f"Solver binary path default: {binary} \n")
        ...
        if (the_found_binary is None) and (not silent):
            ...
            CivilCvCAD.Console.PrintLog(f"Found solver binary path: {the_found_binary}\n")
