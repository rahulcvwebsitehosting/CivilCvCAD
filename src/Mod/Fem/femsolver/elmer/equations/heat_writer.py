...
__title__ = "CivilCvCAD FEM Heat Elmer writer"
        ...
        if obj.Mode == "Dissipation Rate":
            ...
            for mat in self.write.getMember("App::MaterialObject"):
                ...
                if (ref_feat, ref_sub_obj) in mat_ref:
                    density = CivilCvCAD.Units.Quantity(mat.Material["Density"])
                    break
            ...
            heatSource = (
                (obj.TotalPower / (density * CivilCvCAD.Units.Quantity(volume, "mm^3")))
                .getValueAs("W/kg")
                .Value
            )
