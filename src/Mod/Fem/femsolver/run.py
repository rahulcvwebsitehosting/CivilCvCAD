...
__title__ = "CivilCvCAD FEM solver run"
        ...
        try:
            ...
            if App.GuiUp:
                QtGui.QMessageBox.critical(
                    CivilCvCADGui.getMainWindow(), "Can't start Solver", error_message
                )
            return
...
def _getBesideBase(solver):
    ...
    # doc=App.newDocument()
    # doc.FileName
    # the above returns an empty string in CivilCvCAD 0.19
    # https://forum.civilcvcad.org/viewtopic.php?f=10&t=48842
    if path == "":
        error_message = (
            "Please save the file before executing the solver. "
            "This must be done because the location of the working "
