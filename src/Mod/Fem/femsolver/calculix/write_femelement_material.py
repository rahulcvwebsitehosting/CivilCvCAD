...
__title__ = "CivilCvCAD FEM calculix write inpfile materials"
...
def write_femelement_material(f, ccxwriter):
    ...
    for femobj in ccxwriter.member.mats_linear:
        ...
        if mat_obj.Category == "Solid":
            YM = CivilCvCAD.Units.Quantity(mat_obj.Material["YoungsModulus"])
            YM_in_MPa = YM.getValueAs("MPa").Value
            PR = float(mat_obj.Material["PoissonRatio"])
        if is_density_needed() is True:
            density = CivilCvCAD.Units.Quantity(mat_obj.Material["Density"])
            density_in_tonne_per_mm3 = density.getValueAs("t/mm^3").Value
        if ccxwriter.analysis_type == "thermomech":
            TC = CivilCvCAD.Units.Quantity(mat_obj.Material["ThermalConductivity"])
            ...
            SH = CivilCvCAD.Units.Quantity(mat_obj.Material["SpecificHeat"])
        ...
        if (
            ...
            rel_perm = CivilCvCAD.Units.Quantity(mat_obj.Material["RelativePermittivity"]).Value
            vacuum_perm = CivilCvCAD.Units.Quantity(constants.vacuum_permittivity())
            abs_perm = vacuum_perm.getValueAs("C/(mV*mm)").Value * rel_perm
        # write material properties
        f.write(f"** CivilCvCAD material name: {mat_info_name}\n")
        f.write(f"** {mat_label}\n")
