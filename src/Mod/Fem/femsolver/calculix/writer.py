...
__title__ = "CivilCvCAD FEM solver CalculiX writer"
...
# Interesting forum topic: https://forum.civilcvcad.org/viewtopic.php?&t=48451
...
**  The user is the CivilCvCAD calculix writer module ;-)
**
...
# https://forum.civilcvcad.org/viewtopic.php?f=18&p=516518#p516433
# https://forum.civilcvcad.org/viewtopic.php?f=18&t=22759&#p176578
# example "{:.13G}".format(math.sqrt(2.)*-1e100) and count chars
# a property type is best checked in CivilCvCAD objects definition
...
class FemInputWriterCcx(writerbase.FemInputWriter):
    ...
    def write_solver_input(self):
        ...
        CivilCvCAD.Console.PrintMessage("CalculiX solver input writing...\n")
        CivilCvCAD.Console.PrintMessage(f"Input file:{self.file_name}\n")

        if self.solver_obj.SplitInputWriter is True:
            CivilCvCAD.Console.PrintMessage("Split input file.\n")
            self.split_inpfile = True
        else:
            CivilCvCAD.Console.PrintMessage("One monster input file.\n")
        ...
        CivilCvCAD.Console.PrintMessage(f"Writing time CalculiX input file: {writetime} seconds.\n")

        # return
        if self.femelement_count_test is True:
            return self.file_name
        else:
            CivilCvCAD.Console.PrintError("Problems on writing input file, check report prints.\n\n")
            return ""
