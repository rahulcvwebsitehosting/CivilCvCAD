...
__title__ = "CivilCvCAD FEM calculix constraint temperature"
...
def write_constraint(f, femobj, temp_obj, ccxwriter):
    ...
    if temp_obj.ConstraintType == "Temperature":
        ...
        f.write(
            "{},11,11,{}\n".format(
                temp_obj.Name, CivilCvCAD.Units.Quantity(temp_obj.Temperature.getValueAs("K"))
            )
        )
        f.write("\n")
        ...
        f.write(
            "{},11,{}\n".format(
                temp_obj.Name,
                CivilCvCAD.Units.Quantity(temp_obj.ConcentratedHeatFlux.getValueAs("mW"))
                / NumberOfNodes,
            )
        )
        f.write("\n")
