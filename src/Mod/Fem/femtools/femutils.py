...
def createObject(doc, name, proxy, viewProxy=None):
    ...
    proxy(obj)
    if CivilCvCAD.GuiUp and viewProxy is not None:
...
def is_derived_from(obj, t):
    ...
    :note:
     ...
     https://forum.civilcvcad.org/viewtopic.php?f=10&t=32625
    """
...
def get_beside_base(obj):
    ...
    if fcstdPath == "":
        ...
        CivilCvCAD.Console.PrintError(f"{error_message}\n")
...
def getBoundBoxOfAllDocumentShapes(doc):
    ...
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=52920
    for o in doc.Objects:
        ...
        try:
            ...
            CivilCvCAD.Console.PrintMessage(f"{bb}\n")  # debug only
        except Exception:
            CivilCvCAD.Console.PrintMessage("exception \n")  # debug only

        if bb is None:
            try:
                CivilCvCAD.Console.PrintMessage(f"trying: {o.Label}: FemMesh\n")  # debug only
        ...
        if bb:
            ...
            CivilCvCAD.Console.PrintMessage("no bb\n")  # debug only

    CivilCvCAD.Console.PrintMessage("overallBB:" + str(overallboundbox) + "\n")  # debug only
...
def getSelectedFace(selectionex):
    ...
    if len(selectionex) != 1:
        CivilCvCAD.Console.PrintMessage("No or more than one object selected.\n")
    else:
        sel = selectionex[0]
        if len(sel.SubObjects) != 1:
            CivilCvCAD.Console.PrintMessage("More than one element selected.\n")
        else:
            aFace = sel.SubObjects[0]
            if aFace.ShapeType != "Face":
                CivilCvCAD.Console.PrintMessage("Not a Face selected.\n")
            else:
                CivilCvCAD.Console.PrintMessage(":-)\n")
                return aFace
    return aFace
...
def expandParentObject():
    ...
    trees = CivilCvCADGui.getMainWindow().findChildren(QtGui.QTreeWidget)
    for tree in trees:
...
def getOutputWinColor(type):
    ...
    """
    col_int = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/OutputWindow").GetUnsigned(
        "color" + type
    )
