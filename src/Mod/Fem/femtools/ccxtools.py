...
class FemToolsCcx(QtCore.QRunnable, QtCore.QObject):
    ...
    def find_analysis(self):
        ...
        for m in CivilCvCAD.activeDocument().Objects:
    ...
    def check_prerequisites(self):
        ...
        CivilCvCAD.Console.PrintMessage("Check prerequisites...\n")
        message = ""
        # analysis
        if not self.analysis:
    ...
    def setup_working_dir(self, param_working_dir=None, create=False):
        ...
        fem_general_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/General")
        if param_working_dir is not None:
            ...
            if femutils.check_working_dir(self.working_dir) is not True:
                ...
                self.working_dir = femutils.get_pref_working_dir(self.solver)
                CivilCvCAD.Console.PrintMessage(f"Dir '{self.working_dir}' will be used instead.\n")
        else:
            self.working_dir = femutils.get_pref_working_dir(self.solver)
    ...
    def setup_ccx(self, ccx_binary=None, ccx_binary_sig="CalculiX"):
        ...
        if self.ccx_binary is None:
            self.ccx_binary = CivilCvCAD.ParamGet(
        ...
        try:
            ...
            CivilCvCAD.Console.PrintError(f"{e}\n")
            if e.errno == 2:
            ...
            error_message = self.tr(
                ...
                "Check if ccx runs standalone without CivilCvCAD."
    ...
    def start_ccx(self):
        ...
        self.ccx_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/Ccx")
    ...
    def ccx_run(self):
        ret_code = None
        CivilCvCAD.Console.PrintMessage("\n")  # because of time print in separate line
        CivilCvCAD.Console.PrintMessage("CalculiX solver run...\n")
        if self.test_mode:
            CivilCvCAD.Console.PrintError("CalculiX can not be run if test_mode is True.\n")
            return
        self.setup_ccx()
        if self.ccx_binary_present is False:
            ...
            return
        progress_bar = CivilCvCAD.Base.ProgressIndicator()
        ...
        if ret_code or self.ccx_stderr:
            if ret_code == 201 and self.solver.AnalysisType == "check":
                ...
                # https://forum.civilcvcad.org/viewtopic.php?f=18&t=31303&start=10#p260743
                ret_code = 0
            else:
                CivilCvCAD.Console.PrintError(f"CalculiX failed with exit code {ret_code}\n")
                CivilCvCAD.Console.PrintMessage("--------start of stderr-------\n")
                CivilCvCAD.Console.PrintMessage(self.ccx_stderr)
                CivilCvCAD.Console.PrintMessage("--------end of stderr---------\n")
                CivilCvCAD.Console.PrintMessage("--------start of stdout-------\n")
                CivilCvCAD.Console.PrintMessage(self.ccx_stdout)
                CivilCvCAD.Console.PrintMessage("\n--------end of stdout---------\n")
                CivilCvCAD.Console.PrintMessage("--------start problems---------\n")
            ...
            CivilCvCAD.Console.PrintMessage("CalculiX finished without error.\n")
        return ret_code
    ...
    def run(self):
        ...
        if message:
            ...
            error_gui = f"{text}\n{message}"
            CivilCvCAD.Console.PrintError(error_app)
            if CivilCvCAD.GuiUp:
            ...
            if self.inp_file_name == "":
                ...
                CivilCvCAD.Console.PrintLog("Writing CalculiX input file completed.\n")
                ...
                if ret_code != 0:
                    ...
                    CivilCvCAD.Console.PrintLog("Try to read result files\n")
                    self.load_results()
    ...
    def has_nonpositive_jacobians(self):
        if "*ERROR in e_c3d: nonpositive jacobian" in self.ccx_stdout:
            ...
            CivilCvCAD.Console.PrintMessage(command_for_nonposjacnodes + "\n")
            if CivilCvCAD.GuiUp:
                import CivilCvCADGui

                # with this the list nonpositive_jacobian_elenodes
                # will be available for further user interaction
                CivilCvCADGui.doCommand(command_for_nonposjacnodes)
                CivilCvCAD.Console.PrintMessage("\n")
                CivilCvCADGui.doCommand(command_to_highlight)
            CivilCvCAD.Console.PrintMessage(
                "\nFollowing some commands to copy. "
    ...
    def load_results(self):
        CivilCvCAD.Console.PrintMessage("\n")  # because of time print in separate line
        CivilCvCAD.Console.PrintMessage("CalculiX read results...\n")
        self.results_present = False
        ...
        if os.path.isfile(frd_result_file):
            ...
            for m in self.analysis.Group:
                ...
                if self.solver.AnalysisType == "check":
                    ...
                    CivilCvCAD.Console.PrintError("FEM: No result object in active Analysis.\n")
        else:
            CivilCvCAD.Console.PrintError(f"FEM: No frd result file found at {frd_result_file}\n")
    ...
    def load_results_ccxdat(self):
        ...
        if os.path.isfile(dat_result_file):
            ...
            CivilCvCAD.Console.PrintError(f"FEM: No dat result file found at {dat_result_file}\n")
