...
def gui_available():
    ...
    try:
        return CivilCvCADGui.getMainWindow() is not None
...
class SketcherGuiTestCase(unittest.TestCase):
    ...
    def cleanup_gui_document(self, doc, timeout_ms=80):
        ...
        gui_doc = CivilCvCADGui.ActiveDocument
        if gui_doc is not None:
            gui_doc.resetEdit()
            self.flush_gui(timeout_ms)

        if CivilCvCADGui.Control.activeDialog() is not None:
            CivilCvCADGui.Control.closeDialog()
            self.flush_gui(timeout_ms)

        if doc is not None and doc.Name in CivilCvCAD.listDocuments():
