...
class SketcherGuiTestCases(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        CivilCvCADGui.getMainWindow().show()
        cls.pump_gui_events()
    ...
    def build_issue_25840_sketch(sketch):
        ...
        first_line = sketch.addGeometry(
            Part.LineSegment(
                CivilCvCAD.Vector(-17.60407066, 31.05172348, 0.0),
                CivilCvCAD.Vector(44.00962448, -33.86270142, 0.0),
            ),
            False,
        )
        second_line = sketch.addGeometry(
            Part.LineSegment(
                CivilCvCAD.Vector(50.4888, 6.2351, 0.0),
                CivilCvCAD.Vector(30.973626440922192, -20.12834717, 0.0),
            ),
            False,
        )
        ...
        return constraint_id, CivilCvCAD.Vector(30.973626440922192, -20.12834717, 0.0)
    ...
    def setUp(self):
        self.doc = CivilCvCAD.newDocument("SketchGuiTest")
        self.sketch = self.doc.addObject("Sketcher::SketchObject", "Sketch")
        self.doc.recompute()

        CivilCvCADGui.getMainWindow().show()
        self.pump_gui_events()
        CivilCvCADGui.ActiveDocument.setEdit(self.sketch.Name)
        self.pump_gui_events(6)

        self.view = CivilCvCADGui.ActiveDocument.ActiveView

    def tearDown(self):
        CivilCvCADGui.Selection.clearPreselection()
    ...
    def testPointOnObjectPreselectionMatchesTiltedHitArea(self):
        ...
        tilt_y = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 1, 0), 2.0)
    ...
    def testPointMarkerWinsOverOverlappingConstraintLabel(self):
        start_point = CivilCvCAD.Vector(80.0, 100.0, 0.0)
        end_point = CivilCvCAD.Vector(120.0, 140.0, 0.0)
        marker_point = CivilCvCAD.Vector(92.0, 88.0, 0.0)
    ...
    def testCurveWinsOverOverlappingDistanceDimensionLine(self):
        start_point = CivilCvCAD.Vector(80.0, 100.0, 0.0)
        end_point = CivilCvCAD.Vector(130.0, 100.0, 0.0)
        midpoint = CivilCvCAD.Vector(105.0, 100.0, 0.0)
