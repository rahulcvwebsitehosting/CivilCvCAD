...
try:
    ...
    GUI_AVAILABLE = CivilCvCADGui.getMainWindow() is not None
...
class TestExternalFacePreselection(SketcherGuiTestCase):
    ...
    def setUp(self):
        ...
        CivilCvCADGui.activateWorkbench("PartDesignWorkbench")
        self.doc = CivilCvCAD.newDocument("TestExtFacePresel")
        ...
        # 40x40 rectangle
        sketch.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(-20, -20, 0), CivilCvCAD.Vector(20, -20, 0))
        )
        sketch.addGeometry(Part.LineSegment(CivilCvCAD.Vector(20, -20, 0), CivilCvCAD.Vector(20, 20, 0)))
        sketch.addGeometry(Part.LineSegment(CivilCvCAD.Vector(20, 20, 0), CivilCvCAD.Vector(-20, 20, 0)))
        sketch.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(-20, 20, 0), CivilCvCAD.Vector(-20, -20, 0))
        )
    ...
    def hover_for_preselection(self, viewport, center_pos, span=6, step=2):
        for dy in range(-span, span + 1, step):
            for dx in range(-span, span + 1, step):
                ...
                presel = CivilCvCADGui.Selection.getPreselection()
                if presel.ObjectName:
                    return presel

        return CivilCvCADGui.Selection.getPreselection()
    ...
    def testNoDepthBufferInEditRoot(self):
        ...
        CivilCvCADGui.ActiveDocument.setEdit(self.testSketch.Name)
        self.pump(300)
    ...
    def testFacePickableUnderExternalTool(self):
        ...
        self.testSketch.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(-10, -10, 0), CivilCvCAD.Vector(10, -10, 0))
        )
        self.testSketch.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(10, -10, 0), CivilCvCAD.Vector(10, 10, 0))
        )
        ...
        face_center_3d = CivilCvCAD.Vector(0, -20, 10)
        ...
        CivilCvCADGui.runCommand("Sketcher_Projection", 0)
        self.pump(300)
