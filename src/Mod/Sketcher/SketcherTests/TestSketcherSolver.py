...
import CivilCvCAD, Part, Sketcher
from Part import Precision

App = CivilCvCAD

xy_normal = CivilCvCAD.Vector(0, 0, 1)


def vec(x, y):
    """Shorthand to create a vector in the XY-plane"""
    return CivilCvCAD.Vector(x, y, 0)
...
def CreateRectangleSketch(SketchFeature, corner, lengths):
    ...
    SketchFeature.addGeometry(
        Part.LineSegment(CivilCvCAD.Vector(hmin, vmax), CivilCvCAD.Vector(hmax, vmax, 0))
    )
    SketchFeature.addGeometry(
        Part.LineSegment(CivilCvCAD.Vector(hmax, vmax, 0), CivilCvCAD.Vector(hmax, vmin, 0))
    )
    SketchFeature.addGeometry(
        Part.LineSegment(CivilCvCAD.Vector(hmax, vmin, 0), CivilCvCAD.Vector(hmin, vmin, 0))
    )
    SketchFeature.addGeometry(
        Part.LineSegment(CivilCvCAD.Vector(hmin, vmin, 0), CivilCvCAD.Vector(hmin, vmax, 0))
    )
...
def CreateBoxSketchSet(SketchFeature):
    SketchFeature.addGeometry(
        Part.LineSegment(
            CivilCvCAD.Vector(-99.230339, 36.960674, 0),
            CivilCvCAD.Vector(69.432587, 36.960674, 0),
        )
    )
    SketchFeature.addGeometry(
        Part.LineSegment(
            CivilCvCAD.Vector(69.432587, 36.960674, 0),
            CivilCvCAD.Vector(69.432587, -53.196629, 0),
        )
    )
    SketchFeature.addGeometry(
        Part.LineSegment(
            CivilCvCAD.Vector(69.432587, -53.196629, 0),
            CivilCvCAD.Vector(-99.230339, -53.196629, 0),
        )
    )
    SketchFeature.addGeometry(
        Part.LineSegment(
...
# ---------------------------------------------------------------------------
# define the test cases to test the CivilCvCAD Sketcher module
# ---------------------------------------------------------------------------


class TestSketcherSolver(unittest.TestCase):
    def setUp(self):
        self.Doc = CivilCvCAD.newDocument("SketchSolverTest")
    ...
    def testIssue3245(self):
        self.Doc2 = CivilCvCAD.newDocument("Issue3245")
        ...
        CivilCvCAD.closeDocument("Issue3245")
    ...
    def testBlockConstraintEllipse(self):
        self.Doc3 = CivilCvCAD.newDocument("BlockConstraintTests")
        ...
        ActiveSketch.solve()
        self.assertTrue(status == 0)  # no redundants/conflicts/convergence issues
        CivilCvCAD.closeDocument(self.Doc3.Name)
    ...
    def testCircleLineTangentOriented(self):
        # Test to ratchet issue https://github.com/CivilCvCAD/CivilCvCAD/issues/29007
        ...
        circle_q3_idx = sketch.addGeometry(Part.Circle(App.Vector(-25.0, -25.0), xy_normal, 10))
        ...
        sketch.addConstraint(
            [
                ...
                Sketcher.Constraint("Tangent", arc_q2_idx, -2),  # with vertical axis
                Sketcher.Constraint("Tangent", circle_q3_idx, -1),  # with horizontal axis
        ...
        # should still be in q2
        self.assertLess(sketch.Geometry[arc_q2_idx].Location.x, 0)
        self.assertGreater(sketch.Geometry[arc_q2_idx].Location.y, 0)

        # should still be in q3
        self.assertLess(sketch.Geometry[circle_q3_idx].Location.x, 0)
        self.assertLess(sketch.Geometry[circle_q3_idx].Location.y, 0)
    ...
    def testRemovedExternalGeometryReference(self):
        if "BUILD_PARTDESIGN" in CivilCvCAD.__cmake__:
    ...
    def testSaveLoadWithExternalGeometryReference(self):
        if "BUILD_PARTDESIGN" in CivilCvCAD.__cmake__:
            ...
            self.Doc = CivilCvCAD.openDocument(filename)
    ...
    def testConstructionToggleTNP(self):
        ...
        if "BUILD_PARTDESIGN" in CivilCvCAD.__cmake__:
            ...
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(-10, 10, 0)), True
            )
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(-10, 10, 0), CivilCvCAD.Vector(-5, 20, 0)), True
            )
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(-5, 20, 0), CivilCvCAD.Vector(-10, 25, 0)), True
            )
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(-10, 25, 0), CivilCvCAD.Vector(0, 30, 0)), True
            )
    ...
    def tearDown(self):
        # closing doc
        CivilCvCAD.closeDocument("SketchSolverTest")
