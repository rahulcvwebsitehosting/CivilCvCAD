...
void SketchObject::reverseAngleConstraintToSupplementary(Constraint* constr, int constNum)
{
    ...
    if (expr) {
        ...
        auto op = civilcvcad_cast<App::OperatorExpression*>(expr.get());
        if (op && op->getOperator() == App::OperatorExpression::SUB) {
            auto leftNum = civilcvcad_cast<App::NumberExpression*>(op->getLeft());
            if (leftNum && leftNum->getQuantity() == Quantity(180)) {
                newExpr = op->getRight()->copy();
            }
            auto leftOp = civilcvcad_cast<App::OperatorExpression*>(op->getLeft());
            auto leftOpNum = leftOp ? civilcvcad_cast<App::NumberExpression*>(leftOp->getLeft()) : nullptr;
            auto leftOpUnit = leftOp ? civilcvcad_cast<App::UnitExpression*>(leftOp->getRight()) : nullptr;
        ...
        if (!newExpr) {
            ...
            auto* number = civilcvcad_cast<App::NumberExpression*>(result.get());
        ...
        auto* newNumber = civilcvcad_cast<App::NumberExpression*>(newResult.get());
...
{
    if (auto* asCirc = civilcvcad_cast<const Part::GeomCircle*>(geo)) {
    ...
    if (auto* asArcOfCirc = civilcvcad_cast<const Part::GeomArcOfCircle*>(geo)) {
...
void SketchObject::setOrientationDistance(Constraint* constr)
{
    ...
    if (constr->FirstPos != PointPos::none && constr->Second != GeoEnum::GeoUndef) {
        auto* geo1AsLine = civilcvcad_cast<const Part::GeomLineSegment*>(getGeometry(constr->Second));
        if (geo1AsLine) {
    ...
    if (constr->FirstPos == PointPos::none && constr->SecondPos == PointPos::none && constr->Second != GeoEnum::GeoUndef) {
        ...
        auto* geo2AsLine = civilcvcad_cast<const Part::GeomLineSegment*>(secGeo);
