                               ...
                               double tol)
{
    ...
    for (auto &geo : arcs) {
        if (auto arc = civilcvcad_cast<Part::GeomArcOfCircle*>(geo.get())) {
            if (radius == 0.0) {
                 ...
                 TopoDS_Shape& aProjFace)
{
    ...
    if (curve.GetType() == GeomAbs_Line) {
        ...
        //
        // Doing that solves:
        // https://forum.civilcvcad.org/viewtopic.php?f=3&t=55284#p477522
...
void SketchObject::rebuildExternalGeometry(std::optional<ExternalToAdd> extToAdd)
{
    ...
    for (int i=0; i < int(Objects.size()); i++) {
        ...
        try {
            ...
            if (auto* datum = civilcvcad_cast<const Part::Datum*>(resolvedObj)) {
                refSubShape = datum->getShape();
            }
            else if (auto* refObj = civilcvcad_cast<const Part::Feature*>(resolvedObj)) {
                const Part::TopoShape& refShape = refObj->Shape.getShape();
                refSubShape = refShape.getSubShape(SubElement.c_str());
            }
            else if (auto* pl = civilcvcad_cast<const App::Plane*>(resolvedObj)) {
            ...
            else if (auto* line = civilcvcad_cast<const Part::DatumLine*>(resolvedObj)) {
            ...
            else if (auto* point = civilcvcad_cast<const Part::DatumPoint*>(resolvedObj)) {
            ...
            else if (auto* line = civilcvcad_cast<const App::Line*>(resolvedObj)) {
            ...
            else if (auto* point = civilcvcad_cast<const App::Point*>(resolvedObj)) {
