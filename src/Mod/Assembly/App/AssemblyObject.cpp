...
AssemblyObject::AssemblyObject()
    ...
    , lastSolverStatus(0)
{
    mbdAssembly->externalSystem->civilcvcadAssemblyObject = this;
...
void AssemblyObject::onChanged(const App::Property* prop)
{
    if (prop == &Group) {
        for (auto* obj : getInList()) {
            if (auto* assemblyLink = civilcvcad_cast<AssemblyLink*>(obj)) {
...
void AssemblyObject::updateSolveStatus()
{
    ...
    mbdAssembly->mbdSystem->jointsMotionsDo([&](std::shared_ptr<MbD::Joint> jm) {
        ...
        // We only care about objects that are actual joints in the CivilCvCAD document.
...
std::shared_ptr<ASMTAssembly> AssemblyObject::makeMbdAssembly()
{
    ...
    assembly->externalSystem->civilcvcadAssemblyObject = this;
...
T* AssemblyObject::getGroup()
{
    ...
    for (auto group : groups) {
        if (hasObject(group)) {
            return civilcvcad_cast<T*>(group);
        }
    }
    return nullptr;
}
...
ViewGroup* AssemblyObject::getExplodedViewGroup() const
{
    ...
    for (auto viewGroup : viewGroups) {
        if (hasObject(viewGroup)) {
            return civilcvcad_cast<ViewGroup*>(viewGroup);
        }
    }
    return nullptr;
}
...
std::vector<AssemblyLink*> AssemblyObject::getSubAssemblies()
{
    ...
    for (auto assembly : assemblies) {
        if (hasObject(assembly)) {
            subAssemblies.push_back(civilcvcad_cast<AssemblyLink*>(assembly));
        }
    }
    ...
    for (auto part : allParts) {
        ...
        if (isReadOnly && !hasJoint) {
            ...
            try {
                ...
                std::string code = "import CivilCvCAD\n"
                                   "try:\n"
                                   "    import JointObject\n"
                                   "    import UtilsAssembly\n"
                                   "    doc = CivilCvCAD.getDocument('"
                    + docName
                    + "')\n"
                      "    asm = doc.getObject('"
                    + asmName
                    ...
                    + "')\n"
                      ...
                      "    CivilCvCAD.Console.PrintError(str(e) + '\\n')\n";
