...
JointGroup* getJointGroup(const App::Part* part)
{
    ...
    for (auto jointGroup : jointGroups) {
        if (part->hasObject(jointGroup)) {
            return civilcvcad_cast<JointGroup*>(jointGroup);
        }
    }
    return nullptr;
}
        ...
        if (nextIt != names.end()) {
            for (auto* obji : obj->getOutList()) {
                if (*nextIt == obji->getNameInDocument() && isBodySubObject(obji)) {
                    ...
                    if (auto* lcs = civilcvcad_cast<App::LocalCoordinateSystem*>(obji)) {
...
)
{
    for (auto* obj : objects) {
        ...
        if (auto* asmLink = civilcvcad_cast<Assembly::AssemblyLink*>(obj)) {
        ...
        else if (auto* group = civilcvcad_cast<App::DocumentObjectGroup*>(obj)) {
            collectComponentsRecursively(group->Group.getValues(), results);
            continue;
        }
        else if (auto* link = civilcvcad_cast<App::Link*>(obj)) {
