...
void AssemblyLink::synchronizeComponents()
{
    ...
    for (auto* obj : topLevelComponents) {
        ...
        for (auto* obj2 : assemblyLinkGroup) {
            ...
            auto* subAsmLink = civilcvcad_cast<AssemblyLink*>(obj2);
...
{
    auto sourceProp = civilcvcad_cast<T*>(source->getPropertyByName(propertyName));
    auto targetProp = civilcvcad_cast<T*>(target->getPropertyByName(propertyName));
...
App::DocumentObject* AssemblyLink::getLinkedObject2(bool recursive) const
{
    ...
    auto* assembly = civilcvcad_cast<AssemblyObject*>(obj);
    if (assembly) {
        return assembly;
    }
    else {
        auto* assemblyLink = civilcvcad_cast<AssemblyLink*>(obj);
        if (assemblyLink) {
            if (recursive) {
...
{
    return civilcvcad_cast<AssemblyObject*>(getLinkedObject2());
}
