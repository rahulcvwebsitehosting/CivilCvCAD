...
)
{
    ...
    for (auto* child : objs) {
        ...
        if (auto* asmLink = civilcvcad_cast<AssemblyLink*>(child)) {
...
std::string BomObject::getBomPropertyValue(App::DocumentObject* obj, const std::string& baseName)
{
    ...
    if (auto propStr = civilcvcad_cast<App::PropertyString*>(prop)) {
        return propStr->getValue();
    }
    if (auto propQuantity = civilcvcad_cast<App::PropertyQuantity*>(prop)) {
        return propQuantity->getQuantityValue().getUserString();
    }
    if (auto propEnum = civilcvcad_cast<App::PropertyEnumeration*>(prop)) {
        return propEnum->getValueAsString();
    }
    if (auto propFloat = civilcvcad_cast<App::PropertyFloat*>(prop)) {
        return std::to_string(propFloat->getValue());
    }
    if (auto propInt = civilcvcad_cast<App::PropertyInteger*>(prop)) {
        return std::to_string(propInt->getValue());
    }
    if (auto propBool = civilcvcad_cast<App::PropertyBool*>(prop)) {
