...
def get_rgb(color, testbw=True):
    """Return an RRGGBB value #000000 from a CivilCvCAD color.
...
def argb_to_rgba(color):
    """Change byte order of a 4 byte color int from ARGB (Qt) to RGBA (CivilCvCAD).
    ...
    Usage:
        ...
        CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View")\
...
def rgba_to_argb(color):
    """Change byte order of a 4 byte color int from RGBA (CivilCvCAD) to ARGB (Qt)."""
