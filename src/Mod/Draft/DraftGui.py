...
__title__ = "CivilCvCAD Draft Workbench - GUI part"
    ...
    def eventFilter(self, widget, event):
        if (
            ...
            if hasattr(CivilCvCADGui, "Snapper"):
                CivilCvCADGui.Snapper.cycleSnapObject()
            return True
...
class DraftTaskPanel:
    ...
    def accept(self):
        if hasattr(CivilCvCADGui, "draftToolBar"):
            return CivilCvCADGui.draftToolBar.validatePoint()
        else:
            if CivilCvCADGui.ActiveDocument:
                CivilCvCADGui.ActiveDocument.resetEdit()
            return True

    def reject(self):
        # https://github.com/CivilCvCAD/CivilCvCAD/issues/17027
        ...
        if not CivilCvCADGui.draftToolBar.isTaskOn:
            return
        CivilCvCADGui.draftToolBar.isTaskOn = False
        CivilCvCADGui.draftToolBar.escape()
        if CivilCvCADGui.ActiveDocument:
            CivilCvCADGui.ActiveDocument.resetEdit()
        return True
...
class DraftToolBar:
    ...
    def __init__(self):
        ...
        self.radius = 0
        self.offset = 0
        self.uiloader = CivilCvCADGui.UiLoader()
        self.autogroup = None
        ...
        self.tray = CivilCvCADGui.UiLoader().createWidget("Gui::ToolBar")
    ...
    def _on_lock_change(self, key, locked):
        if locked:
            ...
            if key in ("x", "y", "z"):
                ...
                self.mask = None
                if hasattr(CivilCvCADGui, "Snapper"):
                    CivilCvCADGui.Snapper.mask = None
    ...
    def constrain_point(self, point, last=None):
        ...
        if last is None:
            last = CivilCvCAD.Vector() if self.globalMode else plane.position
        if self.relativeMode:
            raw = CivilCvCAD.Vector(point) - last
            delta = raw if self.globalMode else plane.get_local_coords(raw, as_vector=True)
        else:
            delta = CivilCvCAD.Vector(point) if self.globalMode else plane.get_local_coords(point)
        ...
        if self.globalMode:
            base = last if self.relativeMode else CivilCvCAD.Vector()
            return base + delta
    ...
    def setupToolBar(self, task=False):
        ...
        self.xValue.setText(CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length).UserString)
        ...
        self.yValue.setText(CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length).UserString)
        ...
        self.zValue.setText(CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length).UserString)
        ...
        self.lengthValue.setText(CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length).UserString)
        self.labelangle = self._label("labelangle", al)
        self.angleValue = self._inputfield("angleValue", al)
        self.angleValue.setText(CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Angle).UserString)
        ...
        self.radiusValue.setText(CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length).UserString)
        bl = QtWidgets.QHBoxLayout()
        self.layout.addLayout(bl)
        ...
        if getattr(CivilCvCAD, "activeDraftCommand", None) and getattr(
            CivilCvCAD.activeDraftCommand, "featureName", None
        ):
            self.continueMode = params.get_param(
                CivilCvCAD.activeDraftCommand.featureName, "Mod/Draft/ContinueMode", silent=True
            )
    ...
    def _show_dialog(self, panel):
        task = CivilCvCADGui.Control.showDialog(panel)
        task.setDocumentName(CivilCvCADGui.ActiveDocument.Document.Name)
    ...
    def taskUi(self, title="Draft", extra=None, icon="Draft_Draft"):
        ...
        self.isTaskOn = True
        todo.delay(CivilCvCADGui.Control.closeDialog, None)
        self.baseWidget = DraftBaseWidget()
        self.layout = QtWidgets.QVBoxLayout(self.baseWidget)
        self.setupToolBar(task=True)
        if hasattr(CivilCvCADGui, "Snapper"):
            CivilCvCADGui.Snapper.setPointConstraintProvider(self)
    ...
    def setFocus(self, f=None):
        ...
        if f in ("x", "y", "z"):
            if not self.globalMode:
                ...
                constraint_dir = CivilCvCAD.Vector(
    ...
    def offUi(self):
        self._locks.dispose()
        if hasattr(CivilCvCADGui, "Snapper"):
            CivilCvCADGui.Snapper.clearPointConstraintProvider(self)
        todo.delay(CivilCvCADGui.Control.closeDialog, None)
        self.cancel = None
        self.sourceCmd = None
        self.pointcallback = None
    ...
    def setContinue(self, val):
        params.set_param(
            CivilCvCAD.activeDraftCommand.featureName,
        ...
        if self.sourceCmd or self.pointcallback:
            if self.labelRadius.isVisible():
                ...
                try:
                    ...
                    delta = CivilCvCAD.Vector(numx, numy, numz)
    ...
    def checkSpecialChars(self, txt):
        ...
        if txt in _get_incmd_shortcut("Relative"):
            ...
            if hasattr(CivilCvCADGui, "Snapper"):
                CivilCvCADGui.Snapper.addHoldPoint()
            spec = True
        elif txt in _get_incmd_shortcut("Recenter"):
            if hasattr(CivilCvCADGui, "Snapper"):
                CivilCvCADGui.Snapper.recenter_workingplane()
            spec = True
        ...
        if spec:
            ...
            unit = getattr(CivilCvCAD.Units, field["unit"])
            v = CivilCvCAD.Units.Quantity(value, unit).getUserPreferred()[0]
    ...
    def updateSnapper(self):
        ...
        if not self.xValue.isVisible():
            return
        if (
            hasattr(CivilCvCADGui, "Snapper")
            and CivilCvCADGui.Snapper.trackLine
            and CivilCvCADGui.Snapper.trackLine.Visible
        ):
            point = self.get_new_point(CivilCvCAD.Vector(self.x, self.y, self.z))
            CivilCvCADGui.Snapper.trackLine.p2(point)
    ...
    def displayPoint(self, point=None, last=None, plane=None, mask=None):
        ...
        if point is not None:
            ...
            if not last:
                if self.globalMode:
                    last = CivilCvCAD.Vector()
                else:
                    last = plane.position

            self.new_point = CivilCvCAD.Vector(point)
            self.last_point = CivilCvCAD.Vector(last)
    ...
    def toggleContinue(self):
        CivilCvCAD.Console.PrintMessage("toggle continue\n")
    ...
    def selectplane(self):
        CivilCvCADGui.runCommand("Draft_SelectPlane")

    def setstyle(self):
        CivilCvCADGui.runCommand("Draft_SetStyle")
    ...
    def setStyleButton(self):
        ...
        txt = (
            str(linewidth)
            + "px | "
            + CivilCvCAD.Units.Quantity(fontsize, CivilCvCAD.Units.Length).UserString
        )
    ...
    def popupMenu(self, llist, ilist=None, pos=None):
        ...
        if not pos:
            pos = CivilCvCADGui.getMainWindow().cursor().pos()
    ...
    def runAutoGroup(self):
        CivilCvCADGui.runCommand("Draft_AutoGroup")
    ...
    def setAutoGroup(self, value=None):
        if value is None:
            ...
            obj = CivilCvCAD.ActiveDocument.getObject(value)
            if obj:
    ...
    def togglesnap(self):
        CivilCvCADGui.doCommand('CivilCvCADGui.runCommand("Draft_Snap_Lock")')

    def toggleradius(self, val):
        if hasattr(CivilCvCADGui, "Snapper"):
            par = params.get_param("snapRange")
            params.set_param("snapRange", max(0, par + val))
            CivilCvCADGui.Snapper.showradius()
    ...
    def constrain(self, val):
        ...
        if self.mask == val:
            ...
            self.mask = val
            if hasattr(CivilCvCADGui, "Snapper"):
                CivilCvCADGui.Snapper.mask = val
                if self.new_point is not None:
                    self.new_point = CivilCvCADGui.Snapper.constrain(
    ...
    def _apply_snapper_angle_lock(self):
        if not hasattr(CivilCvCADGui, "Snapper"):
            return
        ...
        CivilCvCADGui.Snapper.setAngle(angle_vec)

    def _clear_snapper_angle_lock(self):
        if hasattr(CivilCvCADGui, "Snapper"):
            CivilCvCADGui.Snapper.setAngle()
        self._angle_lock_axis = None
        self.angle = None
    ...
    def update_spherical_coords(self):
        ...
        self.avalue = math.degrees(phi)
        self.angle = CivilCvCAD.Vector(DraftVecUtils.get_cartesian_coords(1, theta, phi))
    ...
    def setWatchers(self):
        class DraftCreateWatcher:
            ...
            def shouldShow(self):
                return (CivilCvCAD.ActiveDocument is not None) and (
                    not CivilCvCADGui.Selection.getSelection()
                )
        ...
        CivilCvCADGui.Control.addTaskWatcher([DraftCreateWatcher(), DraftModifyWatcher()])
    ...
    def Deactivated(self):
        if CivilCvCAD.activeDraftCommand is not None:
            self.continueMode = False
            CivilCvCAD.activeDraftCommand.finish()
        CivilCvCADGui.Control.clearTaskWatcher()
        # self.tray = None
        if hasattr(self, "tray"):
...
class FacebinderTaskPanel:
    ...
    def addElement(self):
        if self.obj:
            for sel in CivilCvCADGui.Selection.getSelectionEx("", 0):
    ...
    def removeElement(self):
        if self.obj:
            ...
            if it:
                obj = CivilCvCAD.ActiveDocument.getObject(str(it.text(0)))
                elt = str(it.text(1))
                flist = []
...
if not hasattr(CivilCvCADGui, "draftToolBar"):
    CivilCvCADGui.draftToolBar = DraftToolBar()
