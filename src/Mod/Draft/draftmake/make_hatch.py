...
def make_hatch(selection, filename, pattern, scale, rotation, translate=True):
    ...
    filename: string
        ...
        To use a relative path the CivilCvCAD document must have been saved.
    ...
    translate: Bool
        ...
        See https://wiki.civilcvcad.org/Draft_Hatch.
    ...
    if not isinstance(selection, list):
        selection = [selection]
    obj = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "Hatch")
    Hatch(obj)
    obj.File = filename
    obj.Pattern = pattern
    obj.Scale = scale
