...
"""This module contains CivilCvCAD commands for the Draft workbench"""
...
class Draft_Hatch(gui_base.GuiCommandNeedsSelection):
    ...
    def Activated(self):
        ...
        task = CivilCvCADGui.Control.showDialog(
            Draft_Hatch_TaskPanel(CivilCvCADGui.Selection.getSelection()[0])
        )
...
class Draft_Hatch_TaskPanel:
    ...
    def __init__(self, baseobj):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogHatch.ui")
    ...
    def accept(self):
        ...
        if hasattr(self.baseobj, "File") and hasattr(self.baseobj, "Pattern"):
            ...
            o = 'CivilCvCAD.ActiveDocument.getObject("' + self.baseobj.Name + '")'
            CivilCvCADGui.doCommand(o + ".File=" + repr(self.form.File.property("fileName")))
            CivilCvCADGui.doCommand(o + ".Pattern=" + repr(self.form.Pattern.currentText()))
            CivilCvCADGui.doCommand(o + ".Scale=" + str(self.form.Scale.value()))
            CivilCvCADGui.doCommand(o + ".Rotation=" + str(self.form.Rotation.value()))
            CivilCvCADGui.doCommand(o + ".Translate=" + str(self.form.Translate.isChecked()))
        else:
            # create new hatch object
            CivilCvCAD.ActiveDocument.openTransaction("Create Hatch")
            CivilCvCADGui.addModule("Draft")
            cmd = "Draft.make_hatch("
            cmd += "CivilCvCADGui.Selection.getSelectionEx('', 0)"
            ...
            cmd += ")"
            CivilCvCADGui.doCommand("hatch = " + cmd)
            CivilCvCADGui.doCommand("Draft.autogroup(hatch)")
    ...
    def onFileChanged(self, filename):
        ...
        if filename[0] == "." and self.baseobj:
            # File path relative to the CivilCvCAD file directory.
...
if CivilCvCAD.GuiUp:
    ...
    CivilCvCADGui.addCommand("Draft_Hatch", Draft_Hatch())
