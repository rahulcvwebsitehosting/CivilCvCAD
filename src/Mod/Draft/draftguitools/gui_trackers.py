...
__title__ = "CivilCvCAD Draft Trackers"
...
class Tracker:
    ...
    def setColor(self, color=None):
        ...
        if color is not None:
            ...
            hasattr(CivilCvCAD, "activeDraftCommand")
            and CivilCvCAD.activeDraftCommand is not None
            and hasattr(CivilCvCAD.activeDraftCommand, "featureName")
            and CivilCvCAD.activeDraftCommand.featureName in ("Dimension", "Label", "Text")
        ):
    ...
    def _get_wp(self):
        return CivilCvCAD.DraftWorkingPlane
...
class snapTracker(Tracker):
    ...
    def __init__(self):
        ...
        self.marker.markerIndex = CivilCvCADGui.getMarkerIndex(
...
class ghostTracker(Tracker):
    ...
    def getMatrix(self):
        ...
        r = (
            CivilCvCADGui.ActiveDocument.ActiveView.getViewer()
            .getSoRenderManager()
            .getViewportRegion()
        )
        ...
        if m:
            m = m.getValue()
            # fmt: off
            return CivilCvCAD.Matrix(m[0][0], m[0][1], m[0][2], m[0][3],
                                  m[1][0], m[1][1], m[1][2], m[1][3],
                                  m[2][0], m[2][1], m[2][2], m[2][3],
                                  m[3][0], m[3][1], m[3][2], m[3][3])
            # fmt: on
        else:
            return CivilCvCAD.Matrix()
    ...
    def _do_flip(self, root):
        ...
        # Code by wmayer:
        # https://forum.civilcvcad.org/viewtopic.php?p=702640#p702640
    ...
    def __init__(
        ...
        if inactive:
            ...
            if name:
                ...
                self.selnode.documentName.setValue(CivilCvCAD.ActiveDocument.Name)
...
class gridTracker(Tracker):
    ...
    def update(self):
        ...
        if self.space == 0:
            ...
            self.pts = []
            CivilCvCAD.Console.PrintWarning("Draft Grid: Spacing value is zero\n")
            return
        if self.mainlines == 0:
            self.lines1.numVertices.deleteValues(0)
            self.lines2.numVertices.deleteValues(0)
        ...
        if pts != self.pts:
            ...
            if params.get_param("gridBorder"):
                ...
                txt = CivilCvCAD.Units.Quantity(
                    self.space * self.mainlines, CivilCvCAD.Units.Length
                ).UserString
    ...
    def getGridColors(self):
        ...
        if params.get_param("coloredGridAxes"):
            vp = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View")
    ...
    def displayHumanFigure(self, wp):
        ...
        if (
            ...
            and wp.axis.getAngle(CivilCvCAD.Vector(0, 0, 1)) < 0.001
        ):
            loc = CivilCvCAD.Vector(-bound + self.space / 2, -bound + self.space / 2, 0)
    ...
    def setAxesColor(self, wp):
        ...
        cols = [0, 0]
        if params.get_param("coloredGridAxes"):
            if round(wp.u.getAngle(CivilCvCAD.Vector(1, 0, 0)), 2) in (0, 3.14):
                cols[0] = 1
            elif round(wp.u.getAngle(CivilCvCAD.Vector(0, 1, 0)), 2) in (0, 3.14):
                cols[0] = 2
            elif round(wp.u.getAngle(CivilCvCAD.Vector(0, 0, 1)), 2) in (0, 3.14):
                cols[0] = 3
            if round(wp.v.getAngle(CivilCvCAD.Vector(1, 0, 0)), 2) in (0, 3.14):
                cols[1] = 1
            elif round(wp.v.getAngle(CivilCvCAD.Vector(0, 1, 0)), 2) in (0, 3.14):
                cols[1] = 2
            elif round(wp.v.getAngle(CivilCvCAD.Vector(0, 0, 1)), 2) in (0, 3.14):
                cols[1] = 3
    ...
    def reset(self):
        ...
        try:
            self.space = CivilCvCAD.Units.Quantity(
    ...
    def update(self, line=None, normal=None):
        ...
        if lvec.Length > tol and normal.Length > tol:
            ...
            if not lvec.isEqual(normal, tol) and not lvec.isEqual(normal.negative(), tol):
                rot = CivilCvCAD.Rotation(lvec, CivilCvCAD.Vector(), normal, "XZY")
...
class radiusTracker(Tracker):
    ...
    def __init__(self, position=CivilCvCAD.Vector(0, 0, 0), radius=1):
        self.trans = coin.SoTransform()
...
class archDimTracker(Tracker):
    ...
    def __init__(self, p1=CivilCvCAD.Vector(0, 0, 0), p2=CivilCvCAD.Vector(1, 0, 0), mode=1):
        import SketcherGui
    ...
    def setString(self, text=None):
        ...
        text = CivilCvCAD.Units.Quantity(self.Distance, CivilCvCAD.Units.Length).UserString
