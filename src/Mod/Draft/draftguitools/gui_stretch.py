    ...
    def doStretch(self):
        ...
        if self.displacement:
            if self.displacement.Length > 0:
                _doc = "CivilCvCAD.ActiveDocument."
                ...
                for ops in self.ops:
                    ...
                    if tp in ["Wire", "BSpline", "BezCurve"]:
                        ...
                        pts = str(pts).replace("Vector ", "CivilCvCAD.Vector")
                        ...
                        points_str = (
                            "["
                            + ", ".join([f"CivilCvCAD.Vector({p.x}, {p.y}, {p.z})" for p in npts])
                            + "]"
                        )

                        commitops.append("import CivilCvCAD")
                        commitops.append(
                            f"wall_obj = CivilCvCAD.ActiveDocument.getObject('{ops[0].Name}')"
                        )
                        commitops.append(
                        ...
                        commitops.append(_pl)
        if commitops:
            commitops.append("CivilCvCAD.ActiveDocument.recompute()")
            Gui.addModule("Draft")
