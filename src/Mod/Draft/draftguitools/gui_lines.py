    ...
    def finish(self, cont=False, closed=False):
        ...
        if len(self.node) > 1:
            ...
            if len(self.node) == 2 and params.get_param("UsePartPrimitives"):
                ...
                _cmd = "CivilCvCAD.ActiveDocument."
        ...
        if len(Gui.Selection.getSelection()) > 1:
            ...
            if edges:
                try:
                    ...
                    # 'CivilCvCAD.Vector(x,y,z), CivilCvCAD.Vector(x1,y1,z1), ...'
                    pts = ", ".join([str(v.Point) for v in w.Vertexes])
                    pts = pts.replace("Vector ", "CivilCvCAD.Vector")

                    # List of commands to remove the old objects
                    rems = list()
                    for o in Gui.Selection.getSelection():
                        rems.append("CivilCvCAD.ActiveDocument." 'removeObject("' + o.Name + '")')
