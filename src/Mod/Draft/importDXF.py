...
__title__ = "CivilCvCAD Draft Workbench - DXF importer/exporter"
...
from CivilCvCAD import Console as FCC
...
gui = CivilCvCAD.GuiUp
draftui = None
if gui:
    import CivilCvCADGui

    try:
        draftui = CivilCvCADGui.draftToolBar
...
from menu Tools → Addon Manager""",
                    ...
                    FCC.PrintWarning(
                        "The DXF import/export libraries needed by CivilCvCAD to handle the DXF format are not installed.\n"
                    )
                    FCC.PrintWarning(
                ...
                break
        progressbar.stop()
        sys.path.append(CivilCvCAD.ConfigGet("UserAppData"))
    else:
        if gui:
            message = translate(
                "draft",
                """The DXF import/export libraries needed by CivilCvCAD to handle
...
To enabled CivilCvCAD to download these libraries, answer Yes.""",
            )
...
def getDXFlibs():
    ...
    """
    try:
        if CivilCvCAD.ConfigGet("UserAppData") not in sys.path:
            sys.path.append(CivilCvCAD.ConfigGet("UserAppData"))
...
def getGroup(ob):
    ...
    Use local variables, not global variables.
    """
    all_objs = CivilCvCAD.ActiveDocument.Objects
    if dxfUseDraftVisGroups:
...
def placementFromDXFOCS(ent):
    ...
    if draftWPlane.axis == CivilCvCAD.Vector(1.0, 0.0, 0.0):
        draftWPlane.u = CivilCvCAD.Vector(0.0, 1.0, 0.0)
        draftWPlane.v = CivilCvCAD.Vector(0.0, 0.0, 1.0)
    elif draftWPlane.axis == CivilCvCAD.Vector(-1.0, 0.0, 0.0):
        draftWPlane.u = CivilCvCAD.Vector(0.0, -1.0, 0.0)
        draftWPlane.v = CivilCvCAD.Vector(0.0, 0.0, 1.0)
    else:
        if (abs(ent.extrusion[0]) < (1.0 / 64.0)) and (abs(ent.extrusion[1]) < (1.0 / 64.0)):
            draftWPlane.u = CivilCvCAD.Vector(0.0, 1.0, 0.0).cross(draftWPlane.axis)
        else:
            draftWPlane.u = CivilCvCAD.Vector(0.0, 0.0, 1.0).cross(draftWPlane.axis)
...
def drawMesh(mesh, forceShape=False):
    ...
    try:
        ...
        if forceShape:
            ...
            s = s.removeSplitter()
            m = s
    except CivilCvCAD.Base.CivilCvCADError:
        warn(mesh)
    else:
        return m
    return None
...
def drawInsert(insert, num=None, clone=False):
    ...
    if clone:
        if insert.block in blockobjects:
            ...
            sc = insert.scale
            sc = vec([sc[0], sc[1], 0])
            newob.Placement = CivilCvCAD.Placement(tsf)
            newob.Scale = sc
            return newob
        else:
            shape = None
...
def processdxf(document, filename, getShapes=False, reComputeFlag=True):
    ...
    layers = [o for o in CivilCvCAD.ActiveDocument.Objects if utils.get_type(o) == typ]
    global doc
    doc = document
    global blockshapes
    blockshapes = {}
    ...
    for polyline in polylines:
        if dxfImportLayouts or (not rawValue(polyline, 67)):
            ...
            if shape:
                if dxfCreateSketch:
                    if isinstance(shape, Part.Shape):
                        t = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "Shape")
                        t.Shape = shape
                        shape = t
                    CivilCvCAD.ActiveDocument.recompute()
                    if dxfMakeBlocks or dxfJoin:
...
def _import_dxf_file(filename, doc_name=None):
    ...
    """
    hGrp = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft")
    ...
    if doc_name:  # INSERT operation
        try:
            doc = CivilCvCAD.getDocument(doc_name)
        except NameError:
            doc = CivilCvCAD.newDocument(doc_name)
        CivilCvCAD.setActiveDocument(doc_name)
    else:  # OPEN operation
            ...
            if lwPoly:
                if hasattr(dxfLibrary, "LwPolyLine"):
                    ...
                    FCC.PrintWarning(
                        ...
                        "from your CivilCvCAD user directory "
        ...
        if (len(exportList) == 1) and (utils.get_type(exportList[0]) == "ArchSectionView"):
            ...
            for ob in exportList:
                ...
                if obtype == "PanelSheet":
                    ...
                    p2 = CivilCvCAD.Vector(_v)
                    lspc = CivilCvCAD.Vector(_h)
                    ...
                    if t2:
                        ofs = CivilCvCAD.Vector(0, -lspc.Length, 0)
                        if rotation:
                            Z = CivilCvCAD.Vector(0, 0, 1)
                            ofs = CivilCvCAD.Rotation(Z, rotation).multVec(ofs)
                        dxf.append(
                            dxfLibrary.Text(
                                t2,
                                p1.add(ofs),
                    ...
                    if gui:
                        ...
                        for text in vobj.Proxy.getTextData():
                            pos = text[1].add(CivilCvCAD.Vector(0, -h / 2, 0))
                            dxf.append(
                                dxfLibrary.Text(
                                    text[0],
                                    pos,
                    ...
                    if ob.isDerivedFrom("Sketcher::SketchObject"):
                        ...
                        _view = CivilCvCADGui.ActiveDocument.ActiveView
...
def exportPage(page, filename):
    ...
    if os.path.exists(template):
        ...
        svgtemplate = f.read()
        f.close()
        editables = re.findall(r"civilcvcad:editable=\"(.*?)\"", svgtemplate)
        ...
        _v = CivilCvCAD.Version()
        _version = _v[0] + "." + _v[1] + "-" + _v[2]
        template = "999\nCivilCvCAD DXF exporter v" + _version + "\n"
        template += "0\nSECTION\n2\nHEADER\n9\n$ACADVER\n1\nAC1009\n0\nENDSEC\n"
        template += "0\nSECTION\n2\nBLOCKS\n999\n$blocks\n0\nENDSEC\n"
        template += "0\nSECTION\n2\nENTITIES\n999\n$entities\n0\nENDSEC\n"
        template += "0\nEOF"
...
class DxfImportReporter:
    ...
    def to_console_string(self):
        ...
        lines.append(f"CivilCvCAD objects created: {self.stats.get('totalEntitiesCreated', 0)}")
    ...
    def report_to_console(self):
        """
        Prints the formatted statistics string to the CivilCvCAD console.
        """
...
class DxfDraftPostProcessor:
    ...
    def _create_draft_object_from_part(self, part_obj):
        ...
        if part_obj.isDerivedFrom("Part::Line"):
            ...
            start_point = CivilCvCAD.Vector(part_obj.X1.Value, part_obj.Y1.Value, part_obj.Z1.Value)
            end_point = CivilCvCAD.Vector(part_obj.X2.Value, part_obj.Y2.Value, part_obj.Z2.Value)
            ...
            if hasattr(part_obj, "Radius"):
                new_obj.Radius = CivilCvCAD.Units.Quantity(part_obj.Radius.Value, "mm")
            ...
            if hasattr(part_obj, "Angle1") and hasattr(part_obj, "Angle2"):
                ...
                new_obj.FirstAngle = CivilCvCAD.Units.Quantity(start_angle, "deg")
                new_obj.LastAngle = CivilCvCAD.Units.Quantity(end_angle, "deg")
            ...
            if hasattr(part_obj, "Placement"):
                ...
                new_obj.Placement = CivilCvCAD.Placement()
            ...
            if is_full_ellipse:
                ...
                center_at_origin = CivilCvCAD.Vector(0, 0, 0)
                geom = Part.Ellipse(
                ...
                if placeholder.DxfEntityType == "DIMENSION":
                    ...
                    if hasattr(placeholder, "DxfDimensionType"):
                        ...
                        if dim_type == 0 and hasattr(placeholder, "DxfRotation"):
                            ...
                            direction_vector = CivilCvCAD.Vector(math.cos(angle), math.sin(angle), 0)
    ...
    def _get_canonical_angles(self, start_angle_deg, end_angle_deg, radius_mm):
        ...
        CivilCvCAD's App::PropertyAngle, which constrains values to [-360, 360].
