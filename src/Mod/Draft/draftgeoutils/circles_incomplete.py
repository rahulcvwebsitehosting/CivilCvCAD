...
def circleFrom2tan1pt(tan1, tan2, point):
    ...
    """
    if geomType(tan1) == "Line" and geomType(tan2) == "Line" and isinstance(point, CivilCvCAD.Vector):
        return circlefrom2Lines1Point(tan1, tan2, point)

    elif (
        geomType(tan1) == "Circle"
        and geomType(tan2) == "Line"
        and isinstance(point, CivilCvCAD.Vector)
    ):
...
def circleFrom1tan2pt(tan1, p1, p2):
    ...
    if (
        geomType(tan1) == "Line"
        and isinstance(p1, CivilCvCAD.Vector)
        and isinstance(p2, CivilCvCAD.Vector)
    ):
...
def circleFrom1tan1pt1rad(tan1, p1, rad):
    ...
    or circular edges, so two combinations are possible.
    """
    if geomType(tan1) == "Line" and isinstance(p1, CivilCvCAD.Vector):
        return circleFromPointLineRadius(p1, tan1, rad)

    elif geomType(tan1) == "Circle" and isinstance(p1, CivilCvCAD.Vector):
