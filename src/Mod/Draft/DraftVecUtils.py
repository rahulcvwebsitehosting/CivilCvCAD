...
__title__ = "CivilCvCAD Draft Workbench - Vector library"
...
def toString(u):
    ...
    u : list, or Base::Vector3
        A list of CivilCvCAD.Vectors, or a single vector.
...
def equals(u, v, precision=None):
    ...
    in the parameter database, accessed through `CivilCvCAD.ParamGet()`.
    ::
        x1 - x2 < precision
        y1 - y2 < precision
        z1 - z2 < precision
...
def scale(u, scalar):
    ...
    ----------
    u : Base::Vector3
        The CivilCvCAD.Vector to scale.
    scalar : float
        The scaling factor.
...
def rotate(u, angle, axis=Vector(0, 0, 1)):
    ...
    # fmt: off
    m = CivilCvCAD.Matrix(c + x*x*t,   xyt - zs,   xzt + ys,   0,
...
def getPlaneRotation(u, v, _=None):
    ...
    # fmt: off
    m = CivilCvCAD.Matrix(u.x, v.x, w.x, 0,
                       u.y, v.y, w.y, 0,
                       u.z, v.z, w.z, 0,
                       0.0, 0.0, 0.0, 1.0)
    # fmt: on
