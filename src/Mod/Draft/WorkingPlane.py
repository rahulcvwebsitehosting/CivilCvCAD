...
in CivilCvCAD and a couple of utility functions.
...
CivilCvCADGui = lz.LazyLoader("CivilCvCADGui", globals(), "CivilCvCADGui")

__title__ = "CivilCvCAD Working Plane utility"
...
class PlaneBase:
    ...
    def align_to_straight_edge(self, edge, offset=0):
        ...
        rot = CivilCvCAD.Rotation(u, self.v, self.axis, "XZY")
    ...
    def align_to_point_and_axis(self, point, axis, offset=0, upvec=Vector(1, 0, 0)):
        ...
        If the axis and up-vector are parallel the CivilCvCAD.Rotation algorithm
        ...
        rot = CivilCvCAD.Rotation(Vector(), upvec, axis, "ZYX")
    ...
    def get_global_coords(self, point, as_vector=False):
        ...
        mtx = CivilCvCAD.Matrix(self.u, self.v, self.axis, pos)
    ...
    def get_placement(self):
        ...
        return CivilCvCAD.Placement(self.position, CivilCvCAD.Rotation(self.u, self.v, self.axis, "ZYX"))
    ...
    def is_ortho(self):
        ...
        rot = CivilCvCAD.Rotation(self.u, self.v, self.axis, "ZYX")
...
class Plane(PlaneBase):
    ...
    def alignToSelection(self, offset=0):
        ...
        `True`/`False`
            `True` if successful.
        """
        sel_ex = CivilCvCADGui.Selection.getSelectionEx()
        if not sel_ex:
            return False
        ...
        for obj in sel_ex:
            ...
            geom_is_shape = False
            if isinstance(obj.Object, CivilCvCAD.GeoFeature):
    ...
    def setup(self, direction=None, point=None, upvec=None, force=False):
        ...
        This method only works when `CivilCvCAD.GuiUp` is `True`,
        ...
        if self.weak or force:
            ...
            elif CivilCvCAD.GuiUp:
                try:
                    view = gui_utils.get_3d_view()
                    if view is not None:
                        cam = view.getCameraNode()
                        rot = CivilCvCAD.Rotation(*cam.getField("orientation").getValue().getValue())
    ...
    def setTop(self):
        ...
        if CivilCvCAD.GuiUp:
            ...
            if hasattr(CivilCvCADGui, "draftToolBar"):
                CivilCvCADGui.draftToolBar.wplabel.setText(translate("draft", "Top"))
    ...
    def setFront(self):
        ...
        if CivilCvCAD.GuiUp:
            ...
            if hasattr(CivilCvCADGui, "Snapper"):
                CivilCvCADGui.Snapper.setGrid()
            if hasattr(CivilCvCADGui, "draftToolBar"):
                CivilCvCADGui.draftToolBar.wplabel.setText(translate("draft", "Front"))
    ...
    def setSide(self):
        ...
        if CivilCvCAD.GuiUp:
            ...
            if hasattr(CivilCvCADGui, "Snapper"):
                CivilCvCADGui.Snapper.setGrid()
            if hasattr(CivilCvCADGui, "draftToolBar"):
                CivilCvCADGui.draftToolBar.wplabel.setText(translate("draft", "Side"))

    def getRotation(self):
        """Return a placement describing the WP orientation only."""
        return CivilCvCAD.Placement(Vector(), CivilCvCAD.Rotation(self.u, self.v, self.axis, "ZYX"))
...
class PlaneGui(PlaneBase):
    ...
    def align_to_selection(self, offset=0, _hist_add=True):
        ...
        sels = CivilCvCADGui.Selection.getSelectionEx("", 0)
        if not sels:
            return False
        ...
        place = CivilCvCAD.Placement(mtx)
        ...
        if typ in ["App::Part", "Part::DatumPlane", "PartDesign::Plane", "Axis", "SectionPlane"]:
            ...
            pl.move(CivilCvCAD.Vector(0, 0, obj.Elevation.Value))
    ...
    def align_to_wp_proxy(self, obj, offset=0, place=None, _hist_add=True):
        ...
        if (
            ...
            for name, vis in vobj.VisibilityMap.items():
                obj = CivilCvCADGui.ActiveDocument.getObject(name)
                if obj:
    ...
    def _format_coord(self, coord):
        return CivilCvCAD.Units.Quantity(coord, CivilCvCAD.Units.Length).UserString
    ...
    def _update_old_plane(self):
        ...
        """
        if not hasattr(CivilCvCAD, "DraftWorkingPlane"):
            CivilCvCAD.DraftWorkingPlane = Plane()
        for prop in ["u", "v", "axis", "position"]:
            setattr(CivilCvCAD.DraftWorkingPlane, prop, self._copy_value(getattr(self, prop)))
        CivilCvCAD.DraftWorkingPlane.weak = self.auto

    def _update_grid(self):
        # Check for draftToolBar because the trackers (grid) depend on it for its colors.
        if (
            CivilCvCAD.GuiUp
            and hasattr(CivilCvCADGui, "draftToolBar")
            and hasattr(CivilCvCADGui, "Snapper")
            and self._view is not None
        ):
            CivilCvCADGui.Snapper.setGrid()
            CivilCvCADGui.Snapper.restack()  # Required??

    def _update_gui(self):
        if CivilCvCAD.GuiUp and hasattr(CivilCvCADGui, "draftToolBar") and self._view is not None:
            from PySide import QtGui

            button = CivilCvCADGui.draftToolBar.wplabel
...
def get_working_plane(update=True):
    ...
    if not hasattr(CivilCvCAD, "draft_working_planes"):
        CivilCvCAD.draft_working_planes = [[], []]

    view = gui_utils.get_3d_view()

    if view is not None and view in CivilCvCAD.draft_working_planes[0]:
        i = CivilCvCAD.draft_working_planes[0].index(view)
        wp = CivilCvCAD.draft_working_planes[1][i]
        if update is False:
    ...
    if CivilCvCAD.GuiUp:
        ...
        wp.set_to_default()
        if view is not None:
            CivilCvCAD.draft_working_planes[0].append(view)
            CivilCvCAD.draft_working_planes[1].append(wp)
...
if CivilCvCAD.GuiUp:
    ...
    def _update_gui():
        try:
            ...
            if not hasattr(CivilCvCAD, "draft_working_planes"):
                CivilCvCAD.draft_working_planes = [[], []]
            if view in CivilCvCAD.draft_working_planes[0]:
                i = CivilCvCAD.draft_working_planes[0].index(view)
                wp = CivilCvCAD.draft_working_planes[1][i]
                wp._update_gui()
            else:
