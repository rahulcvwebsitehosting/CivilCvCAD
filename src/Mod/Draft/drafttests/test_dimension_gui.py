...
class DraftGuiDimension(test_base.DraftTestCaseDoc):
    ...
    CIVILCVCAD_NAMESPACE = "urn:civilcvcad:svg"
    ...
    def extractOvershootLines(self, svg_text):
        ...
        root = ET.fromstring(f'<svg xmlns:civilcvcad="{self.CIVILCVCAD_NAMESPACE}">{svg_text}</svg>')
        lines = []
        for element in root.iter():
            ...
            skip_value = next(
                (
                    ...
                    if key == "civilcvcad:skip" or key == "skip" or key.endswith("}skip")
                ),
                None,
            )
            if skip_value != "1":
