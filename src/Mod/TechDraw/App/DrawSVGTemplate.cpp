...
QString DrawSVGTemplate::processTemplate()
{
    ...
    // has "civilcvcad:editable" attribute
    query.processItems(QStringLiteral(
        "declare default element namespace \"" SVG_NS_URI "\"; "
        "declare namespace civilcvcad=\"" CIVILCVCAD_SVG_NS_URI "\"; "
        "//text[@" CIVILCVCAD_ATTR_EDITABLE "]/tspan"),
        ...
        QString editableName = tspan.parentNode().toElement().attribute(QString::fromUtf8(CIVILCVCAD_ATTR_EDITABLE));
...
//! find the special fields in the template (civilcvcad:editable or civilcvcad:autofill)
std::map<std::string, std::string> DrawSVGTemplate::getEditableTextsFromTemplate()
{
    ...
    query.processItems(QStringLiteral(
        ...
        [this, &editables](QDomElement& tspan) -> bool {
            ...
            std::string editableName = parent.attribute(QStringLiteral(CIVILCVCAD_ATTR_EDITABLE)).toStdString();
            if (!editableName.empty()) {
                std::string editableValue;

                if (parent.hasAttribute(QStringLiteral(CIVILCVCAD_ATTR_AUTOFILL))) {
                    std::string autofillId = parent.attribute(QStringLiteral(CIVILCVCAD_ATTR_AUTOFILL)).toStdString();
