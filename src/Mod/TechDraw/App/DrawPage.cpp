    ...
    if (prop == &KeepUpdated && KeepUpdated.getValue()) {
        ...
        if (!isRestoring()) {
            for (auto* obj : getViews()) {
                auto* view = civilcvcad_cast<DrawView*>(obj);
        ...
        for (auto* obj : getViews()) {
            auto* view = civilcvcad_cast<DrawProjGroup*>(obj);
...
int DrawPage::addView(App::DocumentObject* docObj, bool setPosition)
{
    ...
    auto* view = civilcvcad_cast<DrawView*>(docObj);
    ...
    if (!view) {
        ...
        view = civilcvcad_cast<DrawView*>(link->getLinkedObject());
        if (!view) {
            return -1;
        }
    }
...
void DrawPage::updateAllViews()
{
    ...
    for (auto& v : featViews) {
        auto* part = civilcvcad_cast<DrawViewPart*>(v);
        if (part) {
    ...
    for (auto& v : featViews) {
        ...
        auto* view = civilcvcad_cast<DrawView*>(v);
        if (view) {
