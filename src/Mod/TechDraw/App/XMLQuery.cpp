...
// elements of name "text" with an attribute "civilcvcad:editable"
                            ...
                            const std::function<bool(QDomElement&)>& process)
{
    ...
    if (editable.count() > 0) {
        for(int i = 0; i < editable.count(); i++) {
            ...
            if (element.hasAttribute(QString(QLatin1String(CIVILCVCAD_ATTR_EDITABLE)))) {
                if (find_tspan) {
...
{
    // The actual query string is of the form "//text[@civilcvcad:editable]"
    // or "//text[@civilcvcad:editable]/tspan"
