...
class DrawViewScaleTypeTest(unittest.TestCase):
    ...
    def setUp(self):
        self.document = CivilCvCAD.newDocument("TDScaleType")
        self.page = createPageWithSVGTemplate(self.document)
        self.page.Scale = 0.05
        self.savedFile = None

    def tearDown(self):
        for name in list(CivilCvCAD.listDocuments().keys()):
            CivilCvCAD.closeDocument(name)
    ...
    def testScaleTypePagePersistsOnReload(self):
        ...
        reloaded = CivilCvCAD.openDocument(self.savedFile)
