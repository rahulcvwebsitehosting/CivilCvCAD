...
void QGIView::snapPosition(QPointF& newPosition)
{
    ...
    auto* dvp = civilcvcad_cast<DrawViewPart*>(feature);
    if (dvp  &&
        !dvp->hasGeometry()) {
    ...
    for (auto* view : views) {
        ...
        auto viewDvp = civilcvcad_cast<DrawViewPart*>(viewFeature);
                              ...
                              QPointF& newPosition)
{
    ...
    auto* vpdv = civilcvcad_cast<ViewProviderDrawingView*>(getViewProvider(baseView));
    if (!vpdv) {
        return;
    }
...
void QGIView::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (auto* dpgi = civilcvcad_cast<TechDraw::DrawProjGroupItem*>(feature)) {
...
ViewProviderPage* QGIView::getViewProviderPage(TechDraw::DrawView* dView)
{
    ...
    return civilcvcad_cast<ViewProviderPage*>(activeGui->getViewProvider(page));
}
...
bool QGIView::isExporting() const
{
    ...
    auto* view = civilcvcad_cast<TechDraw::DrawView*>(obj);
    if (!view) {
        return false;
    }
