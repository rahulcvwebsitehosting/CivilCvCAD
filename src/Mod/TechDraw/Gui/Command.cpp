...
void CmdTechDrawView::activated(int iMsg)
{
    ...
    auto* vpp = civilcvcad_cast<ViewProviderPage*>
        (Gui::Application::Instance->getViewProvider(page));
    if (vpp) {
        vpp->show();
    }
    ...
    doCommand(Doc,
              "App.activeDocument().%s.Direction = CivilCvCAD.Vector(%.12f, %.12f, %.12f)",
              FeatName.c_str(), checkedDir.x, checkedDir.y, checkedDir.z);
    doCommand(Doc,
              "App.activeDocument().%s.RotationVector = CivilCvCAD.Vector(%.12f, %.12f, %.12f)",
              FeatName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    doCommand(Doc,
              "App.activeDocument().%s.XDirection = CivilCvCAD.Vector(%.12f, %.12f, %.12f)",
              FeatName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
...
void CmdTechDrawBrokenView::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().%s.Direction = CivilCvCAD.Vector(%.6f,%.6f,%.6f)",
              FeatName.c_str(), projDir.x, projDir.y, projDir.z);
    doCommand(Doc, "App.activeDocument().%s.XDirection = CivilCvCAD.Vector(%.6f,%.6f,%.6f)",
                  FeatName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    getDocument()->setStatus(App::Document::Status::SkipRecompute, true);
...
void CmdTechDrawProjectionGroup::activated(int iMsg)
{
    ...
    getDocument()->setStatus(App::Document::Status::SkipRecompute, true);
    doCommand(Doc,
        "App.activeDocument().%s.Anchor.Direction = CivilCvCAD.Vector(%.12f, %.12f, %.12f)",
        multiViewName.c_str(), checkedDir.x, checkedDir.y, checkedDir.z);
    doCommand(Doc,
        "App.activeDocument().%s.Anchor.RotationVector = CivilCvCAD.Vector(%.12f, %.12f, %.12f)",
        multiViewName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    doCommand(Doc,
        "App.activeDocument().%s.Anchor.XDirection = CivilCvCAD.Vector(%.12f, %.12f, %.12f)",
        multiViewName.c_str(), dirs.second.x, dirs.second.y, dirs.second.z);
    getDocument()->setStatus(App::Document::Status::SkipRecompute, false);
...
void CmdTechDrawBalloon::activated(int iMsg)
{
    ...
    ViewProviderPage* pageVP = civilcvcad_cast<ViewProviderPage*>(guiDoc->getViewProvider(page));
    ViewProviderDrawingView* viewVP =
        civilcvcad_cast<ViewProviderDrawingView*>(guiDoc->getViewProvider(objFeat));
...
void CmdTechDrawDraftView::activated(int iMsg)
{
    ...
    for (auto* obj : objects) {
        ...
        doCommand(Doc, "if App.activeDocument().%s.Scale: App.activeDocument().%s.Scale = App.activeDocument().%s.Scale",
                  PageName.c_str(), FeatName.c_str(), PageName.c_str());
        doCommand(Doc, "App.activeDocument().%s.Direction = CivilCvCAD.Vector(%.12f, %.12f, %.12f)",
                  FeatName.c_str(), checkedDir.x, checkedDir.y, checkedDir.z);
        updateActive();
        commitCommand();
    }
...
void CmdTechDrawExportPageSVG::activated(int iMsg)
{
    ...
    ViewProviderPage* vpPage = civilcvcad_cast<ViewProviderPage*>(vp);
    ...
    if (vpPage) {
        ...
        QMessageBox::warning(Gui::getMainWindow(),
                             ...
                             QObject::tr("CivilCvCAD could not find a page to export"));
        return;
    }
}
