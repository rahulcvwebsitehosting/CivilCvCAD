...
void QGIViewClip::drawClip()
{
    ...
    auto* vpClip = civilcvcad_cast<ViewProviderViewClip*>(getViewProvider(viewClip));
    if (!vpClip) {
        return;
    }
...
DrawView* QGIViewClip::selectionIsInGroup() const
{
    ...
    auto* clipGroup = civilcvcad_cast<DrawViewClip*>(getViewObject());
    for (auto& selItem : selection) {
        auto view = civilcvcad_cast<DrawView*>(selItem.getObject());
        if (view &&
