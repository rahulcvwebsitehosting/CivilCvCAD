...
{
    auto ViewProvider = civilcvcad_cast<ViewProviderLeader*>(getViewProvider(getLeaderFeature()));
    if (!ViewProvider) {
...
QColor QGILeaderLine::prefNormalColor()
{
    ...
    auto vp = civilcvcad_cast<ViewProviderLeader*>(getViewProvider(getViewObject()));
    if (vp) {
