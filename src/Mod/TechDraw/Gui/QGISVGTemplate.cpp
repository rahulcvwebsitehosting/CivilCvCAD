...
namespace {
    ...
    std::vector<QDomElement> getFCElements(QDomDocument& doc) {
        ...
        for(int i = 0; i < textElements.size(); i++) {
            ...
            if(textElement.hasAttribute(QStringLiteral(CIVILCVCAD_ATTR_EDITABLE))) {
    ...
    void applyWorkaround(QByteArray& svgCode)
    {
        ...
        for(QDomElement& textElement : textElements) {
            if (!textElement.hasAttribute(QStringLiteral("id")) ||
                ...
                QString id = QStringLiteral("civilcvcad_id_") + QString::number(counter);
...
void QGISVGTemplate::createClickHandles()
{
    ...
    for(QDomElement& textElement : textElements) {
        ...
        QString name = textElement.attribute(QStringLiteral(CIVILCVCAD_ATTR_EDITABLE));
        auto item(new TemplateTextField(this, svgTemplate, name.toStdString()));
        item->setAutofillId(textElement.attribute(QStringLiteral(CIVILCVCAD_ATTR_AUTOFILL)).toStdString());
