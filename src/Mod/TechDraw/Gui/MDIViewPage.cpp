...
void MDIViewPage::closeEvent(QCloseEvent* event)
{
    ...
    if (_pcDocument && !m_objectName.empty()) {
        ...
        if (doc) {
            ...
            if (auto* vpPage = civilcvcad_cast<ViewProviderPage*>(
...
void MDIViewPage::exportAsPdf() const
{
    ...
    if (fi.exists() && !fi.isWritable()) {
        ...
        QMessageBox::critical(
            ...
            QObject::tr("CivilCvCAD is unable to open file %1 for writing.  The file may be open in another program.").arg(filename));
        return;
    }
