...
{
    return civilcvcad_cast<DrawTemplate*>(getDrawPage()->Template.getValue());
}
...
QGITemplate* ViewProviderPage::getQTemplate() const
{
    ...
    if (guiDoc) {
        ...
        auto vpTemplate = civilcvcad_cast<ViewProviderTemplate*>(vp);
        if (vpTemplate) {
...
void ViewProviderPage::setTemplateMarkers(bool state) const
{
    ...
    auto* vpt = civilcvcad_cast<ViewProviderTemplate*>(vp);
    if (vpt) {
