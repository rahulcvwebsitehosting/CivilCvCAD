...
__title__ = "CivilCvCAD Arch Frame"
        ...
        if obj.Base.Shape.Solids:
            ...
            if hasattr(obj, "Edges"):
                if obj.Edges == "Vertical edges":
                    rv = obj.Base.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 1, 0))
                    edges = [
                        e
                        for e in edges
                    ...
                    rv = obj.Base.Placement.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0))
                    edges = [
                        e
                        for e in edges
                        if round(rv.getAngle(e.tangentAt(e.FirstParameter)), 4) in [0, 3.1416]
            ...
            for e in edges:
                ...
                # Supplying CivilCvCAD.Rotation() with two parallel vectors and
                # a null vector may seem strange, but the function is perfectly
                # able to handle this. Its algorithm will use default axes in
                # such cases.
                if obj.Align:
                    if normal is None:
                        rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(), bvec, bvec, "ZYX")
                    else:
                        rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(), normal, bvec, "ZYX")
