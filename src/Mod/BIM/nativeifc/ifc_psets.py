    ...
    for gname, pset in psets.items():
        for pname, pvalue in pset.items():
            ...
            if ptype in [
                ...
                # CivilCvCAD-specific: split strings by :: delimiter
...
def load_psets(obj):
    ...
    show_psets(obj)
    if isinstance(obj, CivilCvCAD.DocumentObject) and hasattr(obj, "Group"):
        for child in obj.Group:
            load_psets(child)
    elif isinstance(obj, CivilCvCAD.Document):
...
def get_CivilCvCAD_type(ptype):
    """Returns a CivilCvCAD property type corresponding to an IFC property type"""
...
def get_ifc_type(fctype):
    """Returns an IFC property type corresponding to a CivilCvCAD property type"""
...
def read_properties_conversion():
    ...
    csvfile = os.path.join(
        CivilCvCAD.getResourceDir(), "Mod", "BIM", "Presets", "properties_conversion.csv"
    )
    result = {}
...
def remove_property(obj, prop):
    ...
    for psetname, props in psets.items():
        if prop in props:
            ...
            if pset:
                CivilCvCAD.Console.PrintMessage(translate("BIM", "Removing property") + ": " + prop)
