...
class ifc_object:
    ...
    def get_section_data(self, obj):
        ...
        if not objs:
            ...
            if isinstance(proj, CivilCvCAD.DocumentObject):
                objs.append(proj)
            objs.extend(ifc_tools.get_CivilCvCAD_children(proj))
        if objs:
            s = []
            for o in objs:
                ...
                if o.ShapeMode != "Shape":
                    s.append(o)
            if s:
                CivilCvCAD.Console.PrintLog("DEBUG: Generating shapes. This might take some time...\n")
                for o in s:
                    o.ShapeMode = "Shape"
                    o.recompute()
            l = 1
            ...
            plane = Part.makePlane(l, h, CivilCvCAD.Vector(l / 2, -h / 2, 0), CivilCvCAD.Vector(0, 0, 1))
