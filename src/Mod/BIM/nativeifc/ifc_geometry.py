...
def add_geom_properties(obj):
    """Adds geometry properties to a CivilCvCAD object"""
        ...
        if rep.RepresentationIdentifier == "Body":
            if len(rep.Items) == 1:
                ...
                if rep.Items[0].is_a("IfcExtrudedAreaSolid"):
                    ...
                    obj.ExtrusionDirection = CivilCvCAD.Vector(ext.ExtrudedDirection.DirectionRatios)
                    ...
                    # Extrusion of a polyline
                        ...
                        if ext.SweptArea.OuterCurve.is_a("IfcPolyline"):
                            ...
                            points = [CivilCvCAD.Vector(p).multiply(scaling) for p in points]
                        ...
                        if rep.Items[0].Segments[0].is_a("IfcCompositeCurveSegment"):
                            if rep.Items[0].Segments[0].ParentCurve.is_a("IfcPolyline"):
                                ...
                                if len(pol.Points) == 2:
                                    ...
                                    obj.AxisStart = CivilCvCAD.Vector(
                            ...
                            if rep.Items[0].SweptArea.OuterCurve.is_a("IfcPolyline"):
                                ...
                                if len(points) == len(elem_points):
                                    for i in range(len(points)):
                                        v = CivilCvCAD.Vector(points[i]).multiply(scaling)
                                        coord = tuple(v)[:psize]
                                        ifc_tools.api_run(
                                            "attribute.edit_attributes",
                                            ifcfile,
