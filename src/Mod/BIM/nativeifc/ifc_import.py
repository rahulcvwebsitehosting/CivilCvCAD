...
def insert(
    ...
    """Inserts an IFC document in a CivilCvCAD document.
    ...
    stime = time.time()
    try:
        document = CivilCvCAD.getDocument(docname)
    except NameError:
        document = CivilCvCAD.newDocument()
    ...
    if CivilCvCAD.GuiUp and PARAMS.GetBool("IfcFileToConsole", False):
        if isinstance(prj_obj, CivilCvCAD.DocumentObject):
            pstr = "CivilCvCAD.getDocument('{}').{}.Proxy.ifcfile"
    ...
    if CivilCvCAD.GuiUp and switchwb:
...
def get_options(strategy=None, shapemode=None, switchwb=None, silent=False):
    ...
    if ask and CivilCvCAD.GuiUp:
        import CivilCvCADGui

        dlg = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogImport.ui")
...
def get_project_type(silent=False):
    ...
    if ask and CivilCvCAD.GuiUp:
        ...
        dlg = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogCreateProject.ui")
        result = dlg.exec_()
...
def unset_modified():
    try:
        CivilCvCADGui.ActiveDocument.Modified = False
