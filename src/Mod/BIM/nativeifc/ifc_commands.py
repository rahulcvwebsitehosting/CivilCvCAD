...
"""This module contains IFC-related CivilCvCAD commands"""
...
def get_project():
    ...
    if CivilCvCADGui.Selection.getSelection():
        return ifc_tools.get_project(CivilCvCADGui.Selection.getSelection()[0])
    else:
        return ifc_tools.get_project(CivilCvCAD.ActiveDocument)
...
class IFC_MakeProject:
    ...
    def IsActive(self):
        return bool(CivilCvCADGui.Selection.getSelection())
...
class IFC_Save:
    ...
    def Activated(self):
        ...
        if getattr(doc, "IfcFilePath", None):
            ifc_tools.save(doc)
            gdoc = CivilCvCADGui.getDocument(doc.Name)
            try:
                gdoc.Modified = False
            except:
                pass
        else:
            CivilCvCADGui.runCommand("IFC_SaveAs")
...
# initialize commands
CivilCvCADGui.addCommand("IFC_Diff", IFC_Diff())
CivilCvCADGui.addCommand("IFC_Expand", IFC_Expand())
CivilCvCADGui.addCommand("IFC_ConvertDocument", IFC_ConvertDocument())
CivilCvCADGui.addCommand("IFC_MakeProject", IFC_MakeProject())
CivilCvCADGui.addCommand("IFC_Save", IFC_Save())
CivilCvCADGui.addCommand("IFC_SaveAs", IFC_SaveAs())
