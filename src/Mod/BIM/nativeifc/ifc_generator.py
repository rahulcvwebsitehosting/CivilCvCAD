...
"""This module contains all the tools to create CivilCvCAD geometry from IFC objects.
...
def generate_geometry(obj, cached=False):
    ...
    This is the main function called by the execute method of CivilCvCAD document objects
    ...
    obj: CivilCvCAD document object
    ...
    while True:
        ...
        if item and item.id not in done:
            ...
            if hasattr(item.transformation.matrix, "data"):
                # IfcOpenShell 0.7
                mat = ifc_tools.get_CivilCvCAD_matrix(item.transformation.matrix.data)
            else:
                # IfcOpenShell 0.8
                mat = ifc_tools.get_CivilCvCAD_matrix(item.transformation.matrix)
...
def generate_coin(ifcfile, elements, cached=False):
    ...
    while True:
        ...
        if item and item.id not in done:
            ...
            if hasattr(item.transformation.matrix, "data"):
                # IfcOpenShell 0.7
                matrix = ifc_tools.get_CivilCvCAD_matrix(item.transformation.matrix.data)
            else:
                # IfcOpenShell 0.8
                matrix = ifc_tools.get_CivilCvCAD_matrix(item.transformation.matrix)
            placement = CivilCvCAD.Placement(matrix)
            verts = item.geometry.verts
            verts = [CivilCvCAD.Vector(verts[i : i + 3]) for i in range(0, len(verts), 3)]
...
def apply_placement(node, placement):
    ...
    verts = [tuple(placement.multVec(CivilCvCAD.Vector(v))) for v in node[1]]
...
def create_ghost(document, ifcfile, project):
    ...
    sg = CivilCvCADGui.getDocument(document.Name).ActiveView.getSceneGraph()
