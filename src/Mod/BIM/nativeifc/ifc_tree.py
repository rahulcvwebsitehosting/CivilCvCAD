...
def show_geometry_tree(element):
    ...
    import Arch_rc
    import CivilCvCADGui  # lazy import
    from . import ifc_tools
    from PySide import QtWidgets

    if isinstance(element, CivilCvCAD.DocumentObject):
        element = ifc_tools.get_ifc_element(element)
        if not element:
            return
    dlg = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogTree.ui")
    tops = {}
    for line in get_geometry_tree(element):
...
def show_properties(current, previous):
    ...
    if "Position" in dir(elt):
        ...
        position = CivilCvCAD.Vector(elt.Position.Location.Coordinates)
        axis = CivilCvCAD.Vector(elt.Position.Axis.DirectionRatios)
        xref = CivilCvCAD.Vector(elt.Position.RefDirection.DirectionRatios)
        rotation = CivilCvCAD.Rotation(axis, xref, CivilCvCAD.Vector(), "ZXY").toEulerAngles("XYZ")
        rotation = CivilCvCAD.Vector(rotation)
        for c in ["x", "y", "z"]:
