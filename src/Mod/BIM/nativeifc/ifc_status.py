...
def on_add_property():
    ...
    form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogAddProperty.ui")
    ...
    if not pset:
        ...
        CivilCvCAD.Console.PrintError(t + "\n")
    ...
    fctype = ifc_psets.get_CivilCvCAD_type(ptype)
    CivilCvCAD.ActiveDocument.openTransaction(translate("BIM", "add property"))
    for obj in sel:
...
def on_add_pset():
    ...
    custompath = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "CustomPsets.csv")
    psetdefs = read_csv(psetpath)
    psetdefs.update(read_csv(custompath))
    psetkeys = list(psetdefs.keys())
    psetkeys.sort()
    form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogAddPSet.ui")
    # center the dialog over CivilCvCAD window
    form.move(mw.frameGeometry().topLeft() + mw.rect().center() - form.rect().center())
    form.field_pset.clear()
    ...
    if pset in existing_psets:
        ...
        CivilCvCAD.Console.PrintError(t + ": " + pset + "\n")
        return
    props = [psetdefs[pset][i : i + 2] for i in range(0, len(psetdefs[pset]), 2)]
    props = [[p[0], p[1]] for p in props]
    CivilCvCAD.ActiveDocument.openTransaction(translate("BIM", "add property set"))
    for obj in sel:
        existing_psets = list(set([obj.getGroupOfProperty(p) for p in obj.PropertiesList]))
        ...
        for prop in props:
            if prop[0] in obj.PropertiesList:
                ...
                CivilCvCAD.Console.PrintWarning(t + ": " + obj.Label + "," + prop[0] + "\n")
            else:
...
def set_menu(locked=False):
    ...
    mw = CivilCvCADGui.getMainWindow()
    wb = CivilCvCADGui.activeWorkbench()
    save_action = mw.findChild(QtGui.QAction, "Std_Save")
    if locked and "IFC_Save" in CivilCvCADGui.listCommands():
        if not hasattr(CivilCvCADGui, "IFC_WBManipulator"):
            CivilCvCADGui.IFC_WBManipulator = IFC_WBManipulator()
        # we need to void the shortcut otherwise it keeps active
        # even if the command is not shown
        CivilCvCADGui.IFC_saveshortcut = save_action.shortcut()
        save_action.setShortcut("")
        CivilCvCADGui.addWorkbenchManipulator(CivilCvCADGui.IFC_WBManipulator)
        wb.reloadActive()
    else:
        if hasattr(CivilCvCADGui, "IFC_saveshortcut"):
            save_action.setShortcut(CivilCvCADGui.IFC_saveshortcut)
            del CivilCvCADGui.IFC_saveshortcut
        if hasattr(CivilCvCADGui, "IFC_WBManipulator"):
            CivilCvCADGui.removeWorkbenchManipulator(CivilCvCADGui.IFC_WBManipulator)
            del CivilCvCADGui.IFC_WBManipulator
        wb.reloadActive()
...
# https://github.com/CivilCvCAD/CivilCvCAD/pull/10933
