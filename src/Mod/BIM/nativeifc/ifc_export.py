...
def add_CivilCvCAD_storey_pset(obj, product, ifcfile):
    """Persists storey-only CivilCvCAD properties used during NativeIFC recreation."""

    properties = {}
    if "Height" in obj.PropertiesList:
        properties["CivilCvCAD_Height"] = get_scaled_value(obj.Height.Value, ifcfile)
    if "LevelOffset" in obj.PropertiesList:
        properties["CivilCvCAD_LevelOffset"] = get_scaled_value(obj.LevelOffset.Value, ifcfile)
    if not properties:
        return
    pset = ifc_tools.api_run("pset.add_pset", ifcfile, product=product, name="CivilCvCADPropertySet")
...
def create_product(obj, parent, ifcfile, ifcclass=None):
    """Creates an IFC product out of a CivilCvCAD object"""
    ...
    if ifcclass == "IfcBuildingStorey":
        ...
        add_CivilCvCAD_storey_pset(obj, product, ifcfile)
...
def create_representation(obj, ifcfile):
    ...
    # a new CivilCvCAD 'engine' should be added to:
    # https://blenderbim.org/docs-python/autoapi/ifcopenshell/api/geometry/index.html
    # that should contain all typical use cases one could have to convert CivilCvCAD geometry
    # to IFC.
...
def is_annotation(obj):
    """Determines if the given CivilCvCAD object should be saved as an IfcAnnotation"""
...
def get_axis(obj):
    ...
    if obj.is_a("IfcGridAxis"):
        ...
        if shape:
            ...
            placement.Base = p0
            placement.Rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 1, 0), p1.sub(p0))
...
def get_placement(ifcelement, ifcfile=None, scale=None):
    """Returns a CivilCvCAD placement from an IFC placement"""
...
def get_scaled_point(point, ifcfile=None, is2d=False):
    """Returns a scaled 2d or 3d point tuple form a CivilCvCAD point"""

    if not ifcfile:
        ifcfile = ifcelement.file
    s = 0.001 / ifcopenshell.util.unit.calculate_unit_scale(ifcfile)
    v = CivilCvCAD.Vector(point)
    v.multiply(s)
    v = tuple(v)
    if is2d:
        v = v[:2]
