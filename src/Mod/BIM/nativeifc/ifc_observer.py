...
def add_observer():
    """Adds this observer to the running CivilCvCAD instance"""

    if not has_ifcopenshell(report=True):
        return

    CivilCvCAD.BIMobserver = ifc_observer()
    CivilCvCAD.addDocumentObserver(CivilCvCAD.BIMobserver)


def remove_observer():
    """Removes this observer if present"""

    if hasattr(CivilCvCAD, "BIMobserver"):
        CivilCvCAD.removeDocumentObserver(CivilCvCAD.BIMobserver)
        del CivilCvCAD.BIMobserver
...
class ifc_observer:
    ...
    def __init__(self):
        ...
        if CivilCvCAD.ActiveDocument:
            self.slotActivateDocument(CivilCvCAD.ActiveDocument)
    ...
    def save(self):
        ...
        if not hasattr(self, "docname"):
            return
        if self.docname not in CivilCvCAD.listDocuments():
            return
        doc = CivilCvCAD.getDocument(self.docname)
        del self.docname
        projects = []
        ...
        if projects:
            ...
            if ask and CivilCvCAD.GuiUp:
                ...
                dlg = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogExport.ui")
                result = dlg.exec_()
                if not result:
                    return
    ...
    def convert(self):
        ...
        if (
            ...
            CivilCvCAD.Console.PrintLog("Converting " + obj.Label + " to IFC\n")
