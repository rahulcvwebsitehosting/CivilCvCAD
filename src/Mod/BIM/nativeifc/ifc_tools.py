...
SCALE = 1000.0  # IfcOpenShell works in meters, CivilCvCAD works in mm
...
PARAMS = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/NativeIFC")
...
def create_document(document, filename=None, shapemode=0, strategy=0, silent=False):
    """Creates a IFC document object in the given CivilCvCAD document or converts that
...
def create_document_object(document, filename=None, shapemode=0, strategy=0, silent=False):
    """Creates a IFC document object in the given CivilCvCAD document.
...
def convert_document(document, filename=None, shapemode=0, strategy=0, silent=False):
    """Converts the given CivilCvCAD document to an IFC document.
...
def setup_project(proj, filename, shapemode, silent):
    ...
    # bug in CivilCvCAD - to avoid a crash, pre-populate the enum with one value
...
def create_ifcfile():
    ...
    param = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Document")
    ...
    application = "CivilCvCAD"
    version = CivilCvCAD.Version()
    version = ".".join([str(v) for v in version[0:3]])
    civilcvcadorg = api_run(
        "owner.add_organisation", ifcfile, identification="CivilCvCAD.org", name="The CivilCvCAD project"
    )
    application = api_run(
        "owner.add_application",
        ifcfile,
        application_developer=civilcvcadorg,
    ...
    # TODO allow to set this at creation, from the current CivilCvCAD units schema
...
def api_run(*args, **kwargs):
    ...
    if len(args) > 1:
        ifcfile = args[1]
        for d in CivilCvCAD.listDocuments().values():
            for o in d.Objects:
...
def create_object(ifcentity, document, ifcfile, shapemode=0, objecttype=None):
    """Creates a CivilCvCAD object from an IFC entity"""
...
def create_children(
    ...
    def create_child(parent, element):
        ...
        if element.id() not in [getattr(c, "StepId", 0) for c in get_parent_objects(parent)]:
            ...
            if isinstance(parent, CivilCvCAD.DocumentObject):
...
def assign_groups(children, ifcfile=None):
    """Fill the groups in this list. Returns a list of processed CivilCvCAD objects"""
...
def get_CivilCvCAD_children(obj):
    ...
    for child in children:
        ...
        if childobj:
            objs.extend(get_CivilCvCAD_children(childobj))
    return objs
...
def get_object(element, document=None, ifcfile=None):
    ...
    if document:
        ...
        ldocs = CivilCvCAD.listDocuments()
...
def get_ifcfile(obj):
    ...
    if getattr(project, "IfcFilePath", None):
        ...
        if filepath[0] == ".":
            # path relative to the CivilCvCAD file directory
        ...
        try:
            ...
            CivilCvCAD.Console.PrintError("Error: Cannot open IFC file: " + filepath + "\n")
            return None
        if hasattr(project, "Proxy"):
            if project.Proxy is None:
                if not isinstance(project, CivilCvCAD.DocumentObject):
...
def add_object(document, otype=None, oname="IfcObject"):
    """adds a new object to a CivilCvCAD document.
    otype can be:
    'project',
    'group',
    'material',
    ...
    if otype == "schedule":
        ...
        obj = Draft.make_dimension(CivilCvCAD.Vector(), CivilCvCAD.Vector(1, 0, 0))
        obj.Proxy = ifc_objects.ifc_object(otype)
...
def add_properties(obj, ifcfile=None, ifcentity=None, links=False, shapemode=0, short=SHORT):
    """Adds the properties of the given IFC object to a CivilCvCAD object"""
    ...
    if isinstance(obj, CivilCvCAD.DocumentObject) and "Group" not in obj.PropertiesList:
...
def restore_CivilCvCAD_property(obj, ifcentity, property_name, ifcfile, pset=None, scale=None):
    """Restores a CivilCvCAD property from the exporter's CivilCvCADPropertySet."""

    if property_name not in obj.PropertiesList:
        return False
    if not pset:
        pset = ifc_psets.get_pset("CivilCvCADPropertySet", ifcentity)
    if not pset:
        return False
    for prop in getattr(pset, "HasProperties", []) or []:
        if prop.Name != f"CivilCvCAD_{property_name}" or not getattr(prop, "NominalValue", None):
            continue
...
def restore_spatial_data(obj, ifcentity, ifcfile):
    ...
    if ifcentity.is_a("IfcBuildingStorey"):
        ...
        if (not placement) and ("Placement" in obj.PropertiesList) and (elevation is not None):
            restored = CivilCvCAD.Placement(obj.Placement)
            restored.Base.z = elevation * (1 / get_scale(ifcfile))
            obj.Placement = restored
        if "LevelOffset" in obj.PropertiesList:
            restore_CivilCvCAD_property(obj, ifcentity, "LevelOffset", ifcfile)
        if ("Height" in obj.PropertiesList) and not restore_CivilCvCAD_property(
        ...
        if not has_representation(elem):
            ...
            if elem.Representation.Representations:
                ...
                if rep.Items and rep.Items[0].is_a() == "IfcPolyline" and elem.IsDecomposedBy:
                    ...
                    # see https://github.com/yorikvanhavre/CivilCvCAD-NativeIFC/issues/28
...
def set_attribute(ifcfile, element, attribute, value):
    ...
    if hasattr(element, attribute):
        if attribute == "Name" and getattr(element, attribute) is None and value.startswith("_"):
            # do not consider default CivilCvCAD names given to unnamed alements
            return False
...
def set_colors(obj, colors):
    ...
    if CivilCvCAD.GuiUp and colors:
        try:
            vobj = obj.ViewObject
...
def get_CivilCvCAD_matrix(ios_matrix):
    """Converts an IfcOpenShell matrix tuple into a CivilCvCAD matrix"""
    ...
    for i in range(3):
        ...
        line[-1] *= SCALE
        m_l.extend(line)
    return CivilCvCAD.Matrix(*m_l)


def get_ios_matrix(m):
    """Converts a CivilCvCAD placement or matrix into an IfcOpenShell matrix tuple"""

    if isinstance(m, CivilCvCAD.Placement):
        m = m.Matrix
    mat = [
...
def set_placement(obj):
    ...
    if not hasattr(element, "ObjectPlacement"):
        ...
        return False
    placement = CivilCvCAD.Placement(obj.Placement)
    placement.Base = CivilCvCAD.Vector(placement.Base).multiply(get_scale(ifcfile))
...
def set_axis_points(obj, element, ifcfile):
    ...
    if element.AxisCurve.is_a("IfcPolyline"):
        ...
        p2 = obj.Placement.multVec(CivilCvCAD.Vector(0, obj.Length.Value, 0))
        api_run(
...
def save_ifc(obj, filepath=None):
    ...
    if filepath:
        ...
        CivilCvCAD.Console.PrintMessage("Saved " + filepath + "\n")
...
def aggregate(obj, parent, mode=None):
    """Takes any CivilCvCAD object and aggregates it to an existing IFC object.
    Mode can be 'opening' to force-create a subtraction"""

    proj = get_project(parent)
    if not proj:
        CivilCvCAD.Console.PrintError("The parent object is not part of an IFC project\n")
        return
    ...
    if CivilCvCAD.GuiUp:
        ...
        autogroup = getattr(getattr(CivilCvCADGui, "draftToolBar", None), "autogroup", None)
        if autogroup is not None:
            layer = CivilCvCAD.ActiveDocument.getObject(autogroup)
...
def deaggregate(obj, parent):
    """Removes a CivilCvCAD object form its parent"""
...
def get_subvolume(obj):
    ...
    if tempshape:
        if len(tempshape.Faces) == 6:
            ...
            tempobj.Dir = CivilCvCAD.Vector(ext).normalize()
...
def create_relationship(old_obj, obj, parent, element, ifcfile, mode=None):
    ...
    if isinstance(parent, (CivilCvCAD.DocumentObject, CivilCvCAD.Document)):
    ...
    if parent_element.is_a("IfcGroup"):
        ...
        # IFC objects can be part of multiple groups but we do the CivilCvCAD way here
    ...
    # case 1: element inside spatiual structure
        ...
        # first remove the CivilCvCAD object from any parent
        if old_obj:
...
def remove_ifc_element(obj, delete_obj=False):
    ...
    If delete_obj is True, the CivilCvCAD object is also deleted"""
...
def load_orphans(obj):
    ...
    if isinstance(obj, CivilCvCAD.DocumentObject):
        doc = obj.Document
    else:
        doc = obj
    ifcfile = get_ifcfile(obj)
    ...
    if rest:
        ...
        if isinstance(project, CivilCvCAD.DocumentObject):
            for o in rest:
