...
__title__ = "CivilCvCAD Arch Truss"
...
class Truss(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not "Normal" in pl:
            ...
            obj.Normal = CivilCvCAD.Vector(0, 0, 1)
    ...
    def execute(self, obj):
        ...
        if not obj.StrutWidth.Value or not obj.StrutHeight.Value:
            CivilCvCAD.Console.PrintLog(obj.Label + ": Invalid strut size\n")
            return
        if not obj.HeightStart.Value or not obj.HeightEnd.Value:
            CivilCvCAD.Console.PrintLog(obj.Label + ": Invalid truss heights\n")
            return
        if not obj.RodSections or not obj.RodSize.Value:
            CivilCvCAD.Console.PrintLog(obj.Label + ": Invalid rod config\n")
            return
        if not obj.Base:
            CivilCvCAD.Console.PrintLog(obj.Label + ": No base\n")
            return
        if (not hasattr(obj.Base, "Shape")) or (len(obj.Base.Shape.Vertexes) > 2):
            CivilCvCAD.Console.PrintLog(obj.Label + ": No base edge\n")
            return
    ...
    def makeTruss(self, obj, v0, v1):
        ...
        if not sidedir.Length:
            CivilCvCAD.Console.PrintLog(obj.Label + ": normal and base are parallel\n")
            return
        sidedir.normalize()
        p0 = v0.add(CivilCvCAD.Vector(sidedir).negative().multiply(obj.StrutWidth.Value / 2))
        p1 = p0.add(CivilCvCAD.Vector(sidedir).multiply(obj.StrutWidth.Value / 2).multiply(2))
        p2 = p1.add(CivilCvCAD.Vector(normal).multiply(obj.StrutHeight))
        p3 = p0.add(CivilCvCAD.Vector(normal).multiply(obj.StrutHeight))
        ...
        # create top strut
        v2 = v0.add(CivilCvCAD.Vector(normal).multiply(obj.HeightStart.Value))
        v3 = v1.add(CivilCvCAD.Vector(normal).multiply(obj.HeightEnd.Value))
        topdir = v3.sub(v2)
        ...
        topstrut.translate(CivilCvCAD.Vector(normal).multiply(-obj.StrutHeight.Value))
        ...
        if obj.RodType == "Round":
            ...
            rodprofile = Part.Face(
                Part.makePlane(
                    obj.RodSize, obj.RodSize, CivilCvCAD.Vector(-obj.RodSize / 2, -obj.RodSize / 2, 0)
                )
            )

        # create rods
        rods = []
        bottomrodstart = v0.add(CivilCvCAD.Vector(normal).multiply(obj.StrutHeight.Value / 2))
        toprodstart = v2.add(CivilCvCAD.Vector(normal).multiply(obj.StrutHeight.Value / 2).negative())
        bottomrodvec = CivilCvCAD.Vector(maindir).multiply(1 / obj.RodSections)
        toprodvec = CivilCvCAD.Vector(topdir).multiply(1 / obj.RodSections)
    ...
    def makeRod(self, profile, p1, p2):
        ...
        rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), p2.sub(p1))
