...
class Nester:
    ...
    def addObjects(self, objects):
        """addObjects(objects): adds CivilCvCAD DocumentObjects to the nester"""
    ...
    def addContainer(self, container):
        """addContainer(object): adds a CivilCvCAD DocumentObject as the container"""
    ...
    def run(self):
        ...
        while faces:
            ...
            print(
                ...
                facesnumber,
                "Area:",
                CivilCvCAD.Units.Quantity(faces[-1][1].Area, CivilCvCAD.Units.Area).getUserPreferred()[0],
                ": ",
                end="",
            )
            ...
            for rotation in ROTATIONS:
                ...
                v2 = v1.add(CivilCvCAD.Vector(0, boc.YLength - bof.YLength, 0))
                v3 = v2.add(CivilCvCAD.Vector(boc.XLength - bof.XLength, 0, 0))
                v4 = v3.add(CivilCvCAD.Vector(0, -(boc.YLength - bof.YLength), 0))
    ...
    def order(self, face, right=False):
        ...
        for v in verts:
            ...
            if vy.getAngle(wp.v) > 1:
                ly = -ly
            pverts.append(CivilCvCAD.Vector(lx, ly, 0))
    ...
    def show(self, result=None):
        ...
        offset = CivilCvCAD.Vector(0, 0, 0)
        feats = []
        for sheet in result:
            ...
            o = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "Nest")
            o.Shape = comp
            feats.append(o)
            offset = offset.add(CivilCvCAD.Vector(1.1 * self.container.BoundBox.XLength, 0, 0))
    ...
    def getPlacements(self, result=None):
        ...
        for sheet in result:
            for face in sheet:
                ...
                pla = CivilCvCAD.Placement(deltav, rot)
                d[face[0]] = pla
            offset = offset.add(CivilCvCAD.Vector(1.1 * self.container.BoundBox.XLength, 0, 0))
        return d
