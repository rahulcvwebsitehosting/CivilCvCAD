...
__title__ = "CivilCvCAD Arch Curtain Wall"
...
class CurtainWall(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not "VerticalDirection" in pl:
            ...
            obj.VerticalDirection = CivilCvCAD.Vector(0, 0, 1)
    ...
    def execute(self, obj):
        ...
        if not hasattr(obj.Base, "Shape"):
            CivilCvCAD.Console.PrintLog(obj.Label + ": invalid base\n")
            return
        if obj.VerticalMullionProfile:
            if not hasattr(obj.VerticalMullionProfile, "Shape"):
                CivilCvCAD.Console.PrintLog(obj.Label + ": invalid vertical mullion profile\n")
                return
        if obj.HorizontalMullionProfile:
            if not hasattr(obj.HorizontalMullionProfile, "Shape"):
                CivilCvCAD.Console.PrintLog(obj.Label + ": invalid horizontal mullion profile\n")
                return
        if obj.DiagonalMullionProfile:
            if not hasattr(obj.DiagonalMullionProfile, "Shape"):
                CivilCvCAD.Console.PrintLog(obj.Label + ": invalid diagonal mullion profile\n")
                return
        ...
        if obj.Base.Shape.Faces:
            ...
            ext = CivilCvCAD.Vector(obj.VerticalDirection)
            ext.normalize()
        ...
        if not faces:
            CivilCvCAD.Console.PrintLog(obj.Label + ": unable to build base faces\n")
            return
        ...
        for face in faces:
            ...
            if face_plane:
                if (
                    ...
                    # https://github.com/CivilCvCAD/CivilCvCAD/issues/21866
        ...
        if not facets:
            CivilCvCAD.Console.PrintLog(obj.Label + ": failed to subdivide shape\n")
            return
        ...
        if vprofile and vertsec:
            for vedge in vedges:
                ...
                if (vn.x != 0) or (vn.y != 0):
                    avn = CivilCvCAD.Vector(vn.x, vn.y, 0)
                    rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, -1, 0), avn)
                else:
                    rot = CivilCvCAD.Rotation()
                if obj.VerticalMullionAlignment:
                    ev = vedge.Vertexes[-1].Point.sub(vedge.Vertexes[0].Point)
                    rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), ev).multiply(rot)
        ...
        if hprofile and horizsec:
            for hedge in hedges:
                rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 1, 0), -90)
                vn = self.edgenormals[hedge.hashCode()]
                if (vn.x != 0) or (vn.y != 0):
                    avn = CivilCvCAD.Vector(vn.x, vn.y, 0)
                    rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, -1, 0), avn).multiply(rot)
                    if obj.HorizontalMullionAlignment:
                        rot = CivilCvCAD.Rotation(avn, vn).multiply(rot)
    ...
    def makeMullion(self, edge, profile, rotation, recenter=False):
        ...
        center = CivilCvCAD.Vector(0, 0, 0)
        if recenter:
    ...
    def getMullionProfile(self, obj, direction):
        ...
        if prof:
            ...
            if (not proh) or (not prow):
                return None
            profile = Part.Face(Part.makePlane(prow, proh, CivilCvCAD.Vector(-prow / 2, -proh / 2, 0)))
        return profile
