...
if CivilCvCAD.GuiUp:
    # Runtime override using native CivilCvCAD.Qt abstraction
...
class _Covering(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        - Defaults are applied immediately after adding a property, so they too only take effect
          on fresh creation.
        - Editor modes are re-applied unconditionally in both cases because CivilCvCAD does not save
    ...
    def onDocumentRestored(self, obj):
        ...
        `doc.removeObject()` is not safe to call directly from `onDocumentRestored`: CivilCvCAD calls
...
def build_tiling_frame(obj, persist_reference=False):
    ...
    rotation = CivilCvCAD.Rotation(normal, obj.Rotation.Value)
...
def apply_setback(base_face, border_setback):
    ...
    try:
        ...
        CivilCvCAD.Console.PrintWarning(_setback_too_large + "\n")
        return base_face
...
def get_tile_grid_origin(
    ...
    center_point : CivilCvCAD.Vector
        Face center in world space, as returned by ``getFaceFrame``.
    u_vec, v_vec : CivilCvCAD.Vector
    ...
    offset : CivilCvCAD.Vector or None
