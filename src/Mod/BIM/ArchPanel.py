...
class _Panel(ArchComponent.Component):
    ...
    def execute(self, obj):
        ...
        if obj.Base:
            ...
            if not base.Solids:
                # p = CivilCvCAD.Placement(obj.Base.Placement)
                if base.Faces:
                    baseprofile = base
                    if not normal:
        ...
        if hasattr(obj, "WaveLength"):
            if baseprofile and obj.WaveLength.Value and obj.WaveHeight.Value:
                ...
                CivilCvCAD.upwire = upwire  # REMOVE
                if isinstance(downsegment, Part.Shape):
                    downwire = Part.Wire(downedges)
                    CivilCvCAD.downwire = downwire  # REMOVE
                    e1 = Part.LineSegment(
                ...
                CivilCvCAD.basewire = basewire
                if not basewire.isClosed():
                    print("Error closing base wire - check CivilCvCAD.basewire")
                    return

                baseface = Part.Face(basewire)
                base = baseface.extrude(Vector(0, bb.YLength, 0))
                if normal.cross(CivilCvCAD.Vector(0, 0, 1)).Length > 1e-6:
                    rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, -1, 0), CivilCvCAD.Vector(*normal[:2], 0))
                    base.rotate(bb.Center, rot.Axis, math.degrees(rot.Angle))
                rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), normal)
                base.rotate(bb.Center, rot.Axis, math.degrees(rot.Angle))
        ...
        if base and (obj.Sheets > 1) and normal and thickness:
            ...
            for i in range(1, obj.Sheets):
                n = CivilCvCAD.Vector(normal).normalize().multiply(i * thickness)
                b = base.copy()
                b.translate(n)
                bases.append(b)
...
class _ViewProviderPanel(ArchComponent.ViewProviderComponent):
    ...
    def setEdit(self, vobj, mode):
        if mode != 0:
            return None
        CivilCvCADGui.Control.showDialog(PanelTaskPanel(vobj.Object))
        return True
                ...
                if baseobj:
                    if hasattr(baseobj, "Shape"):
                        ...
                        if base and n:
                            ...
                            r = CivilCvCAD.Rotation(n, Vector(0, 0, 1))
    ...
    def execute(self, obj):
        ...
        if obj.Width.Value and obj.Height.Value:
            ...
            if obj.FontFile and obj.TagText and obj.TagSize.Value:
                ...
                base = Part.Compound([base, textshape])
            base.scale(obj.Scale, CivilCvCAD.Vector())
            obj.Shape = base
            obj.Placement = pl
        ...
        for p in obj.Group:
            ...
            if hasattr(p, "Proxy"):
                if hasattr(p.Proxy, "getWires"):
                    ...
                    if w[0]:
                        w = w[0]
                        w.scale(obj.Scale, CivilCvCAD.Vector())
                        if transform:
    ...
    def updateData(self, obj, prop):
        ...
        if prop in ["Width", "Height"]:
            ...
            if hasattr(self, "texcoords"):
                if self.texcoords:
                    s = CivilCvCAD.Vector(self.texcoords.directionS.getValue().getValue()).Length
                    vS = DraftVecUtils.rotate(
                        CivilCvCAD.Vector(s, 0, 0), -math.radians(obj.GrainDirection.Value)
                    )
                    vT = DraftVecUtils.rotate(
                        CivilCvCAD.Vector(0, s, 0), -math.radians(obj.GrainDirection.Value)
                    )
