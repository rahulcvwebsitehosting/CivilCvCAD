...
class BIM_IfcExplorer:
    ...
    def Activated(self):
        ...
        toolbar = CivilCvCADGui.UiLoader().createWidget("Gui::ToolBar")
    ...
    def open(self):
        ...
        self.filename = ""
        lastfolder = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/BIM").GetString(
        ...
        if filename and filename[0]:
            ...
            if not os.path.exists(self.filename):
                CivilCvCAD.Console.PrintError(translate("BIM", "File not found") + "\n")
                return
        else:
            return
    ...
    def close(self):
        ...
        if CivilCvCAD.ActiveDocument:
            if getattr(self, "mesh", None):
                CivilCvCAD.ActiveDocument.removeObject(self.mesh.Name)
            if getattr(self, "currentmesh", None):
                CivilCvCAD.ActiveDocument.removeObject(self.currentmesh.Name)
                ...
                if self.mesh:
                    ...
                    for product in self.products:
                        try:
                            ...
                            v = g.verts
                            f = g.faces
                            verts = [CivilCvCAD.Vector(v[i : i + 3]) for i in range(0, len(v), 3)]
                            ...
                            pass
                    self.mesh = CivilCvCAD.ActiveDocument.addObject("Mesh::Feature", "IFCMesh")
                    ...
                    CivilCvCADGui.Selection.addSelection(self.mesh)
    ...
    def onSelectTree(self, item, previous):
        ...
        entity = self.ifc[eid]
        if entity.is_a("IfcProduct") and CivilCvCAD.ActiveDocument:
        ...
        if omesh:
            if not self.currentmesh:
                self.currentmesh = CivilCvCAD.ActiveDocument.addObject(
...
CivilCvCADGui.addCommand("BIM_IfcExplorer", BIM_IfcExplorer())
