...
class BIM_Text:
    ...
    def IsActive(self):
        act_win = CivilCvCADGui.getMainWindow().getActiveWindow()
    ...
    def Activated(self):
        ...
        if hasattr(self.view, "getPage") and self.view.getPage():
            ...
            CivilCvCADGui.draftToolBar.taskUi()
            CivilCvCADGui.draftToolBar.textUi()
        else:
            CivilCvCADGui.runCommand("Draft_Text")
    ...
    def createObject(self):
        ...
        if self.text:
            ...
            if not pagescale:
                pagescale = 1
            anno = CivilCvCAD.ActiveDocument.addObject("TechDraw::DrawViewAnnotation", "Annotation")
            anno.Text = self.text
            page.addView(anno)
            param = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft")
...
CivilCvCADGui.addCommand("BIM_Text", BIM_Text())
