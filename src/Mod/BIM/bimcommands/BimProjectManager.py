...
def _parse_vector(text):
    ...
    return CivilCvCAD.Vector(float(match.group(1)), float(match.group(2)), float(match.group(3)))
...
class BIM_ProjectManager:
    ...
    def Activated(self):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogProjectManager.ui")
        self.project = None
        self.site = None
        self.building = None
    ...
    def accept(self):
        ...
        # reading form values
        buildingWidth = CivilCvCAD.Units.Quantity(self.form.buildingWidth.text()).Value
        buildingLength = CivilCvCAD.Units.Quantity(self.form.buildingLength.text()).Value
        distVAxes = CivilCvCAD.Units.Quantity(self.form.distVAxes.text()).Value
        distHAxes = CivilCvCAD.Units.Quantity(self.form.distHAxes.text()).Value
        levelHeight = CivilCvCAD.Units.Quantity(self.form.levelHeight.text()).Value
        ...
        if doc:
            CivilCvCAD.setActiveDocument(doc.Name)
        ...
        if self.form.addHumanFigure.isChecked():
            ...
            human = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "Human")
            human.Shape = Part.makePolygon(pts)
            human.Placement.move(CivilCvCAD.Vector(500, 500, 0))
        ...
        if self.form.groupSite.isChecked():
            ...
            elev = CivilCvCAD.Units.Quantity(self.form.siteElevation.text()).Value
            if hasattr(self.site, "Elevation"):
        ...
        if self.form.groupBuilding.isChecked():
            ...
            if self.form.countHAxes.value() and distHAxes:
                ...
                axisH.Placement.Rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), 90)
            ...
            if self.form.countLevels.value() and levelHeight:
                ...
                for i in range(self.form.countLevels.value()):
                    ...
                    lev.Placement.move(CivilCvCAD.Vector(0, 0, h))
                    ...
                    h += levelHeight
                    for group in groups:
                        levGroup = CivilCvCAD.ActiveDocument.addObject("App::DocumentObjectGroup")
        ...
        if outline:
            ...
            CivilCvCADGui.Selection.addSelection(outline)
    ...
    def savePreset(self):
        ...
        if res[1]:
            name = res[0]
            presetdir = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM")
            ...
            preset = f"# CivilCvCAD BIM Project setup preset { name }\n"
    ...
    def getPreset(self, preset):
        ...
        pfile = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", preset + ".txt")
    ...
    def saveTemplate(self):
        ...
        d = CivilCvCAD.ActiveDocument
        if not d:
            d = CivilCvCAD.newDocument()
        ...
        values["unit"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt("UserSchema", 0)
        )
        values["textsize"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetFloat(
                "textheight", 10
            )
        )
        values["textfont"] = str(
        ...
        values["dimstyle"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetInt("dimsymbol", 0)
        )
        ...
        values["decimals"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt("Decimals", 2)
        )
        values["grid"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetFloat(
                "gridSpacing", 10
        ...
        values["squares"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetInt("gridEvery", 10)
        )
        values["linewidth"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetInt(
                "DefautShapeLineWidth", 2
            )
        )
        values["colFace"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned(
        ...
        values["colHelp"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetUnsigned(
                "ColorHelpers", 674321151
            )
        )
        values["colConst"] = str(
            CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Draft").GetUnsigned(
        ...
        if filename:
            ...
            if not filename.lower().endswith(".FCStd"):
                filename += ".FCStd"
            d.saveCopy(filename)
            CivilCvCAD.Console.PrintMessage(translate("BIM", "Template saved successfully") + "\n")
            self.reject()
    ...
    def loadTemplate(self):
        ...
        if filename:
            ...
            if CivilCvCAD.ActiveDocument:
                d = CivilCvCAD.ActiveDocument
                td = CivilCvCAD.openDocument(filename, True)  # hidden
                tname = td.Name
                values = td.Meta
                CivilCvCAD.closeDocument(tname)
                d.mergeProject(filename)
                CivilCvCADGui.ActiveDocument = CivilCvCADGui.getDocument(d.Name)  # fix b/c hidden doc
            else:
                d = CivilCvCAD.openDocument(filename)
                CivilCvCAD.ActiveDocument = d
                values = d.Meta
            bimunit = 0
            wp = WorkingPlane.get_working_plane()
            for key, attr in [
            ...
            if "unit" in values:
                CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Units").SetInt(
                    "UserSchema", int(values["unit"])
                )
                if hasattr(CivilCvCAD.Units, "setSchema"):
                    CivilCvCAD.Units.setSchema(int(values["unit"]))
            ...
            if "textsize" in values:
                ...
                if hasattr(CivilCvCADGui, "draftToolBar"):
                    CivilCvCADGui.draftToolBar.fontsizeButton.setValue(float(values["textsize"]))
            ...
            if "linewidth" in values:
                ...
                if hasattr(CivilCvCADGui, "draftToolBar"):
                    CivilCvCADGui.draftToolBar.widthButton.setValue(int(values["linewidth"]))
...
CivilCvCADGui.addCommand("BIM_ProjectManager", BIM_ProjectManager())
