...
class BIM_Material:
    ...
    def Activated(self):
        ...
        if matList.count():
            ...
            self.onReject()
            CivilCvCADGui.runCommand("Arch_Material")
                    ...
                    for par in mat.InList:
                        for prop in par.PropertiesList:
                            if getattr(par, prop) == mat:
                                ...
                                if first:
                                    CivilCvCAD.ActiveDocument.openTransaction("Merge materials")
                                    first = False
            ...
            for tod in todelete:
                if not tod.InList:
                    ...
                    if first:
                        CivilCvCAD.ActiveDocument.openTransaction("Merge materials")
                        first = False
                    CivilCvCAD.ActiveDocument.removeObject(tod.Name)
    ...
    def onDeleteUnused(self):
        ...
        if self.dlg:
            for i in range(self.dlg.matList.count()):
                ...
                if item:
                    obj = CivilCvCAD.ActiveDocument.getObject(item.toolTip())
                    if obj:
                        parents = [
                            parent
                        ...
                        if not parents:
                            ...
                            label = obj.Label
                            if first:
                                CivilCvCAD.ActiveDocument.openTransaction("Delete materials")
                                first = False
                            CivilCvCAD.Console.PrintMessage(
    ...
    def onDuplicate(self):
        if self.dlg:
            ...
            if item:
                oldmat = CivilCvCAD.ActiveDocument.getObject(item.toolTip())
                if oldmat:
                    import Arch
    ...
    def onMergeTo(self):
        if self.dlg:
            ...
            if item:
                oldmat = CivilCvCAD.ActiveDocument.getObject(item.toolTip())
                # load dialog
                form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogListWidget.ui")
                ...
                if result:
                    ...
                    if mergeto:
                        mergemat = CivilCvCAD.ActiveDocument.getObject(mergeto.toolTip())
                        if oldmat and mergemat:
                            ...
                            name = oldmat.Name
                            CivilCvCAD.ActiveDocument.openTransaction("Merge material")
            ...
            if item:
                ...
                if obj:
                    ...
                    if parents:
                        ...
                        name = obj.Name
                        CivilCvCAD.ActiveDocument.openTransaction("Delete material")
                        CivilCvCAD.ActiveDocument.removeObject(name)
                        CivilCvCAD.ActiveDocument.commitTransaction()
                        CivilCvCAD.ActiveDocument.recompute()
                        self.rescan()
    ...
    def onMulti(self):
        self.onReject()
        CivilCvCADGui.runCommand("Arch_MultiMaterial")

    def onAccept(self, item=None):
        if self.dlg:
            item = self.dlg.matList.currentItem()
            if item and self.dlg.objects:
                mat = CivilCvCAD.ActiveDocument.getObject(item.toolTip())
                if mat:
                    if self.dlg.objects:
                        CivilCvCAD.ActiveDocument.openTransaction("Change material")
                        ...
                        CivilCvCAD.ActiveDocument.commitTransaction()
                        CivilCvCAD.ActiveDocument.recompute()
            p = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/BIM")
...
class Arch_Material:
    ...
    def Activated(self):
        ...
        CivilCvCAD.ActiveDocument.openTransaction(translate("Arch", "Create material"))
        CivilCvCADGui.addModule("Arch")
        CivilCvCADGui.Control.closeDialog()
        CivilCvCADGui.doCommand("mat = Arch.makeMaterial()")
        for obj in sel:
            if hasattr(obj, "Material") and hasattr(obj, "MoveWithHost"):  # 'isComponent' check
                CivilCvCADGui.doCommand(
                    'CivilCvCAD.ActiveDocument.getObject("' + obj.Name + '").Material = mat'
                )
        CivilCvCADGui.doCommandGui("mat.ViewObject.Document.setEdit(mat.ViewObject, 0)")
        CivilCvCAD.ActiveDocument.commitTransaction()
        CivilCvCAD.ActiveDocument.recompute()
...
class Arch_MultiMaterial:
    ...
    def Activated(self):
        ...
        sel = CivilCvCADGui.Selection.getSelection()
        CivilCvCAD.ActiveDocument.openTransaction(translate("Arch", "Create multi-material"))
        CivilCvCADGui.addModule("Arch")
        CivilCvCADGui.Control.closeDialog()
        CivilCvCADGui.doCommand("mat = Arch.makeMultiMaterial()")
        for obj in sel:
            if hasattr(obj, "Material"):
                if not obj.isDerivedFrom("App::MaterialObject"):
                    CivilCvCADGui.doCommand("CivilCvCAD.ActiveDocument." + obj.Name + ".Material = mat")
        CivilCvCADGui.doCommandGui("mat.ViewObject.Document.setEdit(mat.ViewObject, 0)")
        CivilCvCAD.ActiveDocument.commitTransaction()
        CivilCvCAD.ActiveDocument.recompute()
...
CivilCvCADGui.addCommand("BIM_Material", BIM_Material())
CivilCvCADGui.addCommand("Arch_Material", Arch_Material())
CivilCvCADGui.addCommand("Arch_MultiMaterial", Arch_MultiMaterial())
CivilCvCADGui.addCommand("Arch_MaterialTools", Arch_MaterialToolsCommand())
