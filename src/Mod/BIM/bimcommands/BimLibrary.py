...
TEMPLIBPATH = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "OfflineLibrary")
...
# CivilCvCAD.Console.PrintMessage() or CivilCvCAD.Console.PrintWarning() or
# CivilCvCAD.Console.PrintError() and the text should be placed in a translate()
# function and "\n" should be added to it.
# Example CivilCvCAD.Console.PrintError(translate("BIM","Please save the document first")+"\n")
...
class BIM_Library:
    ...
    def Activated(self):
        ...
        pr = CivilCvCAD.ParamGet("User parameter:Plugins/parts_library")
        libok = False
        ...
        if self.librarypath:
            ...
            addondir = os.path.join(CivilCvCAD.getUserAppDataDir(), "Mod", "parts_library")
...
class BIM_Library_TaskPanel:
    ...
    def __init__(self, offlinemode=False):
        ...
        self.mainDocName = CivilCvCAD.Gui.ActiveDocument.Document.Name
        self.previewDocName = "Viewer"

        self.linked = False

        self.librarypath = CivilCvCAD.ParamGet("User parameter:Plugins/parts_library").GetString(
            "destination", ""
        )
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogLibrary.ui")
        ...
        # External catalog search is removed from the offline distribution.
        self.form.comboSearch.hide()

        # retrieve preferences
        PARAMS.SetBool("LibraryOnline", False)
        self.form.checkOnline.setChecked(False)
        self.form.checkOnline.setEnabled(False)
        self.form.checkOnline.hide()
    ...
    def onItemSelected(self, selected, deselected):
        ...
        if False:
            ...
            if self.previewOn == True or self.linked == True:
                if self.isFile == True:
                    ...
                    if self.linked == False:
                        try:
                            CivilCvCAD.closeDocument(self.previewDocName)
                        except:
                            pass
                    ...
                    if (
                        ...
                        self.previewDocName = "Viewer"
                        CivilCvCAD.newDocument(self.previewDocName)
                        CivilCvCAD.setActiveDocument(self.previewDocName)
                        Part.show(Part.read(self.path))
                        CivilCvCADGui.SendMsgToActiveView("ViewFit")
                    elif self.path.lower().endswith(".fcstd"):
                        openedDoc = CivilCvCAD.openDocument(self.path)
                        CivilCvCADGui.SendMsgToActiveView("ViewFit")
                        self.previewDocName = CivilCvCAD.ActiveDocument.Name
                        thumbnailSave = PARAMS.GetBool("SaveThumbnails", False)
                        if thumbnailSave == True:
                            CivilCvCAD.ActiveDocument.save()
            if self.linked == False:
            ...
            if self.path.lower().endswith(".fcstd"):
                ...
                # check for meta-file if it's really a CivilCvCAD document
    ...
    def link(self, index):
        ...
        try:
            ...
            if CivilCvCAD.getDocument(self.mainDocName).FileName == "":
                ...
                if self.fileDialog[0] != "":
                    ...
                    for obj in CivilCvCAD.ActiveDocument.RootObjects:
                    ...
                    for obj in objects:
                        try:
                            link = (
                                CivilCvCAD.getDocument(self.mainDocName)
                                .addObject("App::Link", "Link")
                                .setLink(obj)
                            )
                            # CivilCvCAD.getDocument(self.mainDocName).getObject('Link').Label=CivilCvCAD.ActiveDocument.ActiveObject.Label
                            CivilCvCAD.getDocument(self.mainDocName).getObject(
                                link
                            ).Label = CivilCvCAD.ActiveDocument.ActiveObject.Label
                        except:
                            pass
                    CivilCvCAD.setActiveDocument(self.mainDocName)
                    self.librarypath = CivilCvCAD.ParamGet(
    ...
    def addtolibrary(self):
        ...
        if self.fileDialog[0] != "":
            ...
            CivilCvCAD.ActiveDocument.saveAs(FCfilename)
            if self.stepCB.isChecked() or self.stlCB.isChecked():
                toexport = []
                objs = CivilCvCAD.ActiveDocument.Objects
                for obj in objs:
        ...
        def add_line(f, dp, sep):
            if self.isAllowed(f) and (text.lower() in f.lower()):
                ...
                if f.lower().endswith(".fcstd"):
                    it.setIcon(QtGui.QIcon(":icons/civilcvcad-doc.png"))
    ...
    def reject(self):
        ...
        if self.previewDocName in CivilCvCAD.listDocuments():
            CivilCvCAD.closeDocument(self.previewDocName)
    ...
    def insert(self, index=None):
        ...
        before = CivilCvCAD.ActiveDocument.Objects
        self.name = os.path.splitext(os.path.basename(path))[0]
        ext = os.path.splitext(path.lower())[1]
        if ext in [".stp", ".step", ".brp", ".brep"]:
            self.place(path)
        elif ext == ".fcstd":
            CivilCvCADGui.ActiveDocument.mergeProject(path)
            ...
            importIFC.insert(path, CivilCvCAD.ActiveDocument.Name)
            from draftutils import todo
            ...
            try:
                ...
                try:
                    ...
                    path = CadExchangerIO.insert(path, CivilCvCAD.ActiveDocument.Name, returnpath=True)
                    self.place(path)
            else:
                path = importerIL.insert(path, CivilCvCAD.ActiveDocument.Name)
        CivilCvCADGui.Selection.clearSelection()
        for o in CivilCvCAD.ActiveDocument.Objects:
            if not o in before:
                CivilCvCADGui.Selection.addSelection(o)
        CivilCvCADGui.SendMsgToActiveView("ViewSelection")

    def download(self, url):
        del url
        from civilcvcad.offline import reject_network

        reject_network("BIM library download")
        ...
        return (
            [
                CivilCvCADGui.InputHint(
                    translate("BIM", "%1 pick insertion point"), CivilCvCADGui.UserInput.MouseLeft
                )
            ]
    ...
    def getDelta(self):
        ...
        d = CivilCvCAD.Vector(-self.shape.BoundBox.Center.x, -self.shape.BoundBox.Center.y, 0)
        idx = self.origin.comboOrigin.currentIndex()
        if idx <= 0:
            return CivilCvCAD.Vector()
        elif idx == 1:
            return d.add(
                CivilCvCAD.Vector(self.shape.BoundBox.XLength / 2, -self.shape.BoundBox.YLength / 2, 0)
            )
        elif idx == 2:
            return d.add(CivilCvCAD.Vector(0, -self.shape.BoundBox.YLength / 2, 0))
        elif idx == 3:
            return d.add(
                CivilCvCAD.Vector(
            ...
            return d.add(CivilCvCAD.Vector(self.shape.BoundBox.XLength / 2, 0, 0))
        elif idx == 5:
            return d
        elif idx == 6:
            return d.add(CivilCvCAD.Vector(-self.shape.BoundBox.XLength / 2, 0, 0))
    ...
    def onCheckOnline(self, state=None):
        """Keep the library in local-files-only mode."""
    ...
    def onRefresh(self):
        """Refresh the local library tree."""

        CivilCvCAD.Console.PrintWarning(
            "Online library refresh is disabled in this offline-only CivilCvCAD build.\n"
        )
        self.setFileModel()
    ...
    def onCheck3DPreview(self, state):
        ...
        if self.previewOn == True:
            self.previewDocName = "Viewer"
            self.doc = CivilCvCAD.newDocument(self.previewDocName)
            CivilCvCADGui.ActiveDocument.ActiveView.viewIsometric()
...
if CivilCvCAD.GuiUp:
    ...
    class LibraryModel(QtGui.QFileSystemModel):
        "a custom QFileSystemModel that displays CivilCvCAD file icons"
        ...
        def data(self, index, role):
            ...
            if index.column() == 0 and role == QtCore.Qt.DecorationRole:
                if index.data().lower().endswith(".fcstd"):
                    return QtGui.QIcon(":icons/civilcvcad-doc.png")
...
CivilCvCADGui.addCommand("BIM_Library", BIM_Library())
