...
class BIM_Windows:
    ...
    def Activated(self):
        CivilCvCADGui.Control.showDialog(BIM_Windows_TaskPanel())
...
class BIM_Windows_TaskPanel:
    ...
    def __init__(self):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogWindows.ui")
    ...
    def update(self, index=None):
        ...
        windows = [o for o in CivilCvCAD.ActiveDocument.Objects if Draft.getType(o) == "Window"]
    ...
    def editWindow(self, item, column):
        ...
        if len(self.form.windows.selectedItems()) == 1:
            ...
            window = CivilCvCAD.ActiveDocument.getObject(item.toolTip(0))
            if window:
        ...
        for item in self.form.windows.selectedItems():
            o = CivilCvCAD.ActiveDocument.getObject(item.toolTip(0))
            if o:
    ...
    def showWindow(self, item, column):
        ...
        if window:
            CivilCvCADGui.Selection.clearSelection()
            CivilCvCADGui.Selection.addSelection(window)
            CivilCvCADGui.SendMsgToActiveView("ViewSelection")

    def setWidth(self):
        val = CivilCvCAD.Units.Quantity(self.form.windowWidth.text()).Value
        if val:
            for it in self.form.windows.selectedItems():
                window = CivilCvCAD.ActiveDocument.getObject(it.toolTip(0))
                if window:
                    window.Width = val
            self.update()

    def setHeight(self):
        val = CivilCvCAD.Units.Quantity(self.form.windowHeight.text()).Value
        if val:
            for it in self.form.windows.selectedItems():
                window = CivilCvCAD.ActiveDocument.getObject(it.toolTip(0))
                if window:
    ...
    def setMaterial(self):
        ...
        form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogMaterialChooser.ui")
        mw = CivilCvCADGui.getMainWindow()
        form.move(mw.frameGeometry().topLeft() + mw.rect().center() - form.rect().center())
        materials = [o for o in CivilCvCAD.ActiveDocument.Objects if Draft.getType(o) == "Material"]
        ...
        if result:
            ...
            if sel:
                ...
                if sel.toolTip() != "__None__":
                    mat = CivilCvCAD.ActiveDocument.getObject(str(sel.toolTip()))
                for it in self.form.windows.selectedItems():
                    window = CivilCvCAD.ActiveDocument.getObject(it.toolTip(0))
                    if window:
                        if mat:
...
CivilCvCADGui.addCommand("BIM_Windows", BIM_Windows())
