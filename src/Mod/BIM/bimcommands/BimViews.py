...
class BIM_Views:
    ...
    def Activated(self):
        ...
        if vm:
            ...
            # create the dialog
            self.dialog = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogViews.ui")
            ...
            # delay connecting after CivilCvCAD finishes setting up
    ...
    def onClose(self, event):
        ...
        st = CivilCvCADGui.getMainWindow().statusBar()
        statuswidget = st.findChild(QtGui.QToolBar, "BIMStatusWidget")
        if statuswidget and hasattr(statuswidget, "bimviewsbutton"):
    ...
    def update(self, retrigger=True):
        ...
        if vm and vm.isVisible():
            if CivilCvCAD.isRestoring() or not CivilCvCAD.ActiveDocument:
            ...
            if allItemsInTrees:
                ...
                objSelected = CivilCvCADGui.Selection.getSelection()
                objNameSelected = [obj.Name for obj in objSelected]
                objActive = CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIFC")
                if not objActive:
                    objActive = CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("Arch")
                ...
                for item in allItemsInTrees:
                    ...
                    if objActive and item.toolTip(0) == objActive.Name:
                        tparam = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/TreeView")
    ...
    def select(self, item, column=None):
        ...
        name = item.toolTip(0)
        if name:
            obj = CivilCvCAD.ActiveDocument.getObject(name)
            if obj:
    ...
    def addLevel(self):
        ...
        siblings = []
        scope = (
            parent.Group if parent and hasattr(parent, "Group") else CivilCvCAD.ActiveDocument.Objects
        )
        for o in scope:
            t = Draft.getType(o)
            if (
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Create Level")
        obj = Arch.makeFloor()
    ...
    def addProxy(self):
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Create WP Proxy")
    ...
    def delete(self):
        ...
        if vm:
            if vm.tree.selectedItems():
                CivilCvCAD.ActiveDocument.openTransaction("Delete")
                for item in vm.tree.selectedItems():
                    obj = CivilCvCAD.ActiveDocument.getObject(item.toolTip(0))
                    if obj:
    ...
    def editObject(self, item, column):
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Edit level")
        try:
            if column == 0:
                obj.Label = text
            elif column == 1:
                if text:
                    setObjectElevation(obj, CivilCvCAD.Units.parseQuantity(text))
            elif column == 2:
                if text and hasattr(obj, "Height"):
                    obj.Height = CivilCvCAD.Units.parseQuantity(text)
        finally:
            CivilCvCAD.ActiveDocument.commitTransaction()
        CivilCvCAD.ActiveDocument.recompute()
    ...
    def isolate(self):
        ...
        for item in self.allItemsInTree:
            ...
            obj = CivilCvCAD.ActiveDocument.getObject(toolTip)
            if obj:
    ...
    def onContextMenu(self, pos):
        ...
        if selobj:
            selobj = CivilCvCAD.ActiveDocument.getObject(selobj.toolTip(0))
            if selobj:
                if Draft.getType(selobj).startswith("Ifc"):
                    self.dialog.buttonAddProxy.setEnabled(False)
                if CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIFC") == selobj:
                    self.dialog.buttonActive.setChecked(True)
                elif CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("Arch") == selobj:
                    self.dialog.buttonActive.setChecked(True)
                else:
    ...
    def getViews(self):
        ...
        bps = [o for o in CivilCvCAD.ActiveDocument.Objects if Draft.getType(o) == "BuildingPart"]
    ...
    def getPages(self):
        ...
        return [o for o in CivilCvCAD.ActiveDocument.Objects if o.isDerivedFrom("TechDraw::DrawPage")]
...
def show(item, column=None):
    ...
    if isinstance(item, str) or ((sys.version_info.major < 3) and isinstance(item, unicode)):
        ...
        obj = CivilCvCAD.ActiveDocument.getObject(item)
    else:
        # called from GUI
        if column in (1, 2):
    ...
    if obj:
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCADGui.Selection.addSelection(obj)
        vparam = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View")
        if obj.isDerivedFrom("TechDraw::DrawPage"):
            ...
            if not hasattr(CivilCvCADGui.ActiveDocument.ActiveView, "getSceneGraph"):
                ...
                for w in CivilCvCADGui.getMainWindow().getWindows():
                    if hasattr(w, "getSceneGraph"):
                        CivilCvCADGui.getMainWindow().setActiveWindow(w)
                        break
            CivilCvCADGui.runCommand("Std_OrthographicCamera")
            CivilCvCADGui.ActiveDocument.ActiveView.viewTop()
...
def getTreeViewItem(obj):
    ...
    elevStr = CivilCvCAD.Units.Quantity(z, CivilCvCAD.Units.Length).UserString
...
def _toggle_active_container(obj, action=None, dialog=None):
    ...
    active_obj = CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIFC")
    if active_obj is None:
        active_obj = CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("Arch")
    ...
    if is_active:
        ...
        CivilCvCADGui.ActiveDocument.ActiveView.setActiveObject("NativeIFC", None)
        CivilCvCADGui.ActiveDocument.ActiveView.setActiveObject("Arch", None)
        return False
    else:
        # Activate the object
        import Draft
        ...
        CivilCvCADGui.ActiveDocument.ActiveView.setActiveObject(context, obj)
        return True


CivilCvCADGui.addCommand("BIM_Views", BIM_Views())
