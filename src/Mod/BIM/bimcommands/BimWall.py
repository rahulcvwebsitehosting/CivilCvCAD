...
class Arch_Wall:
    ...
    https://wiki.civilcvcad.org/Arch_Wall
    """
                ...
                for selobj in sel:
                    if (
                        ...
                        CivilCvCADGui.doCommand(
                            "obj = Arch.makeWall(CivilCvCAD.ActiveDocument."
                            + selobj.Object.Name
                            + ",face="
                            + str(idx)
                            + ")"
                        ...
                        CivilCvCADGui.doCommand(
                            "obj = Arch.makeWall(CivilCvCAD.ActiveDocument." + selobj.Object.Name + ")"
                        )
    ...
    def _create_baseless_wall(self, p0, p1):
        ...
        rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), direction)

        # This placement is local to the working plane.
        local_placement = CivilCvCAD.Placement(midpoint, rotation)
        ...
        placement_str = (
            f"CivilCvCAD.Placement(CivilCvCAD.Vector({final_placement.Base.x}, {final_placement.Base.y}, {final_placement.Base.z}), "
            f"CivilCvCAD.Rotation({final_placement.Rotation.Q[0]}, {final_placement.Rotation.Q[1]}, {final_placement.Rotation.Q[2]}, {final_placement.Rotation.Q[3]}))"
        )
        ...
        CivilCvCADGui.doCommand("import Arch")
        CivilCvCADGui.doCommand(make_wall_cmd)
        CivilCvCADGui.doCommand(f"{wall_var}.Placement = {placement_str}")
        if self.MultiMat:
            CivilCvCADGui.doCommand(
                f"{wall_var}.Material = CivilCvCAD.ActiveDocument.{self.MultiMat.Name}"
            )
        ...
        CivilCvCADGui.doCommand("import Draft")
        CivilCvCADGui.doCommand(f"Draft.autogroup({wall_var})")
    ...
    def _create_baseline_object(self, p0, p1):
        ...
        placement_str = (
            f"CivilCvCAD.Placement(CivilCvCAD.Vector({placement.Base.x}, {placement.Base.y}, {placement.Base.z}), "
            f"CivilCvCAD.Rotation({placement.Rotation.Q[0]}, {placement.Rotation.Q[1]}, {placement.Rotation.Q[2]}, {placement.Rotation.Q[3]}))"
        )

        trace_cmd = (
            f"trace = Part.LineSegment(CivilCvCAD.Vector({p0.x}, {p0.y}, {p0.z}), "
            f"CivilCvCAD.Vector({p1.x}, {p1.y}, {p1.z}))"
        )

        CivilCvCADGui.doCommand("import CivilCvCAD")
        CivilCvCADGui.doCommand("import Part")
        CivilCvCADGui.addModule("WorkingPlane")
        CivilCvCADGui.doCommand(trace_cmd)

        if self.baseline_mode == WallBaselineMode.DRAFT_LINE:
            CivilCvCADGui.doCommand("import Draft")

            # Execute creation command. CivilCvCAD will ensure a unique name.
            CivilCvCADGui.doCommand("base = Draft.make_line(trace)")

            # Execute placement command.
            CivilCvCADGui.doCommand(f"base.Placement = {placement_str}")
            ...
            useArchSketch = hasattr(ArchSketchObject, "makeArchSketch") and getattr(
                CivilCvCAD, "ArchSketchLock", True
            )

            if not useArchSketch:
                # Regular path without SketchArch add-on installed. Execute creation command with a
                # suggested name. CivilCvCAD will ensure uniqueness.
                CivilCvCADGui.doCommand(
                    "base = CivilCvCAD.ActiveDocument.addObject('Sketcher::SketchObject', 'WallTrace')"
                )
                user_label = translate("BimWall", "Wall Trace")
                # Set the user-facing, translated label.
                CivilCvCADGui.doCommand(f"base.Label = {repr(user_label)}")
            else:
                # Use ArchSketch if SketchArch add-on is present
                CivilCvCADGui.doCommand("import ArchSketchObject")
                CivilCvCADGui.doCommand("base = ArchSketchObject.makeArchSketch()")

            # Apply placement and geometry using the correctly identified object name.
            CivilCvCADGui.doCommand(f"base.Placement = {placement_str}")
            CivilCvCADGui.doCommand(f"base.addGeometry(trace)")
    ...
    def _create_wall_from_baseline(self, base_obj):
        ...
        make_wall_cmd = (
            f"{wall_var} = Arch.makeWall(CivilCvCAD.ActiveDocument.{base_obj.Name}, "
            f"width={self.Width}, height={self.Height}, align='{self.Align}')"
        )
        set_normal_cmd = f"{wall_var}.Normal = CivilCvCAD.{self.wp.axis}"

        # Execute creation and property-setting commands
        CivilCvCADGui.doCommand("import Arch")
        CivilCvCADGui.doCommand(make_wall_cmd)
        CivilCvCADGui.doCommand(set_normal_cmd)
        if self.MultiMat:
            CivilCvCADGui.doCommand(
                f"{wall_var}.Material = CivilCvCAD.ActiveDocument.{self.MultiMat.Name}"
            )
        ...
        if wall_obj and self.existing:
            ...
            if wallGrp == oldWallGrp:
                ...
                if (
                    ...
                    CivilCvCADGui.doCommand(
                        f"Arch.joinWalls([CivilCvCAD.ActiveDocument.{wall_obj.Name}, CivilCvCAD.ActiveDocument.{oldWall.Name}], delete=True, deletebase=True)"
                    )
                    joined = True
                ...
                if not joined and AUTOJOIN:
                    if wallGrp:
                        ...
                        CivilCvCADGui.doCommand(
                            f"CivilCvCAD.ActiveDocument.{wallGrp.Name}.removeObject(CivilCvCAD.ActiveDocument.{wall_obj.Name})"
                        )
                    CivilCvCADGui.doCommand(
                        f"Arch.addComponents(CivilCvCAD.ActiveDocument.{wall_obj.Name}, CivilCvCAD.ActiveDocument.{oldWall.Name})"
                    )
    ...
    def update(self, point, info):
        ...
        if CivilCvCADGui.Control.activeDialog():
            ...
            if self.Length:
                self.Length.setText(
                    CivilCvCAD.Units.Quantity(bv.Length, CivilCvCAD.Units.Length).UserString
                )
    ...
    def taskbox(self):
        ...
        if hasattr(CivilCvCAD, "LastArchMultiMaterial"):
            for i, o in enumerate(self.multimats):
                if o.Name == CivilCvCAD.LastArchMultiMaterial:
        ...
        inputWidth.setText(CivilCvCAD.Units.Quantity(self.Width, CivilCvCAD.Units.Length).UserString)
        ...
        inputHeight.setText(CivilCvCAD.Units.Quantity(self.Height, CivilCvCAD.Units.Length).UserString)
        ...
        inputOffset.setText(CivilCvCAD.Units.Quantity(self.Offset, CivilCvCAD.Units.Length).UserString)
    ...
    def setMat(self, d):
        ...
        if d == 0:
            self.MultiMat = None
            del CivilCvCAD.LastArchMultiMaterial
        elif d <= len(self.multimats):
            self.MultiMat = self.multimats[d - 1]
            CivilCvCAD.LastArchMultiMaterial = self.MultiMat.Name

    def setLength(self, d):
        """Simple callback for the interactive mode gui widget to set length."""

        if isinstance(d, CivilCvCAD.Units.Quantity):
            d = d.Value
        self.lengthValue = d
    ...
    def createFromGUI(self):
        ...
        if self.MultiMat:
            CivilCvCADGui.doCommand("wall.Material = CivilCvCAD.ActiveDocument." + self.MultiMat.Name)
        self.doc.commitTransaction()
        self.doc.recompute()
...
CivilCvCADGui.addCommand("Arch_Wall", Arch_Wall())
