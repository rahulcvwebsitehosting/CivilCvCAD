...
class BIM_Box:
        ...
        if len(self.points) == 1:
            ...
            self.Length.setText(
                CivilCvCAD.Units.Quantity(
                    self.points[-1].sub(point).Length, CivilCvCAD.Units.Length
                ).UserString
            )
            self.Length.setFocus()
            self.Length.setSelection(0, CivilCvCADGui.draftToolBar.number_length(self.Length.text()))
            ...
            self.Width.setText(
                CivilCvCAD.Units.Quantity(
                    self.cubetracker[0].getSize()[1], CivilCvCAD.Units.Length
                ).UserString
            )
            self.Width.setFocus()
            self.Width.setSelection(0, CivilCvCADGui.draftToolBar.number_length(self.Width.text()))
            ...
            self.Height.setText(CivilCvCAD.Units.Quantity(h, CivilCvCAD.Units.Length).UserString)
            self.Height.setFocus()
            self.Height.setSelection(0, CivilCvCADGui.draftToolBar.number_length(self.Height.text()))
    ...
    def taskbox(self):
        ...
        l = CivilCvCAD.Units.Quantity(self.LengthValue, CivilCvCAD.Units.Length)
        ...
        l = CivilCvCAD.Units.Quantity(self.WidthValue, CivilCvCAD.Units.Length)
        ...
        l = CivilCvCAD.Units.Quantity(self.HeightValue, CivilCvCAD.Units.Length)
        ...
        self.Length.textEdited.connect(CivilCvCADGui.draftToolBar.checkSpecialChars)
        self.Width.textEdited.connect(CivilCvCADGui.draftToolBar.checkSpecialChars)
        self.Height.textEdited.connect(CivilCvCADGui.draftToolBar.checkSpecialChars)
    ...
    def _makeBox(self):
        ...
        if self.normal.isEqual(pla.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1)), 1e-6):
...
CivilCvCADGui.addCommand("BIM_Box", BIM_Box())
