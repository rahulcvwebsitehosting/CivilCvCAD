...
class BIM_IfcElements:
    ...
    def Activated(self):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogIfcElements.ui")
        ...
        for group in groups.keys():
            ...
            for name, mat in groups[group]:
                ...
                if obj:
                    ...
                    matlabel = ""
                    if mat:
                        matobj = CivilCvCAD.ActiveDocument.getObject(mat)
                        if matobj:
    ...
    def onClickTree(self, index=None):
        ...
        for index in sel:
            if index.column() == 0:
                obj = CivilCvCAD.ActiveDocument.getObject(self.model.itemFromIndex(index).toolTip())
                if obj:
        ...
        for index in sel:
            if index.column() == 2:
                ...
                m = CivilCvCAD.ActiveDocument.getObject(item.toolTip())
                if mat:
                    if m != mat:
                        mat = None
                        break
    ...
    def onMaterialChanged(self, index=-1):
        ...
        if index == 1:
            ...
            for index in sel:
                if index.column() == 2:
                    if mat:
                        mobj = CivilCvCAD.ActiveDocument.getObject(mat)
                        if mobj:
                            item = self.model.itemFromIndex(index)
                            if item.toolTip() != mat:
    ...
    def checkMatChanged(self):
        ...
        if len(mats) != len(self.materials):
            ...
            for m in self.materials:
                o = CivilCvCAD.ActiveDocument.getObject(m)
                if o:
                    self.form.globalMaterial.addItem(
        ...
        for name, rolemat in self.objectslist.items():
            ...
            if obj:
                if hasattr(obj, "IfcRole") and (obj.IfcRole != role):
                    if not changed:
                        CivilCvCAD.ActiveDocument.openTransaction("Change IFC role")
                        changed = True
                    obj.IfcRole = role
                elif hasattr(obj, "IfcType") and (obj.IfcType != role):
                    if not changed:
                        CivilCvCAD.ActiveDocument.openTransaction("Change IFC type")
                        changed = True
                    obj.IfcType = role
...
CivilCvCADGui.addCommand("BIM_IfcElements", BIM_IfcElements())
