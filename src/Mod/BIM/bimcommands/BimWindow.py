...
class Arch_Window:
    ...
    def Activated(self):
        ...
        self.sel = CivilCvCADGui.Selection.getSelection()
        ...
        # autobuild mode
        if CivilCvCADGui.Selection.getSelectionEx():
            ...
            if hasattr(obj, "Shape"):
                if obj.Shape.Wires and (not obj.Shape.Solids) and (not obj.Shape.Shells):
                    ...
                    CivilCvCADGui.doCommand(
                        "win = Arch.makeWindow(CivilCvCAD.ActiveDocument." + obj.Name + ")"
                    )
                    if host and self.Include:
                        CivilCvCADGui.doCommand(
                            "win.Hosts = [CivilCvCAD.ActiveDocument." + host.Name + "]"
                        )
                        ...
                        for sibling in siblings:
                            if not sibling in sibs:
                                sibs.append(sibling)
                                CivilCvCADGui.doCommand(
                                    "win.Hosts = win.Hosts+[CivilCvCAD.ActiveDocument."
                                    + sibling.Name
                                    + "]"
                                )
                ...
                # Try to detect an object to use as a window type - TODO we must make this safer
                    ...
                    self.doc.openTransaction(translate("Arch", "Create Window"))
                    CivilCvCADGui.addModule("Arch")
                    CivilCvCADGui.doCommand("Arch.makeWindow(CivilCvCAD.ActiveDocument." + obj.Name + ")")
                    self.doc.commitTransaction()
                    self.doc.recompute()
                    return
        ...
        # CivilCvCADGui.Snapper.setSelectMode(True)
        ...
        return (
            [
                CivilCvCADGui.InputHint(
                    translate("Arch", "%1 pick point on host"), CivilCvCADGui.UserInput.MouseLeft
                )
            ]
    ...
    def getPoint(self, point=None, obj=None):
        ...
        if self.sel:
            obj = self.sel[0]
        point = point.add(CivilCvCAD.Vector(0, 0, self.SillHeight))
        self.doc.openTransaction(translate("Arch", "Create Window"))

        CivilCvCADGui.doCommand("import CivilCvCAD, Arch, DraftGeomUtils, WorkingPlane")
        CivilCvCADGui.doCommand("wp = WorkingPlane.get_working_plane()")

        if self.baseFace is not None:
            CivilCvCADGui.doCommand(
                "face = CivilCvCAD.ActiveDocument."
                + self.baseFace[0].Name
                + ".Shape.Faces["
                + str(self.baseFace[1])
                + "]"
            )
            CivilCvCADGui.doCommand("pl = DraftGeomUtils.placement_from_face(face, vec_z = wp.axis)")
        else:
            CivilCvCADGui.doCommand("pl = CivilCvCAD.Placement()")
            CivilCvCADGui.doCommand("pl.Rotation = CivilCvCAD.Rotation(wp.u, wp.axis, -wp.v, 'XZY')")

        CivilCvCADGui.doCommand(
            "pl.Base = CivilCvCAD.Vector("
            + str(point.x)
            + ", "
            + str(point.y)
            + ", "
        ...
        if self.Preset >= len(WindowPresets):
            ...
            CivilCvCADGui.doCommand("CivilCvCADGui.ActiveDocument.mergeProject(" + repr(path) + ")")
            ...
            for o in nol[len(col) :]:
                if Draft.getType(o) == "Window":
                    if Draft.getType(o.Base) != "Sketcher::SketchObject":
                        ...
                        self.Include = False
                        break
                    CivilCvCADGui.doCommand("win = CivilCvCAD.ActiveDocument.getObject('" + o.Name + "')")
                    CivilCvCADGui.doCommand("win.Base.Placement = pl")
                    ...
                    # CivilCvCADGui.doCommand("win.Normal = pl.Rotation.multVec(CivilCvCAD.Vector(0, 0, -1))")
                    CivilCvCADGui.doCommand("win.Width = " + str(self.Width))
                    CivilCvCADGui.doCommand("win.Height = " + str(self.Height))
                    CivilCvCADGui.doCommand("win.Base.recompute()")
            ...
            if (
                ...
                and hasattr(CivilCvCAD, "ArchSketchLock")
                and CivilCvCAD.ArchSketchLock
            ):
                if self.Include:
                    ...
                    # - see https://github.com/CivilCvCAD/CivilCvCAD/issues/24903#issuecomment-3475455946
                    # placement = CivilCvCAD.Placement(App.Vector(0,0,0),App.Rotation(App.Vector(1,0,0),90))
                    ...
                    CivilCvCADGui.doCommand(
                        "pl90 = CivilCvCAD.Placement(App.Vector(0,0,0),App.Rotation(App.Vector(1,0,0),90))"
                    )
                    CivilCvCADGui.doCommand(
                    ...
                    CivilCvCADGui.doCommand("win.AttachToAxisOrSketch = 'None'")
                CivilCvCADGui.doCommand("CivilCvCADGui.Selection.addSelection(win)")
                w = CivilCvCADGui.Selection.getSelection()[0]
                CivilCvCADGui.doCommand("CivilCvCAD.SketchArchPl = pl")
                wPl = CivilCvCAD.SketchArchPl
                SketchArch = True
            else:
                CivilCvCADGui.doCommand(
                    "win = Arch.makeWindowPreset('"
        ...
        if self.Include:
            if Draft.getType(host) in ALLOWEDHOSTS:
                CivilCvCADGui.doCommand("win.Hosts = [CivilCvCAD.ActiveDocument." + host.Name + "]")
                siblings = host.Proxy.getSiblings(host)
                for sibling in siblings:
                    CivilCvCADGui.doCommand(
                        "win.Hosts = win.Hosts + [CivilCvCAD.ActiveDocument." + sibling.Name + "]"
                    )
                if SketchArch:
    ...
    def update(self, point, info):
        ...
        delta = CivilCvCAD.Vector(self.Width / 2, self.W1 / 2, self.Height / 2)
        delta = delta.add(CivilCvCAD.Vector(0, 0, self.SillHeight))

        if self.baseFace is None:
            rot = CivilCvCAD.Rotation(self.wp.u, self.wp.v, -self.wp.axis, "XZY")
        ...
        if r != (0, 0, 0, 1):
            delta = CivilCvCAD.Rotation(r[0], r[1], r[2], r[3]).multVec(
                CivilCvCAD.Vector(delta.x, -delta.y, -delta.z)
            )
    ...
    def taskbox(self):
        ...
        # because of the use of CivilCvCADGui.doCommand() backslashes in the
        ...
        presetdir = CivilCvCAD.getUserAppDataDir().replace("\\", "/") + "/Arch"
        ...
        for param in self.wparams:
            ...
            if param == "W1":
                wid.setText(CivilCvCAD.Units.Quantity(self.W1, CivilCvCAD.Units.Length).UserString)
            elif param == "Width":
                wid.setText(CivilCvCAD.Units.Quantity(self.Width, CivilCvCAD.Units.Length).UserString)
            elif param == "Height":
                wid.setText(CivilCvCAD.Units.Quantity(self.Height, CivilCvCAD.Units.Length).UserString)
            else:
                n = params.get_param_arch("Window" + param)
                wid.setText(CivilCvCAD.Units.Quantity(n, CivilCvCAD.Units.Length).UserString)
        ...
        values.setText(CivilCvCAD.Units.Quantity(d, CivilCvCAD.Units.Length).UserString)
    ...
    def setPreset(self, i):
        ...
        if i >= 0:
            CivilCvCADGui.Snapper.setSelectMode(False)
            self.tracker.length(self.Width)
            self.tracker.height(self.Height)
            self.tracker.width(self.W1)
            self.tracker.on()
...
CivilCvCADGui.addCommand("Arch_Window", Arch_Window())
