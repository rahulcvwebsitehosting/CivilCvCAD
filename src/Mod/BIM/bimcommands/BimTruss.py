...
class Arch_Truss:
    ...
    def Activated(self):
        ...
        if len(sel) > 1:
            ...
            # build on selection
            basename = "CivilCvCAD.ActiveDocument." + CivilCvCADGui.Selection.getSelection()[0].Name
    ...
    def createTruss(self, basename=""):
        ...
        if not basename:
            if self.points:
                cmd = "base = Draft.makeLine(CivilCvCAD."
                cmd += str(self.points[0]) + ",CivilCvCAD." + str(self.points[1]) + ")"
                CivilCvCADGui.doCommand(cmd)
                basename = "base"
        CivilCvCADGui.doCommand("obj = Arch.makeTruss(" + basename + ")")
        CivilCvCADGui.doCommand("Draft.autogroup(obj)")
        self.doc.commitTransaction()
        self.doc.recompute()


CivilCvCADGui.addCommand("Arch_Truss", Arch_Truss())
