...
class BIM_Classification:
    ...
    def Activated(self):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogClassification.ui")
        ...
        self.form.labelDownload.setText(
            self.form.labelDownload.text().replace(
                "%s", os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "Classification")
            )
        )

        # hide materials list if we are editing a particular object
        if len(CivilCvCADGui.Selection.getSelection()) == 1:
            self.isEditing = CivilCvCADGui.Selection.getSelection()[0]
        ...
        p = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetString(
            "DefaultClassificationSystem", ""
        )
        presetdir = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "Classification")
    ...
    def updateByMaterial(self):
        ...
        for name in self.matlist.keys():
            mat = CivilCvCAD.ActiveDocument.getObject(name)
            if mat:
        ...
        for group in groups.keys():
            matobj = CivilCvCAD.ActiveDocument.getObject(group)
            if matobj:
        ...
        for name, code in d.items():
            ...
            if obj:
                if (
                    ...
                    it.setToolTip(0, name)
                    self.form.treeObjects.addTopLevelItem(it)
                    if obj in CivilCvCADGui.Selection.getSelection():
    ...
    def updateClasses(self, search=""):
        ...
        # save as default
        CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").SetString(
    ...
    def build(self, system):
        ...
        preset = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "Classification", system + ".ifc")
        if os.path.exists(preset):
            return self.build_ifc(system)
        else:
            preset = os.path.join(
                CivilCvCAD.getUserAppDataDir(), "BIM", "Classification", system + ".xml"
            )
            if os.path.exists(preset):
                return self.build_xml(system)
            else:
                CivilCvCAD.Console.PrintError("Unable to find classification file:" + system + "\n")
                return []
    ...
    def build_xml(self, system):
        ...
        preset = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "Classification", system + ".xml")
        if not os.path.exists(preset):
            return None
        import re
                ...
                for item in items:
                    ...
                    if item.toolTip(0):
                        ...
                        if obj:
                            ...
                            if label != obj.Label:
                                if not changed:
                                    CivilCvCAD.ActiveDocument.openTransaction("Change standard codes")
                                    changed = True
                                obj.Label = label
            if changed:
...
CivilCvCADGui.addCommand("BIM_Classification", BIM_Classification())
