...
class BIM_Layers:
    "The BIM_Layers CivilCvCAD command"
    ...
    def Activated(self):
        ...
        # create the dialog
        self.dialog = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogLayersIFC.ui")
        ...
        for row in range(self.model.rowCount()):
            ...
            if not obj:
                ...
                if self.model.item(row, 1).icon().isNull():
                    ...
                    active = CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIFC")
                    project = None
                    if active:
    ...
    def reject(self):
        ...
        # save dialog size
        pref = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/BIM")
        pref.SetInt("LayersManagerWidth", self.dialog.width())
        pref.SetInt("LayersManagerHeight", self.dialog.height())

        # wipe to let CivilCvCAD know the dialog has been closed
        del self.dialog
    ...
    def update(self):
        ...
        # populate
        objs = [obj for obj in CivilCvCAD.ActiveDocument.Objects if Draft.getType(obj) == "Layer"]
    ...
    def addItem(self, obj=None):
        ...
        if CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("NativeIFC"):
    ...
    def getPref(self, value, default, valuetype="Unsigned"):
        ...
        p = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View")
    ...
    def onAssign(self):
        ...
        for index in self.dialog.tree.selectedIndexes():
            if index.column() == 1:
                name = self.model.itemFromIndex(index).toolTip()
                if name:
                    target = CivilCvCAD.ActiveDocument.getObject(name)
                    if target:
                        selected = CivilCvCADGui.Selection.getSelection()
                        if selected:
...
CivilCvCADGui.addCommand("BIM_Layers", BIM_Layers())
