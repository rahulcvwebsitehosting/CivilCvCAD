...
class BIM_Preflight:
    ...
    def Activated(self):
        CivilCvCADGui.BIMPreflightDone = False
        CivilCvCADGui.Control.showDialog(BIM_Preflight_TaskPanel())
...
class BIM_Preflight_TaskPanel:
    ...
    def __init__(self):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogPreflight.ui")
        ...
        # setup custom tests
        self.customTests = {}
        customModulePath = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "Preflight")
        ...
        if (test in self.results) and self.results[test]:
            if (test in self.culprits) and self.culprits[test]:
                ...
                for c in self.culprits[test]:
                    CivilCvCADGui.Selection.addSelection(c)
            if not self.rform:
                self.rform = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogPreflightResults.ui")
    ...
    def toReport(self):
        ...
        if self.rform and hasattr(self.rform, "test") and self.rform.test:
            if self.results[self.rform.test]:
                CivilCvCAD.Console.PrintMessage(self.results[self.rform.test] + "\n")
    ...
    def getObjects(self):
        ...
        if self.form.getAll.isChecked():
            ...
            objs = [o for o in CivilCvCAD.ActiveDocument.Objects if o.ViewObject.Visibility == True]
        else:
    ...
    def testAll(self):
        ...
        CivilCvCADGui.BIMPreflightDone = True
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            try:
                ...
                msg = (
                    translate(
                        "BIM",
                        "IfcOpenShell is not installed or is unavailable to CivilCvCAD. IFC support is disabled. Install it from trusted local media and restart CivilCvCAD.",
                    )
                    + " "
                )
                self.failed(test)
    ...
    def testTinyLines(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            if edges:
                ...
                result = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "TinyLinesResult")
    ...
    def testRectangleProfileDef(self):
        ...
        if getattr(self.form, test).text() == "Failed":
            ...
            self.culprits[test] = None
            msg = None
            if CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Arch").GetBool(
...
CivilCvCADGui.addCommand("BIM_Preflight", BIM_Preflight())
