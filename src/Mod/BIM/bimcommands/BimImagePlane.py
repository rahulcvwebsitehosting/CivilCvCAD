...
class BIM_ImagePlane:
    ...
    def MoveCallback(self, point, snapinfo):
        ...
        if point and self.basepoint and (point != self.basepoint):
            ...
            self.opposite = (
                CivilCvCAD.Vector(self.tracker.u)
                .multiply(length)
                .add(CivilCvCAD.Vector(self.tracker.v).multiply(height))
            )
    ...
    def PointCallback(self, point, snapinfo):
        ...
        if not point:
            ...
            image.Placement = CivilCvCAD.Placement(midpoint, rotation)
            image.XSize = length
            image.YSize = height
...
CivilCvCADGui.addCommand("BIM_ImagePlane", BIM_ImagePlane())
