...
class BIM_IfcQuantities:
    ...
    def Activated(self):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogIfcQuantities.ui")
    ...
    def get_qtos(self):
        ...
        qtopath = os.path.join(
            CivilCvCAD.getResourceDir(), "Mod", "BIM", "Presets", "qto_definitions.csv"
        )
        custompath = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "CustomQtos.csv")
    ...
    def add_qto(self):
        ...
        CivilCvCAD.Console.PrintMessage(translate("BIM", "Adding quantity set") + ": " + qto + "\n")
                ...
                if i > -1:
                    for j, p in enumerate(QPROPS):
                        ...
                        if t:
                            ...
                            qval = CivilCvCAD.Units.Quantity(t).Value
                if qval:
    ...
    def accept(self):
        ...
        if self.ifcqtolist:
            if not changed:
                CivilCvCAD.ActiveDocument.openTransaction("Change quantities")
            changed = True
    ...
    def onClickTree(self, index=None):
        ...
        for index in sel:
            if index.column() == 0:
                obj = CivilCvCAD.ActiveDocument.getObject(self.qmodel.itemFromIndex(index).toolTip())
                if obj:
                    CivilCvCADGui.Selection.addSelection(obj)


CivilCvCADGui.addCommand("BIM_IfcQuantities", BIM_IfcQuantities())
