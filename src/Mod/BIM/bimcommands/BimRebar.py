...
class Arch_Rebar:
    ...
    def Activated(self):
        ...
        if sel:
            ...
            if hasattr(obj, "Shape") and obj.Shape.Solids:
                ...
                if len(sel) > 1:
                    ...
                    if hasattr(sk, "Shape"):
                        if len(sk.Shape.Wires) == 1:
                            ...
                            CivilCvCADGui.doCommand(
                                "Arch.makeRebar(CivilCvCAD.ActiveDocument."
                                + obj.Name
                    ...
                    CivilCvCADGui.runCommand("Sketcher_NewSketch")
                    CivilCvCAD.ArchObserver = ArchComponent.ArchSelectionObserver(
                        obj,
                        CivilCvCAD.ActiveDocument.Objects[-1],
                        hide=False,
                    ...
                    if hasattr(obj, "AttachmentSupport"):
                        if obj.AttachmentSupport:
                            if len(obj.AttachmentSupport) != 0:
                                support = (
                                    "CivilCvCAD.ActiveDocument." + obj.AttachmentSupport[0][0].Name
                                )
                    CivilCvCAD.ActiveDocument.openTransaction(translate("Arch", "Create Rebar"))
                    CivilCvCADGui.addModule("Arch")
                    CivilCvCADGui.doCommand(
                        "Arch.makeRebar(" + support + ",CivilCvCAD.ActiveDocument." + obj.Name + ")"
                    )
                    CivilCvCAD.ActiveDocument.commitTransaction()
                    CivilCvCAD.ActiveDocument.recompute()
                    return
        ...
        CivilCvCAD.ArchObserver = ArchComponent.ArchSelectionObserver(nextCommand="Arch_Rebar")
        CivilCvCADGui.Selection.addObserver(CivilCvCAD.ArchObserver)


CivilCvCADGui.addCommand("Arch_Rebar", Arch_Rebar())
