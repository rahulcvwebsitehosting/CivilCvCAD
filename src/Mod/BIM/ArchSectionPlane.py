...
def getSectionData(source):
    ...
    if hasattr(source, "Objects"):
        ...
        objs = source.Group
        cutplane = Part.makePlane(1000, 1000, CivilCvCAD.Vector(-500, -500, 0))
        m = 1
        if source.ViewObject and hasattr(source.ViewObject, "CutMargin"):
            m = source.ViewObject.CutMargin.Value
        cutplane.translate(CivilCvCAD.Vector(0, 0, m))
    ...
    if hasattr(source, "Clip"):
        clip = source.Clip
    p = CivilCvCAD.Placement(source.Placement)
    direction = p.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))
    if objs:
...
def getFillForObject(o, defaultFill, source):
    ...
    if hasattr(source, "UseMaterialColorForFill") and source.UseMaterialColorForFill:
        ...
        if hasattr(o, "SectionColor"):
            ...
            material = CivilCvCAD.ActiveDocument.getObject(o)
        if material:
...
def isOriented(obj, plane):
    ...
    if hasattr(obj, "Placement"):
        norm2 = obj.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))
...
def getSVG(
    ...
    if renderMode in ["Coin", 2, "Coin mono", 3]:
        ...
        if should_update_svg_cache:
            ...
            pl = CivilCvCAD.Placement(source.Placement)
            if source.ViewObject and hasattr(source.ViewObject, "CutMargin"):
                mv = pl.multVec(CivilCvCAD.Vector(0, 0, 1))
...
def getCoinSVG(cutplane, objs, cameradata=None, linewidth=0.2, singleface=False, facecolor=None):
    ...
    markervec = CivilCvCAD.Vector(10, 10, 10)
    ...
    if cameradata:
        ...
        # rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0,0,1),cutplane.normalAt(0,0))
        vx = cutplane.Placement.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0))
        vy = cutplane.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 1, 0))
        vz = cutplane.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))
        rot = CivilCvCAD.Rotation(vx, vy, vz, "ZXY")
    ...
    orlength = CivilCvCAD.Vector(p.x, p.y, 0).Length
    ...
    if marker:
        ...
        x2 = float(marker[5])
        y2 = float(marker[7])
        p1 = CivilCvCAD.Vector(x1, y1, 0)
        p2 = CivilCvCAD.Vector(x2, y2, 0)
        factor = orlength / p2.sub(p1).Length
        if factor:
            orig = wp.get_local_coords(CivilCvCAD.Vector(boundbox.XMin, boundbox.YMin, boundbox.ZMin))
            orig = CivilCvCAD.Vector(orig.x, -orig.y, 0)
            scaledp1 = CivilCvCAD.Vector(p1.x * factor, p1.y * factor, 0)
...
class _SectionPlane:
    ...
    def execute(self, obj):
        ...
        target_normal = obj.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))
...
class _ViewProviderSectionPlane:
    ...
    def attach(self, vobj):
        ...
        # fs = coin.SoType.fromName("SoBrepFaceSet").createInstance() # this causes a CivilCvCAD freeze for me
    ...
    def onChanged(self, vobj, prop):
        ...
        if prop == "LineColor":
            ...
            if (
                ...
                and CivilCvCADGui.ActiveDocument.ActiveView
                and hasattr(CivilCvCADGui.ActiveDocument.ActiveView, "getSceneGraph")
            ):
...
class SectionPlaneTaskPanel:
    ...
    def removeElement(self):
        if self.obj:
            ...
            if it:
                comp = CivilCvCAD.ActiveDocument.getObject(str(it.toolTip(0)))
    ...
    def rotateX(self):
        self.rotate(CivilCvCAD.Vector(1, 0, 0))

    def rotateY(self):
        self.rotate(CivilCvCAD.Vector(0, 1, 0))

    def rotateZ(self):
        self.rotate(CivilCvCAD.Vector(0, 0, 1))

    def getBB(self):
        bb = CivilCvCAD.BoundBox()
        if self.obj:
    ...
    def resize(self):
        if self.obj and self.obj.ViewObject:
            ...
            if (n.getAngle(CivilCvCAD.Vector(1, 0, 0)) < 0.1) or (
                n.getAngle(CivilCvCAD.Vector(-1, 0, 0)) < 0.1
            ):
                self.obj.ViewObject.DisplayLength = bb.YLength + margin
                self.obj.ViewObject.DisplayHeight = bb.ZLength + margin
            elif (n.getAngle(CivilCvCAD.Vector(0, 1, 0)) < 0.1) or (
                n.getAngle(CivilCvCAD.Vector(0, -1, 0)) < 0.1
            ):
                self.obj.ViewObject.DisplayLength = bb.XLength + margin
                self.obj.ViewObject.DisplayHeight = bb.ZLength + margin
            elif (n.getAngle(CivilCvCAD.Vector(0, 0, 1)) < 0.1) or (
                n.getAngle(CivilCvCAD.Vector(0, 0, -1)) < 0.1
            ):
                self.obj.ViewObject.DisplayLength = bb.XLength + margin
