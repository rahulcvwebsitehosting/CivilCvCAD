...
profilefiles = [
    os.path.join(CivilCvCAD.getResourceDir(), "Mod", "BIM", "Presets", "profiles.csv"),
    os.path.join(os.path.dirname(__file__), "Presets", "profiles.csv"),
    os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "profiles.csv"),
]
...
class _ProfileTSLOT(_Profile):
    ...
    def execute(self, obj):
        ...
        hole = Part.makeCircle(obj.HoleDiameter / 2, CivilCvCAD.Vector(0, 0, 0))
    ...
    def changeCategory(self, text):
        ...
        for pre in self.presets:
            if pre[1] == text:
                ...
                f = CivilCvCAD.Units.Quantity(pre[4], CivilCvCAD.Units.Length).getUserPreferred()
