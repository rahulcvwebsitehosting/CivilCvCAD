...
if CivilCvCAD.GuiUp:
    ICON_STATUS_OK = CivilCvCADGui.getIcon(":/icons/edit_OK.svg")
    ICON_STATUS_WARN = CivilCvCADGui.getIcon(":/icons/Warning.svg")
    ICON_STATUS_ERROR = CivilCvCADGui.getIcon(":/icons/delete.svg")
    ICON_STATUS_INCOMPLETE = CivilCvCADGui.getIcon(":/icons/button_invalid.svg")
    ICON_EDIT = CivilCvCADGui.getIcon(":/icons/edit-edit.svg")
    ICON_ADD = CivilCvCADGui.getIcon(":/icons/list-add.svg")
    ICON_REMOVE = CivilCvCADGui.getIcon(":/icons/list-remove.svg")
    ICON_DUPLICATE = CivilCvCADGui.getIcon(":/icons/edit-copy.svg")
...
def _get_preset_paths(preset_type):
    ...
    # Path to the bundled presets installed with CivilCvCAD
    system_path = os.path.join(
        CivilCvCAD.getResourceDir(), "Mod", "BIM", "Presets", "ArchReport", subdir
    )
    # Path to the user's custom presets in their AppData directory
    user_path = os.path.join(CivilCvCAD.getUserAppDataDir(), "BIM", "Presets", "ArchReport", subdir)
...
def _save_preset(preset_type, name, data):
    ...
    try:
        ...
        CivilCvCAD.Console.PrintError(f"BIM Report: Could not save preset to '{file_path}': {e}\n")
...
def _rename_preset(preset_type, filename, new_name):
    ...
    try:
        ...
        CivilCvCAD.Console.PrintError(f"BIM Report: Failed to rename preset file '{file_path}': {e}\n")
...
def _delete_preset(preset_type, filename):
    ...
    try:
        ...
        CivilCvCAD.Console.PrintError(f"BIM Report: Failed to delete preset file '{file_path}': {e}\n")
    ...
    def slotRecomputedDocument(self, doc):
        ...
        try:
            if not self.report.isAttachedToDocument():
                CivilCvCAD.removeDocumentObserver(self)
                return
        except RuntimeError:
            CivilCvCAD.removeDocumentObserver(self)
            return
...
class _ArchReport:
    ...
    def getSpreadSheet(self, obj, force=False):
        ...
        if force:
            sheet = CivilCvCAD.ActiveDocument.addObject("Spreadsheet::Sheet", "ReportResult")
        ...
        if prop == "AutoUpdate":
            if obj.AutoUpdate:
                if getattr(self, "docObserver", None) is None:
                    self.docObserver = _ArchReportDocObserver(CivilCvCAD.ActiveDocument, obj)
                    CivilCvCAD.addDocumentObserver(self.docObserver)
            else:
                if getattr(self, "docObserver", None) is not None:
                    CivilCvCAD.removeDocumentObserver(self.docObserver)
                    self.docObserver = None
    ...
    def _write_cell(self, spreadsheet, cell_address, value):
        ...
        # Handle CivilCvCAD Quantity objects by extracting their raw numerical value.
    ...
    def setSpreadsheetData(
        ...
        if data_rows:
            for i, cell_value in enumerate(data_rows[0]):
                if isinstance(cell_value, CivilCvCAD.Units.Quantity):
...
class ViewProviderReport:
        ...
        if mode == 0:
            if CivilCvCAD.GuiUp:
                ...
                try:
                    ...
                    CivilCvCAD.Console.PrintError(f"Could not open Report editor: {e}\n")
                    return False
            return True
        return False
    ...
    def onDelete(self, vobj, subelements):
        ...
        if proxy and getattr(proxy, "docObserver", None) is not None:
            CivilCvCAD.removeDocumentObserver(proxy.docObserver)
...
class ReportTaskPanel:
    ...
    Exposes `self.form` as a QWidget so it works with CivilCvCADGui.Control.showDialog(panel).
    ...
    def __init__(self, report_obj):
        # Create two top-level widgets so CivilCvCAD will wrap each into a TaskBox.
        ...
        self.btn_save_template.setIcon(CivilCvCADGui.getIcon(":/icons/document-save.svg"))
        ...
        self.btn_toggle_preview.setIcon(CivilCvCADGui.getIcon(":/icons/Std_ToggleVisibility.svg"))
        ...
        self.btn_show_cheatsheet.setIcon(CivilCvCADGui.getIcon(":/icons/help-browser.svg"))
        ...
        self.btn_refresh_preview.setIcon(CivilCvCADGui.getIcon(":/icons/view-refresh.svg"))
        ...
        self.btn_save.setIcon(CivilCvCADGui.getIcon(":/icons/document-save.svg"))
        ...
        self.btn_discard.setIcon(CivilCvCADGui.getIcon(":/icons/delete.svg"))
        ...
        # Expose form as a list of the two top-level widgets so CivilCvCAD creates
        ...
        self.icon_show_preview = CivilCvCADGui.getIcon(":/icons/Std_ToggleVisibility.svg")
        self.icon_hide_preview = CivilCvCADGui.getIcon(":/icons/Invisible.svg")
    ...
    def accept(self):
        ...
        if spreadsheet:
            CivilCvCADGui.ActiveDocument.setEdit(spreadsheet.Name, 0)

        # Close the task panel.
        try:
            CivilCvCADGui.Control.closeDialog()
        except Exception as e:
            CivilCvCAD.Console.PrintLog(f"Could not close Report Task Panel: {e}\n")
        ...
        def _on_edit_source(self):
            ...
            if not open_local_url(url):
                QtWidgets.QMessageBox.warning(
                    ...
                    translate(
                        "Arch",
                        "CivilCvCAD could not open the file. Please check if you have a default text editor configured in your operating system.",
                    ),
                )
