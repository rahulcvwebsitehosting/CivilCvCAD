...
__title__ = "CivilCvCAD Arch Commands"
...
def getStringList(objects):
    ...
    for o in objects:
        if len(result) > 1:
            result += ","
        result += "CivilCvCAD.ActiveDocument." + o.Name
    result += "]"
    return result
...
def makeComponent(baseobj=None, name=None, delete=False):
    ...
    if not CivilCvCAD.ActiveDocument:
        CivilCvCAD.Console.PrintError("No active document. Aborting\n")
        return
    obj = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "Component")
    ...
    if baseobj:
        ...
        if hasattr(baseobj, "Shape"):
            ...
            if delete:
                CivilCvCAD.ActiveDocument.removeObject(baseobj.Name)
            else:
                obj.Base = baseobj
                if CivilCvCAD.GuiUp:
...
def splitMesh(obj, mark=True):
    ...
    if comps:
        basename = obj.Name
        CivilCvCAD.ActiveDocument.removeObject(basename)
        for c in comps:
            newobj = CivilCvCAD.ActiveDocument.addObject("Mesh::Feature", basename)
            newobj.Mesh = c
...
def getCutVolume(cutplane, shapes, clip=False, depth=None):
    ...
    try:
        ...
        CivilCvCAD.Console.PrintMessage(translate("Arch", "Invalid cut plane") + "\n")
        return None, None, None
    ce = p.CenterOfMass
    ax = p.normalAt(0, 0)
    ...
    if not bb.isCutPlane(ce, ax):
        # CivilCvCAD.Console.PrintMessage(translate("Arch","No objects are cut by the plane)+"\n")
        return None, None, None
    else:
        corners = [
            CivilCvCAD.Vector(bb.XMin, bb.YMin, bb.ZMin),
            CivilCvCAD.Vector(bb.XMin, bb.YMax, bb.ZMin),
            CivilCvCAD.Vector(bb.XMax, bb.YMin, bb.ZMin),
            CivilCvCAD.Vector(bb.XMax, bb.YMax, bb.ZMin),
            CivilCvCAD.Vector(bb.XMin, bb.YMin, bb.ZMax),
            CivilCvCAD.Vector(bb.XMin, bb.YMax, bb.ZMax),
            CivilCvCAD.Vector(bb.XMax, bb.YMin, bb.ZMax),
            CivilCvCAD.Vector(bb.XMax, bb.YMax, bb.ZMax),
        ]
        for c in corners:
            dv = c.sub(ce)
...
def getShapeFromMesh(mesh, fast=True, tolerance=0.001, flat=False, cut=True):
    ...
    if mesh.isSolid() and (mesh.countComponents() == 1) and fast:
        ...
        for f in mesh.Facets:
            ...
            pts = []
            for pp in p:
                pts.append(CivilCvCAD.Vector(pp[0], pp[1], pp[2]))
            try:
...
def meshToShape(obj, mark=True, fast=True, tol=0.001, flat=False, cut=True):
    ...
    if "Mesh" in obj.PropertiesList:
        ...
        if solid:
            ...
            newobj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", name)
            newobj.Shape = solid
        ...
        if DraftGeomUtils.isCubic(obj.Shape):
            ...
            if dims:
                ...
                if tp == "Structure":
                    ...
                    CivilCvCAD.ActiveDocument.removeObject(name)
        elif CivilCvCAD.GuiUp and mark:
    ...
    for o in objectslist:
        ...
        for c in o.Components:
            if not c in l:
                l.append(c)
        base.Components = l
        CivilCvCAD.ActiveDocument.removeObject(o.Name)
    CivilCvCAD.ActiveDocument.recompute()
    return base


def download(url, force=False):
    """Reject legacy network downloads in the offline-only distribution."""

    del url, force
    CivilCvCAD.Console.PrintError(
        "Network downloads are disabled. Supply the required file from local media.\n"
    )
    return None
...
def pruneIncluded(objectslist, strict=False, silent=False):
    ...
    for obj in objectslist:
        ...
        if toplevel:
            ...
            CivilCvCAD.Console.PrintWarning("pruning " + obj.Label + "\n")
    return newlist
...
def survey(callback=False):
    ...
    if not callback:
        if hasattr(CivilCvCAD, "SurveyObserver"):
            for label in CivilCvCAD.SurveyObserver.labels:
                CivilCvCAD.ActiveDocument.removeObject(label)
            CivilCvCADGui.Selection.removeObserver(CivilCvCAD.SurveyObserver)
            del CivilCvCAD.SurveyObserver
            CivilCvCADGui.Control.closeDialog()
            if hasattr(CivilCvCAD, "SurveyDialog"):
                del CivilCvCAD.SurveyDialog
        else:
            CivilCvCAD.SurveyObserver = _SurveyObserver(callback=survey)
            CivilCvCADGui.Selection.addObserver(CivilCvCAD.SurveyObserver)
            CivilCvCAD.SurveyDialog = SurveyTaskPanel()
            CivilCvCADGui.Control.showDialog(CivilCvCAD.SurveyDialog)
    else:
        sel = CivilCvCADGui.Selection.getSelectionEx()
        if hasattr(CivilCvCAD, "SurveyObserver"):
            if not sel:
                if CivilCvCAD.SurveyObserver.labels:
                    for label in CivilCvCAD.SurveyObserver.labels:
                        CivilCvCAD.ActiveDocument.removeObject(label)
                    tl = CivilCvCAD.SurveyObserver.totalLength
                    ta = CivilCvCAD.SurveyObserver.totalArea
                    CivilCvCAD.SurveyObserver.labels = []
                    CivilCvCAD.SurveyObserver.selection = []
                    CivilCvCAD.SurveyObserver.totalLength = 0
                    CivilCvCAD.SurveyObserver.totalArea = 0
                    CivilCvCAD.SurveyObserver.totalVolume = 0
                    if not CivilCvCAD.SurveyObserver.cancellable:
                        CivilCvCAD.Console.PrintMessage("\n---- Reset ----\n\n")
                        CivilCvCAD.SurveyObserver.cancellable = True
                        if hasattr(CivilCvCAD, "SurveyDialog"):
                            CivilCvCAD.SurveyDialog.newline(tl, ta)
                    else:
                        CivilCvCADGui.Selection.removeObserver(CivilCvCAD.SurveyObserver)
                        del CivilCvCAD.SurveyObserver
                        CivilCvCADGui.Control.closeDialog()
                ...
                CivilCvCAD.SurveyObserver.cancellable = False
                basesel = CivilCvCAD.SurveyObserver.selection
                newsels = []
                for o in sel:
                    found = False
                    for eo in basesel:
                ...
                if newsels:
                    for o in newsels:
                        if hasattr(o.Object, "Shape"):
                            ...
                            if not o.HasSubObjects:
                                ...
                                CivilCvCAD.SurveyObserver.labels.append(anno.Name)
                                if o.Object.Shape.Solids:
                                    u = CivilCvCAD.Units.Quantity(
                                        o.Object.Shape.Volume, CivilCvCAD.Units.Volume
                                    )
                                    ...
                                    CivilCvCAD.SurveyObserver.totalVolume += u.Value
                                elif o.Object.Shape.Faces:
                                    u = CivilCvCAD.Units.Quantity(
                                        o.Object.Shape.Area, CivilCvCAD.Units.Area
                                    )
                                    t = u.getUserPreferred()[0]
                                    t = t.replace("^2", "²")
                                    anno.LabelText = "a " + t
                                    ...
                                    CivilCvCAD.SurveyObserver.totalArea += u.Value
                                    if hasattr(CivilCvCAD, "SurveyDialog"):
                                        CivilCvCAD.SurveyDialog.update(2, t)
                                else:
                                    u = CivilCvCAD.Units.Quantity(
                                        o.Object.Shape.Length, CivilCvCAD.Units.Length
                                    )
                                    t = u.getUserPreferred()[0]
                                    anno.LabelText = "l " + t
                                    CivilCvCAD.Console.PrintMessage(
                                    ...
                                    CivilCvCAD.SurveyObserver.totalLength += u.Value
                                    if hasattr(CivilCvCAD, "SurveyDialog"):
                                        CivilCvCAD.SurveyDialog.update(1, t)
                                if CivilCvCAD.GuiUp and t:
                                    if showUnit:
                                ...
                                for el in o.SubElementNames:
                                    ...
                                    CivilCvCAD.SurveyObserver.labels.append(anno.Name)
                                    if "Face" in el:
                                        u = CivilCvCAD.Units.Quantity(e.Area, CivilCvCAD.Units.Area)
                                        t = u.getUserPreferred()[0]
                                        t = t.replace("^2", "²")
                                        anno.LabelText = "a " + t
                                        CivilCvCAD.Console.PrintMessage(
                                        ...
                                        u = CivilCvCAD.Units.Quantity(e.Length, CivilCvCAD.Units.Length)
                                        t = u.getUserPreferred()[0]
                                        anno.LabelText = "l " + t
                                        CivilCvCAD.Console.PrintMessage(
                                            "Object: "
                                        ...
                                        u = CivilCvCAD.Units.Quantity(e.Z, CivilCvCAD.Units.Length)
                                        t = u.getUserPreferred()[0]
                                        anno.LabelText = "z " + t
                                        CivilCvCAD.Console.PrintMessage(
                                            "Object: "
                    ...
                    CivilCvCAD.SurveyObserver.selection.extend(newsels)
            if hasattr(CivilCvCAD, "SurveyObserver"):
                if (
                    CivilCvCAD.SurveyObserver.totalLength
                    or CivilCvCAD.SurveyObserver.totalArea
                    or CivilCvCAD.SurveyObserver.totalVolume
                ):
                    msg = " Total:"
                    if CivilCvCAD.SurveyObserver.totalLength:
                        u = CivilCvCAD.Units.Quantity(
                            CivilCvCAD.SurveyObserver.totalLength, CivilCvCAD.Units.Length
                        )
                        t = u.getUserPreferred()[0]
                        msg += " Length: " + t
                    if CivilCvCAD.SurveyObserver.totalArea:
                        u = CivilCvCAD.Units.Quantity(
                            CivilCvCAD.SurveyObserver.totalArea, CivilCvCAD.Units.Area
                        )
                        t = u.getUserPreferred()[0]
                        t = t.replace("^2", "²")
                        msg += " Area: " + t
                    if CivilCvCAD.SurveyObserver.totalVolume:
                        u = CivilCvCAD.Units.Quantity(
                            CivilCvCAD.SurveyObserver.totalVolume, CivilCvCAD.Units.Volume
                        )
                        t = u.getUserPreferred()[0]
                        t = t.replace("^3", "³")
                        msg += " Volume: " + t
                    CivilCvCAD.Console.PrintMessage(msg + "\n")
...
class SurveyTaskPanel:
    ...
    def clipLength(self):
        if hasattr(CivilCvCAD, "SurveyObserver"):
            u = CivilCvCAD.Units.Quantity(CivilCvCAD.SurveyObserver.totalLength, CivilCvCAD.Units.Length)
            t = u.getUserPreferred()[0]
    ...
    def clipArea(self):
        if hasattr(CivilCvCAD, "SurveyObserver"):
            u = CivilCvCAD.Units.Quantity(CivilCvCAD.SurveyObserver.totalArea, CivilCvCAD.Units.Area)
            t = u.getUserPreferred()[0]
            t = t.replace("^2", "²")
            if params.get_param_arch("surveyUnits"):
                QtGui.QApplication.clipboard().setText(t)
    ...
    def newline(self, length=0, area=0):
        ...
        self.tree.setCurrentItem(item)
        if length:
            u = CivilCvCAD.Units.Quantity(length, CivilCvCAD.Units.Length)
            t = u.getUserPreferred()[0]
            item.setText(1, t)
        if area:
            u = CivilCvCAD.Units.Quantity(area, CivilCvCAD.Units.Area)
            t = u.getUserPreferred()[0]
            t = t.replace("^2", "²")
            item.setText(2, t)
        if length or area:
            ...
            if filename:
                with open(filename[0].encode("utf8"), "w") as csvfile:
                    ...
                    for i in range(rows):
                        ...
                        if item.text(1):
                            u = CivilCvCAD.Units.Quantity(item.text(1))
...
def makeCompoundFromSelected(objects=None):
    ...
    so = []
    if not objects:
        objects = CivilCvCADGui.Selection.getSelectionEx()
...
def cleanArchSplitter(objects=None):
    ...
    if not objects:
        objects = CivilCvCADGui.Selection.getSelection()
    if not isinstance(objects, list):
        objects = [objects]
    for obj in objects:
...
def rebuildArchShape(objects=None):
    ...
    if not objects and CivilCvCAD.GuiUp:
        objects = CivilCvCADGui.Selection.getSelection()
    if not isinstance(objects, list):
        objects = [objects]
    for obj in objects:
...
def printMessage(message):
    CivilCvCAD.Console.PrintMessage(message)
    if CivilCvCAD.GuiUp:
...
def makeIfcSpreadsheet(archobj=None):
    ifc_container = None
    for obj in CivilCvCAD.ActiveDocument.Objects:
        if obj.Name == "IfcPropertiesContainer":
            ifc_container = obj
    if not ifc_container:
        ifc_container = CivilCvCAD.ActiveDocument.addObject(
            "App::DocumentObjectGroup", "IfcPropertiesContainer"
        )
    import Spreadsheet

    ifc_spreadsheet = CivilCvCAD.ActiveDocument.addObject("Spreadsheet::Sheet", "IfcProperties")
    ...
    if archobj:
        if hasattr(obj, "IfcProperties"):
            ...
            CivilCvCAD.ActiveDocument.removeObject(ifc_spreadsheet)
    else:
        return ifc_spreadsheet
...
def override_link_properties(link_obj, prop_names):
    ...
    See: https://wiki.civilcvcad.org/Std_LinkMake#Copy_on_Change
...
def pickMainFaceName(shape, view_vector=None):
    ...
    view_vector : CivilCvCAD.Vector, optional
        ...
        if not face_ref.Shape or face_ref.Shape.isNull():
            ...
            try:
                ...
                if closed_wires:
                    ...
                    CivilCvCAD.Console.PrintWarning(translate("Arch", "No closed wires found.") + "\n")
            except Exception as e:
                CivilCvCAD.Console.PrintWarning(
...
def getFaceFrame(face, reference_direction=None):
    ...
    reference_direction : CivilCvCAD.Vector or None
    ...
    Returns
    -------
    tuple[CivilCvCAD.Vector, CivilCvCAD.Vector, CivilCvCAD.Vector, CivilCvCAD.Vector]
...
def _derive_u_from_reference(reference_direction, normal):
    ...
    ref = CivilCvCAD.Vector(reference_direction)
...
def _derive_u_from_longest_edge(face, normal):
    ...
    world_axes = [
        ...
        CivilCvCAD.Vector(0, 1, 0),
        CivilCvCAD.Vector(0, 0, 1),
    ]
...
def read_pat_pattern_names(filename):
    ...
    try:
        ...
        CivilCvCAD.Console.PrintWarning(f"Arch: could not read pattern file {filename}: {e}\n")
