...
# _getShapes(CivilCvCADGui.Selection.getSelectionEx("", 0))
def _getShapes(sels):
...
def cutComponentwithPlane(baseObj, cutterShp=None, side=0):
    ...
    baseObj: Part::FeaturePython object or selection set (a list of Gui::SelectionObject objects)
        Object to be cut or a selection set: `CivilCvCADGui.Selection.getSelectionEx("", 0)`.
    ...
    if cutVolume:
        obj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", "CutVolume")
        if baseParent is not None:
        ...
        if "Additions" in baseObj.PropertiesList:
            ...
            cutObj = CivilCvCAD.ActiveDocument.addObject("Part::Cut", "CutPlane")
            if baseParent is not None:
