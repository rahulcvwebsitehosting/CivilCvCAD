...
def export(exportList, filename, colors=None):
    ...
    Called when civilcvcad exports a file. exportList is a list
    ...
    outfile.write("# CivilCvCAD v" + ver[0] + "." + ver[1] + " build" + ver[2] + " Arch module\n")
    outfile.write("# CivilCvCAD offline build\n")
    offsetv = 1
    offsetvn = 1
    ...
    for obj in objectslist:
        ...
        if vlist is None:
            CivilCvCAD.Console.PrintError("Unable to export object " + obj.Label + ". Skipping.\n")
        else:
            offsetv += len(vlist)
            offsetvn += len(vnlist)
    ...
    outfile.close()
    CivilCvCAD.Console.PrintMessage(translate("Arch", "Successfully written") + " " + filename + "\n")
    if materials:
...
def insert(filename, docname):
    ...
    if activeobject:
        makeMesh(doc, group, activeobject, verts, medges, facets, material, colortable)
    CivilCvCAD.Console.PrintMessage(translate("Arch", "Successfully imported") + " " + filename + "\n")
    return doc
...
def makeMesh(doc, group, activeobject, verts, edges, facets, material, colortable):
    ...
    if facets:
        for facet in facets:
            if len(facet) > 3:
                vecs = [CivilCvCAD.Vector(*verts[i - 1]) for i in facet]
                vecs.append(vecs[0])
    ...
    if mfacets:
        ...
        if CivilCvCAD.GuiUp and material and material in colortable:
    ...
    if medges:
        ...
        features = []
        for strip in medges:
            points = [CivilCvCAD.Vector(*verts[i - 1]) for i in strip]
