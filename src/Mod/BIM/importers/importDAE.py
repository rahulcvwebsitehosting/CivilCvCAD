...
__title__ = "CivilCvCAD Collada importer"
...
def import_collada() -> bool:
    ...
    try:
        ...
        CivilCvCAD.Console.PrintError("pycollada not found, collada support is disabled.\n")
        return False
    return True
...
def read(filename):
    ...
    if col.errors:
        CivilCvCAD.Console.PrintWarning("File was read but some errors occurred:\n")
    for e in col.errors:
        CivilCvCAD.Console.PrintWarning(str(e) + "\n")
    if CivilCvCAD.GuiUp:
        CivilCvCAD.Gui.SendMsgToActiveView("ViewFit")
...
def export(
    exports: list[CivilCvCAD.DocumentObject],
    filename: str,
    tessellation: int = 1,
    colors: Optional[dict[str, tuple]] = None,
):
    """Export CivilCvCAD contents to a DAE file.
    ...
    scale = scale * 0.001  # from millimeters (CivilCvCAD) to meters (Collada)
    ...
    try:
        author = CivilCvCAD.ActiveDocument.CreatedBy
    except UnicodeEncodeError:
        author = CivilCvCAD.ActiveDocument.CreatedBy.encode("utf8")
    author = xml_escape(author)
    col_contributor.author = author
    ver = CivilCvCAD.Version()
    appli = f"CivilCvCAD v{ver[0]}.{ver[1]} build {ver[2]}"
    ...
    CivilCvCAD.Console.PrintMessage(f'file "{filename}" successfully created.\n')
...
def _read_bound_triangle_set(
    doc: CivilCvCAD.Document,
    bound_geom: "collada.geometry.BoundGeometry",
    prim: "collada.primitive.BoundPrimitive",
    unit: float,
) -> list[CivilCvCAD.DocumentObject]:
    """
...
def _add_meshes_to_doc(
    ...
    objects: list[CivilCvCAD.DocumentObject] = []
    for triangle_set in triangle_sets:
        ...
        # In order to prevent CivilCvCAD from incrementing the number (because
        ...
        for col_field, fc_field in field_map.items():
            try:
                ...
                # The collada value is not compatible with CivilCvCAD.
                pass
            except TypeError:
