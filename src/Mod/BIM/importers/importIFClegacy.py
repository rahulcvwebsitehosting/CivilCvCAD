...
__title__ = "CivilCvCAD IFC importer"
...
MAKETEMPFILES = False  # if True, shapes are passed from ifcopenshell to civilcvcad through temp files
...
def getIfcOpenShell():
    ...
    try:
        ...
        try:
            ...
            CivilCvCAD.Console.PrintMessage(translate("Arch", "Could not locate IfcOpenShell") + "\n")
            return False
        else:
            IFCOPENSHELL5 = True
            return True
...
def read(filename, skip=None):
    ...
    if getIfcOpenShell() and not FORCE_PYTHON_PARSER:
        ...
        ifcObjects = {}  # a table to relate ifc id with civilcvcad object
        ...
        while True:
            ...
            # check if object was already processed, to workaround an ifcopenshell bug
                ...
                # furniture
                    ...
                    nobj = CivilCvCAD.ActiveDocument.addObject("Part::Feature", n)
                    nobj.Shape = shape
                ...
                # spaces
                    ...
                    if hasattr(obj, "mesh"):
                        ...
                        me, pl = getMesh(obj)
                        nobj = CivilCvCAD.ActiveDocument.addObject("Mesh::Feature", n)
                        nobj.Label = n
                        nobj.Mesh = me
                        nobj.Placement = pl
                    else:
...
def makeWall(entity, shape=None, name="Wall"):
    "makes a wall in the civilcvcad document"
    try:
        if shape:
            # use ifcopenshell
            if isinstance(shape, Part.Shape):
                body = CivilCvCAD.ActiveDocument.addObject("Part::Feature", name + "_body")
                body.Shape = shape
            else:
                body = CivilCvCAD.ActiveDocument.addObject("Mesh::Feature", name + "_body")
                body.Mesh = shape
...
def makeWindow(entity, shape=None, name="Window"):
    "makes a window in the civilcvcad document"
    try:
        if shape:
            # use ifcopenshell
            if isinstance(shape, Part.Shape):
...
def makeStructure(entity, shape=None, ifctype=None, name="Structure"):
    "makes a structure in the civilcvcad document"
    try:
        if shape:
            # use ifcopenshell
            if isinstance(shape, Part.Shape):
...
def makeSite(entity, shape=None, name="Site"):
    "makes a site in the civilcvcad document"
    try:
        body = None
        if shape:
            # use ifcopenshell
...
def makeSpace(entity, shape=None, name="Space"):
    "makes a space in the civilcvcad document"
    try:
        if shape:
            # use ifcopenshell
            if isinstance(shape, Part.Shape):
...
def makeRoof(entity, shape=None, name="Roof"):
    "makes a roof in the civilcvcad document"
    try:
        if shape:
            # use ifcopenshell
            if isinstance(shape, Part.Shape):
...
def getMesh(obj):
    ...
    m = obj.matrix
    # fmt: off
    mat = CivilCvCAD.Matrix(m[0], m[3], m[6], m[9],
                         m[1], m[4], m[7], m[10],
                         m[2], m[5], m[8], m[11],
                         0, 0, 0, 1)
    # fmt: on
    pl = CivilCvCAD.Placement(mat)
    return me, pl
...
def getShape(obj, objid):
    ...
    if not IFCOPENSHELL5:
        ...
        mat = CivilCvCAD.Matrix(m[0], m[3], m[6], m[9],
                             ...
                             0, 0, 0, 1)
        # fmt: on
        sh.Placement = CivilCvCAD.Placement(mat)
...
def getVector(entity):
    ...
    if entitytype == "IFCDIRECTION":
        ...
        if len(DirectionRatios) == 3:
            return CivilCvCAD.Vector(tuple(DirectionRatios))
        else:
            return CivilCvCAD.Vector(tuple(DirectionRatios + [0]))
    elif entitytype == "IFCCARTESIANPOINT":
        Coordinates = getAttr(entity, "Coordinates")
        if len(Coordinates) == 3:
            return CivilCvCAD.Vector(tuple(Coordinates))
        else:
            return CivilCvCAD.Vector(tuple(Coordinates + [0]))
    return None
...
def getSchema():
    ...
    if custom:
        if os.path.exists(custom):
            ...
            return custom
    p = None
    p = os.path.join(CivilCvCAD.ConfigGet("UserAppData"), SCHEMA.split(os.sep)[-1])
    if os.path.exists(p):
        return p
    import ArchCommands
            ...
            for g in groups:
                for t in g[1]:
                    if e.type.upper() == t.upper():
                        if hasattr(CivilCvCAD.ActiveDocument, g[0] + str(e.id)):
                            g[2].append(CivilCvCAD.ActiveDocument.getObject(g[0] + str(e.id)))
        print("groups:", groups)
            ...
            for g in groups:
                if g[2]:
                    if g[0] in ["Building", "Floor"]:
                        ...
                        fcg = CivilCvCAD.ActiveDocument.addObject(
...
def getWire(entity, placement=None):
    "returns a wire (created in the civilcvcad document) from the given entity"
    # only used by the internal parser
    if DEBUG:
...
def export(exportList, filename):
    "called when civilcvcad exports a file"
    global ifcw
    ifcw = None
    try:
    ...
    ver = CivilCvCAD.Version()
    version = ver[0] + "." + ver[1] + " build" + ver[2]
    owner = CivilCvCAD.ActiveDocument.CreatedBy
    company = CivilCvCAD.ActiveDocument.Company
    project = CivilCvCAD.ActiveDocument.Name
        ...
        if obj.isDerivedFrom("App::DocumentObjectGroup"):
            ...
            if product:
                ...
                for g in groups.keys():
                    group = CivilCvCAD.ActiveDocument.getObject(g)
                    if group:
    ...
    for name, entities in groups.items():
        if entities:
            o = CivilCvCAD.ActiveDocument.getObject(name)
            if o:
                if DEBUG:
    ...
    if exporttxt:
        ...
        txtstring = "List of objects exported by CivilCvCAD in file\n"
        txtstring += filename + "\n"
...
def getTuples(data, scale=1, placement=None, normal=None, close=True):
    ...
    if isinstance(data, CivilCvCAD.Vector):
        if placement:
...
def getIfcExtrusionData(obj, scale=1, nosubs=False):
    ...
    if hasattr(obj, "Proxy"):
        if hasattr(obj.Proxy, "getProfiles"):
            ...
            if (len(p) == 1) and v:
                p = p[0]
                r = CivilCvCAD.Placement()
                # b = p.CenterOfMass
                r = obj.Proxy.getPlacement(obj)
                # b = obj.Placement.multVec(CivilCvCAD.Vector())
                # r.Rotation = DraftVecUtils.getRotation(v,CivilCvCAD.Vector(0,0,1))
                d = [
                    r.Base,
                    DraftVecUtils.rounded(r.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0))),
                    DraftVecUtils.rounded(r.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))),
                ]
                # r = r.inverse()
...
class IfcSchema:
    ...
    def __init__(self, filename):
        ...
        if not os.path.exists(filename):
            filename = CivilCvCAD.getUserMacroDir(True) + os.sep + filename
            if not os.path.exists(filename):
