...
def buildRelMattable(ifcfile):
    ...
    for r in ifcfile.by_type("IfcRelAssociatesMaterial"):
        ...
        # https://forum.civilcvcad.org/viewtopic.php?f=39&t=58607
        if r.RelatedObjects:
...
def buildRelColors(ifcfile, prodrepr):
    ...
    for r in ifcfile.by_type("IfcStyledItem"):
        ...
        # see https://forum.civilcvcad.org/viewtopic.php?f=39&t=37940&start=10#p329491
        # last code change in these color code https://github.com/CivilCvCAD/CivilCvCAD/commit/2d1f6ab1
        """
        if r.Item:
            # print(r.id())
...
def getColorFromStyledItem(styled_item):
    ...
    if len(styled_item.Styles) == 0:
        ...
        # see https://forum.civilcvcad.org/viewtopic.php?f=39&t=33560&p=437056#p437056
...
def getIfcPropertySets(ifcfile, pid):
    ...
    for rel in ifcfile[pid].IsDefinedBy:
        ...
        # https://forum.civilcvcad.org/viewtopic.php?f=39&t=37892#p322884
...
def getIfcProperties(ifcfile, pid, psets, d):
    """builds valid property values for CivilCvCAD"""
...
def getIfcPsetProperties(ifcfile, pid):
    """directly build the property table from pid and ifcfile for CivilCvCAD"""
...
def getRotation(entity):
    """returns a CivilCvCAD rotation from an IfcProduct with a IfcMappedItem representation"""
    try:
        u = CivilCvCAD.Vector(entity.Axis1.DirectionRatios)
        v = CivilCvCAD.Vector(entity.Axis2.DirectionRatios)
        w = CivilCvCAD.Vector(entity.Axis3.DirectionRatios)
    except AttributeError:
        return CivilCvCAD.Rotation()
    return CivilCvCAD.Rotation(u, v, w, "ZYX")
...
def getPlacement(entity, scaling=1000):
    ...
    if entity.is_a("IfcAxis2Placement3D"):
        ...
        pl.move(loc)
    if pl:
        pl.Base = CivilCvCAD.Vector(pl.Base).multiply(scaling)
    return pl
...
def getVector(entity, scaling=1000):
    ...
    if entity.is_a("IfcDirection"):
        if len(entity.DirectionRatios) == 3:
            v = CivilCvCAD.Vector(tuple(entity.DirectionRatios))
        else:
            v = CivilCvCAD.Vector(tuple(entity.DirectionRatios + [0]))
    elif entity.is_a("IfcCartesianPoint"):
        if len(entity.Coordinates) == 3:
            v = CivilCvCAD.Vector(tuple(entity.Coordinates))
        else:
            v = CivilCvCAD.Vector(tuple(entity.Coordinates + [0]))
    # if v:
    #     v.multiply(scaling)
    return v
...
def get2DShape(representation, scaling=1000, notext=False):
    ...
    def getPolyline(ent):
        ...
        for p in ent.Points:
            c = p.Coordinates
            c = CivilCvCAD.Vector(c[0], c[1], c[2] if len(c) > 2 else 0)
            c.multiply(scaling)
            pts.append(c)
        ...
        for el in elts:
            if el.is_a("IfcPolyline"):
                ...
                if base.is_a("IfcPolyline"):
                    ...
                    v = CivilCvCAD.Vector(d[0], d[1], d[2] if len(d) > 2 else 0)
                    a = -DraftVecUtils.angle(v)
                    e.rotate(bc.Curve.Center, CivilCvCAD.Vector(0, 0, 1), math.degrees(a))
                    result.append(e)
                ...
                if not el.Segments:
                    # use all points
                    verts = [CivilCvCAD.Vector(c[0], c[1], c[2] if len(c) > 2 else 0) for c in coords]
...
def createFromProperties(propsets, ifcfile, parametrics):
    """
    Creates a CivilCvCAD parametric object from a set of properties.
    """
    ...
    for pset in propsets.keys():
        if ifcfile[pset].Name == "CivilCvCADPropertySet":
            appset = {}
            for pid in propsets[pset]:
                p = ifcfile[pid]
                appset[p.Name] = p.NominalValue.wrappedValue
        elif ifcfile[pset].Name == "CivilCvCADGuiPropertySet":
            guiset = {}
            for pid in propsets[pset]:
                p = ifcfile[pid]
    ...
    if appset:
        oname = None
        otype = None
        if "CivilCvCADType" in appset:
            if "CivilCvCADName" in appset:
                obj = CivilCvCAD.ActiveDocument.addObject(appset["CivilCvCADType"], appset["CivilCvCADName"])
                if "CivilCvCADAppObject" in appset:
                    mod, cla = appset["CivilCvCADAppObject"].split(".")
                    if "'" in mod:
                        mod = mod.split("'")[-1]
                    if "'" in cla:
                        cla = cla.split("'")[0]
                ...
                if CivilCvCAD.GuiUp:
                    if guiset:
                        if "CivilCvCADGuiObject" in guiset:
                            mod, cla = guiset["CivilCvCADGuiObject"].split(".")
                            if "'" in mod:
                                mod = mod.split("'")[-1]
                            if "'" in cla:
                                cla = cla.split("'")[0]
    ...
    if obj and sets:
        for realm, pset in sets:
            ...
            for key, val in pset.items():
                if key.startswith("CivilCvCAD_") or key.startswith("CivilCvCADGui_"):
                    name = key.split("_")[1]
                    if name in target.PropertiesList:
                        if not target.getEditorMode(name):
                            ...
                            if ptype in [
                                ...
                                v = CivilCvCAD.Vector([float(s) for s in data[0].split(",")])
                                r = CivilCvCAD.Rotation(*[float(s) for s in data[1].split(",")])
                                setattr(target, name, CivilCvCAD.Placement(v, r))
                            elif ptype == "App::PropertyLink":
                                link = val.split("_")[1]
                                parametrics.append([target, name, link])
                            else:
                                print("Unhandled CivilCvCAD property:", name, " of type:", ptype)
    ...
    if annotation.is_a("IfcGrid"):
        ...
        for axis in uvwaxes:
            if axis.AxisCurve:
                ...
                if sh and (len(sh[0].Vertexes) == 2):  # currently only straight axes are supported
                    ...
                    pl.Rotation = CivilCvCAD.Rotation(
                        CivilCvCAD.Vector(0, 1, 0), sh.Vertexes[-1].Point.sub(sh.Vertexes[0].Point)
                    )
                    o = Arch.makeAxis(1, l)
                    o.Length = l
                    o.Placement = pl
        ...
        if axes:
            ...
            if annotation.ObjectPlacement:
                # https://forum.civilcvcad.org/viewtopic.php?f=39&t=40027
