...
# SweetHome3D is in cm while CivilCvCAD is in mm
FACTOR = 10
TOLERANCE = 1
...
class SH3DImporter:
    ...
    def _import_home(self, home):
        ...
        # Get all the CivilCvCAD object in the active doc, in order to allow
    ...
    def _create_groups(self):
        """Create CivilCvCAD Group for the different imported elements"""
        doc = App.ActiveDocument
...
class BaseHandler:
    ...
    def setp(self, obj, type_, name, description, value=None, valid_values=None):
        ...
        Args:
            obj (CivilCvCAD): the object on which to set the property
...
class WallHandler(BaseHandler):
    ...
    def _create_wall(self, floor, prev, next, elm):
        ...
        # See https://github.com/CivilCvCAD/CivilCvCAD/issues/18658 and related OCCT
        #   ticket
...
class DoorOrWindowHandler(BaseFurnitureHandler):
    ...
    def _create_door(self, floor, elm):
        ...
        # CivilCvCAD is rather confusing and tricky to get right.
        ...
        # See the https://wiki.civilcvcad.org/Arch_Window for details about these values
...
class FurnitureHandler(BaseFurnitureHandler):
    ...
    def _create_furniture(self, floor, elm):
        ...
        if len(mesh.Points) == 0:
            # Until https://github.com/CivilCvCAD/CivilCvCAD/issues/19456 is solved...
...
def dim_sh2fc(dimension):
    """Convert SweetHome dimension (cm) to CivilCvCAD dimension (mm)

    Args:
        dimension (float): The dimension in SweetHome

    Returns:
        float: the CivilCvCAD dimension
    """
    return float(dimension) * FACTOR


def dim_fc2sh(dimension):
    """Convert CivilCvCAD dimension (mm) to SweetHome dimension (cm)

    Args:
        dimension (float): The dimension in CivilCvCAD
...
def coord_sh2fc(vector):
    """Converts SweetHome to CivilCvCAD coordinate

    Args:
        CivilCvCAD.Vector (CivilCvCAD.Vector): The coordinate in SweetHome

    Returns:
        CivilCvCAD.Vector: the CivilCvCAD coordinate
    """
    return App.Vector(vector.x * FACTOR, -vector.y * FACTOR, vector.z * FACTOR)


def ang_sh2fc(angle: float):
    """Convert SweetHome angle (º) to CivilCvCAD angle (º)

    SweetHome angles are clockwise positive while CivilCvCAD are anti-clockwise
    positive. Further more angle in CivilCvCAD are always positive between 0º and
    360º.
