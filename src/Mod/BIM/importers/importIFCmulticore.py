...
"""CivilCvCAD IFC importer - Multicore version"""
...
# global dicts to store ifc object/civilcvcad object relationships
...
def insert(filename, docname=None, preferences=None):
    ...
    if annotations:
        ...
        for annotation in annotations:
            importIFCHelper.createAnnotation(
                annotation, CivilCvCAD.ActiveDocument, ifcscale, preferences
            )
    ...
    return CivilCvCAD.ActiveDocument
...
def storeColorDict():
    ...
    if colors and not CivilCvCAD.GuiUp:
        import json

        d = CivilCvCAD.ActiveDocument.Meta
        d["colordict"] = json.dumps(colors)
        CivilCvCAD.ActiveDocument.Meta = d
