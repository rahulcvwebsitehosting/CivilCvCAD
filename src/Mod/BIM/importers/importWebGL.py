...
# https://forum.civilcvcad.org/viewtopic.php?t=51245
# https://forum.civilcvcad.org/viewtopic.php?t=29487
...
#    The 'camera' string for the active document may be generated from: import OfflineRenderingUtils; OfflineRenderingUtils.getCamera(CivilCvCAD.ActiveDocument.FileName);
#
# Development reload oneliner:
# def re(): from importlib import reload;import importWebGL;reload(importWebGL);o=CivilCvCAD.getDocument("YourDocName");importWebGL.export([o.getObject("YourBodyName")],u"C:/path/to/your/file.htm");

## @package importWebGL
#  \ingroup ARCH
#  \brief CivilCvCAD WebGL Exporter
#
...
def getHTMLTemplate():
    ...
    if using_custom_template:
        ...
        if custom_content:
            CivilCvCAD.Console.PrintMessage(f"Using custom template file '{custom_path}'.\n")
            return custom_content
        else:
            # Custom template failed - ask user or auto-fallback
            if not CivilCvCADGui:
    ...
    if default_content:
        CivilCvCAD.Console.PrintMessage(f"Using default template file '{default_template_path}'.\n")
    ...
    if CivilCvCADGui:
        ...
        message = translate(
            ...
            " {}\n\nPlease check your CivilCvCAD installation or provide a "
            "custom template under menu Preferences → Import-Export → WebGL.",
        ).format(default_template_path)

        # Use getMainWindow() as parent following CivilCvCAD patterns
        parent = CivilCvCADGui.getMainWindow()
...
def export(
    ...
    for obj in objectslist:
            ...
            while True:  # drill down to get to the actual obj
                if obj.isDerivedFrom("App::Link"):
                    ...
                    if hasattr(obj, "__len__"):
                        CivilCvCAD.Console.PrintMessage(f"{label}: Sub-Links are Unsupported.\n")
                        break
        ...
        if obj.isDerivedFrom("Part::Feature"):
            deviation = 0.5
            if CivilCvCADGui and hasattr(obj.ViewObject, "Deviation"):
                ...
                for f in objShape.Faces:
                    ...
                    for fv in range(len(faceData[0])):
                        ...
                        if not found:
                            break
                    if not found:
                        CivilCvCAD.Console.PrintMessage("Facet to Face Mismatch.\n")
                    ...
                    for ff in faceData[1]:  # face facets
                        ...
                        if not found:
                            break
                    if not found:
                        CivilCvCAD.Console.PrintMessage("Facet List Mismatch.\n")
                        objdata["facesToFacets"] = []
                        break
    ...
    html = html.replace("$pagetitle", CivilCvCAD.ActiveDocument.Label)
    ...
    with pyopen(filename, "w", encoding="utf-8") as outfile:
        outfile.write(html)
    CivilCvCAD.Console.PrintMessage(translate("Arch", "Successfully written") + f" {filename}\n")
    return True
...
def get_view_properties(obj, label: str, colors: dict[str, str] | None) -> tuple[str, float]:
    ...
    color = "#cccccc"
    opacity = 1.0
    if CivilCvCADGui and hasattr(obj.ViewObject, "ShapeColor"):
...
def populate_camera(data: CameraDict, camera: str | None):
    if not CivilCvCADGui and not camera:
        camera = OfflineRenderingUtils.getCamera(CivilCvCAD.ActiveDocument.FileName)
