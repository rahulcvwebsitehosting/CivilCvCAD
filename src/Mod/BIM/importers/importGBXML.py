...
__title__ = "CivilCvCAD GbXml exporter"
...
def export(objectslist, filename):
    ...
    filestream.write(
        "<!-- Exported by CivilCvCAD %s -->\n" % CivilCvCAD.Version()[0]
        + CivilCvCAD.Version()[1]
        + CivilCvCAD.Version()[2]
    )
    ...
    for building in site.Group:
        if Draft.getType(building) == "Building":
            ...
            for space in Draft.getObjectsOfType(
                ...
                filestream.write(
                    "            <Volume>$f</Volume>\n"
                    % CivilCvCAD.Units.Quantity(space.Shape.Volume, CivilCvCAD.Units.Volume).getValueAs(
                        "m^3"
                    )
                )
