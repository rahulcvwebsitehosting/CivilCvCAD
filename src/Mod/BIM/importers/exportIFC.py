...
__title__ = "CivilCvCAD IFC export"
...
# Specific CivilCvCAD <-> IFC slang translations
translationtable = {
    "Foundation": "Footing",
...
#4=IFCAPPLICATION(#2,'$version','CivilCvCAD','118df2cf_ed21_438e_a41');
...
def _prepare_export_list_skipping_std_groups(initial_export_list, preferences_dict):
    ...
    standard groups is active. Standard CivilCvCAD groups (App::DocumentObjectGroup that would become
    ...
    The re-parenting of children of a skipped CivilCvCAD group in the resulting IFC file is achieved
    implicitly:

    1. The skipped CivilCvCAD group itself is not converted into an IFC product. It will not exist as
    ...
    3. These IFC products, initially "orphaned" from the skipped CivilCvCAD group's potential IFC
    ...
    for obj in all_potential_objects:
        ...
        # Determine if the current object is a standard CivilCvCAD group that should be skipped
...
def getPreferences():
    ...
    preferences = {
        ...
        "FULL_PARAMETRIC": params.get_param_arch("IfcExportCivilCvCADProperties"),
...
def export(exportList, filename, colors=None, preferences=None):
    ...
    try:
        ...
        _err(
            ...
            "Install it from trusted local media and restart CivilCvCAD."
        )
        return
    ...
    if isinstance(filename, ifcopenshell.file):
        ...
        version = CivilCvCAD.Version()
        owner = CivilCvCAD.ActiveDocument.CreatedBy
        email = ""
        ...
        template = template.replace("$company", CivilCvCAD.ActiveDocument.Company)
    ...
    if preferences["IGNORE_STD_GROUPS"]:
        if preferences["DEBUG"]:
            print("IFC Export: Skipping standard CivilCvCAD groups and processing their children.")
    ...
    uids = (
        []
    )  # store used UIDs to avoid reuse (some CivilCvCAD objects might have same IFC UID, ex. copy/pasted objects
    ...
    for obj in objectslist:
        ...
        if ifctype == "IfcArray":
            ...
            if hasattr(obj, "Group"):
                ...
                group = [CivilCvCAD.ActiveDocument.getObject(n[:-1]) for n in obj.getSubObjects()]
            for subobj in group:
            ...
            for axg in axgroups:
                ...
                for ax in axg:
                    p1 = ifcbin.createIfcCartesianPoint(
                        tuple(CivilCvCAD.Vector(ax[0]).multiply(preferences["SCALE_FACTOR"])[:2])
                    )
                    p2 = ifcbin.createIfcCartesianPoint(
                        tuple(CivilCvCAD.Vector(ax[1]).multiply(preferences["SCALE_FACTOR"])[:2])
                    )
        ...
        if hasattr(obj, "IfcProperties"):
            ...
            if obj.IfcProperties:
                ...
                if isinstance(obj.IfcProperties, dict):
                    ...
                    # IfcProperties is a dictionary (CivilCvCAD 0.18)
        ...
        if preferences["FULL_PARAMETRIC"]:
            ...
            CivilCvCADProps = []
            CivilCvCADGuiProps = []
            CivilCvCADProps.append(
                ifcbin.createIfcPropertySingleValue("CivilCvCADType", "IfcText", obj.TypeId)
            )
            CivilCvCADProps.append(
                ifcbin.createIfcPropertySingleValue("CivilCvCADName", "IfcText", obj.Name)
            )
            sets = [("App", obj)]
            if hasattr(obj, "Proxy"):
                if obj.Proxy:
                    CivilCvCADProps.append(
                        ifcbin.createIfcPropertySingleValue(
                            "CivilCvCADAppObject", "IfcText", str(obj.Proxy.__class__)
                        )
                    )
            if CivilCvCAD.GuiUp:
                if obj.ViewObject:
                        ...
                        if not (
                            ...
                            if itype:
                                ...
                                if realm == "Gui":
                                    CivilCvCADGuiProps.append(
                                        ifcbin.createIfcPropertySingleValue(
                                            "CivilCvCADGui_" + prop, itype, ivalue
                                        )
                                    )
                                else:
                                    CivilCvCADProps.append(
                                        ifcbin.createIfcPropertySingleValue(
                                            "CivilCvCAD_" + prop, itype, ivalue
                                        )
                                    )
            if CivilCvCADProps:
                pset = ifcfile.createIfcPropertySet(
                    ifcopenshell.guid.new(), history, "CivilCvCADPropertySet", None, CivilCvCADProps
                )
                ifcfile.createIfcRelDefinesByProperties(
                    ifcopenshell.guid.new(), history, None, None, [product], pset
                )
            if CivilCvCADGuiProps:
                pset = ifcfile.createIfcPropertySet(
                    ifcopenshell.guid.new(), history, "CivilCvCADGuiPropertySet", None, CivilCvCADGuiProps
                )
                ifcfile.createIfcRelDefinesByProperties(
                    ifcopenshell.guid.new(), history, None, None, [product], pset
                )
    ...
    for k, v in products.items():
        if not (k in treated):
            if (not buildings) or (k != buildings[0].Name):
                if not (
                    Draft.getType(CivilCvCAD.ActiveDocument.getObject(k))
                    in ["Site", "Building", "Floor", "BuildingPart"]
                ):
                    untreated.append(v)
                elif Draft.getType(CivilCvCAD.ActiveDocument.getObject(k)) == "BuildingPart":
                    if not (
                        CivilCvCAD.ActiveDocument.getObject(k).IfcType
    ...
    while groups:
        for g in groups.keys():
            okay = True
            for c in groups[g]:
                if Draft.getType(CivilCvCAD.ActiveDocument.getObject(c)) in ["Group", "VisGroup"]:
                    okay = False
    ...
    for g in sortedgroups:
        if g[1]:
            ...
            if children:
                name = CivilCvCAD.ActiveDocument.getObject(g[0]).Label
    ...
    stack = {}
    for g in sortedgroups:
        go = CivilCvCAD.ActiveDocument.getObject(g[0])
...
def getPropertyData(key, value, preferences):
    ...
    if ptype in ["IfcLabel", "IfcText", "IfcIdentifier", "IfcDescriptiveMeasure"]:
        ...
        try:
            ...
            try:
                pvalue = CivilCvCAD.Units.Quantity(pvalue).Value
            except Exception:
                if preferences["DEBUG"]:
                    print(
...
def getIfcTypeFromObj(obj):
    ...
    if (dtype == "BuildingPart") and hasattr(obj, "IfcType") and (obj.IfcType == "Undefined"):
        ...
        # https://forum.civilcvcad.org/viewtopic.php?p=662934#p662927
...
def exportIfcAttributes(obj, kwargs, scale=0.001):
    ...
    for property in obj.PropertiesList:
        if obj.getGroupOfProperty(property) == "IFC Attributes" and obj.getPropertyByName(property):
            ...
            if isinstance(value, CivilCvCAD.Units.Quantity):
                value = float(value)
...
def getRepresentation(
        ...
        for k, v in clones.items():
            if (obj.Name == k) or (obj.Name in v):
                if k in sharedobjects:
                    ...
                    pos = CivilCvCAD.Vector(pla.Base)
                    if isinstance(forceclone, CivilCvCAD.Vector):
                        pos += forceclone
                    axis1 = ifcbin.createIfcDirection(
                        tuple(pla.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0)))
                    )
                    axis2 = ifcbin.createIfcDirection(
                        tuple(pla.Rotation.multVec(CivilCvCAD.Vector(0, 1, 0)))
                    )
                    axis3 = ifcbin.createIfcDirection(
                        tuple(pla.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1)))
                    )
    ...
    if (not shapes) and (not forcebrep) and (not skipshape):
        ...
        if hasattr(obj, "Proxy"):
            ...
            if (not shapes) and hasattr(obj.Proxy, "getExtrusionData"):
                ...
                if extdata:
                    ...
                    for evi in ev:
                        if not isinstance(evi, CivilCvCAD.Vector):
                    ...
                    if simpleExtrusion:
                        for i in range(len(p)):
                            ...
                            if i < len(ev):
                                evi = CivilCvCAD.Vector(ev[i])
                            else:
                                evi = CivilCvCAD.Vector(ev[-1])
                            ...
                            if pstr in profiledefs:
                                ...
                                # https://forum.civilcvcad.org/viewtopic.php?p=771954#p771954
                            ...
                            if profiles and not (DraftVecUtils.isNull(evi)):
                                for profile in profiles:
                                    ...
                                    xvc = ifcbin.createIfcDirection(
                                        tuple(pli.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0)))
                                    )
                                    zvc = ifcbin.createIfcDirection(
                                        tuple(pli.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1)))
                                    )
                                    ...
                                    edir = ifcbin.createIfcDirection(
                                        tuple(CivilCvCAD.Vector(evi).normalize())
                                    )
        ...
        if (not shapes) and obj.isDerivedFrom("Part::Extrusion"):
            ...
            xvc = ifcbin.createIfcDirection(tuple(pl.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0))))
            zvc = ifcbin.createIfcDirection(tuple(pl.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))))
            ovc = ifcbin.createIfcCartesianPoint(tuple(pl.Base))
            lpl = ifcbin.createIfcAxis2Placement3D(ovc, zvc, xvc)
            edir = ifcbin.createIfcDirection(tuple(CivilCvCAD.Vector(ev).normalize()))
                            ...
                            if obj.Base.isDerivedFrom("Part::Feature"):
                                if obj.Base.Shape:
                                    if obj.Base.Shape.Solids:
                                        ...
                                        subplacement = CivilCvCAD.Placement(obj.Placement)
                    if not fcshape:
                        if obj.Shape:
                ...
                if shapedef in shapedefs:
                    ...
                    if (
                        ...
                        if obj.Shape.Faces:
                            ...
                            # https://forum.civilcvcad.org/viewtopic.php?p=675760#p675760
                    ...
                    if not serialized:
                        ...
                        if tostore:
                            fcshape.Placement = CivilCvCAD.Placement()
    ...
    if shapes:
        ...
        if tostore:
            ...
            if isinstance(forceclone, CivilCvCAD.Vector):
                pla.Base += forceclone
            axis1 = ifcbin.createIfcDirection(tuple(pla.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0))))
            axis2 = ifcbin.createIfcDirection(tuple(pla.Rotation.multVec(CivilCvCAD.Vector(0, 1, 0))))
            origin = ifcbin.createIfcCartesianPoint(
                tuple(CivilCvCAD.Vector(pla.Base).multiply(preferences["SCALE_FACTOR"]))
            )
            axis3 = ifcbin.createIfcDirection(tuple(pla.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))))
            transf = ifcbin.createIfcCartesianTransformationOperator3D(
                axis1, axis2, origin, 1.0, axis3
            )
            mapitem = ifcfile.createIfcMappedItem(repmap, transf)
        ...
        if colors:
            ...
            if obj.Name in colors:
                ...
                if isinstance(color[0], tuple):
                    ...
                    diffusecolor = color
                    shapecolor = color[0]
        elif CivilCvCAD.GuiUp and (not subtraction) and hasattr(obj.ViewObject, "ShapeColor"):
...
def createProduct(
    ...
    # https://forum.civilcvcad.org/viewtopic.php?f=39&t=50085
    if ifctype not in ArchIFCSchema.IfcProducts:
...
def createAxis(ifcfile, obj, preferences, delta=None):
    ...
    shape = None
    pla = CivilCvCAD.Placement(obj.Placement)
    if isinstance(delta, CivilCvCAD.Vector):
        pla.Base += delta
...
def create_annotation(anno, ifcfile, context, history, preferences):
    ...
    if anno.isDerivedFrom("Part::Feature"):
        ...
        objectType = "TEXT"
        l = CivilCvCAD.Vector(anno.Position).multiply(preferences["SCALE_FACTOR"])
        ...
        objectType = "TEXT"
        l = CivilCvCAD.Vector(anno.Placement.Base).multiply(preferences["SCALE_FACTOR"])
        pos = ifcbin.createIfcCartesianPoint((0.0, 0.0, 0.0))
        tpl = ifcbin.createIfcAxis2Placement3D(pos, None, None)
        ovc = ifcbin.createIfcCartesianPoint((l.x, l.y, l.z))
        zvc = ifcbin.createIfcDirection(
            tuple(anno.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1)))
        )
        xvc = ifcbin.createIfcDirection(
            tuple(anno.Placement.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0)))
        )
        alg = "LEFT"
        if CivilCvCAD.GuiUp and hasattr(anno.ViewObject, "Justification"):
        ...
        if CivilCvCAD.GuiUp:
            ...
            # Append text
            l = CivilCvCAD.Vector(vp.tbase).multiply(preferences["SCALE_FACTOR"])
            zdir = None
            xdir = None
            if hasattr(vp, "trot"):
                r = CivilCvCAD.Rotation(vp.trot[0], vp.trot[1], vp.trot[2], vp.trot[3])
                zdir = ifcbin.createIfcDirection(tuple(r.multVec(CivilCvCAD.Vector(0, 0, 1))))
                xdir = ifcbin.createIfcDirection(tuple(r.multVec(CivilCvCAD.Vector(1, 0, 0))))
        ...
        p = CivilCvCAD.Vector(anno.Placement.Base).multiply(preferences["SCALE_FACTOR"])
        ...
        if anno.Depth.Value:
            h = anno.Depth.Value
        l = CivilCvCAD.Vector(l, w, h).multiply(preferences["SCALE_FACTOR"])
