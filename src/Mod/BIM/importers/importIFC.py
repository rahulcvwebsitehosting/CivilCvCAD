...
__title__ = "CivilCvCAD IFC importer - Enhanced IfcOpenShell-only version"
...
# which IFC type must create which CivilCvCAD type
...
def insert(srcfile, docname, skip=[], only=[], root=None, preferences=None):
    ...
    # See https://forum.civilcvcad.org/viewtopic.php?f=39&t=37892
    ...
    # start the actual import, set CivilCvCAD UI
    count = 0
    ...
    if preferences["FITVIEW_ONIMPORT"] and CivilCvCAD.GuiUp:
    ...
    if not preferences["REPLACE_PROJECT"]:
        if len(ifcfile.by_type("IfcProject")) > 0:
            ...
            # https://forum.civilcvcad.org/viewtopic.php?f=39&t=40624
    ...
    for product in products:
        ...
        # checking for full CivilCvCAD parametric definition, overriding everything else
        if psets and params.get_param_arch("IfcImportCivilCvCADProperties"):
            if "CivilCvCADPropertySet" in [ifcfile[pset].Name for pset in psets.keys()]:
                if preferences["DEBUG"]:
        ...
        if brep:
            ...
            shape.scale(
                1000.0
            )  # IfcOpenShell always outputs in meters, we convert to mm, the civilcvcad internal unit
                        ...
                        if preferences["GET_EXTRUSIONS"] and (preferences["MERGE_MODE_ARCH"] != 1):
                            ...
                            if ex:
                                ...
                                if profileid and (profileid in profiles):
                                    ...
                                    addplacement = CivilCvCAD.Placement()
        ...
        if preferences["MERGE_MODE_ARCH"] == 0 and archobj:
            ...
            for civilcvcadtype, ifctypes in typesmap.items():

                if ptype not in ifctypes:
                    continue
                if clone:
                    obj = getattr(Arch, "make" + civilcvcadtype)(name=name)
                    obj.CloneOf = clone
                    ...
                    # no clone
                    obj = getattr(Arch, "make" + civilcvcadtype)(baseobj=baseobj, name=name)
                    if (
                        civilcvcadtype in ["Wall", "Structure"]
                        and baseobj
                        ...
                        obj.Normal = CivilCvCAD.Vector(baseobj.Dir).normalize()
                        bn = baseobj.Name
                        doc.removeObject(bn)
                    if (civilcvcadtype in ["Structure", "Wall"]) and not baseobj:
                        # remove sizes to prevent auto shape creation for types that don't require a base object
                        obj.Height = 0
                        obj.Width = 0
                        obj.Length = 0
                    if (civilcvcadtype in ["Rebar"]) and baseobj:
            ...
            if hasattr(obj, "IfcData"):
                ...
                for attribute in ArchIFCSchema.IfcProducts[product.is_a()]["attributes"]:
                    ...
                    if (
                        ...
                        # TODO: ArchIFCSchema.IfcProducts uses the IFC version from the CivilCvCAD prefs.
    ...
    for material in materials:
        ...
        # https://forum.civilcvcad.org/viewtopic.php?f=23&t=63260
                ...
                if o in objects:
                    if hasattr(objects[o], "Material"):
                        ...
                        if CivilCvCAD.GuiUp:
                            ...
                            # all viewers use the shape color whereas in CivilCvCAD the shape color will be
                            # overwritten by the material color (if there is a material with a color).
                            # In such a case CivilCvCAD shows a different color than all common ifc viewers
                            # https://forum.civilcvcad.org/viewtopic.php?f=39&t=38440
    ...
    if colordict and not CivilCvCAD.GuiUp:
        import json
    ...
    if CivilCvCAD.GuiUp and ZOOMOUT:
        Gui.SendMsgToActiveView("ViewFit")
