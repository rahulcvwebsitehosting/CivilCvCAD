...
"""Core API for architectural and Building Information Modeling (BIM) in CivilCvCAD.
...
- Integration with CivilCvCAD's core (Part, Draft) and other workbenches
- Object creation utilities for architectural workflows

## Usage
Designed for:
1. Internal API for CivilCvCAD's built-in BIM commands
...
def makeAxis(num=1, size=1000, name=None):
    ...
    if not CivilCvCAD.ActiveDocument:
        ...
        return
    obj = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "Axis")
...
def makeAxisSystem(axes, name=None):
    ...
    if not isinstance(axes, list):
        axes = [axes]
    obj = CivilCvCAD.ActiveDocument.addObject("App::FeaturePython", "AxisSystem")
...
def makeBuildingPart(objectslist=None, baseobj=None, name=None):
    ...
    obj = CivilCvCAD.ActiveDocument.addObject("App::GeometryPython", "BuildingPart")
    # obj = CivilCvCAD.ActiveDocument.addObject("App::FeaturePython","BuildingPart")
...
def make2DDrawing(objectslist=None, baseobj=None, name=None):
    ...
    if CivilCvCAD.GuiUp:
        obj.ViewObject.DisplayOffset = CivilCvCAD.Placement()
        obj.ViewObject.ShowLevel = False
    return obj
...
def convertFloors(floor=None):
    ...
    if floor:
        ...
        objset = CivilCvCAD.ActiveDocument.Objects
    for obj in objset:
        if Draft.getType(obj) in ["Floor", "Building"]:
            ...
            if obj.ViewObject:
                ...
                # in case CivilCvCAD doesn't allow 2 objs with same label
            obj.Label = obj.Label + " to delete"
            nobj.Label = label
    for n in todel:
        from draftutils import todo

        todo.ToDo.delay(CivilCvCAD.ActiveDocument.removeObject, n)
...
def makeEquipment(baseobj=None, placement=None, name=None):
    ...
    placement : CivilCvCAD.Placement, optional
    ...
    if CivilCvCAD.GuiUp and baseobj:
        baseobj.ViewObject.hide()
    return equipment
...
def makeProfile(profile=[0, "REC", "REC100x100", "R", 100, 100]):
    ...
    profile : list, optional
        ...
        4. dimensionsList: int
            ...
            See https://wiki.civilcvcad.org/Arch_Profile for more details on profile presets.
    ...
    if not CivilCvCAD.ActiveDocument:
        CivilCvCAD.Console.PrintError("No active document. Aborting\n")
        return
    obj = CivilCvCAD.ActiveDocument.addObject("Part::Part2DObjectPython", "Profile")
...
def makeRebar(
    baseobj: Optional[CivilCvCAD.DocumentObject] = None,
    sketch: Optional[CivilCvCAD.DocumentObject] = None,
    diameter: Optional[float] = None,
    amount: int = 1,
    offset: Optional[float] = None,
    name: Optional[str] = None,
) -> Optional[CivilCvCAD.DocumentObject]:
    """
    ...
    Parameters
    ----------
    baseobj : CivilCvCAD.DocumentObject, optional
    ...
    sketch : CivilCvCAD.DocumentObject, optional
    ...
    Returns
    -------
    CivilCvCAD.DocumentObject or None
        The created rebar object, or None if creation fails.

    Examples
    --------
    >>> import CivilCvCAD, Arch, Part, Sketcher
    ...
    >>> rebar_sketch.addGeometry(Part.LineSegment(CivilCvCAD.Vector(25, 25, 0),
    ...                                         CivilCvCAD.Vector(1975, 25, 0)), False)
...
def makeStairs(baseobj=None, length=None, width=None, height=None, steps=None, name=None):
    ...
    if baseobj:
        ...
        if lenSelection > 1:
            stair = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "Stairs")
            stair.Label = label
        ...
        obj = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "Stairs")
        obj.Label = label
...
def makeWall(
    ...
    if wall.Base and CivilCvCAD.GuiUp:
...
def joinWalls(walls, delete=False, deletebase=False):
        ...
        if Draft.getType(base.Base) == "ArchSketch":
            ...
            try:
                ...
                if Draft.getType(base.Base) != "Sketcher::SketchObject":
                    newSk = CivilCvCAD.ActiveDocument.addObject("Sketcher::SketchObject", "WallTrace")
                else:
                    newSk = None
            if newSk:
    ...
    if delete:
        for n in deleteList:
            CivilCvCAD.ActiveDocument.removeObject(n)
    CivilCvCAD.ActiveDocument.recompute()
    if base.Base and CivilCvCAD.GuiUp:
...
def makeWindow(
    ...
    if baseobj and Draft.getType(baseobj) == "Window" and CivilCvCAD.ActiveDocument:
    ...
    if baseobj:
        ...
        # here.
        # obj.Normal = baseobj.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, -1))
        window.Base = baseobj
    ...
    if window.Base and CivilCvCAD.GuiUp:
...
def is_debasable(wall):
    ...
    Parameters
    ----------
    wall : CivilCvCAD.DocumentObject
...
def debaseWall(wall):
    ...
    if not is_debasable(wall):
        CivilCvCAD.Console.PrintWarning(f"Wall '{wall.Label}' is not eligible for debasing.\n")
        return False
    ...
    try:
        ...
        normal = wall.Normal
        if normal.Length == 0:
            normal = base_obj.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1))
            if normal.Length == 0:
                normal = CivilCvCAD.Vector(0, 0, 1)
        ...
        final_rotation = CivilCvCAD.Rotation(x_axis, y_axis, z_axis)
        ...
        final_placement = CivilCvCAD.Placement(final_position, final_rotation)
        ...
        CivilCvCAD.Console.PrintError(f"Error debasing wall '{wall.Label}': {e}\n")
        return False
...
def _initializeArchObject(
    ...
    obj = CivilCvCAD.ActiveDocument.addObject(objectType, internalName)
    if not obj:
        return None
    ...
    try:
        ...
        CivilCvCAD.Console.PrintError(f"Failed to import module '{moduleName}': {e}\n")
        return None
...
def makeReport(name=None):
    ...
    if CivilCvCAD.GuiUp:
        ...
        CivilCvCADGui.ActiveDocument.setEdit(report_obj.Name, 0)
...
def makeStructuralSystem(objects=[], axes=[], name=None):
    ...
    for base_obj in objects:
        ...
        if CivilCvCAD.GuiUp and base_obj:
...
def placeAlongEdge(p1, p2, horizontal=False):
    ...
    placement = CivilCvCAD.Placement()
    placement.Base = p1
    ...
    if cross_section_horizontal.Length > 0:
        ...
        if horizontal:
            placement.Rotation = CivilCvCAD.Rotation(
            ...
            placement.Rotation = CivilCvCAD.Rotation(
                placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, 1)), 90
