...
#  \ingroup UTILITIES
#  \brief Utility functions to work with CivilCvCAD files in console mode
#
...
"""@package docstring
OfflineRenderingUtils - Utilities to help producing files with colors from CivilCvCAD in non-GUI mode
...
civilcvcadFile = os.path.join(os.getcwd(),"testfile.FCStd")
baseFileName = os.path.splitext(civilcvcadFile)[0]

# open CivilCvCAD file

doc = CivilCvCAD.open(civilcvcadFile)
...
colors = OfflineRenderingUtils.getColors(civilcvcadFile,nodiffuse=True)

# get the camera data from the file (used in some functions below)

camera = OfflineRenderingUtils.getCamera(civilcvcadFile)
...
class CivilCvCADGuiHandler(xml.sax.ContentHandler):
    "A XML handler to process the CivilCvCAD GUI xml data, used by getGuiData()"
...
def getGuiData(filename):
    """getGuiData(filename): Extract visual data from a saved CivilCvCAD file.
    ...
    if zdoc:
        if "GuiDocument.xml" in zdoc.namelist():
            ...
            for key, properties in guidata.items():
                ...
                # https://forum.civilcvcad.org/viewtopic.php?t=29382
...
def getColors(filename, nodiffuse=False):
    """getColors(filename,nodiffuse): Extracts the colors saved in a CivilCvCAD file
...
def buildScene(objects, colors=None):
    """buildScene(objects,colors=None): builds a coin node from a given list of CivilCvCAD
...
def getCamera(filepath):
    """getCamera(filepath): Returns a string representing a coin camera node from a given CivilCvCAD
...
def save(document, filename=None, guidata=None, colors=None, camera=None):
    ...
    The type of the "value" contents depends on the type (int, string, float,tuple...) see inside the CivilCvCADGuiHandler
    ...
    in that case the objects will keep their colors when opened in the CivilCvCAD GUI. If given, camera is a string
...
def getUnsigned(color):
    ...
    # https://forum.civilcvcad.org/viewtopic.php?t=19074
...
def buildGuiDocumentFromColors(document, colors, camera=None):
    ...
    guidoc += "<!--\n"
    guidoc += " CivilCvCAD Document, see  for more information…\n"
    guidoc += "-->\n"
...
def extract(filename, inputpath, outputpath=None):
    ...
    stored in filename (a CivilCvCAD or zip file). If outputpath is given, the file is saved as outputpath and
