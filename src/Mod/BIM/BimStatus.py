...
def setStatusIcons(show=True):
    ...
    def toggleBackground(state):
        CivilCvCADGui.runCommand("BIM_Background")

    def setNudge(action):
        utext = action.text().replace("&", "")
        if utext == nudgeLabelsM[0]:
            # load dialog
            form = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogNudgeValue.ui")
    ...
    if mw:
        ...
        if show:
            if statuswidget:
                ...
                statuswidget = CivilCvCADGui.UiLoader().createWidget("Gui::ToolBar")
