...
#
# This file is part of the CivilCvCAD BIM workbench.
...
class TessellationResult:
    ...
    def __init__(
        ...
        self.placement = placement or CivilCvCAD.Placement()
...
def _to_local_space(substrate, origin, u_vec, normal):
    ...
    """
    n_vec = CivilCvCAD.Vector(normal).normalize()
    u_vec = CivilCvCAD.Vector(u_vec).normalize()
    ...
    placement = CivilCvCAD.Placement(origin, CivilCvCAD.Rotation(u_vec, v_vec, n_vec))
...
class Tessellator(ABC):
    ...
    def _overlay_lines(self, substrate, lines, thickness):
        ...
        Part.Compound
        """
        local_normal = CivilCvCAD.Vector(0, 0, 1)
        if thickness > 0:
...
class RectangularTessellator(Tessellator):
    ...
    def _extrude_or_offset(self, substrate):
        ...
        if self.extrude and self.thickness > MIN_EXTRUSION_THICKNESS:
            return substrate.extrude(CivilCvCAD.Vector(0, 0, self.thickness))
        offset_matrix = CivilCvCAD.Matrix()
        offset_matrix.move(CivilCvCAD.Vector(0, 0, VIEWPORT_Z_OFFSET))
    ...
    def _make_h_edges(self, count, step, tile_width, span_start, span_len, bbox):
        ...
        for i in range(-count, count):
            ...
            line_start = CivilCvCAD.Vector(span_start, pos, 0)
            line_end = CivilCvCAD.Vector(span_start + span_len, pos, 0)
            edge_bb = CivilCvCAD.BoundBox()
    ...
    def _make_v_edges(self, count, step, tile_length, span_start, span_len, bbox):
        ...
        for i in range(-count, count):
            ...
            line_start = CivilCvCAD.Vector(pos, span_start, 0)
            line_end = CivilCvCAD.Vector(pos, span_start + span_len, 0)
            edge_bb = CivilCvCAD.BoundBox()
            edge_bb.add(line_start)
            edge_bb.add(line_end)
            if bbox.intersect(edge_bb):
    ...
    def _generate_physical_geo(self, substrate, params):
        ...
        for j in range(-params.count_v, params.count_v):
            ...
            for i in range(-params.count_u, params.count_u):
                ...
                if self.extrude and self.thickness > MIN_EXTRUSION_THICKNESS:
                    tile = Part.makeBox(
                        self.length, self.width, self.thickness, CivilCvCAD.Vector(u_pos, v_pos, 0)
                    )
                else:
                    tile = Part.makePlane(self.length, self.width, CivilCvCAD.Vector(u_pos, v_pos, 0))
                tiles.append(tile)
        ...
        if self.filename and pattern_name:
            ...
            try:
                ...
                if hatch_succeeded:
                    ...
                    final_local_geo = substrate.extrude(CivilCvCAD.Vector(0, 0, self.thickness))

            except Exception as e:
                CivilCvCAD.Console.PrintError(f"ArchTessellation error: {e}\n")
