...
__title__ = "CivilCvCAD Arch Component"
...
class Component(ArchIFC.IfcProduct):
    ...
    Components here: https://wiki.civilcvcad.org/Arch_Component
    ...
    def setProperties(self, obj):
        ...
        https://wiki.civilcvcad.org/property
        """
    ...
    def execute(self, obj):
        ...
        if hasattr(obj.Base, "Shape") and not obj.Base.Shape.isNull():
            ...
            base_shape.Placement = CivilCvCAD.Placement()
    ...
    def onBeforeChange(self, obj, prop):
        ...
        """
        if prop == "Placement":
            self.oldPlacement = CivilCvCAD.Placement(obj.Placement)
        ...
        if obj.Base:
            ...
            # the base is a Part Extrusion
                ...
                if obj.Base.Base and len(obj.Base.Base.Shape.Wires) == 1:
                    ...
                    extrusion = CivilCvCAD.Vector(obj.Base.Dir).normalize()
                    if extrusion.Length == 0:
                        extrusion = CivilCvCAD.Vector(0, 0, 1)
                    else:
                ...
                for sub in obj.Base.Shapes:
                    if sub.isDerivedFrom("Part::Extrusion"):
                        if sub.Base:
                            ...
                            extrusion = CivilCvCAD.Vector(sub.Dir).normalize()
                            if extrusion.Length == 0:
                                extrusion = CivilCvCAD.Vector(0, 0, 1)
                            else:
                                extrusion = placement.inverse().Rotation.multVec(extrusion)
    ...
    def rebase(self, shape, hint=None):
        ...
        r = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 0, 1), n)
        ...
        for s in shape:
            ...
            s.rotate(CivilCvCAD.Vector(0, 0, 0), r.Axis, math.degrees(-r.Angle))
            shapes.append(s)
    ...
    def processSubShapes(self, obj, base, placement=None):
        ...
        if placement:
            if self.isIdentity(placement):
                ...
                placement = CivilCvCAD.Placement(placement)
        ...
        for o in obj.Additions:
            ...
            if not base or base.isNull():
                if hasattr(o, "Shape"):
                    ...
                    if placement:
                        # see https://forum.civilcvcad.org/viewtopic.php?p=579754#p579754
    ...
    def isStandardCase(self, obj):
        ...
        if obj.IfcType + " Standard Case" in ArchIFC.IfcTypes:
            ...
            if obj.Additions or obj.Subtractions:
                return False
            if obj.Placement.Rotation.Axis.getAngle(CivilCvCAD.Vector(0, 0, 1)) > 0.01:
            ...
            if obj.IfcType == "Wall":
                ...
                if (not obj.Base) or (len(obj.Base.Shape.Edges) == 1):
                    if hasattr(obj, "Normal"):
                        if obj.Normal in [CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(0, 0, 1)]:
                            return True
                ...
                if hasattr(obj, "Tool") and obj.Tool:
                    return False
                if obj.Normal == CivilCvCAD.Vector(0, 0, 0):
                    return True
        ...
        if getattr(obj, "Base", None):
            if obj.Base.isDerivedFrom("Part::Feature"):
                ...
                if isinstance(getattr(obj.Base, "Shape", None), Part.Shape):
                    ...
                    CivilCvCAD.Console.PrintError(obj.Label + ": " + t + "\n")
                    return False
    ...
    def _isInternalLinkgroup(self, obj):
        ...
        # https://forum.civilcvcad.org/viewtopic.php?p=820487#p820428
...
class AreaCalculator:
    ...
    def isFaceVertical(self, face, face_index=None):
        ...
        https://github.com/CivilCvCAD/CivilCvCAD/issues/14687.
        """
        import Part
        import DraftGeomUtils
        import TechDraw
        ...
        if face.Surface.TypeId == "Part::GeomCylinder":
            angle = face.Surface.Axis.getAngle(CivilCvCAD.Vector(0, 0, 1))
            return self.isZeroAngle(angle)
        elif face.Surface.TypeId == "Part::GeomSurfaceOfExtrusion":
            angle = face.Surface.Direction.getAngle(CivilCvCAD.Vector(0, 0, 1))
            return self.isZeroAngle(angle)
            ...
            try:
                edges = TechDraw.project(face, CivilCvCAD.Vector(0, 0, 1))[0].Edges
        ...
        try:
            angle = face.normalAt(0, 0).getAngle(CivilCvCAD.Vector(0, 0, 1))
    ...
    def _computeHorizontalAreaAndPerimeter(self, horizontalAreaFaces):
        ...
        param_grp = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/TechDraw/debug")
        ...
        direction = CivilCvCAD.Vector(0, 0, 1)
        projectedFaces = []
...
class ViewProviderComponent:
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        menu: <PySide2.QtWidgets.QMenu>
            ...
            called.
        """
        if CivilCvCADGui.activeWorkbench().name() != "BIMWorkbench":
            return
    ...
    def edit(self):
        CivilCvCADGui.ActiveDocument.setEdit(self.Object, 0)

    def toggleSubcomponents(self):
        CivilCvCADGui.runCommand("Arch_ToggleSubs")
...
class ArchSelectionObserver:
    ...
    def addSelection(self, document, object, element, position):
        ...
        if not self.watched:
            CivilCvCADGui.Selection.removeObserver(CivilCvCAD.ArchObserver)
            if self.nextCommand:
                CivilCvCADGui.runCommand(self.nextCommand)
            del CivilCvCAD.ArchObserver
        elif object == self.watched.Name:
            if not element:
                CivilCvCAD.Console.PrintMessage(translate("Arch", "Closing Sketch edit"))
                if self.hide:
                    if self.origin:
                        self.origin.ViewObject.Transparency = 0
                        self.origin.ViewObject.Selectable = True
                    self.watched.ViewObject.hide()
                CivilCvCADGui.activateWorkbench("BIMWorkbench")
                if hasattr(CivilCvCAD, "ArchObserver"):
                    CivilCvCADGui.Selection.removeObserver(CivilCvCAD.ArchObserver)
                    del CivilCvCAD.ArchObserver
                if self.nextCommand:
                    CivilCvCADGui.Selection.clearSelection()
                    CivilCvCADGui.Selection.addSelection(self.watched)
                    CivilCvCADGui.runCommand(self.nextCommand)
...
class ComponentTaskPanel:
    ...
    def __init__(self):
        ...
        try:
            ...
            if not "BIM_Classification" in CivilCvCADGui.listCommands():
                CivilCvCADGui.activateWorkbench("BIMWorkbench")
    ...
    def removeElement(self):
        ...
        element_to_remove = CivilCvCAD.ActiveDocument.getObject(str(element_selected.toolTip(0)))
    ...
    def editObject(self, wid, col):
        ...
        if wid.parent():
            obj = CivilCvCAD.ActiveDocument.getObject(str(wid.toolTip(0)))
            if obj:
                ...
                CivilCvCAD.ArchObserver = ArchSelectionObserver(self.obj, obj)
                CivilCvCADGui.Selection.addObserver(CivilCvCAD.ArchObserver)
                CivilCvCADGui.ActiveDocument.setEdit(obj.Name, 0)
    ...
    def editIfcProperties(self):
        ...
        psetspath = os.path.join(
            CivilCvCAD.getResourceDir(), "Mod", "BIM", "Presets", "pset_definitions.csv"
        )
        ...
        self.psetkeys.sort()
        self.ifcEditor = CivilCvCADGui.PySideUic.loadUi(":/ui/dialogIfcPropertiesRedux.ui")
    ...
    def editClass(self):
        ...
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCADGui.Selection.addSelection(self.obj)
        CivilCvCADGui.runCommand("BIM_Classification")
...
class ComponentOptionsTaskPanel(ComponentTaskPanel):
    ...
    def __init__(self, obj, property_definitions):
        """
        obj: The CivilCvCAD object being edited.
        ...
        for item in self.property_definitions:
            ...
            if prop_type_id in [
                ...
                CivilCvCADGui.ExpressionBinding(widget).bind(target_obj, prop_name)
            ...
            # Float
                ...
                widget.setDecimals(
                    CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Units").GetInt(
                        "Decimals", 2
                    )
                )
    ...
    def accept(self):
        ...
        for prop_name, data in self.property_widgets.items():
            ...
            try:
                ...
                CivilCvCAD.Console.PrintError(f"{msg} {prop_name}: {str(e)}\n")
        ...
        def createEditor(self, parent, option, index):
            ...
            if index.column() == 0:  # property name
                ...
                if "Integer" in ptype:
                    ...
                    editor = CivilCvCADGui.UiLoader().createWidget("Gui::InputField")
