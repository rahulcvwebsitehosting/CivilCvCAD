...
# CivilCvCAD-Reporting workbench (https://github.com/furti/CivilCvCAD-Reporting).
...
def _get_property(obj, prop_name):
    """Gets a property from a CivilCvCAD object, including sub-properties.
    ...
    if not is_nested_property(prop_name):
        ...
        if hasattr(obj, prop_name):
            ...
            if isinstance(value, CivilCvCAD.DocumentObject):
                return value.Label
            return value
        return None
    else:
...
def _map_results_to_objects(headers, data_rows):
    """
    Maps the raw data rows from a query result back to CivilCvCAD DocumentObjects.
    ...
    Returns
    -------
    list of CivilCvCAD.DocumentObject
        A list of unique CivilCvCAD DocumentObject instances that correspond to the
    ...
    objects_by_name = {o.Name: o for o in CivilCvCAD.ActiveDocument.Objects}
    ...
    if "Name" in headers:
        ...
        objects_by_label = {o.Label: o for o in CivilCvCAD.ActiveDocument.Objects}
...
def _get_bim_type(obj):
    """
    Gets the most architecturally significant type for a CivilCvCAD object.
...
def _get_direct_children(obj, discover_hosted_elements, include_components_from_additions):
    ...
    Encapsulates the different ways an object can be a "child" in CivilCvCAD's BIM context, checking
...
class ConvertFunction(FunctionBase):
    ...
    def get_value(self, obj):
        ...
        if not isinstance(input_value, CivilCvCAD.Units.Quantity):
            raise SqlEngineError(
...
class FromFunctionBase:
    ...
    def _get_parent_objects(self, source_objects=None):
        ...
        into a list of CivilCvCAD document objects.
        """
        ...
        headers, rows = self.args.execute(CivilCvCAD.ActiveDocument.Objects)
...
class ChildrenRecursiveFromFunction(FromFunctionBase):
    ...
    def _get_parent_objects_from_subquery(self, substatement, source_objects=None):
        ...
        if source_objects:
            ...
            headers, rows = substatement.execute(CivilCvCAD.ActiveDocument.Objects)
        ...
        if self.order_by_clause:
            ...
            def sort_key(row):
                ...
                for index in sort_column_indices:
                    ...
                    elif isinstance(value, (int, float, CivilCvCAD.Units.Quantity)):
                        num_val = (
                            value.Value if isinstance(value, CivilCvCAD.Units.Quantity) else value
                        )
                    ...
                    if extractor.function_name == "count":
                        ...
                        for obj in object_list:
                            ...
                            if prop_val is not None:
                                # Handle CivilCvCAD.Quantity by using its value
                                if isinstance(prop_val, CivilCvCAD.Units.Quantity):
...
class BooleanComparison:
    ...
    def evaluate(self, obj):
        ...
        if isinstance(left_val, CivilCvCAD.Units.Quantity):
            left_val = left_val.Value
        if isinstance(right_val, CivilCvCAD.Units.Quantity):
...
class ReferenceExtractor:
    ...
    def get_objects(self, source_objects=None):
        ...
        if self.value == "document" and not self.base:
            found_objects = CivilCvCAD.ActiveDocument.Objects
            return found_objects
        return []
...
try:
    ...
    CivilCvCAD.Console.PrintError(f"BIM SQL engine failed to initialize: {e}\n")
...
def _run_query(query_string: str, mode: str, source_objects: Optional[List] = None):
    ...
    source_objects : list of CivilCvCAD.DocumentObject, optional
...
class ReportStatement:
    ...
    allowing it to be persisted within a CivilCvCAD document object.
...
def selectObjectsFromPipeline(statements: list) -> list:
    """
    Executes a multi-statement pipeline and returns a final list of CivilCvCAD objects.
    ...
    -------
    list of CivilCvCAD.DocumentObject
        A list of the final CivilCvCAD.DocumentObject instances that result from
        the pipeline.
    """
...
def selectObjects(query_string: str) -> List["CivilCvCAD.DocumentObject"]:
    """
    ...
    -------
    list of App::DocumentObject
        A list of the CivilCvCAD document objects that match the query.
        Returns an empty list if the query is invalid or finds no objects.
    """
    if not CivilCvCAD.ActiveDocument:
        CivilCvCAD.Console.PrintError("Arch.selectObjects() requires an active document.\n")
        return []
    ...
    try:
        ...
        CivilCvCAD.Console.PrintError(f"Arch.selectObjects query failed: {e}\n")
        return []
