...
class _Rebar(ArchComponent.Component):
    ...
    def getRebarData(self, obj):
        ...
        if not axis:
            axis = obj.Base.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 0, -1))
        if hasattr(obj, "Direction"):
            if not DraftVecUtils.isNull(obj.Direction):
                axis = CivilCvCAD.Vector(obj.Direction)
                axis.normalize()
        size = 0
        if father:
    ...
    def execute(self, obj):
        ...
        # Drop this check to solve issue as discussed here: https://github.com/CivilCvCAD/CivilCvCAD/pull/2550
        ...
        if spacinglist:
            ...
            if round(influenceArea) > round(reqInfluenceArea):
                ...
                CivilCvCAD.Console.PrintWarning("Last span is greater that end offset.\n")
    ...
    def setEdit(self, vobj, mode):
        ...
        if hasattr(vobj.Object, "RebarShape"):
            try:
                ...
                CivilCvCAD.Console.PrintError("Unable to import RebarShape module\n")
                return False
...
def CalculatePlacement(
    ...
    placement = CivilCvCAD.Placement(barplacement, rotation)
    return placement
...
def getLengthOfRebar(rebar):
    ...
    # When rebar is derived from Sketch
        ...
        CivilCvCAD.Console.PrintError("Cannot calculate rebar length from its base object\n")
        return None
