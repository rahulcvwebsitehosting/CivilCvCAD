...
def makeSolarDiagram(longitude, latitude, scale=1, complete=False, tz=None):
    ...
    with temp_logger_level(logging.WARNING):
        try:
            ...
            # CivilCvCAD.Console.PrintWarning("Ladybug module not found, using pysolar instead. Warning, this will be deprecated in the future\n")
            ladybug = False
            try:
                import pysolar
            except ImportError:
    ...
    for ad in range(0, 360, 15):
        a = math.radians(ad)
        p1 = CivilCvCAD.Vector(math.cos(a) * scale, math.sin(a) * scale, 0)
        p2 = CivilCvCAD.Vector(math.cos(a) * scale * 0.125, math.sin(a) * scale * 0.125, 0)
        p3 = CivilCvCAD.Vector(math.cos(a) * scale * 1.08, math.sin(a) * scale * 1.08, 0)
    ...
    for i, d in enumerate(m):
        ...
        for h in range(24):
            ...
            xc = math.cos(az) * ic
            yc = math.sin(az) * ic
            p = CivilCvCAD.Vector(xc, yc, zc)
            pts.append(p)
            hpts[h].append(p)
            if i in [0, 6]:
                ep = CivilCvCAD.Vector(p)
                ep.multiply(1.08)
                if ep.z >= 0:
                    if not oldversion:
    ...
    cube.translate(CivilCvCAD.Vector(-sz / 2, -sz / 2, -sz))
...
def makeWindRose(epwfile, scale=1, sectors=24):
    ...
    if not epwfile:
        CivilCvCAD.Console.PrintWarning("No EPW file, unable to generate wind rose.\n")
        return None
    ...
    for divider in dividers:
        l = Part.makeLine(CivilCvCAD.Vector(), divider)
...
class _Site(ArchIFC.IfcProduct):
    ...
    def setProperties(self, obj):
        ...
        if not "ExtrusionVector" in pl:
            ...
            obj.ExtrusionVector = CivilCvCAD.Vector(0, 0, -100000)
    ...
    def onDocumentRestored(self, obj):
        """CivilCvCAD hook run after the object is restored from a file.

        This method serves a dual purpose due to CivilCvCAD framework limitations:
        ...
        1.  **Standard Role:** Restore properties on the data object (`_Site` proxy) itself by
            ...
            properties defined in the current CivilCvCAD version to an object loaded from an older
            file.
        ...
        if CivilCvCAD.GuiUp and hasattr(obj, "ViewObject"):
            # Manually ensure the view provider's properties are up-to-date.
            #
            # When loading a document, CivilCvCAD's C++ document loading mechanism restores Python
            ...
            # CivilCvCAD adds new properties.
            try:
                ...
                CivilCvCAD.Console.PrintError(f"ArchSite: proxy.setProperties failed: {e}\n")
            ...
            # the way CivilCvCAD's GUI and data layers interact.
            #
    ...
    def execute(self, obj):
        ...
        pl = CivilCvCAD.Placement(obj.Placement)
        shape = None
        if (
            ...
            shape.Placement = CivilCvCAD.Placement()
    ...
    def computeAreas(self, obj):
        ...
        for face in obj.Shape.Faces:
            if face.normalAt(0, 0).getAngle(CivilCvCAD.Vector(0, 0, 1)) < 1.5707:
                edges.extend(TechDraw.project(face, CivilCvCAD.Vector(0, 0, 1))[0].Edges)
...
class _ViewProviderSite:
    ...
    def setProperties(self, vobj):
        ...
        You can learn more about properties here: https://wiki.civilcvcad.org/property
        """
    ...
    def restoreConstraints(self, vobj):
        ...
        try:
            ...
            # see https://github.com/CivilCvCAD/CivilCvCAD/issues/24543
            return
    ...
    def addDisplaymodeTerrainSwitches(self, vobj):
        ...
        See:
        https://forum.civilcvcad.org/viewtopic.php?f=10&t=74731
        https://forum.civilcvcad.org/viewtopic.php?t=75658
        https://forum.civilcvcad.org/viewtopic.php?t=75883
        """
    ...
    def addTrueNorthRotation(self):
        ...
        if hasattr(self, "trueNorthRotation") and self.trueNorthRotation is not None:
            return
        if not CivilCvCADGui.ActiveDocument.ActiveView:
            return
        if not hasattr(CivilCvCADGui.ActiveDocument.ActiveView, "getSceneGraph"):
            return

        from pivy import coin

        self.trueNorthRotation = coin.SoTransform()
        sg = CivilCvCADGui.ActiveDocument.ActiveView.getSceneGraph()
    ...
    def _migrate_legacy_solar_diagram_scale(self, vobj):
        ...
        Historically older CivilCvCAD files (1.0.0 and prior) sometimes stored
        ...
        try:
            ...
            CivilCvCAD.Console.PrintError(f"ArchSite: Failed during legacy migration: {e}\n")
    ...
    def _on_viewobject_ready(self, vobj):
        ...
        try:
            ...
            CivilCvCAD.Console.PrintError(f"ArchSite: Failed to set properties during restore: {e}\n")
    ...
    def updateSunPosition(self, vobj):
        ...
        obj = vobj.Object
        declination_rot = CivilCvCAD.Rotation(
            ...
            if alt > 0:
                ...
                if hasattr(vobj, "ShowHourLabels") and vobj.ShowHourLabels:
                    if vobj.ShowHourLabels and (hour_float in [9.0, 12.0, 15.0]):
                        ...
                        direction = CivilCvCAD.Vector(point).normalize()
        ...
        sun_pos_local = CivilCvCAD.Vector(x, y, z)
        ...
        if not ray_object:
            ...
            # is not disrupted.
            saved_selection = CivilCvCADGui.Selection.getSelection()
            ...
            for sel_obj in saved_selection:
                CivilCvCADGui.Selection.addSelection(sel_obj)
