...
def _rotate_profile(prof, pt, vec):
    ...
    if vec.getAngle(CivilCvCAD.Vector(0, 0, 1)) > 0.01:
        up = CivilCvCAD.Vector(0, 0, 1)
    else:
        up = CivilCvCAD.Vector(0, 1, 0)
    vec_x = up.cross(vec)
    vec_y = vec.cross(vec_x)
    rot = CivilCvCAD.Rotation(vec_x, vec_y, vec, "ZYX")
...
class _ArchPipe(ArchComponent.Component):
    ...
    def onDocumentRestored(self, obj):
        ...
        if (
            ...
            and CivilCvCAD.ActiveDocument.getProgramVersion().split()[0] < "1.1"
        ):
    ...
    def execute(self, obj):
        ...
        w = self.getWire(obj)
        if not w:
            CivilCvCAD.Console.PrintError(translate("Arch", "Unable to build the base path") + "\n")
            return
        ...
        if not p:
            CivilCvCAD.Console.PrintError(translate("Arch", "Unable to build the profile") + "\n")
            return
        ...
        try:
            ...
            CivilCvCAD.Console.PrintError(translate("Arch", "Unable to build the pipe") + "\n")
        else:
            if len(shapes) == 0:
                return
            elif len(shapes) == 1:
    ...
    def getWire(self, obj):
        ...
        if obj.Base:
            ...
            if obj.Base.Shape.Wires[0].isClosed():
                CivilCvCAD.Console.PrintError(translate("Arch", "The base wire is closed") + "\n")
                return
            ...
            w = Part.Wire(
                [
                    Part.LineSegment(
                        CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(0, 0, obj.Length.Value)
                    ).toShape()
                ]
            )
        return w
    ...
    def getProfile(self, obj):
        ...
        if obj.Profile:
            if not obj.Profile.getLinkedObject().isDerivedFrom("Part::Part2DObject"):
                CivilCvCAD.Console.PrintError(translate("Arch", "The profile is not a 2D Part") + "\n")
                return
            if not obj.Profile.Shape.Wires[0].isClosed():
                CivilCvCAD.Console.PrintError(translate("Arch", "The profile is not closed") + "\n")
                return
            ...
            if obj.ProfileType == "Square":
                ...
                p = Part.makePlane(
                    obj.Width.Value,
                    obj.Width.Value,
                    CivilCvCAD.Vector(-obj.Width.Value / 2, -obj.Width.Value / 2, 0),
                )
                ...
                p = Part.makePlane(
                    obj.Width.Value,
                    obj.Height.Value,
                    CivilCvCAD.Vector(-obj.Width.Value / 2, -obj.Height.Value / 2, 0),
                )
                if (
                ...
                p = Part.Wire(
                    [
                        Part.Circle(
                            CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(0, 0, 1), obj.Diameter.Value / 2
                        ).toShape()
                    ]
                )
                p = Part.Face(p)
...
class _ArchPipeConnector(ArchComponent.Component):
    ...
    def execute(self, obj):
        ...
        if wires[0].Vertexes[0].Point.sub(wires[1].Vertexes[0].Point).Length <= ptol:
            ...
            CivilCvCAD.Console.PrintError(translate("Arch", "Common vertex not found") + "\n")
            return
        if order[0] == "start":
        ...
        if len(obj.Pipes) == 2:
            ...
            if round(v1.getAngle(v2), tol) in [0, round(math.pi, tol)]:
                CivilCvCAD.Console.PrintError(translate("Arch", "Pipes are already aligned") + "\n")
                return
            normal = v2.cross(v1)
            ...
            if round(v1.getAngle(v2), tol) in [0, round(math.pi, tol)]:
                ...
                CivilCvCAD.Console.PrintError(translate("Arch", "At least 2 pipes must align") + "\n")
                return
