...
__title__ = "CivilCvCAD Arch BuildingPart"
        ...
        for child in g:
            if Draft.getType(child) in ["Wall", "Structure"]:
                if not child.Height.Value:
                    CivilCvCAD.Console.PrintLog("Auto-updating Height of " + child.Name + "\n")
...
class ViewProviderBuildingPart:
    ...
    def setProperties(self, vobj):
        ...
        if not "DisplayOffset" in pl:
            ...
            vobj.DisplayOffset = CivilCvCAD.Placement(
                CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), 90)
            )
        ...
        if not "SaveInventor" in pl:
            vobj.addProperty(
                ...
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "If this is enabled, the OpenInventor representation of this object will be saved in the CivilCvCAD file, allowing to reference it in other files in lightweight mode.",
                ),
                locked=True,
            )
        if not "SavedInventor" in pl:
    ...
    def attach(self, vobj):
        ...
        self.tra.rotation.setValue(CivilCvCAD.Rotation(0, 0, 90).Q)
    ...
    def onChanged(self, vobj, prop):
        ...
        if prop == "ShapeAppearance":
            ...
            if (
                ...
                q = CivilCvCAD.Units.Quantity(z, CivilCvCAD.Units.Length)
                txt = ""
                if vobj.ShowLabel:
            ...
            if (
                ...
                if vobj.CutView:
                    ...
                    norm = vobj.Object.Placement.multVec(CivilCvCAD.Vector(0, 0, 1))
            ...
            if hasattr(vobj, "AutogroupBox") and hasattr(vobj, "AutogroupSize"):
                if vobj.AutogroupBox:
                    if len(vobj.AutogroupSize) >= 6:
                        self.autobbox = CivilCvCAD.BoundBox(*vobj.AutogroupSize[0:6])
    ...
    def setEdit(self, vobj, mode):
        ...
        # the Tree view. Using CivilCvCADGui.getUserEditMode() as a workaround.
        if CivilCvCADGui.getUserEditMode() in ("Transform", "Cutting"):
            return None
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        if CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("Arch") == self.Object:
    ...
    def writeCamera(self):
        ...
        if hasattr(self, "Object"):
            ...
            n = CivilCvCADGui.ActiveDocument.ActiveView.getCameraNode()
            CivilCvCAD.Console.PrintMessage(
    ...
    def createGroup(self):
        ...
        if hasattr(self, "Object"):
            s = (
                'CivilCvCAD.ActiveDocument.getObject("%s").newObject("App::DocumentObjectGroup","Group")'
                % self.Object.Name
            )
            CivilCvCADGui.doCommand(s)
