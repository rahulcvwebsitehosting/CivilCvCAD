...
__title__ = "CivilCvCAD Axis System"
...
class ArchGrid:
    ...
    def getSizes(self, obj):
        ...
        if obj.AutoHeight.Value:
            if obj.AutoHeight.Value > obj.Height.Value:
                CivilCvCAD.Console.PrintError(translate("Arch", "Auto height is larger than height"))
                return [], [], []
        ...
        if obj.AutoWidth.Value:
            if obj.AutoWidth.Value > obj.Width.Value:
                CivilCvCAD.Console.PrintError(translate("Arch", "Auto width is larger than width"))
                return [], [], []
    ...
    def execute(self, obj):
        ...
        for row in rowsizes:
            ...
            for column in columnsizes:
                v1 = CivilCvCAD.Vector(columnoffset, rowoffset, 0)
                v2 = v1.add(CivilCvCAD.Vector(column, 0, 0))
                v3 = v2.add(CivilCvCAD.Vector(0, -row, 0))
                v4 = v3.add(CivilCvCAD.Vector(-column, 0, 0))
    ...
    def getPoints(self, obj):
        ...
        if obj.PointsOutput == "Vertices":
            ...
            rv = obj.Placement.Rotation.multVec(CivilCvCAD.Vector(0, 1, 0))
            edges = [
                e
            ...
            rv = obj.Placement.Rotation.multVec(CivilCvCAD.Vector(1, 0, 0))
            edges = [
                e
                for e in obj.Shape.Edges
                if round(rv.getAngle(e.tangentAt(e.FirstParameter)), 4) in [0, 3.1416]
...
class ArchGridTaskPanel:
    ...
    def update(self):
        ...
        if self.obj.Rows:
            ...
            for i, v in enumerate(self.obj.RowSize):
                if i < len(vlabels):
                    vlabels[i] = CivilCvCAD.Units.Quantity(v, CivilCvCAD.Units.Length).getUserPreferred()[
                        0
                    ]
        ...
        if self.obj.Columns:
            ...
            for i, v in enumerate(self.obj.ColumnSize):
                if i < len(hlabels):
                    hlabels[i] = CivilCvCAD.Units.Quantity(v, CivilCvCAD.Units.Length).getUserPreferred()[
                        0
                    ]
    ...
    def accept(self):
        ...
        self.obj.RowSize = [
            CivilCvCAD.Units.Quantity(self.table.verticalHeaderItem(i).text()).Value
            for i in range(self.table.rowCount())
        ]
        self.obj.ColumnSize = [
            CivilCvCAD.Units.Quantity(self.table.horizontalHeaderItem(i).text()).Value
