...
"The CivilCvCAD Arch Vector Rendering Module"
...
class Renderer:
    ...
    def setWorkingPlane(self, wp):
        ...
        if isinstance(wp, CivilCvCAD.Placement):
    ...
    def addLabels(self):
        ...
        c = 0
        for f in self.faces:
            l = CivilCvCAD.ActiveDocument.addObject("App::AnnotationLabel", "facelabel")
    ...
    def flattenFace(self, face):
        ...
        for w in face[0].Wires:
            ...
            for e in edges:
                v = e.Vertexes[0].Point
                verts.append(CivilCvCAD.Vector(v.x, v.y, 0))
            verts.append(verts[0])
            wires.append(Part.makePolygon(verts))
        try:
            sh = Part.Face(wires)
    ...
    def buildDummy(self):
        ...
        for f in self.faces[:]:
            ...
            ff.translate(CivilCvCAD.Vector(0, 0, z))
            faces.append(ff)
            z += 1
        if faces:
