...
def recolorize(attr):  # names is [docname,objname]
    ...
    if isinstance(attr, list):
        if attr[0] in CivilCvCAD.listDocuments():
            doc = CivilCvCAD.getDocument(attr[0])
...
class _Window(ArchComponent.Component):
    ...
    def onDocumentRestored(self, obj):
        ...
        # Related PRs:
        #     https://github.com/CivilCvCAD/CivilCvCAD/pull/21005
        #     https://github.com/CivilCvCAD/CivilCvCAD/pull/23014
    ...
    def buildShapes(self, obj):
        ...
        for i in range(int(len(obj.WindowParts) / 5)):
            ...
            if wires:
                ...
                exv = CivilCvCAD.Vector()
                zov = CivilCvCAD.Vector()
                V = 0
                        ...
                        if obj.LouvreWidth and obj.LouvreSpacing:
                            ...
                            if step < bb.ZLength:
                                ...
                                for i in range(int(bb.ZLength / step) + 1):
                                    b = box.copy()
                                    b.translate(
                                        CivilCvCAD.Vector(bb.XMin, bb.YMin, bb.ZMin + i * step)
                                    )
                                    boxes.append(b)
    ...
    def executeSketchArchFeatures(self, obj, linkObj=None, index=None, linkElement=None):
        """
        To execute features in the SketchArch External Add-on  (https://github.com/paullee0/CivilCvCAD_SketchArch)
        -  import ArchSketchObject module, and
        -  execute features that are common to ArchObjects (including Links) and ArchSketch

        To install SketchArch External Add-on, see https://github.com/paullee0/CivilCvCAD_SketchArch#iv-install
        """
        ...
        try:
            ...
            # see https://forum.civilcvcad.org/viewtopic.php?f=23&t=50802
            ...
            if obj.Subvolume:
                if hasattr(obj.Subvolume, "Shape"):
                    if not obj.Subvolume.Shape.isNull():
                        ...
                        pl = CivilCvCAD.Placement(sh.Placement)
...
class _ArchWindowTaskPanel:
    ...
    def selectHole(self):
        ...
        s = CivilCvCADGui.Selection.getSelectionEx()
        if s and self.obj:
    ...
    def update(self):
        ...
        if self.obj:
            ...
            CivilCvCADGui.ExpressionBinding(self.widthWidget).bind(self.obj, "Width")
            self.widthWidget.setProperty("value", self.obj.Width)

            CivilCvCADGui.ExpressionBinding(self.heightWidget).bind(self.obj, "Height")
            self.heightWidget.setProperty("value", self.obj.Height)

            CivilCvCADGui.ExpressionBinding(self.openingWidget).bind(self.obj, "Opening")
                ...
                if comp in self.obj.WindowParts:
                    ...
                    for i in range(5):
                        ...
                        if i == 1:
                            ...
                            f.setProperty(
                                "text",
                                CivilCvCAD.Units.Quantity(float(t), CivilCvCAD.Units.Length).UserString,
                            )
                        else:
                            f.setText(t)
    ...
    def create(self):
        ...
        if ok:
            ...
            CivilCvCAD.Console.PrintWarning(translate("Arch", "Unable to create component") + "\n")
