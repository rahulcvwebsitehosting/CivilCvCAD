...
class _Fence(ArchComponent.Component):
    ...
    def execute(self, obj):
        ...
        if not pathwire:
            CivilCvCAD.Console.PrintLog("ArchFence.execute: path " + obj.Path.Name + " has no edges\n")

            return

        if not obj.Section:
            CivilCvCAD.Console.PrintLog("ArchFence.execute: Section not set\n")

            return

        if not obj.Post:
            CivilCvCAD.Console.PrintLog("ArchFence.execute: Post not set\n")
        ...
        downRotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), -90)
    ...
    def calculatePostPlacements(self, obj, pathwire, rotation):
        ...
        transformationVector = CivilCvCAD.Vector(0, -postWidth / 2, 0)
    ...
    def calculateSections(self, obj, postPlacements, postLength, sectionLength):
        ...
        for i in range(obj.NumberOfSections):
            ...
            if startPlacement.Rotation.isSame(endPlacement.Rotation):
                ...
                sectionRotation = CivilCvCAD.Rotation(EAST, direction)
    ...
    def clipSection(self, shape, length, clipLength):
        ...
        leftBox = Part.makeBox(
            ...
            boundBox.YMax + 1,
            boundBox.ZMax + 1,
            CivilCvCAD.Vector(boundBox.XMin, boundBox.YMin, boundBox.ZMin),
        )
        ...
        newShape.translate(CivilCvCAD.Vector(-newBoundBox.XMin, 0, 0))
