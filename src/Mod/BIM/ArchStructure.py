...
__title__ = "CivilCvCAD Structure"
...
class CommandStructuresFromSelection:
    ...
    def Activated(self):
        selex = CivilCvCADGui.Selection.getSelectionEx()
        if len(selex) >= 2:
            ...
            for selexi in selex[
                ...
                for sub in subelement_names:
                    CivilCvCADGui.doCommand(
                        "structure = Arch.makeStructure(CivilCvCAD.ActiveDocument." + base.Name + ")"
                    )
                    CivilCvCADGui.doCommand(
                        "structure.Tool = (CivilCvCAD.ActiveDocument."
                        + selexi.Object.Name
                        + ", '"
                        + sub
                        + "')"
                    )
                    CivilCvCADGui.doCommand("structure.BasePerpendicularToTool = True")
                    CivilCvCADGui.doCommand("Draft.autogroup(structure)")
...
class _CommandStructure:
    ...
    def Activated(self):
        ...
        if sel:
            ...
            ax = Draft.getObjectsOfType(sel, "Axis")
            if ax:
                CivilCvCADGui.runCommand("Arch_StructuralSystem")
                return
                ...
                for obj in sel:
                    CivilCvCADGui.doCommand(
                        "obj = Arch.makeStructure(CivilCvCAD.ActiveDocument." + obj.Name + ")"
                    )
                    CivilCvCADGui.addModule("Draft")
                    CivilCvCADGui.doCommand("Draft.autogroup(obj)")
    ...
    def getPoint(self, point=None, obj=None):
        ...
        if self.Profile is not None:
            ...
            if ("Precast" in self.Profile) and self.precastvalues:
                ...
                if self.mode == StructureMode.BEAM:
                    delta = CivilCvCAD.Vector(0, 0 - self.Width / 2, 0)
                else:
                    delta = CivilCvCAD.Vector(-self.Length / 2, -self.Width / 2, 0)
                ...
                CivilCvCADGui.addModule("ArchPrecast")
                CivilCvCADGui.doCommand("s = ArchPrecast.makePrecast(" + argstring + ")")
            else:
                # metal profile
                CivilCvCADGui.doCommand("p = Arch.makeProfile(" + str(self.Profile) + ")")
                if self.mode == StructureMode.BEAM:
                    # horizontal
                    CivilCvCADGui.doCommand(
                ...
                CivilCvCADGui.doCommand("s.Profile = " + repr(self.Profile[2]))
        else:
            CivilCvCADGui.doCommand(
        ...
        CivilCvCADGui.addModule("Draft")
        CivilCvCADGui.doCommand("Draft.autogroup(s)")
        self.doc.commitTransaction()
        self.doc.recompute()
    ...
    def _createItemlist(self, baselist):
        ...
        ilist = []
        for p in baselist:
            f = CivilCvCAD.Units.Quantity(p[4], CivilCvCAD.Units.Length).getUserPreferred()
    ...
    def taskbox(self):
        ...
        self.vHeight.setText(CivilCvCAD.Units.Quantity(self.Height, CivilCvCAD.Units.Length).UserString)
        ...
        if CivilCvCADGui.Control.activeDialog():
            ...
            if self.mode == StructureMode.COLUMN:
                ...
                if self.bpoint:
                    ...
                    self.vLength.setText(CivilCvCAD.Units.Quantity(l, CivilCvCAD.Units.Length).UserString)
                else:
                    self.tracker.off()
            ...
            if elt in self.precast.PrecastTypes:
                ...
                if self.mode == StructureMode.BEAM:
                    self.vHeight.setText(
                        CivilCvCAD.Units.Quantity(depth, CivilCvCAD.Units.Length).UserString
                    )
                ...
                self.vWidth.setText(CivilCvCAD.Units.Quantity(width, CivilCvCAD.Units.Length).UserString)
                self.setWidth(width)
                self.Profile = elt
    ...
    def switchLH(self, beam_toggled):
        ...
        self.tracker.length(self.Length)
        if self.mode == StructureMode.COLUMN:
            self.tracker.setRotation(CivilCvCAD.Rotation())
...
class _Structure(ArchComponent.Component):
    ...
    def execute(self, obj):
        ...
        if extdata:
            ...
            for i in range(len(sh)):
                ...
                if i < len(ev):
                    ...
                    evi = ev[-1]
                    if isinstance(evi, CivilCvCAD.Vector):
                        evi = CivilCvCAD.Vector(evi)
                    else:
                        evi = evi.copy()
                if i < len(pla):
                    pli = pla[i]
            ...
            obj.ComputedLength = CivilCvCAD.Units.Quantity(extrusion_length, CivilCvCAD.Units.Length)
        if obj.Base:
        ...
        if (not base) and (not obj.Additions):
            # CivilCvCAD.Console.PrintError(translate("Arch","Error: Invalid base object")+"\n")
            return
    ...
    def getExtrusionData(self, obj):
        ...
        if baseface:
                ...
                if len(edges) > 0:
                    ...
                    if hasattr(obj, "BasePerpendicularToTool") and obj.BasePerpendicularToTool:
                        ...
                        if hasattr(obj, "BaseRotation"):
                            pl.rotate(
                                CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(0, 0, 1), -obj.BaseRotation
                            )
                        if hasattr(obj, "BaseOffsetX") and hasattr(obj, "BaseOffsetY"):
                            pl.translate(CivilCvCAD.Vector(obj.BaseOffsetX, obj.BaseOffsetY, 0))
                        if hasattr(obj, "BaseMirror") and obj.BaseMirror:
                            pl.rotate(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(0, 1, 0), 180)
            ...
            if extrusion:
                ...
                if isinstance(extrusion, CivilCvCAD.Vector):
    ...
    def onChanged(self, obj, prop):
        ...
        if CivilCvCAD.ActiveDocument.Restoring:
            return
        ...
        if prop in ["Shape", "ResetNodes", "NodesOffset"]:
            ...
            if extdata and not isinstance(extdata[0], list):
                ...
                if IfcType in ["Beam", "Column"]:
                    if not isinstance(extdata[1], CivilCvCAD.Vector):
                        nodes = extdata[1]
                ...
                if isinstance(extdata[1], CivilCvCAD.Vector):
                    nodes.Placement = nodes.Placement.multiply(extdata[2])
            offset = CivilCvCAD.Vector()
            if hasattr(obj, "NodesOffset"):
                offset = CivilCvCAD.Vector(0, 0, obj.NodesOffset.Value)
...
class _ViewProviderStructure(ArchComponent.ViewProviderComponent):
    ...
    def setEdit(self, vobj, mode):
        ...
        CivilCvCADGui.Control.showDialog(taskd)
        return True
...
class StructureTaskPanel(ArchComponent.ComponentOptionsTaskPanel):
    ...
    def editNodes(self):
        ...
        CivilCvCADGui.runCommand("Draft_Edit")
    ...
    def extendNodes(self, other=None):
        ...
        if not other:
            ...
            CivilCvCADGui.Selection.addObserver(self.observer)
            CivilCvCAD.Console.PrintMessage(translate("Arch", "Choose another Structure object:"))
        else:
            CivilCvCADGui.Selection.removeObserver(self.observer)
            self.observer = None
                ...
                if not other.Nodes:
                    ...
                    if (len(self.Object.Nodes) != 2) or (len(other.Nodes) != 2):
                        ...
                        if not intersect:
                            ...
                            CivilCvCAD.Console.PrintMessage(translate("Arch", "Intersection found.\n"))
    ...
    def setSelectionFromTool(self):
        ...
        if hasattr(self.Object, "Tool"):
            ...
            if hasattr(tool, "Shape") and tool.Shape:
                CivilCvCADGui.Selection.addSelection(tool)
            else:
                if not isinstance(tool, list):
                    tool = [tool]
                for o, subs in tool:
                    CivilCvCADGui.Selection.addSelection(o, subs)
...
class StructSelectionObserver:
    ...
    def addSelection(self, docName, objName, sub, pos):
        print("got ", objName)
        obj = CivilCvCAD.getDocument(docName).getObject(objName)
        self.callback(obj)
...
if CivilCvCAD.GuiUp:
    CivilCvCADGui.addCommand("Arch_StructuralSystem", CommandStructuralSystem())
    CivilCvCADGui.addCommand("Arch_StructuresFromSelection", CommandStructuresFromSelection())
    ...
    CivilCvCADGui.addCommand("Arch_StructureTools", _ArchStructureGroupCommand())
