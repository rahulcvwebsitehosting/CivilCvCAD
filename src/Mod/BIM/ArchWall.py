...
class _Wall(ArchComponent.Component):
    ...
    def onDocumentRestored(self, obj):
        ...
        # changed to [0, 0, 1].
        if (
            CivilCvCAD.ActiveDocument.getProgramVersion() < "0.22"
    ...
    def getFootprint(self, obj):
        ...
        if obj.Shape:
            for f in obj.Shape.Faces:
                if f.normalAt(0, 0).getAngle(CivilCvCAD.Vector(0, 0, -1)) < 0.01:
            ...
            if hasattr(obj.Base, "Shape"):
                if obj.Base.Shape:
                    ...
                    # in some corner case != getSortedClusters()
                        ...
                        # https://forum.civilcvcad.org/viewtopic.php?f=22&t=39341&p=334275#p334275
                        # normal = obj.Base.Placement.Rotation.multVec(CivilCvCAD.Vector(0,0,1))
                        ...
                        # See discussion - https://forum.civilcvcad.org/viewtopic.php?t=86365
                        # See discussion - https://forum.civilcvcad.org/viewtopic.php?t=82207&start=10
                        ...
                        # See FC discussion -
                        # https://forum.civilcvcad.org/viewtopic.php?f=23&t=48275&p=413745#p413745
                        ...
                        # normal = obj.Base.getGlobalPlacement().Rotation.multVec(CivilCvCAD.Vector(0,0,1))
                        # normal = obj.Base.Placement.Rotation.multVec(CivilCvCAD.Vector(0,0,1))
                        ...
                        for i, wire in enumerate(self.basewires):
                            ...
                            if curAligns == "Left":
                                ...
                                if layers:
                                    ...
                                    # See https://github.com/CivilCvCAD/CivilCvCAD/issues/25485#issuecomment-3566734050
                            ...
                            if face:
                                ...
                                if baseface:
                                    ...
                                    # properly
                                    # "ArchWall - Based on Sketch Issues" - https://forum.civilcvcad.org/viewtopic.php?f=39&t=31235

                                    # "Bug #2408: [PartDesign] .fuse is splitting edges it should not"
                                    # - https://forum.civilcvcad.org/viewtopic.php?f=10&t=20349&p=346237#p346237
                                    # - bugtracker - 

                                    # Try Part.Shell before removeSplitter
                                    # - https://forum.civilcvcad.org/viewtopic.php?f=10&t=20349&start=10
    ...
    def calc_endpoints(self, obj):
        ...
        p1_local = CivilCvCAD.Vector(-obj.Length.Value / 2, 0, 0)
        p2_local = CivilCvCAD.Vector(obj.Length.Value / 2, 0, 0)
    ...
    def set_from_endpoints(self, obj, pts):
        ...
        new_rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), new_direction)
    ...
    def _make_blocks(self, obj, base_face, extv):
        ...
        if obj.BlockHeight.Value:
            ...
            bh = obj.Height.Value
        bvec = CivilCvCAD.Vector(n)
        bvec.multiply(bh)
        svec = CivilCvCAD.Vector(n)
        svec.multiply(fsize)
        if cuts1:
        ...
        for i in range(entire):
            ...
            if i:
                t = CivilCvCAD.Vector(svec)
                t.multiply(i)
                b.translate(t)
            blocks.append(b)
        if rest:
            rest = extv.Length - (entire * fsize)
            rvec = CivilCvCAD.Vector(n)
            rvec.multiply(rest)
            if entire % 2:
    ...
    def build_base_from_scratch(self, obj):
        ...
        Returns
        -------
        tuple of (list of Part.Face, CivilCvCAD.Placement)
...
if CivilCvCAD.GuiUp:
    ...
    class WallTaskPanel(ArchComponent.ComponentTaskPanel):
        def __init__(self, obj):
            ...
            CivilCvCADGui.ExpressionBinding(self.length).bind(self.obj, "Length")
            ...
            CivilCvCADGui.ExpressionBinding(self.width).bind(self.obj, "Width")
            ...
            CivilCvCADGui.ExpressionBinding(self.height).bind(self.obj, "Height")
