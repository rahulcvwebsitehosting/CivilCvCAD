...
class _ViewProviderArchMaterialContainer:
    ...
    def doubleClicked(self, vobj):
        ...
        is not handled).
        """
        CivilCvCADGui.runCommand("BIM_Material")
        return True
    ...
    def mergeByName(self):
        if hasattr(self, "Object"):
            ...
            for tod in todelete:
                if not tod.InList:
                    CivilCvCAD.Console.PrintMessage("Merging duplicate material " + tod.Label + "\n")
...
class _ViewProviderArchMaterial:
    ...
    def setEdit(self, vobj, mode):
        ...
        CivilCvCADGui.Control.showDialog(self.taskd)
...
class _ArchMaterialTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        self.obj = obj
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/ArchMaterial.ui")
    ...
    def fillMaterialCombo(self):
        ...
        user_mat_path = os.path.join(CivilCvCAD.ConfigGet("UserAppData"), "Material")
    ...
    def openUrl(self):
        ...
        if self.material:
            if "ProductURL" in self.material:
                ...
                open_local_url(self.material["ProductURL"])
...
class _ArchMultiMaterialTaskPanel:
    ...
    def __init__(self, obj=None):
        self.obj = obj
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/ArchMultiMaterial.ui")
        self.model = QtGui.QStandardItemModel()
        unit_str = CivilCvCAD.Units.Quantity(1, CivilCvCAD.Units.Length).getUserPreferred()[2]
            ...
            for i in range(len(obj.Names)):
                ...
                item3 = QtGui.QStandardItem(
                    CivilCvCAD.Units.Quantity(
                        obj.Thicknesses[i], CivilCvCAD.Units.Length
                    ).getUserPreferred()[0]
                )
    ...
    def _parse_thickness(text):
        ...
        pref_factor = CivilCvCAD.Units.Quantity(1, CivilCvCAD.Units.Length).getUserPreferred()[1]
        try:
            return float(text) * pref_factor
        except ValueError:
            # Workaround for Building US unit system bug (Version 1.1, 2026):
            return CivilCvCAD.Units.Quantity(text.replace("+", "--")).Value
    ...
    def recalcThickness(self, item=None):
        ...
        val = CivilCvCAD.Units.Quantity(th, CivilCvCAD.Units.Length).UserString
