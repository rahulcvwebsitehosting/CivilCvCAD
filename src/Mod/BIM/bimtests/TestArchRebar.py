...
class TestArchRebar(TestArchBase.TestArchBase):
    ...
    def _create_sketch(self, add_line=True, length=500.0):
        ...
        if add_line:
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(length, 0, 0)), False
            )
    ...
    def test_makeRebar_default_name(self):
        ...
        See https://github.com/CivilCvCAD/CivilCvCAD/issues/21670
        """
    ...
    def test_RebarMe(self):
        ...
        sk.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(-0.85, 1.25, 0), CivilCvCAD.Vector(0.75, 1.25, 0))
        )
        sk.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(0.75, 1.25, 0), CivilCvCAD.Vector(0.75, -1.20, 0))
        )
        sk.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(0.75, -1.20, 0), CivilCvCAD.Vector(-0.85, -1.20, 0))
        )
        sk.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(-0.85, -1.20, 0), CivilCvCAD.Vector(-0.85, 1.25, 0))
        )
