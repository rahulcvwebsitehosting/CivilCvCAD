...
class TestArchBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # aborted test run. CivilCvCAD.getDocument() raises a NameError if the document is not found,
        # so we wrap this check in a try...except block.
        try:
            CivilCvCAD.getDocument(self.doc_name)
            # If getDocument() succeeds, the document exists and must be closed.
            CivilCvCAD.closeDocument(self.doc_name)
        except NameError:
            # This is the expected path on a clean run; do nothing.
            pass

        # Create a fresh document for the current test.
        self.document = CivilCvCAD.newDocument(self.doc_name)
        self.assertEqual(self.document.Name, self.doc_name)
        CivilCvCAD.setActiveDocument(self.document.Name)
        self.assertEqual(CivilCvCAD.ActiveDocument.Name, self.doc_name)
    ...
    def printTestMessage(self, text, prepend_text="Test ", end="\n"):
        ...
        """
        CivilCvCAD.Console.PrintMessage(prepend_text + text + end)
