...
class TestArchSiteGui(TestArchBaseGui.TestArchBaseGui):
    ...
    def test_new_site_save_and_reopen(self):
        ...
        path = tf.name
        try:
            CivilCvCAD.ActiveDocument.saveAs(path)

            # Open the saved document (this returns a new Document instance)
            reopened = CivilCvCAD.openDocument(path)
            try:
                ...
                CivilCvCAD.closeDocument(reopened.Name)
        finally:
            try:
                os.unlink(path)
            except Exception:
    ...
    def test_legacy_site_migration(self):
        ...
        Verifies that loading a legacy (<=1.0.0) CivilCvCAD file triggers the view provider's restore
        ...
        modern default of 20 m radius (https://github.com/CivilCvCAD/CivilCvCAD/pull/22496). The migration
        ...
        d = CivilCvCAD.openDocument(fname)
        try:
            ...
            CivilCvCAD.closeDocument(d.Name)
