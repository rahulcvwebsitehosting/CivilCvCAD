"""GUI tests for ArchReport features that require a running CivilCvCAD GUI.

These tests inherit from the GUI test base `TestArchBaseGui` which skips
the whole class when `CivilCvCAD.GuiUp` is False.
"""
...
class TestArchReportGui(TestArchBaseGui):
    ...
    def setUp(self):
        ...
        self.wall_ext.Height = CivilCvCAD.Units.Quantity(3000, "mm")

        self.wall_int = Arch.makeWall(length=500, name="Interior partition wall")
        self.wall_int.IfcType = "Wall"
        self.wall_int.Height = CivilCvCAD.Units.Quantity(2500, "mm")
