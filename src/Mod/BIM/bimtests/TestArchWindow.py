...
class TestArchWindow(TestArchBase.TestArchBase):
    ...
    def test_window_in_wall_creates_opening(self):
        ...
        line = Draft.makeLine(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(wall_length, 0, 0))
        self.document.recompute()
        ...
        sk.Placement.Rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), 90)
        win_global_x_start = (wall.Length.Value - sketch_profile_width) / 2
        win_global_z_start = 900.0
        sk.Placement.Base = CivilCvCAD.Vector(win_global_x_start, 0, win_global_z_start)
        self.document.recompute()
    ...
    def test_create_window_on_xz_plane(self):
        ...
        sketch.Placement.Rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), 90)
        self.document.recompute()
    ...
    def test_custom_subvolume_creates_opening(self):
        ...
        wall_base = Draft.makeLine(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(4000, 0, 0))
        ...
        cutting_box.Placement.Base = CivilCvCAD.Vector(1600, -50, 800)
        self.document.recompute()
    ...
    def test_cloned_window_in_wall_creates_opening(self):
        ...
        wall_thickness = 200.0
        wall_height = 2500.0
        wall_base = Draft.makeLine(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(wall_length, 0, 0))
        wall = Arch.makeWall(
        ...
        original_sketch.Placement.Rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), 90)
        self.document.recompute()
        ...
        cloned_window.Placement.Base = CivilCvCAD.Vector(clone_x, clone_y, clone_z)
        self.document.recompute()
    ...
    def test_addComponents_window_to_wall(self):
        ...
        wall_base = Draft.makeLine(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(3000, 0, 0))
        ...
        window_sketch.Placement.Rotation = CivilCvCAD.Rotation(CivilCvCAD.Vector(1, 0, 0), 90)
        window_sketch.Placement.Base = CivilCvCAD.Vector(1100, 100, 800)  # Position it within the wall
        self.document.recompute()
    ...
    def _create_sketch_with_wires(
        ...
        """
        sketch: CivilCvCAD.DocumentObject = self.document.addObject("Sketcher::SketchObject", name)

        for i, (x, y, w, h) in enumerate(wire_definitions):
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(x, y, 0), CivilCvCAD.Vector(x + w, y, 0))
            )
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(x + w, y, 0), CivilCvCAD.Vector(x + w, y + h, 0))
            )
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(x + w, y + h, 0), CivilCvCAD.Vector(x, y + h, 0))
            )
            sketch.addGeometry(
                Part.LineSegment(CivilCvCAD.Vector(x, y + h, 0), CivilCvCAD.Vector(x, y, 0))
            )
            base_idx = i * 4
    ...
    def _create_sketch_with_named_constraints(
        ...
        sketch.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(0, 0, 0), CivilCvCAD.Vector(initial_width, 0, 0)), False
        )
        sketch.addGeometry(
            Part.LineSegment(
                CivilCvCAD.Vector(initial_width, 0, 0),
                CivilCvCAD.Vector(initial_width, initial_height, 0),
            ),
            False,
        )
        sketch.addGeometry(
            Part.LineSegment(
                CivilCvCAD.Vector(initial_width, initial_height, 0),
                CivilCvCAD.Vector(0, initial_height, 0),
            ),
            False,
        )
        sketch.addGeometry(
            Part.LineSegment(CivilCvCAD.Vector(0, initial_height, 0), CivilCvCAD.Vector(0, 0, 0)), False
        )
