...
if CivilCvCAD.GuiUp:
    ...
    class _ViewProviderCovering(ArchComponent.ViewProviderComponent):
        ...
        This class manages the visual representation of an Arch Covering within the CivilCvCAD 3D view
        ...
        def setEdit(self, vobj, mode=0):
            ...
            task_control = CivilCvCADGui.Control
        ...
        def _find_flat_root(self, vobj):
            ...
            ``FlatRoot`` is the subtree CivilCvCAD renders for face geometry in both "Flat Lines"
        ...
        def _compute_texture_mapping(self, obj):
            ...
            tuple or None
                ...
                CivilCvCAD.Vectors and *_offset are floats representing the normalized texture
    ...
    class ArchCoveringTaskPanel:
        ...
        command : object, optional
            The CivilCvCAD command instance that invoked this panel.
        ...
        *Edit path* (double-click in Tree View): Tree.cpp opens an "Edit Covering" transaction
        before calling doubleClicked() — this is standard CivilCvCAD behaviour for any document
        ...
        CivilCvCADGui.ActiveDocument.resetEdit(), which triggers C++ _resetEdit(), which commits the
            ...
            if self.obj_to_edit:
                ...
                if not pat_file or not os.path.exists(pat_file):
                    pat_file = os.path.join(
                        CivilCvCAD.getResourceDir(), "data", "examples", "FCPAT.pat"
                    )
                    if not os.path.exists(pat_file):
                        pat_file = os.path.join(
                            CivilCvCAD.getResourceDir(), "Mod", "TechDraw", "PAT", "FCPAT.pat"
                        )
        ...
        def _setupLayoutControls(self):
            ...
            self.sb_u_off = self._setup_bound_spinbox(
                ...
                initial_value=CivilCvCAD.Units.Quantity(offset.x, CivilCvCAD.Units.Length),
            )
            self.sb_v_off = self._setup_bound_spinbox(
                "AlignmentOffset.y",
                translate("Arch", "Shift the grid along V"),
                initial_value=CivilCvCAD.Units.Quantity(offset.y, CivilCvCAD.Units.Length),
            )
        ...
        def _setup_bound_spinbox(self, prop_name, tooltip=None, initial_value=None):
            ...
            tooltip : str, optional
                ...
                ``translate("Arch", ...)``.
            initial_value : CivilCvCAD.Units.Quantity, optional
                Explicit initial value. Required when prop_name contains a dot.
            """
            sb = CivilCvCADGui.UiLoader().createWidget("Gui::QuantitySpinBox")
            if tooltip:
            ...
            CivilCvCADGui.ExpressionBinding(sb).bind(self.template.buffer, prop_name)
        ...
        def _rebind_to_buffer(self):
            ...
            be refreshed to point at the new buffer or the f(x) button will crash CivilCvCAD.
            """
            self.bindings = []
            for sb, prop_name in self._bound_spinboxes:
                binding = CivilCvCADGui.ExpressionBinding(sb)
        ...
        def _get_view_direction(self):
            ...
            if CivilCvCAD.GuiUp and CivilCvCADGui.ActiveDocument:
        ...
        def _onSelectionChanged(self):
            """Syncs internal state with CivilCvCAD selection."""
        ...
        def _on_alignment_mode_changed(self, is_preset):
            ...
            if is_preset:
                self.sb_u_off.setProperty("value", CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length))
                self.sb_v_off.setProperty("value", CivilCvCAD.Units.Quantity(0, CivilCvCAD.Units.Length))
        ...
        def _set_rotation(self, angle_deg):
            ...
            self.sb_rot.setProperty("value", CivilCvCAD.Units.Quantity(angle_deg, CivilCvCAD.Units.Angle))

        def _set_custom_offset(self, u_off, v_off):
            """Writes custom alignment offsets to the buffer, spinboxes, and radio button."""
            self.template.buffer.AlignmentOffset = CivilCvCAD.Vector(u_off, v_off, 0)
            self.sb_u_off.setProperty("value", CivilCvCAD.Units.Quantity(u_off, CivilCvCAD.Units.Length))
            self.sb_v_off.setProperty("value", CivilCvCAD.Units.Quantity(v_off, CivilCvCAD.Units.Length))
        ...
        def _install_view_callback(self):
            ...
            """
            self._view = CivilCvCADGui.ActiveDocument.ActiveView
        ...
        def _wake_snapper(self):
            ...
            CivilCvCAD.activeDraftCommand = self
            if hasattr(CivilCvCADGui, "Snapper"):
                CivilCvCADGui.Snapper.show()
                CivilCvCADGui.Snapper.setTrackers()
        ...
        def _handle_interaction(self, arg):
            ...
            if arg["Type"] == "SoLocation2Event":
                ...
                self._pt = CivilCvCADGui.Snapper.snap(arg["Position"], active=ctrl, constrain=shift)
                self.onMouseMove(self._pt, None)
        ...
        def onMouseMove(self, point, _info):
            ...
            if point and hasattr(self, "tracker") and hasattr(self, "_cached_basis"):
                ...
                base_rot = CivilCvCAD.Rotation(u_basis, v_basis, normal, "XYZ")
                extra_deg = getattr(self, "_pick_rotation_deg", 0.0)
                if extra_deg:
                    extra_rot = CivilCvCAD.Rotation(normal, extra_deg)
                ...
                u_rotated = final_rot.multVec(CivilCvCAD.Vector(1, 0, 0))
                v_rotated = final_rot.multVec(CivilCvCAD.Vector(0, 1, 0))
            ...
            if self.btn_pick.isChecked():
                return [
                    CivilCvCADGui.InputHint(
                        ...
                        CivilCvCADGui.UserInput.MouseLeft,
                    ),
                    CivilCvCADGui.InputHint(
                        translate("Arch", "%1 rotate tile CW / Shift+%1 rotate tile CCW"),
                        CivilCvCADGui.UserInput.KeyR,
                    ),
                ]
            if self.isPicking():
                if self.obj_to_edit:
                    return [
                        ...
                        CivilCvCADGui.InputHint(
                            ...
                            CivilCvCADGui.UserInput.KeyControl,
                            CivilCvCADGui.UserInput.MouseLeft,
                        ),
                    ]
                return [
        ...
        def update_hints(self):
            ...
            if hints:
                CivilCvCADGui.HintManager.show(*hints)
            else:
                QtCore.QTimer.singleShot(0, CivilCvCADGui.HintManager.hide)
        ...
        def _sync_ui_to_target(self):
            ...
            obj.AlignmentOffset = CivilCvCAD.Vector(
            ...
            obj.TextureScale = CivilCvCAD.Vector(
        ...
        def accept(self):
            ...
            try:
                ...
                traceback.print_exc()
                CivilCvCAD.Console.PrintError(f"Error updating covering: {e}\n")
        ...
        def _cleanup_and_close(self):
            ...
            # CivilCvCAD's task panel infrastructure orphans (unparents) the form widgets on close
...
class _CoveringTemplate:
    ...
    ``ExpressionBinding`` requires a real CivilCvCAD document object to bind to, the buffer is a live
    ...
    CivilCvCAD's transaction system tracks each object's lifecycle: when ``doc.removeObject()`` is
