...
class _ArchScheduleDocObserver:
    ...
    # https://forum.civilcvcad.org/viewtopic.php?style=3&p=553377#p553377
...
class _ArchSchedule:
        ...
        try:  # Required as self.spreadsheet may get deleted.
            ...
            for o in CivilCvCAD.ActiveDocument.Objects:
                if o.TypeId == "Spreadsheet::Sheet" and getattr(o, "Schedule", None) == obj:
                    ...
                    return self.spreadsheet
            if force:
                self.spreadsheet = CivilCvCAD.ActiveDocument.addObject("Spreadsheet::Sheet", "Result")
    ...
    def onChanged(self, obj, prop):
        ...
        if prop == "CreateSpreadsheet":
            if obj.CreateSpreadsheet:
                ...
                if sp is not None:
                    CivilCvCAD.ActiveDocument.removeObject(sp.Name)
                    self.spreadsheet = None
        elif prop == "AutoUpdate":
            if obj.AutoUpdate:
                if getattr(self, "docObserver", None) is None:
                    self.docObserver = _ArchScheduleDocObserver(CivilCvCAD.ActiveDocument, obj)
    ...
    def execute(self, obj):
        ...
        for i in range(len(obj.Operation)):
            ...
            if val:
                ...
                if objs:
                    objs = objs.split(";")
                    objs = [CivilCvCAD.ActiveDocument.getObject(o) for o in objs]
                    objs = [o for o in objs if o is not None]
                else:
                    if hasattr(getattr(CivilCvCAD.ActiveDocument, "Proxy", None), "ifcfile"):
                        ifcfile = CivilCvCAD.ActiveDocument.Proxy.ifcfile
    ...
    def update_from_objs(self, objs, val, unit, details):
        """Updates the spreadsheet data from CivilCvCAD objects"""
        ...
        if val.upper() == "COUNT":
            ...
            if unit:
                ...
                unit = unit.replace("³", "^3")
                if "2" in unit:
                    tp = CivilCvCAD.Units.Area
                elif "3" in unit:
                    tp = CivilCvCAD.Units.Volume
                elif "deg" in unit:
                    tp = CivilCvCAD.Units.Angle
                else:
                    tp = CivilCvCAD.Units.Length
            ...
            for o in objs:
                ...
                try:
                    ...
                    CivilCvCAD.Console.PrintWarning(t + ": " + o.Name + "." + ".".join(vals) + "\n")
                else:
                    if VERBOSE:
                        if tp and unit:
                            v = fs.format(CivilCvCAD.Units.Quantity(d, tp).getValueAs(unit).Value)
                            print(v, unit)
                    ...
                    if details:
                        ...
                        if tp and unit:
                            q = CivilCvCAD.Units.Quantity(d, tp)
            ...
            val = sumval
            if tp:
                q = CivilCvCAD.Units.Quantity(val, tp)
            ...
            if VERBOSE:
                if tp and unit:
                    v = fs.format(CivilCvCAD.Units.Quantity(val, tp).getValueAs(unit).Value)
    ...
    def export_ifc(self, obj, ifcfile):
        """Exports the object to IFC (does not modify the CivilCvCAD object)."""
...
class ArchScheduleTaskPanel:
    ...
    def __init__(self, obj=None):
        ...
        self.obj = obj
        self.form = CivilCvCADGui.PySideUic.loadUi(":/ui/ArchSchedule.ui")
        ...
        for i in range(self.form.list.rowCount()):
            for j in range(5):
                ...
                if cell:
                    ...
                    lists[j].append("")
        CivilCvCAD.ActiveDocument.openTransaction("Edited Schedule")
