...
import sys
import io
import CivilCvCAD, unittest, Mesh
import MeshEnums
...
# ---------------------------------------------------------------------------
# define the functions to test the CivilCvCAD mesh module
# ---------------------------------------------------------------------------
...
class MeshSplitTestCases(unittest.TestCase):
    ...
    def testForaminate(self):
        ...
        filtered_result = list(
            filter(
                FilterAngle(self.mesh, CivilCvCAD.Vector(0, 1, 1), math.pi / 2).check_angle,
                results.keys(),
            )
        )
...
def loadFile(name):
    # lock.acquire()
    mesh = Mesh.Mesh()
    # CivilCvCAD.Console.PrintMessage("Create mesh instance\n")
    # lock.release()
    mesh.read(name)
    # CivilCvCAD.Console.PrintMessage("Mesh loaded successfully.\n")


def createMesh(r, s):
    # CivilCvCAD.Console.PrintMessage("Create sphere (%s,%s)...\n"%(r,s))
    mesh = Mesh.createSphere(r, s)
    # CivilCvCAD.Console.PrintMessage("... destroy sphere\n")
...
class LoadMeshInThreadsCases(unittest.TestCase):
    ...
    def testLoadMesh(self):
        ...
        mesh.write(name)
        # CivilCvCAD.Console.PrintMessage("Write mesh to %s\n"%(name))
...
class PolynomialFitCases(unittest.TestCase):
    ...
    def testFitGood(self):
        # symmetric
        v = []
        v.append(CivilCvCAD.Vector(0, 0, 0.0))
        v.append(CivilCvCAD.Vector(1, 0, 0.5))
        v.append(CivilCvCAD.Vector(2, 0, 0.0))
        v.append(CivilCvCAD.Vector(0, 1, 0.5))
        v.append(CivilCvCAD.Vector(1, 1, 1.0))
        v.append(CivilCvCAD.Vector(2, 1, 0.5))
        v.append(CivilCvCAD.Vector(0, 2, 0.0))
        v.append(CivilCvCAD.Vector(1, 2, 0.5))
        v.append(CivilCvCAD.Vector(2, 2, 0.0))
    ...
    def testFitExact(self):
        ...
        v = []
        v.append(CivilCvCAD.Vector(0, 0, 0.0))
        v.append(CivilCvCAD.Vector(1, 0, 0.0))
        v.append(CivilCvCAD.Vector(2, 0, 0.0))
        v.append(CivilCvCAD.Vector(0, 1, 0.0))
        v.append(CivilCvCAD.Vector(1, 1, 1.0))
        v.append(CivilCvCAD.Vector(2, 1, 0.0))
        d = Mesh.polynomialFit(v)
        c = d["Coefficients"]
        # print ("Polynomial: f(x,y)=%f*x^2%+f*y^2%+f*x*y%+f*x%+f*y%+f" % (c[0],c[1],c[2],c[3],c[4],c[5]))
        for i in d["Residuals"]:
    ...
    def testFitBad(self):
        ...
        v.append(CivilCvCAD.Vector(2, 1, 0.0))
        v.append(CivilCvCAD.Vector(0, 2, 0.0))
        v.append(CivilCvCAD.Vector(1, 2, 0.0))
        v.append(CivilCvCAD.Vector(2, 2, 0.0))
        d = Mesh.polynomialFit(v)
        c = d["Coefficients"]
        # print ("Polynomial: f(x,y)=%f*x^2%+f*y^2%+f*x*y%+f*x%+f*y%+f" % (c[0],c[1],c[2],c[3],c[4],c[5]))
...
class NastranReader(unittest.TestCase):
    def setUp(self):
        self.test_dir = join(CivilCvCAD.getHomePath(), "Mod", "Mesh", "App", "TestData")
...
class MeshProperty(unittest.TestCase):
    def setUp(self):
        self.doc = CivilCvCAD.newDocument("MeshTest")
