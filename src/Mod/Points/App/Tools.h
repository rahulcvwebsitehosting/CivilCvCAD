...
)
{
    ...
    if (std::all_of(std::begin(source), std::end(source), [=](auto obj) {
            return civilcvcad_cast<PropertyT*>(obj->getPropertyByName(propertyName)) != nullptr;
        })) {

        auto target_prop = civilcvcad_cast<PropertyT*>(
            target->addDynamicProperty(PropertyT::getClassTypeId().getName(), propertyName)
        );
        if (target_prop) {
            auto values = target_prop->getValues();
            for (auto it : source) {
                auto source_prop = civilcvcad_cast<PropertyT*>(it->getPropertyByName(propertyName));
                if (source_prop) {
