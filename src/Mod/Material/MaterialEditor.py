...
__title__ = "CivilCvCAD material editor"
...
class MaterialEditor:
    ...
    def __init__(self, obj=None, prop=None, material=None, card_path="", category="Solid"):
        ...
        if self.directory is None:
            self.directory = CivilCvCAD.getResourceDir() + "Mod/Material"
        self.materials = {}
        self.cards = {}
        self.icons = {}
        self.initialIndex = -1
        ...
        self.widget = CivilCvCADGui.PySideUic.loadUi(":/ui/materials-editor.ui")
        # remove unused Help button
        self.widget.setWindowFlags(self.widget.windowFlags()
                                   & ~QtCore.Qt.WindowContextHelpButtonHint)

        # restore size and position
        param = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material")
        ...
        if self.prop and self.obj:
            matProperty = CivilCvCAD.ActiveDocument.getObject(self.obj).getPropertyByName(self.prop)
        elif self.material:
    ...
    def implementModel(self):
        ...
        p = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material")
        widget = self.widget
        treeView = widget.treeView
    ...
    def chooseMaterial(self, index):
        ...
        if os.path.isfile(self.card_path):
            ...
            CivilCvCAD.Console.PrintError("Material card not found: {}\n".format(self.card_path))

    def verifyMaterial(self):
        if self.edited:
            reply = QtGui.QMessageBox.question(self.widget, #CivilCvCADGui.getMainWindow(),
    ...
    def openProductURL(self):
        ...
        if url:
            ...
            open_local_url(QtCore.QUrl(url, QtCore.QUrl.TolerantMode))
    ...
    def openfile(self):
        ...
        if os.path.isfile(filetuple[0]):
            ...
            CivilCvCAD.Console.PrintError("Error: Invalid path to the material file\n")
            return
        ...
        # D:/CivilCvCAD-build/data/Mod\Material\FluidMaterial\Air.FCMat
...
class MaterialsDelegate(QtGui.QStyledItemDelegate):
    ...
    def setEditorData(self, editor, index):
        ...
        if Type == "Color":
            ...
            if not hasattr(CivilCvCAD.Units, self.matproperty):
            ...
            if not self.Value:
                ...
                unit = getattr(CivilCvCAD.Units, self.matproperty)
                quantity = CivilCvCAD.Units.Quantity(1, unit)
...
def matProperWidget(parent=None, matproperty=None, Type="String", Value=None,
    ...
    # https://forum.civilcvcad.org/viewtopic.php?f=18&t=56912&start=20#p516811
    # set precision to 12
...
def openEditor(obj=None, prop=None):
    ...
    # param = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Cards")
    # legacy = param.GetBool("LegacyEditor", True)
    # if legacy:
    #     editor = MaterialEditor(obj, prop)
    #     editor.exec_()
    # else:
    CivilCvCADGui.runCommand('Material_Edit',0)
...
doc = CivilCvCAD.open(
    CivilCvCAD.ConfigGet("AppHomePath") + "data/examples/FEMExample.FCStd"
)
import MaterialEditor
