...
void ModelSelect::onURL(bool checked)
{
    ...
    if (url.length() > 0) {
        Gui::OfflinePolicy::openLocalUrl(QUrl(url, QUrl::TolerantMode));
    }
}
...
{
    Q_UNUSED(checked)
    // DOI resolution requires network access and is intentionally disabled.
}
