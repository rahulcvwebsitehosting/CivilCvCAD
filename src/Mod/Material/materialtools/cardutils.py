...
https://forum.civilcvcad.org/viewtopic.php?f=38&t=16714
'''
...
def get_material_preferred_directory(category=None):
    """
        ...
        3. system folder
    """
    mat_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Resources")
    ...
    if use_built_in_materials:
        if category == 'Fluid':
            preferred = join(
                CivilCvCAD.getResourceDir(), "Mod", "Material", "Resources", "Materials", "FluidMaterial"
            )

        elif category == 'Solid':
            preferred = join(
                CivilCvCAD.getResourceDir(), "Mod", "Material", "Resources", "Materials", "StandardMaterial"
            )

        else:
            preferred = join(
                CivilCvCAD.getResourceDir(), "Mod", "Material"
            )

    if use_mat_from_config_dir:
        user = join(
            CivilCvCAD.ConfigGet("UserAppData"), "Material"
        )
...
def get_material_resources(category='Solid'):
    ...
    if use_built_in_materials:
        ...
        resources[builtin_mat_dir] = ":/icons/civilcvcad.svg"

    if use_mat_from_modules:
        module_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Resources/Modules")
...
def get_material_libraries():
    ...
    if use_built_in_materials:
        builtin_mat_dir = join(
            CivilCvCAD.getResourceDir(), "Mod", "Material", "Resources", "Materials"
        )
        resources["System"] = (builtin_mat_dir, ":/icons/civilcvcad.svg")
...
def output_resources(resources):
    CivilCvCAD.Console.PrintMessage('Directories in which we will look for material cards:\n')
...
def add_cards_from_a_dir(materials, cards, icons, mat_dir, icon, template=False):
    ...
    mat_prefs = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Material/Cards")
...
def output_material_param(mat_dict):
    ...
    if not mat_dict:
        CivilCvCAD.Console.PrintMessage('  empty matdict\n')
    else:
        for p in mat_dict:
            CivilCvCAD.Console.PrintMessage('   {} --> {}\n'.format(p, mat_dict[p]))
    CivilCvCAD.Console.PrintMessage('\n')
...
def get_material_template(withSpaces=False):
    # material properties
    # see the following resources in the CivilCvCAD wiki for more
    ...
    template_data = yaml.safe_load(
        open(join(CivilCvCAD.ConfigGet('AppHomePath'), 'Mod/Material/Templatematerial.yml'))
    )
    if withSpaces:
...
def create_mat_template_card(write_group_section=True):
    ...
    if not os.path.isfile(template_card):
        ...
        return
    rev = "{}.{}.{}".format(
        CivilCvCAD.ConfigGet("BuildVersionMajor"),
        CivilCvCAD.ConfigGet("BuildVersionMinor"),
        CivilCvCAD.ConfigGet("BuildRevision")
    )
    template_data = get_material_template()
    f = open(template_card, "w")
    f.write('; TEMPLATE\n')
    f.write('; (c) 2013-2015 Juergen Riegel (CC-BY 3.0)\n')
    f.write('; CivilCvCAD material card metadata\n')
    f.write(': this template card was created by CivilCvCAD ' + rev + '\n\n')
    ...
    f.write('; find unit information in src/App/CivilCvCADInit.py')
    # write sections
...
def get_source_path():
    ...
    source_dir = ''
    make_file = join(CivilCvCAD.ConfigGet('AppHomePath'), 'Makefile')
    f = open(make_file, 'r')
    lines = f.readlines()
    f.close()
    for line in lines:
...
def check_parm_unit(param):
    # check if this parameter is known to CivilCvCAD unit system
...
def check_value_unit(param, value):
    ...
    # CivilCvCAD.Console.PrintMessage('{} --> {}\n'.format(param, value))
    if hasattr(Units, param):
        # get unit and other information known by CivilCvCAD for this parameter
        ...
        try:
            ...
            try:
                ...
                CivilCvCAD.Console.PrintError(
                    '{} Unit {} is known by CivilCvCAD, but wrong for parameter {}.\n'
        ...
        CivilCvCAD.Console.PrintError(
            'Parameter {} is unknown to the CivilCvCAD unit system.\n'
            .format(param)
        )
    return False
...
def output_parm_unit_info(param):
    ...
    if hasattr(Units, param):
        CivilCvCAD.Console.PrintMessage(
            '\nParameter {} is known to CivilCvCAD unit system.'
            .format(param)
        )
...
def output_value_unit_info(param, value):
    ...
    from CivilCvCAD import Units
    some_text = "Parameter: {} --> value: {} -->".format(param, value)
    CivilCvCAD.Console.PrintMessage('{} unit information:'.format(some_text))
    if hasattr(Units, param):
...
def check_mat_units(mat):
    ...
    for param, value in mat.items():
        if param in known_quantities:
            ...
            pass
            # print('{} is not a known CivilCvCAD quantity.'.format(param))
    # print('')
    return units_ok
