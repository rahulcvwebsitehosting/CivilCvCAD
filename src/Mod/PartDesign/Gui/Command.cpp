...
void CmdPartDesignClone::activated(int iMsg)
{
    ...
    if (objs.size() == 1) {
        // As suggested in https://forum.civilcvcad.org/viewtopic.php?f=3&t=25265&p=198547#p207336
...
bool importExternalElements(App::PropertyLinkSub& prop, std::vector<App::SubObjectT> _sobjs)
{
    ...
    auto editObj = civilcvcad_cast<App::DocumentObject*>(prop.getContainer());
    if (!editObj) {
...
)
{
    auto base_worker = [=](App::DocumentObject* feature, const std::vector<std::string>& subs) {
        ...
        auto ProfileFeature = civilcvcad_cast<PartDesign::ProfileBased*>(Feat);
    ...
    if (pcActiveBody && !bNoSketchWasSelected && extReference) {
        ...
        // For more details see: https://forum.civilcvcad.org/viewtopic.php?f=19&t=32164
