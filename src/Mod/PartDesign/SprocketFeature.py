...
if CivilCvCAD.GuiUp:
    ...
    from CivilCvCADGui import PySideUic as uic
...
def makeSprocket(name):
    ...
    """
    obj = CivilCvCAD.ActiveDocument.addObject("Part::Part2DObjectPython", name)
    Sprocket(obj)
    if CivilCvCAD.GuiUp:
        ViewProviderSprocket(obj.ViewObject)
    # CivilCvCAD.ActiveDocument.recompute()
    if CivilCvCAD.GuiUp:
        body = CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("pdbody")
        part = CivilCvCADGui.ActiveDocument.ActiveView.getActiveObject("part")
        if body:
...
class CommandSprocket:
    ...
    def Activated(self):
        ...
        CivilCvCAD.ActiveDocument.openTransaction("Create Sprocket")
        CivilCvCADGui.addModule("SprocketFeature")
        CivilCvCADGui.doCommand("SprocketFeature.makeSprocket('Sprocket')")
        CivilCvCADGui.doCommand("Gui.activeDocument().setEdit(App.ActiveDocument.ActiveObject.Name,0)")
...
class Sprocket:
    ...
    def __init__(self, obj):
        ...
        tempForm = CivilCvCADGui.PySideUic.loadUi(
            CivilCvCAD.getHomePath() + "Mod/PartDesign/SprocketFeature.ui"
        )
...
class SprocketTaskPanel:
    ...
    def __init__(self, obj, mode):
        ...
        self.form = CivilCvCADGui.PySideUic.loadUi(
            CivilCvCAD.getHomePath() + "Mod/PartDesign/SprocketFeature.ui"
        )
