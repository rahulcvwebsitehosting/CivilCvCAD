...
import CivilCvCAD  # just for debug printing to console...
import numpy as np
...
class SegmentFunction:
    ...
    def output(self):
        CivilCvCAD.Console.PrintMessage(self.name + " = ")
        for i in range(len(self.segments)):
            CivilCvCAD.Console.PrintMessage(self.segments[i].asString())
...
class TranslationFunction:
    ...
    def __init__(self, f, E, d, tangents, translations):
        ...
        # CivilCvCAD.Console.PrintMessage(A)
        # CivilCvCAD.Console.PrintMessage(" * x = ")
        # CivilCvCAD.Console.PrintMessage(b)
        # CivilCvCAD.Console.PrintMessage("\n")

        try:
            self.boundaries = np.linalg.solve(A, b)  # A * self.boundaries = b
        except np.linalg.linalg.LinAlgError as e:
            CivilCvCAD.Console.PrintMessage(e.message)
