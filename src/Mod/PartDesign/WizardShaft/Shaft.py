...
class Shaft:
    ...
    def editConstraint(self, index):
        if self.segments[index].constraint is not None:
            CivilCvCADGui.activeDocument().setEdit(self.segments[index].constraint.Name)
    ...
    def equilibrium(self):
        ...
        try:
            ...
            CivilCvCAD.Console.PrintMessage("numpy is not installed on your system\n")
        ...
        for i in range(len(self.segments)):
            ...
            if cType == "Fixed":
                ...
                # CivilCvCAD.Console.PrintMessage("BeltForce1: %f, BeltForce2: %f\n" % (constraint.BeltForce1,  constraint.BeltForce2))
                # CivilCvCAD.Console.PrintMessage("Angle1: %f, Angle2: %f\n" % (forceAngle1,  forceAngle2))
                force = [
                    0,
        ...
        for ax in range(3):
            CivilCvCAD.Console.PrintMessage("Axis: %u\n" % ax)
            ...
            if len(coefficientsF[ax]) <= 1:
                ...
                CivilCvCAD.Console.PrintMessage("Matrix is singular, no solution possible\n")
            ...
            if len(coefficientsF[ax]) == 2:
                ...
                try:
                    ...
                    CivilCvCAD.Console.PrintMessage(str(e))
                    CivilCvCAD.Console.PrintMessage(". No solution possible.\n")
                    self.parent.updateButtons(ax, False)
                    continue
            ...
            CivilCvCAD.Console.PrintMessage(forces[ax])
            CivilCvCAD.Console.PrintMessage("\n")
            CivilCvCAD.Console.PrintMessage(moments[ax])
            CivilCvCAD.Console.PrintMessage("\n")
    ...
    def printEquilibrium(self, var, coeff):
        ...
        for i in range(len(var)):
            if i == 0:
                CivilCvCAD.Console.PrintMessage("%f = " % coeff[i])
            else:
                CivilCvCAD.Console.PrintMessage("%f * %s" % (coeff[i], var[i]))
