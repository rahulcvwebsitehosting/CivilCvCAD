...
xvfb-run \
/path/to/local/civilcvcad/install/bin/CivilCvCAD -t TestPartDesignGui.TestSuppressedStrikethrough
"""
...
class TestSuppressed(unittest.TestCase):
    def setUp(self):
        self.Doc = CivilCvCAD.newDocument("PartDesignTestSuppressed")
    ...
    def testSuppressedPersistsAfterReload(self):
        ...
        # Save, close, reopen
        with tempfile.TemporaryDirectory(prefix="civilcvcad_test_suppressed_") as tmpdir:
            filepath = os.path.join(tmpdir, "test_suppressed.FCStd")
            self.Doc.saveAs(filepath)
            CivilCvCAD.closeDocument(self.Doc.Name)
            self.Doc = CivilCvCAD.openDocument(filepath)
...
def _findTreeWidget():
    """Find the main tree widget (QTreeWidget) in the CivilCvCAD GUI."""
...
class TestSuppressedStrikethrough(unittest.TestCase):
    ...
    def setUp(self):
        if not hasattr(CivilCvCADGui, "getMainWindow") or CivilCvCADGui.getMainWindow() is None:
            self.skipTest("Requires GUI")
        self.Doc = CivilCvCAD.newDocument("TestSuppressedStrikethrough")
    ...
    def _createBodyWithBox(self):
        self.Body = self.Doc.addObject("PartDesign::Body", "Body")
        CivilCvCADGui.activeView().setActiveObject("pdbody", self.Body)
