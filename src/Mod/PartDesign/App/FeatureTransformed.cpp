...
Part::Feature* Transformed::getBaseObject(bool silent) const
{
    ...
    if (firstOriginal) {
        rv = civilcvcad_cast<Part::Feature*>(firstOriginal);
        if (!rv) {
        ...
        if (civilcvcad_cast<const Mirrored*>(this)) {
            err = QT_TRANSLATE_NOOP("Exception", "No features selected to be mirrored.");
        }
        else if (civilcvcad_cast<const LinearPattern*>(this) || civilcvcad_cast<const PolarPattern*>(this)) {
...
std::vector<App::DocumentObject*> Transformed::getOriginals() const
{
    ...
    const auto isSuppressed = [](const DocumentObject* obj) {
        auto feature = civilcvcad_cast<Feature*>(obj);
...
App::DocumentObject* Transformed::getSketchObject() const
{
    ...
    if (auto feature = civilcvcad_cast<PartDesign::ProfileBased*>(firstOriginal)) {
        return feature->getVerifiedSketch(true);
    }
    if (civilcvcad_cast<PartDesign::FeatureAddSub*>(firstOriginal)) {
        return nullptr;
    }
    if (auto pattern = civilcvcad_cast<LinearPattern*>(this)) {
        return pattern->Direction.getValue();
    }
    if (auto pattern = civilcvcad_cast<PolarPattern*>(this)) {
        return pattern->Axis.getValue();
    }
    if (auto pattern = civilcvcad_cast<Mirrored*>(this)) {
...
bool Transformed::isMultiTransformChild() const
{
    ...
    for (auto const* obj : getInList()) {
        auto mt = civilcvcad_cast<PartDesign::MultiTransform*>(obj);
        if (!mt) {
            continue;
        }
...
)
{
    ...
    if (auto property = civilcvcad_cast<App::PropertyFloat*>(prop);
...
App::DocumentObjectExecReturn* Transformed::recomputePreview()
{
    ...
    const auto makeCompoundOfToolShapes = [this, &supportTransform]() {
        ...
        for (const auto& original : getOriginals()) {
            if (auto* feature = civilcvcad_cast<FeatureAddSub*>(original)) {
    ...
    switch (mode) {
        case Mode::Features:
            ...
            for (auto original : originals) {
                ...
                auto feature = civilcvcad_cast<PartDesign::FeatureAddSub*>(original);
                if (!feature) {
