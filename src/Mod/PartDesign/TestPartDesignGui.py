...
# ---------------------------------------------------------------------------
# define the test cases to test the CivilCvCAD PartDesign module
# ---------------------------------------------------------------------------
class PartDesignGuiTestCases(unittest.TestCase):
    ...
    def testMoveSingleFeature(self):
        CivilCvCAD.Console.PrintMessage("Testing moving one feature from one body to another\n")
        self.BodySource = self.Doc.addObject("PartDesign::Body", "Body")
        Gui.activateView("Gui::View3DInventor", True)
        Gui.activeView().setActiveObject("pdbody", self.BodySource)
    ...
    def tearDown(self):
        CivilCvCAD.closeDocument("SketchGuiTest")
...
class CreateSketch(unittest.TestCase):
    ...
    def testPDCreateSketch(self):
        ...
        param = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/PartDesign")
        ...
        CivilCvCADGui.activeView().setActiveObject("pdbody", App.activeDocument().Body)
        CivilCvCADGui.Selection.clearSelection()
        CivilCvCADGui.runCommand("Std_OrthographicCamera", 1)
        workflowcheck = CallableCheckExemptionDialog(self)
        QtCore.QTimer.singleShot(100, workflowcheck)
        CivilCvCADGui.runCommand("PartDesign_CompSketches", 0)
        activeDialog = CivilCvCADGui.Control.activeDialog()
...
class TestShapeBinder(unittest.TestCase):
    def setUp(self):
        self.Doc = CivilCvCAD.newDocument("PartDesignTestShapeBinder")
...
class TestSubShapeBinder(unittest.TestCase):
    def setUp(self):
        self.Doc = CivilCvCAD.newDocument("PartDesignTestSubShapeBinder")
...
class TestDatumPlane(unittest.TestCase):
    def setUp(self):
        self.Doc = CivilCvCAD.newDocument("PartDesignTestDatumPlane")
