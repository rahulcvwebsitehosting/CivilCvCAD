    ...
    try {
        ...
        for (auto& info : shapeObjects) {
            ...
            QString cmd = QStringLiteral(
                              "__doc__=CivilCvCAD.getDocument(\"%1\")\n"
...
void Tessellation::setFaceColors(int method, App::Document* doc, App::DocumentObject* obj)
{
    ...
    if (method == Standard) {
        if (ui->meshShapeColors->isChecked()) {
            ...
            auto svp = civilcvcad_cast<PartGui::ViewProviderPartExt*>(
...
bool Mesh2ShapeGmsh::writeProject(QString& inpFile, QString& outFile)
{
    if (!d->shapes.empty()) {
        ...
        if (part) {
            ...
            geoOut << "// geo file for meshing with Gmsh meshing software created by CivilCvCAD\n"
                   << "// open brep geometry\n"
                   << "Merge \"" << d->cadFile << "\";\n\n"
