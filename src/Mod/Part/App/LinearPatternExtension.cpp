...
gp_Dir LinearPatternExtension::getDirectionFromProperty(const App::PropertyLinkSub& dirProp) const
{
    ...
    gp_Dir dir;
    if (auto* refSketch = civilcvcad_cast<Part::Part2DObject*>(refObject)) {
        Base::Axis axis;
    ...
    else if (auto* plane = civilcvcad_cast<App::Plane*>(refObject)) {
        Base::Vector3d d = plane->getDirection();
        dir = gp_Dir(d.x, d.y, d.z);
    }
    else if (auto* line = civilcvcad_cast<App::Line*>(refObject)) {
        Base::Vector3d d = line->getDirection();
        dir = gp_Dir(d.x, d.y, d.z);
    }
    else if (auto* refFeature = civilcvcad_cast<Part::Feature*>(refObject)) {
    ...
    TopLoc_Location loc;
    if (auto* feat = civilcvcad_cast<Part::Feature*>(getExtendedObject())) {
