...
void PropertyGeometryList::setPyObject(PyObject* value)
{
    ...
    Part2DObject* part2d = civilcvcad_cast<Part2DObject*>(this->getContainer());
...
void PropertyGeometryList::trySaveGeometry(Geometry* geom, Base::Writer& writer) const
{
    ...
    try {
        ...
        for (auto& ext : geom->getExtensions()) {
            ...
            auto gpe = civilcvcad_cast<GeometryMigrationPersistenceExtension*>(extension.get());
            if (gpe) {
                gpe->postSave(writer);
            }
        }
...
void PropertyGeometryList::Save(Writer& writer) const
{
    ...
    for (int i = 0; i < getSize(); i++) {
        ...
        for (auto& e : _lValueList[i]->getExtensions()) {
            auto ext = e.lock();
            if (auto gpe = civilcvcad_cast<GeometryMigrationPersistenceExtension*>(ext.get())) {
                gpe->preSave(writer);
            }
        }
