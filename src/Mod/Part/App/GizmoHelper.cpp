...
Base::Vector3d getMidPointFromFace(Part::TopoShape& face)
{
    ...
    auto* surface = civilcvcad_cast<Part::GeomSurface*>(geom.get());
    ...
    if (auto sphere = civilcvcad_cast<Part::GeomSphere*>(geom.get())) {
        midPoint = sphere->getLocation();
    }
    else if (auto cone = civilcvcad_cast<Part::GeomCone*>(geom.get())) {
