    ...
    switch (mmode) {
        ...
        case mmMidpoint: {
            ...
            switch (shape->shapeType()) {
                case TopAbs_VERTEX: {
                    if (auto point = civilcvcad_cast<GeomPoint*>(geom.get())) {
                        placement.setPosition(point->getPoint());
                    }
                } break;

                case TopAbs_EDGE: {
                    if (auto conic = civilcvcad_cast<GeomConic*>(geom.get())) {
                        placement.setPosition(conic->getLocation());
                        placement.setRotation(conic->getRotation().value_or(Base::Rotation {}));
                    }
                    else if (auto line = civilcvcad_cast<GeomCurve*>(geom.get())) {
                ...
                case TopAbs_FACE: {
                    auto surface = civilcvcad_cast<GeomSurface*>(geom.get());
                    ...
                    if (auto sphere = civilcvcad_cast<GeomSphere*>(geom.get())) {
                        placement.setPosition(sphere->getLocation());
                    }
                    else if (auto cone = civilcvcad_cast<GeomCone*>(geom.get())) {
...
double AttachEngine3D::calculateFoldAngle(gp_Vec axA, gp_Vec axB, gp_Vec edA, gp_Vec edB) const
{
    ...
    // https://forum.civilcvcad.org/viewtopic.php?f=8&t=10511&sid=007946a934530ff2a6c9259fb32624ec&start=40#p87584
    axA.Normalize();
    axB.Normalize();
    edA.Normalize();
    edB.Normalize();
