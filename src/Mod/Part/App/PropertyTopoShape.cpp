...
void PropertyPartShape::setValue(const TopoShape& sh)
{
    aboutToSetValue();
    _Shape = sh;
    auto obj = civilcvcad_cast<App::DocumentObject*>(getContainer());
    if (obj) {
...
const TopoShape& PropertyPartShape::getShape() const
{
    ...
    //    else if (!res.Tag) {
    if (!_Shape.Tag) {
        if (auto parent = civilcvcad_cast<App::DocumentObject*>(getContainer())) {
...
{
    auto prop = civilcvcad_cast<const PropertyPartShape*>(&from);
    if (prop) {
...
void PropertyPartShape::RestoreDocFile(Base::Reader& reader)
{
    ...
    // https://github.com/realthunder/CivilCvCAD/blob/a9810d509a6f112b5ac03d4d4831b67e6bffd5b7/src/Mod/Part/App/PropertyTopoShape.cpp#L639
...
{
    auto prop = civilcvcad_cast<PropertyShapeCache*>(obj->getDynamicPropertyByName(SHAPE_CACHE_NAME));
