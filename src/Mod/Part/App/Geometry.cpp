...
bool Geometry::hasSameExtensions(const Geometry& other) const
{
    ...
    for (const auto& ext : extensions) {
        if (auto persistExt = civilcvcad_cast<const GeometryPersistenceExtension*>(ext.get())) {
            for (; index < other.extensions.size(); ++index) {
                if (auto extOther = civilcvcad_cast<const GeometryPersistenceExtension*>(
    ...
    for (; index < other.extensions.size(); ++index) {
        if (civilcvcad_cast<const GeometryPersistenceExtension*>(other.extensions[index].get())) {
            return false;
        }
    }
    return true;
...
{
    // https://forum.civilcvcad.org/viewtopic.php?f=10&t=31700
...
{
    return new BezierCurvePy(civilcvcad_cast<GeomBezierCurve*>(this->clone()));
}
...
{
    return new BSplineCurvePy(civilcvcad_cast<GeomBSplineCurve*>(this->clone()));
}
...
{
    return new LineSegmentPy(civilcvcad_cast<GeomLineSegment*>(this->clone()));
}
...
{
    return new OffsetCurvePy(civilcvcad_cast<GeomOffsetCurve*>(this->clone()));
}
...
gp_Vec GeomCone::getDN(double u, double v, int Nu, int Nv) const
{
    ...
    // https://forum.civilcvcad.org/viewtopic.php?f=10&t=66677
...
)
{
    if (geo1->is<GeomLineSegment>() && geo2->is<GeomLineSegment>()) {
        ...
        if (radius == .0f) {
            // guess a radius
            // https://forum.civilcvcad.org/viewtopic.php?f=3&t=31594&start=50#p266658
            //
