...
class ViewProviderTube:
    ...
    def setupContextMenu(self, viewObject, menu):
        action = menu.addAction(
            CivilCvCAD.Qt.translate("QObject", "Edit %1").replace("%1", viewObject.Object.Label)
        )
    ...
    def startDefaultEditMode(self, viewObject):
        ...
        if not document.HasPendingTransaction:
            text = CivilCvCAD.Qt.translate("QObject", "Edit %1").replace("%1", viewObject.Object.Label)
            document.openTransaction(text)
        viewObject.Document.setEdit(viewObject.Object, 0)

    def setEdit(self, viewObject, mode):
        if mode == 0:
            CivilCvCADGui.Control.showDialog(TaskTubeUI(viewObject))
            return True
...
class TaskTubeUI:
    ...
    def __init__(self, viewObject):
        ...
        CivilCvCADGui.ExpressionBinding(self.form.tubeOuterRadius).bind(object, "OuterRadius")
        CivilCvCADGui.ExpressionBinding(self.form.tubeInnerRadius).bind(object, "InnerRadius")
        CivilCvCADGui.ExpressionBinding(self.form.tubeHeight).bind(object, "Height")
