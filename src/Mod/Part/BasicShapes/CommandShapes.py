...
class CommandTube:
    ...
    def Activated(self):
        text = CivilCvCAD.Qt.translate("QObject", "Create tube")
        CivilCvCAD.ActiveDocument.openTransaction(text)
        tube = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", "Tube")
        Shapes.TubeFeature(tube)
        vp = ViewProviderShapes.ViewProviderTube(tube.ViewObject)
        activePart = CivilCvCADGui.activeView().getActiveObject("part")
        if activePart:
...
CivilCvCADGui.addCommand("Part_Tube", CommandTube())
