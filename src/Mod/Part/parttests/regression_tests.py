...
import CivilCvCAD
from CivilCvCAD import Vector, Base, newDocument, closeDocument
import Part
...
class RegressionTests(unittest.TestCase):
    ...
    def test_TangentMode3_Issue24254(self):
        """In CivilCvCAD 1.1 we changed the behavior of the mmTangentPlane, but added code to handle old files
        that relied upon the original tangent plane behavior. This test ensures that old files open with the expected
        tangent plane setup."""

        location = os.path.dirname(os.path.realpath(__file__))
        CivilCvCAD.openDocument(os.path.join(location, "TestTangentMode3-0.21.FCStd"), True)

        def check_plane(name, expected_pos, expected_normal, expected_xaxis):
            obj = CivilCvCAD.ActiveDocument.getObject(name)
            if not obj:
                CivilCvCAD.Console.PrintError(f"{name}: object not found\n")
                return

            pos = obj.Placement.Base
            rot = obj.Placement.Rotation
            normal = rot.multVec(CivilCvCAD.Vector(0, 0, 1))
            xaxis = rot.multVec(CivilCvCAD.Vector(1, 0, 0))
        ...
        expected_planes = [
            (
                "Sketch001",
                CivilCvCAD.Vector(-1 * r, 0, 0),  # position
                CivilCvCAD.Vector(1, 0, 0),  # normal
                CivilCvCAD.Vector(0, -1, 0),
            ),  # tangent x-axis
            (
                "Sketch002",
                CivilCvCAD.Vector(sin30 * r, cos30 * cos30 * (-r), cos30 * sin30 * r),
                CivilCvCAD.Vector(sin30, cos30 * cos30 * (-1), cos30 * sin30),
                CivilCvCAD.Vector(0, sin30, cos30),
            ),
            (
                "Sketch003",
                CivilCvCAD.Vector(cos30 * cos30 * r, sin30 * r, cos30 * sin30 * r),
                CivilCvCAD.Vector(cos30 * cos30, sin30, cos30 * sin30),
                CivilCvCAD.Vector(sin30, 0, -cos30),
            ),
        ]
    ...
    def test_issue_29733_transformShape_bakes_in_transformation(self):
        ...
        move = CivilCvCAD.Placement(CivilCvCAD.Vector(10, 0, 0), CivilCvCAD.Rotation()).toMatrix()
