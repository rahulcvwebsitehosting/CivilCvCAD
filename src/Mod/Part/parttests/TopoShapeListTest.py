...
class TopoShapeListTest(unittest.TestCase):
    ...
    def testMakeTopoShapeList(self):
        ...
        self.assertLessEqual(
            error, maxError, "TopoShapeList entry 0 has wrong volume: {0}".format(boxes[0].Volume)
        )
        error = abs(2.0 - boxes[1].Volume)
        self.assertLessEqual(
            error, maxError, "TopoShapeList entry 1 has wrong volume: {0}".format(boxes[1].Volume)
        )
        error = abs(3.0 - boxes[2].Volume)
        self.assertLessEqual(
            error, maxError, "TopoShapeList entry 2 has wrong volume: {0}".format(boxes[2].Volume)
        )
        ...
        self.assertEqual(
            len(obj.Shapes),
            2,
            "TopoShapeList has wrong entry count (1): {0}".format(len(obj.Shapes)),
        )
        ...
        self.assertEqual(
            len(obj.Shapes),
            3,
            "TopoShapeList has wrong entry count (2): {0}".format(len(obj.Shapes)),
        )
        App.closeDocument(doc.Name)
