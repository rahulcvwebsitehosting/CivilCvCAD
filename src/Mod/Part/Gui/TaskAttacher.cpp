...
void TaskAttacher::findCorrectObjAndSubInThisContext(App::DocumentObject*& rootObj, std::string& sub)
{
    ...
    // allow because of some workflow see https://github.com/CivilCvCAD/CivilCvCAD/issues/29714
...
{
    if (auto planarViewProvider = civilcvcad_cast<PartGui::ViewProvider2DObject*>(ViewProvider)) {
        overrides.override(planarViewProvider->ShowPlane, true);
    }

    if (auto draggerViewProvider = civilcvcad_cast<Gui::ViewProviderDragger*>(ViewProvider)) {
