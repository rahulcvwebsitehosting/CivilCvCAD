...
void CmdPartPointsFromMesh::activated(int iMsg)
{
    ...
    auto found = std::find_if(geoms.begin(), geoms.end(), [](App::DocumentObject* obj) {
        return civilcvcad_cast<Part::Feature*>(obj);
    });

    if (found != geoms.end()) {

        double defaultDistance = getDefaultDistance(civilcvcad_cast<Part::Feature*>(*found));
    ...
    for (std::vector<Gui::SelectionObject>::iterator it = objs.begin(); it != objs.end(); ++it) {
        try {
            ...
            doCommand(
                ...
                "else:\n"
                "\t\tCivilCvCAD.Console.PrintError('Defeaturing failed\\n')",
                it->getDocName(),
                it->getFeatName(),
                faces.c_str(),
                it->getFeatName()
