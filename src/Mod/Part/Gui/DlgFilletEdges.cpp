...
bool DlgFilletEdges::accept()
{
    ...
    if (!d->fillet) {
        code = QStringLiteral(
                   ...
                   "CivilCvCAD.ActiveDocument.%2.Base = CivilCvCAD.ActiveDocument.%3\n"
        )
                   .arg(type, name, shape);
    }
    ...
    Gui::WaitCursor wc;
    code += QStringLiteral(
                "CivilCvCAD.ActiveDocument.%1.Edges = __fillets__\n"
                "del __fillets__\n"
                "CivilCvCADGui.ActiveDocument.%2.Visibility = False\n"
    )
                .arg(name, shape);
