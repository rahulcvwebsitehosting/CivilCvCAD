...
def cmdCreateToleranceSetFeature(name, minTolerance=1e-7, maxTolerance=0):
    ...
    CivilCvCAD.ActiveDocument.openTransaction("Create ToleranceSet")
    CivilCvCADGui.addModule("BOPTools.ToleranceFeatures")
    CivilCvCADGui.doCommand(
        "j = BOPTools.ToleranceFeatures.makeToleranceSet(name='{name}')".format(name=name)
    )
    CivilCvCADGui.doCommand("j.minTolerance = {minTolerance}".format(minTolerance=minTolerance))
    CivilCvCADGui.doCommand("j.maxTolerance = {maxTolerance}".format(maxTolerance=maxTolerance))
    CivilCvCADGui.doCommand(
...
def addCommands():
    CivilCvCADGui.addCommand("Part_ToleranceSet", CommandToleranceSet())
