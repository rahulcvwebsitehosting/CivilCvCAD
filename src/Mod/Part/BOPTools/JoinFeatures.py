...
def getParamRefine():
    return CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Part/Boolean").GetBool(
        "RefineModel"
    )
...
def cmdCreateJoinFeature(name, mode):
    ...
    CivilCvCAD.ActiveDocument.openTransaction("Create " + mode)
    CivilCvCADGui.addModule("BOPTools.JoinFeatures")
    ...
    if mode == "Embed" or mode == "Cutout":
        CivilCvCADGui.doCommand("j.Base = App.ActiveDocument." + sel[0].Object.Name)
        CivilCvCADGui.doCommand("j.Tool = App.ActiveDocument." + sel[1].Object.Name)
    elif mode == "Connect":
        CivilCvCADGui.doCommand(
...
class CommandEmbed:
    ...
    def Activated(self):
        if len(CivilCvCADGui.Selection.getSelectionEx()) == 2:
...
def addCommands():
    CivilCvCADGui.addCommand("Part_JoinCutout", CommandCutout())
    CivilCvCADGui.addCommand("Part_JoinEmbed", CommandEmbed())
    CivilCvCADGui.addCommand("Part_JoinConnect", CommandConnect())
