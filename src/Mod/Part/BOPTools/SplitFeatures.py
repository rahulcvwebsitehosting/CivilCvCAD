...
def makeBooleanFragments(name):
    ...
    obj = CivilCvCAD.ActiveDocument.addObject("Part::FeaturePython", name)
...
class ViewProviderBooleanFragments:
    ...
    def onDelete(self, feature, subelements):
        try:
            ...
            CivilCvCAD.Console.PrintError("Error in onDelete: " + str(err))
        return True
...
def cmdCreateBooleanFragmentsFeature(name, mode):
    ...
    CivilCvCAD.ActiveDocument.openTransaction("Create Boolean Fragments")
    CivilCvCADGui.addModule("BOPTools.SplitFeatures")
    ...
    CivilCvCADGui.doCommand("j.Mode = {mode}".format(mode=repr(mode)))

    try:
        CivilCvCADGui.doCommand("j.Proxy.execute(j)")
        CivilCvCADGui.doCommand("j.purgeTouched()")
    except Exception as err:
...
class CommandBooleanFragments:
    ...
    def Activated(self):
        if len(CivilCvCADGui.Selection.getSelectionEx()) >= 1:
...
def cmdCreateSliceFeature(name, mode, transaction=True):
    ...
    sel = CivilCvCADGui.Selection.getSelectionEx()
    if transaction:
        CivilCvCAD.ActiveDocument.openTransaction("Create Slice")
    CivilCvCADGui.addModule("BOPTools.SplitFeatures")
    CivilCvCADGui.doCommand("f = BOPTools.SplitFeatures.makeSlice(name='{name}')".format(name=name))
    CivilCvCADGui.doCommand(
        "f.Base = {sel}[0]\n"
    ...
    CivilCvCADGui.doCommand("f.Mode = {mode}".format(mode=repr(mode)))
...
def cmdSliceApart():
    CivilCvCAD.ActiveDocument.openTransaction("Slice apart")
    made = cmdCreateSliceFeature(name="Slice", mode="Split", transaction=False)

    if made:
        CivilCvCADGui.addModule("CompoundTools.Explode")
        CivilCvCADGui.doCommand("CompoundTools.Explode.explodeCompound(f)")
        CivilCvCADGui.doCommand("f.ViewObject.hide()")
        CivilCvCAD.ActiveDocument.commitTransaction()
        CivilCvCADGui.doCommand("App.ActiveDocument.recompute()")
    else:
        CivilCvCAD.ActiveDocument.abortTransaction()
...
def cmdCreateXORFeature(name):
    ...
    sel = CivilCvCADGui.Selection.getSelectionEx()
    CivilCvCAD.ActiveDocument.openTransaction("Create Boolean XOR")
    CivilCvCADGui.addModule("BOPTools.SplitFeatures")
    CivilCvCADGui.doCommand("j = BOPTools.SplitFeatures.makeXOR(name='{name}')".format(name=name))
    CivilCvCADGui.doCommand(
        "j.Objects = {sel}".format(
            sel="[" + ", ".join(["App.ActiveDocument." + so.Object.Name for so in sel]) + "]"
        )
...
def addCommands():
    CivilCvCADGui.addCommand("Part_BooleanFragments", CommandBooleanFragments())
    CivilCvCADGui.addCommand("Part_Slice", CommandSlice())
    CivilCvCADGui.addCommand("Part_SliceApart", CommandSliceApart())
    CivilCvCADGui.addCommand("Part_XOR", CommandXOR())
