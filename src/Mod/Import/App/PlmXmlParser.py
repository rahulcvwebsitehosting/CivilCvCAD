...
def addPart(partElement):
    global CivilCvCAD_On, CivilCvCAD_Doc, CivilCvCAD_ObjList
    print("=== Part ======================================================")
    ...
    if CivilCvCAD_On:
        import CivilCvCAD, Assembly

        print("Create Reference")
        partObject = CivilCvCAD_Doc.addObject("App::Part", id)
        CivilCvCAD_ObjList.append(partObject)
...
def addAssembly(asmElement):
    ...
    if CivilCvCAD_On:
        ...
        print("Create Reference")
        admObject = CivilCvCAD_Doc.addObject("Assembly::Product", id)
        CivilCvCAD_ObjList.append(admObject)
...
def addReference(refElement):
    ...
    if CivilCvCAD_On:
        ...
        print("Create Reference")
        refObject = CivilCvCAD_Doc.addObject("Assembly::ProductRef", id)
        CivilCvCAD_ObjList.append(refObject)
...
def resolveRefs():
    ...
    if CivilCvCAD_On:
        for i in CivilCvCAD_ObjList:
            if i.TypeId == "Assembly::Product":
                objectList = []
                for l in i.Meta["instanceRefs"].split(" "):
                    objectList.append(CivilCvCAD_Doc.getObject(l))
                i.Items = objectList
            if i.TypeId == "Assembly::ProductRef":
                i.Item = CivilCvCAD_Doc.getObject(i.Meta["partRef"])


def open(fileName):
    """called when civilcvcad opens an PlmXml file"""
    global CivilCvCAD_On, CivilCvCAD_Doc
    import CivilCvCAD, os
...
def insert(filename, docname):
    """called when civilcvcad imports an PlmXml file"""
    global CivilCvCAD_On, CivilCvCAD_Doc
    import CivilCvCAD

    CivilCvCAD.setActiveDocument(docname)
    doc = CivilCvCAD.getDocument(docname)
    CivilCvCAD.Console.PrintMessage('Started import of "' + filename + '" file')
    CivilCvCAD_Doc = doc
    CivilCvCAD_On = True
    parse(fileName)
    resolveRefs()
