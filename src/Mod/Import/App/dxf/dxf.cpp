...
//***************************
// writeHeaderSection
// added by Wandererfan 2018 (wandererfan@gmail.com) for CivilCvCAD project
void CDxfWrite::writeHeaderSection()
{
    std::stringstream ss;
    ss << "CivilCvCAD v" << App::Application::Config()["BuildVersionMajor"] << "."
...
void CDxfRead::DoRead(const bool ignore_errors /* = false */)
{
    ...
    try {
        ...
        // This catches specific CivilCvCAD exceptions and re-throws them.
        throw;
    }
    catch (const std::exception& e) {
        // This catches all standard C++ exceptions and converts them
        // to a CivilCvCAD exception, which the binding layer can handle.
...
Base::Color CDxfRead::ObjectColor(ColorIndex_t index)
{
    ...
    // this is just a rendering feature, but CivilCvCAD does not support this so the user has the
