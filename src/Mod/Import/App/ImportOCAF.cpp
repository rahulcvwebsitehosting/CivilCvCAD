...
)
{
    ...
    if (isRef || myRefShapes.find(hash) == myRefShapes.end()) {
        ...
        if (aShapeTool->IsSimpleShape(label) && (isRef || aShapeTool->IsFree(label))) {
            ...
            // Importer/Exporter option within the CivilCvCAD preference menu Currently it is merging
            ...
            if (!localValue.empty()) {
                if (aShapeTool->IsAssembly(label)) {
                    ...
                    // standard CivilCvCAD placement was absolute we are now moving to relative
