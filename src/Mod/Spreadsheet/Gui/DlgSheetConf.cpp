...
)
{
    ...
    if (cell && cell->getExpression()) {
        ...
        if (expr->isDerivedFrom<FunctionExpression>()) {
            auto fexpr = civilcvcad_cast<FunctionExpression*>(cell->getExpression());
            if (fexpr
        ...
        auto vexpr = civilcvcad_cast<VariableExpression*>(expr);
        if (vexpr) {
            auto prop = civilcvcad_cast<PropertyEnumeration*>(vexpr->getPath().getProperty());
            if (prop) {
                auto obj = civilcvcad_cast<DocumentObject*>(prop->getContainer());
                if (obj && prop->hasName()) {
