...
void CmdSpreadsheetMergeCells::activated(int iMsg)
{
    ...
    if (getActiveGuiDocument()) {
        ...
        SpreadsheetGui::SheetView* sheetView = civilcvcad_cast<SpreadsheetGui::SheetView*>(activeWindow);
...
void CmdSpreadsheetExport::activated(int iMsg)
{
    ...
    if (getActiveGuiDocument()) {
        ...
        if (sheetView) {
            ...
            auto* vps = civilcvcad_cast<ViewProviderSheet*>(vp);
            if (vps) {
                vps->exportAsFile();
            }
        }
...
bool CmdSpreadsheetExport::isActive()
{
    if (getActiveGuiDocument()) {
        Gui::MDIView* activeWindow = Gui::getMainWindow()->activeWindow();
        if (activeWindow && civilcvcad_cast<SpreadsheetGui::SheetView*>(activeWindow)) {
            return true;
        }
    }
    return false;
...
bool CmdSpreadsheetSetAlias::isActive()
{
    if (getActiveGuiDocument()) {
        ...
        if (activeWindow) {
            SpreadsheetGui::SheetView* sheetView = civilcvcad_cast<SpreadsheetGui::SheetView*>(
                activeWindow
            );
