...
class SpreadsheetCases(unittest.TestCase):
    ...
    def testCrossDocumentLinks(self):
        ...
        CivilCvCAD.closeDocument(doc2.Name)

        # Open both documents again
        self.doc = CivilCvCAD.openDocument(self.TempPath + os.sep + "cube.fcstd")
        doc2 = CivilCvCAD.openDocument(self.TempPath + os.sep + "cylinder.fcstd")
    ...
    def testMatrix(self):
        ...
        vec = CivilCvCAD.Vector(2, 1, 2)

        rot = CivilCvCAD.Rotation(CivilCvCAD.Vector(0, 1, 0), 45)
        irot = rot.inverted()

        pla = CivilCvCAD.Placement(vec, rot)
        ipla = pla.inverse()
        ...
        self.assertEqual(sheet.D2, CivilCvCAD.Matrix())
        ...
        self.assertTrue(sheet.D3.isSame(CivilCvCAD.Rotation(), rtol))
        ...
        self.assertTrue(plm_equal(sheet.D4, CivilCvCAD.Placement()))
        ...
        self.assertTrue(sheet.A7.Base.isEqual(CivilCvCAD.Vector(1, 2, 3), tol))
        self.assertTrue(sheet.B7.Base.isEqual(CivilCvCAD.Vector(1, -3, 2), tol))
        self.assertTrue(sheet.C7.Base.isEqual(CivilCvCAD.Vector(1, -3, 2), tol))
        self.assertTrue(sheet.D7.Base.isEqual(CivilCvCAD.Vector(3, 2.0, -1), tol))
        self.assertTrue(sheet.E7.Base.isEqual(CivilCvCAD.Vector(-2, 1, 3.0), tol))
        self.assertTrue(sheet.F7.Base.isEqual(CivilCvCAD.Vector(2, 4, 6), tol))
        self.assertTrue(sheet.G7.Base.isEqual(CivilCvCAD.Vector(2, 4, 6), tol))
    ...
    def testIssue3363(self):
        ...
        self.doc = CivilCvCAD.openDocument(self.TempPath + os.sep + "conditionals.fcstd")
    ...
    def test_cross_link_empty_property_name(self):
        # https://forum.civilcvcad.org/viewtopic.php?f=3&t=58603
        base = CivilCvCAD.newDocument("base")
        sheet = base.addObject("Spreadsheet::Sheet", "Spreadsheet")
        sheet.setAlias("A1", "x")
        sheet.set("x", "42mm")
        base.recompute()

        square = CivilCvCAD.newDocument("square")
        ...
        CivilCvCAD.closeDocument(square.Name)
        CivilCvCAD.closeDocument(base.Name)

        ##
        ## preparation done
        base = CivilCvCAD.openDocument(base_path)
        square = CivilCvCAD.openDocument(square_path)
    ...
    def test_expression_with_alias(self):
        # https://forum.civilcvcad.org/viewtopic.php?p=564502#p564502
        ...
        CivilCvCAD.closeDocument(self.doc.Name)
        self.doc = CivilCvCAD.openDocument(project_path)
        ss1 = self.doc.Input
        ss2 = self.doc.Output
    ...
    def testAliasEmptyCell(self):
        # https://github.com/CivilCvCAD/CivilCvCAD/issues/7841
        sheet = self.doc.addObject("Spreadsheet::Sheet", "Spreadsheet")
    ...
    def testIssue6395(self):
        ...
        self.doc = CivilCvCAD.openDocument(self.TempPath + os.sep + "string.fcstd")
    ...
    def testVectorFunctions(self):
        ...
        self.assertEqual(sheet.A1, CivilCvCAD.Vector(-1, -4, 3))

        self.assertEqual(sheet.B1, 12)

        self.assertEqual(sheet.C1, 90)

        self.assertEqual(sheet.D1, CivilCvCAD.Vector(1, 0, 0))
        self.assertLess(
            sheet.D2.distanceToPoint(CivilCvCAD.Vector(1 / sqrt(3), 1 / sqrt(3), 1 / sqrt(3))),
            tolerance,
        )

        self.assertEqual(sheet.E1, CivilCvCAD.Vector(2, 6, 12))
        self.assertEqual(sheet.E2, CivilCvCAD.Vector(-2, 2, 3))
        self.assertEqual(sheet.E3, CivilCvCAD.Vector(1, -4, 3))
        self.assertEqual(sheet.E4, CivilCvCAD.Vector(1, 2, -6))

        self.assertLess(abs(sheet.F1.Value - 0.3464), 0.0001)
        self.assertEqual(sheet.F2, CivilCvCAD.Vector(1, 1, 1))
        self.assertLess(sheet.F3.distanceToPoint(CivilCvCAD.Vector(0.28, 0.04, -0.2)), tolerance)
        self.assertLess(abs(sheet.F4.Value - -1.6971), 0.0001)
        self.assertEqual(sheet.F5, CivilCvCAD.Vector(1.72, 2.96, 4.2))
