...
bool Cell::getStringContent(std::string& s, bool persistent) const
{
    if (expression) {
        ...
        else if (civilcvcad_cast<App::StringExpression*>(expression.get())) {
            s = static_cast<App::StringExpression*>(expression.get())->getText();
            s = "'" + s;
        }
        else if (civilcvcad_cast<App::ConstantExpression*>(expression.get())) {
            s = "=" + expression->toString();
        }
        else if (civilcvcad_cast<App::NumberExpression*>(expression.get())) {
            s = expression->toString();
        }
        else {
            s = "=" + expression->toString(persistent);
...
{
    auto expr = civilcvcad_cast<StringExpression*>(expression.get());
    if (expr) {
        ...
        if (*value == '=') {
            ...
            if (!newExpr && isStartingWithNumber) {
                try {
                    ...
                    if (const auto fraction = civilcvcad_cast<OperatorExpression*>(parsedExpr.get())) {
                        if (fraction->getOperator() == OperatorExpression::UNIT) {
                            const auto left = civilcvcad_cast<NumberExpression*>(fraction->getLeft());
                            const auto right = civilcvcad_cast<UnitExpression*>(fraction->getRight());
                            if (left && right) {
                            ...
                            const bool isNumberNom = civilcvcad_cast<NumberExpression*>(
                                fraction->getLeft()
                            );
                            const bool isNumberDenom = civilcvcad_cast<NumberExpression*>(
                                fraction->getRight()
                            );

                            // check for numbers with units in (de)nominator
                            const auto opNom = civilcvcad_cast<OperatorExpression*>(fraction->getLeft());
                            const auto opDenom = civilcvcad_cast<OperatorExpression*>(
                                fraction->getRight()
                            );
                            ...
                            const auto uDenom = civilcvcad_cast<UnitExpression*>(fraction->getRight());
                    ...
                    else if (civilcvcad_cast<NumberExpression*>(parsedExpr.get())) {
