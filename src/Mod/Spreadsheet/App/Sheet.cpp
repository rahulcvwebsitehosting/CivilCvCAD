...
bool Sheet::exportToFile(const std::string& filename, char delimiter, char quoteChar, char escapeChar) const
{
    ...
    while (i != usedCells.end()) {
        ...
        if (auto p = civilcvcad_cast<PropertyQuantity*>(prop)) {
            field << p->getValue();
        }
        else if (auto p = civilcvcad_cast<PropertyFloat*>(prop)) {
            field << p->getValue();
        }
        else if (auto p = civilcvcad_cast<PropertyInteger*>(prop)) {
            field << p->getValue();
        }
        else if (auto p = civilcvcad_cast<PropertyString*>(prop)) {
            field << p->getValue();
        }
        else {
            assert(0);
...
Property* Sheet::setFloatProperty(CellAddress key, double value)
{
    ...
    if (!prop || !prop->is<PropertyFloat>()) {
        ...
        floatProp = civilcvcad_cast<PropertyFloat*>(addDynamicProperty(
            "App::PropertyFloat",
            name.c_str(),
            nullptr,
            nullptr,
...
Property* Sheet::setIntegerProperty(CellAddress key, long value)
{
    ...
    if (!prop || !prop->is<PropertyInteger>()) {
        ...
        intProp = civilcvcad_cast<PropertyInteger*>(addDynamicProperty(
            "App::PropertyInteger",
            name.c_str(),
            nullptr,
            nullptr,
...
Property* Sheet::setQuantityProperty(CellAddress key, double value, const Base::Unit& unit)
{
    ...
    if (!prop || !prop->is<PropertySpreadsheetQuantity>()) {
        ...
        quantityProp = civilcvcad_cast<PropertySpreadsheetQuantity*>(p);
    }
    else {
...
Property* Sheet::setStringProperty(CellAddress key, const std::string& value)
{
    std::string name = key.toString(CellAddress::Cell::ShowRowColumn);
    Property* prop = props.getDynamicPropertyByName(name.c_str());
    PropertyString* stringProp = civilcvcad_cast<PropertyString*>(prop);
    ...
    if (!stringProp) {
        ...
        stringProp = civilcvcad_cast<PropertyString*>(addDynamicProperty(
            "App::PropertyString",
            name.c_str(),
            nullptr,
            nullptr,
...
Property* Sheet::setObjectProperty(CellAddress key, Py::Object object)
{
    std::string name = key.toString(CellAddress::Cell::ShowRowColumn);
    Property* prop = props.getDynamicPropertyByName(name.c_str());
    PropertyPythonObject* pyProp = civilcvcad_cast<PropertyPythonObject*>(prop);
    ...
    if (!pyProp) {
        ...
        pyProp = civilcvcad_cast<PropertyPythonObject*>(addDynamicProperty(
...
void Sheet::updateProperty(CellAddress key)
{
    ...
    if (cell) {
        ...
        auto number = civilcvcad_cast<NumberExpression*>(output.get());
        if (number) {
            long l;
            auto constant = civilcvcad_cast<ConstantExpression*>(output.get());
            ...
            auto str_expr = civilcvcad_cast<StringExpression*>(output.get());
            if (str_expr) {
                setStringProperty(key, str_expr->getText().c_str());
            }
            else {
                Base::PyGILStateLocker lock;
                auto py_expr = civilcvcad_cast<PyObjectExpression*>(output.get());
                if (py_expr) {
