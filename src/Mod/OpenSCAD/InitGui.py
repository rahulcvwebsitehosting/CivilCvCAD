...
class OpenSCADWorkbench(Workbench):
    ...
    def __init__(self):
        ...
        self.__class__.ToolTip = (
            ...
            "CivilCvCAD utizes OpenSCAD's capability as a script-only based modeller that uses its own description language\n"
    ...
    def Initialize(self):
        ...
        param = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD")
        ...
        if not openscadfilename:
            ...
            if openscadfilename:  # automatic search was successful
                CivilCvCAD.addImportType("OpenSCAD Format (*.scad *.SCAD)", "importCSG")
                param.SetString(
        ...
        if openscadfilename:
            ...
            CivilCvCAD.Console.PrintWarning("OpenSCAD executable not found\n")
        ...
        if openscadfilename and transferMechanism == 0:
            ...
            if "snap" in openscadfilename:
                CivilCvCAD.Console.PrintMessage(
                    ...
                    + translate(
                        "OpenSCAD",
                        "If OpenSCAD execution fails to load the temporary file, use CivilCvCAD's OpenSCAD Workbench Preferences to change the transfer mechanism.",
                    )
                    + "\n"
                )
                ...
                CivilCvCAD.Console.PrintMessage(
                    translate(
                        "OpenSCAD",
                        "It looks like you may be using a sandboxed version of CivilCvCAD.",
                    )
                    + " "
                    + translate(
                        "OpenSCAD",
        ...
        CivilCvCADGui.addPreferencePage(":/ui/openscadprefs-base.ui", "OpenSCAD")
...
CivilCvCAD.__unit_test__ += ["TestOpenSCADGui"]
