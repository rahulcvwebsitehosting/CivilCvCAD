...
__title__ = "CivilCvCAD OpenSCAD Workbench - Utility Functions"
...
try:
    BaseError = CivilCvCAD.Base.CivilCvCADError
...
def getopenscadexe(osfilename=None):
    if not osfilename:
        osfilename = CivilCvCAD.ParamGet(\
...
def callopenscad(inputfilename,outputfilename=None, outputext='csg', keepname=False):
    ...
    def check_output2(*args, **kwargs):
        ...
        if stderrd.strip():
            CivilCvCAD.Console.PrintWarning(stderrd + '\n')
        if stdoutd.strip():
            CivilCvCAD.Console.PrintMessage(stdoutd + '\n')
            return stdoutd

    preferences = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD")
    osfilename = getopenscadexe()
...
def call_openscad_with_pipes(input_filename, output_filename, output_extension, keep_name):
    ...
    # read or write access to the files, only the CivilCvCAD process does. In the future
...
def reverseimporttypes():
    ...
    importtypes = {}
    for key,value in CivilCvCAD.getImportType().items():
...
def fcsubmatrix(m):
    """Extracts the 3x3 Submatrix from a civilcvcad Matrix Object
...
def decomposerotoinversion(m, precision=4):
    ...
    cmat = CivilCvCAD.Matrix()
    if rmat == [[-1,0,0],[0,1,0],[0,0,1]]:
        cmat.scale(-1,1,1)
        return m*cmat,CivilCvCAD.Vector(1)
    elif rmat == [[1,0,0],[0,-1,0],[0,0,1]]:
        cmat.scale(1,-1,1)
        return m*cmat, CivilCvCAD.Vector(0,1)
    elif rmat == [[1,0,0],[0,1,0],[0,0,-1]]:
        cmat.scale(1,1,-1)
        return m*cmat, CivilCvCAD.Vector(0,0,1)
    else:
        cmat.scale(1,1,-1)
        return m*cmat, CivilCvCAD.Vector(0,0,1)
...
def mirror2mat(nv, bv):
    ...
    mbef = CivilCvCAD.Matrix()
    mbef.move(bv * -1)
    maft = CivilCvCAD.Matrix()
    maft.move(bv)
...
def vec2householder(nv):
    ...
    hh = CivilCvCAD.Matrix(nv.x*nv.x*l,nv.x*nv.y*l,nv.x*nv.z*l,0,\
                      nv.y*nv.x*l,nv.y*nv.y*l,nv.y*nv.z*l,0,\
                      nv.z*nv.x*l,nv.z*nv.y*l,nv.z*nv.z*l,0,0,0,0,0)
    return CivilCvCAD.Matrix()-hh
...
-2.0, -1.0)
    ...
    bestrot = CivilCvCAD.Rotation()
    dangle  = comparerotations(r1, bestrot)
    for axis in wkaxes():
        for angle in vangl:
            for axissign in (1.0,-1.0):
                r2=CivilCvCAD.Rotation(axis*axissign, angle)
...
def process2D_ObjectsViaOpenSCAD(ObjList, Operation, doc=None):
    doc = doc or CivilCvCAD.activeDocument()
...
def process_ObjectsViaOpenSCADShape(doc, children, name, maxmeshpoints=None):
    if all((not obj.Shape.isNull() and obj.Shape.Volume == 0) \
        ...
        CivilCvCAD.Console.PrintError( translate('OpenSCAD',\
