...
__title__ = "CivilCvCAD OpenSCAD Workbench - Parametric Features"
...
#        return {'ObjectName' : self.Object.Name}
    ...
    def loads(self,state):
        if state is not None:
            import CivilCvCAD
            doc = CivilCvCAD.ActiveDocument #crap
...
class Resize:
    def __init__(self,obj,target,vector):
        ...
        obj.addProperty("App::PropertyVector","Vector","Resize",
                        " Resize Vector", locked=True).Vector = CivilCvCAD.Vector(vector)
        obj.Proxy = self
...
class MatrixTransform:
    ...
    def execute(self, fp):
        ...
        #else:
            #CivilCvCAD.Console.PrintMessage('base %s\nmat %s/n' % (fp.Base,fp.Matrix))
    ...
    def execute(self, fp):
        if all((fp.Radius1,fp.Radius2,fp.FacesNumber,fp.Height)):
            ...
            for ir,r in enumerate((fp.Radius1,fp.Radius2)):
                ...
                pts = [CivilCvCAD.Vector(r.Value,0,ir*fp.Height.Value)]
                for i in range(fp.FacesNumber-1):
                    ang = (i+1)*angle
                    pts.append(CivilCvCAD.Vector(r.Value*math.cos(ang),\
...
class Twist:
    def __init__(self, obj, child=None, h=1.0, angle=0.0, scale=[1.0,1.0]):
        ...
        obj.Angle = CivilCvCAD.Units.Angle # assign the Angle unit
        ...
        if fp.Base and fp.Angle and fp.Segments and fp.Base.Shape.isValid():
            ...
            for start_face in fp.Base.Shape.Faces:
                ...
                try:
                    ...
                    CivilCvCAD.Console.PrintWarning("Could not create solid: creating compound instead")
                    solids.append(Part.Compound(faces))
            fp.Shape = Part.Compound(solids)
...
def makeSurfaceVolume(filename):
        ...
        for line in f1.readlines():
            ...
            if sline and not sline.startswith('#'):
                ...
                for xcoord, num in enumerate(sline.split()):
                    fnum = float(num)
                    lcoords.append(CivilCvCAD.Vector(float(xcoord),float(ycoord),fnum))
                    min_z = min(fnum,min_z)
                coords.append(lcoords)

    num_rows = len(coords)
    if num_rows == 0:
        CivilCvCAD.Console.PrintWarning(f"No data found in surface file {filename}")
        return None,0,0
    ...
    for row in range(num_rows - 1):
        for col in range(num_cols - 1):
            ...
            try:
                ...
                CivilCvCAD.Console.PrintWarning("Failed to create the face from {},{},{},{}".format(coords[row + 0][col + 0],\
    ...
    lines = []
    corner1 = CivilCvCAD.Vector(coords[0][0].x, coords[0][0].y, min_z - 1)
    lines.append(Part.makeLine(corner1,coords[0][0]))
    for col in range(num_cols - 1):
        a = coords[0][col]
        b = coords[0][col + 1]
        lines.append(Part.makeLine(a, b))
    corner2 = CivilCvCAD.Vector(coords[0][last_col].x, coords[0][last_col].y, min_z - 1)
    ...
    lines = []
    corner1 = CivilCvCAD.Vector(coords[last_row][0].x, coords[last_row][0].y, min_z - 1)
    lines.append(Part.makeLine(corner1,coords[last_row][0]))
    for col in range(num_cols - 1):
        a = coords[last_row][col]
        b = coords[last_row][col + 1]
        lines.append(Part.makeLine(a, b))
    corner2 = CivilCvCAD.Vector(coords[last_row][last_col].x, coords[last_row][last_col].y, min_z - 1)
    ...
    for row in range(num_rows - 1):
        ...
        b = coords[row + 1][0]
        lines.append(Part.makeLine(a, b))
    corner2 = CivilCvCAD.Vector(coords[last_row][0].x, coords[last_row][0].y, min_z - 1)
    ...
    lines = []
    corner1 = CivilCvCAD.Vector(coords[0][last_col].x, coords[0][last_col].y, min_z - 1)
    ...
    a = CivilCvCAD.Vector(coords[0][0].x, coords[0][0].y, min_z - 1)
    b = CivilCvCAD.Vector(coords[0][last_col].x, coords[0][last_col].y, min_z - 1)
    c = CivilCvCAD.Vector(coords[last_row][last_col].x, coords[last_row][last_col].y, min_z - 1)
    d = CivilCvCAD.Vector(coords[last_row][0].x, coords[last_row][0].y, min_z - 1)
    ab = Part.makeLine(a,b)
    bc = Part.makeLine(b,c)
    cd = Part.makeLine(c,d)
    da = Part.makeLine(d,a)
