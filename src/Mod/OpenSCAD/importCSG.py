...
#*   Acknowledgements:                                                     *
#*                                                                         *
#*     Thanks to shoogen on the CivilCvCAD forum and Peter Li                 *
#*     for programming advice and some code.                               *
#*                                                                         *
#*                                                                         *
#***************************************************************************
__title__ = "CivilCvCAD OpenSCAD Workbench - CSG importer"
...
params = CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD")
printverbose = params.GetBool('printVerbose', False)

if CivilCvCAD.GuiUp:
    gui = True
else:
    if printverbose: print("CivilCvCAD Gui not present.")
    gui = False
...
def fixVisibility():
    ...
    for root_object in CivilCvCAD.ActiveDocument.RootObjects:
...
def open(filename):
    "called when civilcvcad opens a file."
    global doc
    global pathName
...
def insert(filename, docname):
    "called when civilcvcad imports a file"
    global doc
    global pathName
...
def processcsg(filename):
    ...
    hassetcolor.clear()
    alreadyhidden.clear()
    CivilCvCAD.Console.PrintMessage('End processing CSG file\n')
    doc.recompute()
...
def placeholder(name, children, arguments):
    ...
    if gui:
        if CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").\
...
def p_resize_action(p):
    ...
    transform_matrix = CivilCvCAD.Matrix()

    scale = CivilCvCAD.Vector(float(new_size[0])/old_size[0],
...
def p_not_supported(p):
    ...
    not_supported : glide LPAREN keywordargument_list RPAREN OBRACE block_list EBRACE
                  ...
                  '''
    if gui and not CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").\
...
def fuse(lst,name):
    ...
    myfuse.Placement = CivilCvCAD.Placement()
    return myfuse
...
def process_rotate_extrude(obj, angle):
    ...
    myrev.Angle = angle
    myrev.Placement = CivilCvCAD.Placement(CivilCvCAD.Vector(),CivilCvCAD.Rotation(0,0,90))
    if gui:
...
def process_rotate_extrude_prism(obj, angle, n):
    ...
    newobj.Placement=CivilCvCAD.Placement(CivilCvCAD.Vector(),CivilCvCAD.Rotation(0,0,90))
    if gui:
        if CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").\
            GetBool('useViewProviderTree'):
            from OpenSCADFeatures import ViewProviderTree
...
def process_linear_extrude(obj,h) :
    ...
    mylinear.Dir = (0,0,h)
    mylinear.Placement=CivilCvCAD.Placement()
...
def process_linear_extrude_with_transform(base,height,twist,scale) :
    ...
    # base is a CivilCvCAD Object, height & twist are floats, scale is a two-component vector of floats
...
def processSVG(fname, ext):
    ...
    docSVG = CivilCvCAD.newDocument(fname+'_tmp')
    CivilCvCAD.ActiveDocument = docSVG
    ...
    CivilCvCAD.closeDocument(docSVG.Name)
...
def process_mesh_file(fname,ext):
    ...
    if mesh1 is not None:
        ...
        CivilCvCAD.Console.PrintError('Mesh not imported %s.%s %s\n' % \
...
def p_multmatrix_action(p):
    ...
    transform_matrix = CivilCvCAD.Matrix(*tuple(m1l))
...
#   If more than one object on the stack for multmatrix fuse first
    ...
    if (isspecialorthogonalpython(fcsubmatrix(transform_matrix))) :
        ...
        if matrixisrounded:
            ...
            plm=CivilCvCAD.Placement(transform_matrix)
            plm=CivilCvCAD.Placement(plm.Base,roundrotation(plm.Rotation))
            part.Placement=plm.multiply(part.Placement)
        else:
            part.Placement=CivilCvCAD.Placement(transform_matrix).multiply(\
                    part.Placement)
        new_part = part
        ...
        if matrixisrounded:
            if printverbose: print("rotation rounded")
            plm=CivilCvCAD.Placement(cmat)
            new_part.Placement=CivilCvCAD.Placement(plm.Base,roundrotation(plm.Rotation))
        else:
            new_part.Placement=CivilCvCAD.Placement(cmat)
        new_part.Label="mirrored %s" % part.Label
        if gui:
            part.ViewObject.hide()
    elif CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").\
...
def center(obj,x,y,z):
    obj.Placement = CivilCvCAD.Placement(\
        CivilCvCAD.Vector(-x/2.0,-y/2.0,-z/2.0),\
        CivilCvCAD.Rotation(0,0,0,1))
...
def myPolygon(n,r1):
    ...
    angle = math.pi*2/n
    nodes = [CivilCvCAD.Vector(r1,0,0)]
    for i in range(n-1) :
        th = (i+1) * angle
        nodes.append(CivilCvCAD.Vector(r1*math.cos(th),r1*math.sin(th),0))
...
def p_cylinder_action(p):
    ...
    if h > 0:
        if ( r1 == r2 and r1 > 0):
            ...
            if n < 3 or fnmax != 0 and n > fnmax:
                ...
                if False: #user Draft Polygon
                    ...
                    try :
                        ...
                        # Linux builds of CivilCvCAD), this is a fallback.
                        # or old level of CivilCvCAD
                        if printverbose:
            ...
            CivilCvCAD.Console.PrintWarning('cylinder with radius zero\n')
            mycyl=doc.addObject("Part::Feature","emptycyl")
            mycyl.Shape = Part.Compound([])
    else: # h == 0
        CivilCvCAD.Console.PrintWarning('cylinder with height <= zero\n')
        mycyl=doc.addObject("Part::Feature","emptycyl")
        mycyl.Shape = Part.Compound([])
...
def p_cube_action(p):
    ...
    if (l > 0 and w > 0 and h >0):
        ...
        CivilCvCAD.Console.PrintWarning('cube with radius zero\n')
...
def p_circle_action(p) :
    ...
    if n == 0 or fnmax != 0 and n >= fnmax:
        mycircle = CivilCvCAD.ActiveDocument.addObject("Part::Part2DObjectPython",'circle')
        ...
        mycircle = CivilCvCAD.ActiveDocument.addObject("Part::Part2DObjectPython",'polygon')
...
def p_text_action(p) :
    ...
    CivilCvCAD.Console.PrintMessage("textmsg : "+t+"\n")
...
def convert_points_list_to_vector(l):
    ...
    for i in l :
        ...
        v.append(CivilCvCAD.Vector(i[0],i[1]))
...
def p_polyhedron_action(p) :
    ...
    for i in p[6] :
        if printverbose: print(i)
        v.append(CivilCvCAD.Vector(float(i[0]),float(i[1]),float(i[2])))
    if printverbose:
        print(v)
...
def p_projection_action(p) :
    ...
    plane.Placement = CivilCvCAD.Placement(CivilCvCAD.Vector(\
                     bbox.XMin,bbox.YMin,0),CivilCvCAD.Rotation())
    if gui:
