...
class Node:
    ...
    def addtocivilcvcad(self,doc=None,fcpar=None):
        ...
        if namel in multifeature:
            if len(self.children)>1:
                ...
                subobjs = [child.addtocivilcvcad(doc,obj) for child in self.children]
                obj.Shapes = subobjs
                for subobj in subobjs:
                    subobj.ViewObject.hide()
            elif len(self.children) == 1:
                obj = self.children[0].addtocivilcvcad(doc,fcpar or True)
            else:
                obj = fcpar
            ...
            if len(self.children) == 1:
                ...
                base = self.children[0].addtocivilcvcad(doc,obj)

                if len(self.children) == 2:
                    tool = self.children[1].addtocivilcvcad(doc,obj)
                else:
                    tool = Node(name='imp_union',\
                        children=self.children[1:]).addtocivilcvcad(doc,obj)
                obj.Base = base
                obj.Tool = tool
            ...
            and self.arguments['$fn']<=Node.fnmin: # polygonal
                if r1 == r2: # prismatic
                    ...
                    if self.arguments['center']:
                        p1.Placement = CivilCvCAD.Placement(\
                        CivilCvCAD.Vector(0.0,0.0,-h/2.0),CivilCvCAD.Rotation())
                        p2.Placement = CivilCvCAD.Placement(\
                        CivilCvCAD.Vector(0.0,0.0,h/2.0), CivilCvCAD.Rotation())
                    else:
                        p2.Placement = CivilCvCAD.Placement(\
                        CivilCvCAD.Vector(0.0,0.0,h),CivilCvCAD.Rotation())
...
#                solid.complement()
            ...
            if len(self.children) == 1:
                ...
                obj = Node(name='imp_union',\
                        children=self.children).addtocivilcvcad(doc,fcpar or True)
            ...
            m1 = CivilCvCAD.Matrix(*tuple(m1l)) #That's the original matrix
            if isspecialorthogonalpython(fcsubmatrix(m1)): #a Placement can represent the transformation
                if len(self.children) == 1:
                    ...
                    obj = Node(name='imp_union',\
                            children = self.children).addtocivilcvcad(doc,fcpar or True)
                    #CivilCvCAD.Console.PrintMessage('obj %s\nmat %s/n' % (obj.Placement,m1))
                obj.Placement=CivilCvCAD.Placement(m1).multiply(obj.Placement)
                ...
                if len(self.children) == 1:
                    child = self.children[0].addtocivilcvcad(doc,obj)
                else:
                    child = Node(name='imp_union',\
                            children=self.children).addtocivilcvcad(doc,obj)
            ...
            if len(self.children)==0:
                base = Node('import',self.arguments).addtocivilcvcad(doc,obj)
            elif len(self.children)==1:
                base = self.children[0].addtocivilcvcad(doc,obj)
            else:
            ...
            obj.Angle = 360.00
            base.ViewObject.hide()
            obj.Placement=CivilCvCAD.Placement(CivilCvCAD.Vector(),CivilCvCAD.Rotation(0,0,90))
            ...
            if self.arguments['cut']:
                ...
                if not plane:
                    ...
                    -Node.planedim,-Node.planedim,0),CivilCvCAD.Rotation(0,0,0,1))
            ...
            if filename:
                ...
                if extension.lower() in reverseimporttypes()['Mesh']:
                    ...
                    try:
                        ...
                        CivilCvCAD.Console.PrintError('processing of dxf import failed\nRework \'%s\' manually\n' % layera)
                    ...
                    CivilCvCAD.Console.ErrorMessage('Filetype of %s not supported\n' % (filename))
                    raise(NotImplementedError)
                if obj: #handle origin and scale
                    if scale is not None and scale !=1:
                        ...
                        placement = CivilCvCAD.Placement(CivilCvCAD.Vector(*[-c for c in origin]),CivilCvCAD.Rotation())
                        obj.Placement = placement.multiply(obj.Placement)
                else:
                    CivilCvCAD.Console.ErrorMessage('Import of %s failed\n' % (filename))
            ...
            if len(self.children) == 2 and \
                ...
                if len(nonsphere) == 1:
                    child = nonsphere[0].addtocivilcvcad(doc,obj)
                else:
                    child = Node(name='imp_union',\
                        children=nonsphere).addtocivilcvcad(doc,obj)
                OffsetShape(obj,child,offset)
                ViewProviderTree(obj.ViewObject)
            elif False:
                raise(NotImplementedError)
                pass # handle rotated cylinders and select edges that
                     #radius = radius0 * m1.multiply(CivilCvCAD.Vector(0,0,1)).dot(edge.Curve.tangent(0)[0])
            else:
                raise(NotImplementedError)
        elif namel == 'surface':
            ...
            if lenchld == 1:
                CivilCvCAD.Console.PrintMessage('Not recognized %s\n' % (self))
                obj = self.children[0].addtocivilcvcad(doc,fcpar)
            elif lenchld >1:
                obj = Node(name='imp_union',\
                        children=self.children).addtocivilcvcad(doc,fcpar or True)
            else:
...
def parseexpression(e):
    ...
    if el == 'true': return True
        ...
        try:
            ...
            import CivilCvCAD
            CivilCvCAD.Console.PrintMessage('%s\n' % (el))
            return 1.0
...
def open(filename):
    ...
    readfile(filename).addtocivilcvcad(doc)
    #doc.recompute()
    return doc
