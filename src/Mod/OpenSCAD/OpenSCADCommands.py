...
__title__ = "CivilCvCAD OpenSCAD Workbench - GUI Commands"
...
class ExplodeGroup:
    "Ungroup Objects"
    def IsActive(self):
        return CivilCvCADGui.Selection.countObjectsOfType('Part::Feature') > 0
    ...
    def Activated(self):
        def isdefault(shapecolor):
            ...
            scol=CivilCvCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned('DefaultShapeColor',0xccccccff)
        ...
        def explode(obj,color=True):
            if obj.isDerivedFrom('Part::Fuse') or \
                ...
                if plm.isNull() or all((len(oo.InList)==1 and \
                    ...
                    CivilCvCAD.Console.PrintError(translate('OpenSCAD', 'Unable to explode %s') % obj.Name + '\n')
...
class ColorCodeShape:
    ...
    def Activated(self):
        ...
        selection=CivilCvCADGui.Selection.getSelectionEx()
        if len(selection) > 0:
...
class Edgestofaces:
    ...
    def Activated(self):
        ...
        Overlappingfaces(edgestofaces(edges,None)).makefeatures(CivilCvCAD.ActiveDocument)
        for selobj in selection:
...
class MirrorMeshFeature:
    def IsActive(self):
        return CivilCvCADGui.Selection.countObjectsOfType('Mesh::Feature') > 0
    ...
    def Activated(self):
        ...
        for selobj in selection:
            ...
            if ok:
                ...
                y = float(splits[1])
                z = float(splits[2])
                vec = CivilCvCAD.Base.Vector(x,y,z)
...
class ExpandPlacements:
    ...
    def Activated(self):
        ...
        for selobj in CivilCvCADGui.Selection.getSelectionEx():
            expandplacements.expandplacements(selobj.Object,CivilCvCAD.Placement())
        CivilCvCAD.ActiveDocument.recompute()
    def GetResources(self):
...
class ReplaceObject:
    def IsActive(self):
        nobj = CivilCvCADGui.Selection.countObjectsOfType('Part::Feature')
        if nobj == 3: return True
        elif nobj == 2: return tuple((len(obj.InList)) for obj in \
                CivilCvCADGui.Selection.getSelection()) in ((0,1),(1,0))
        #else: return False
    ...
    def Activated(self):
        ...
        if len(objs)==3 or \
            ...
            CivilCvCAD.Console.PrintError(translate('OpenSCAD', 'Select 3 objects first')+ '\n')
    def GetResources(self):
...
class RemoveSubtree:
    ...
    def Activated(self):
        import OpenSCADUtils
        import CivilCvCADGui
        OpenSCADUtils.removesubtree(CivilCvCADGui.Selection.getSelection())
...
class AddSCADTask:
    ...
    def addelement(self):
        ...
        try:
            ...
            doc=CivilCvCAD.activeDocument() or CivilCvCAD.newDocument()
            if asmesh:
                import Mesh
            ...
            CivilCvCAD.Console.PrintError(e.value)
...
class OpenSCADMeshBooleanTask:
    ...
    def doboolean(self):
        ...
        newmesh,objsused = meshoponobjs(opname,CivilCvCADGui.Selection.getSelection())
        if len(objsused) > 0:
            newmeshobj = CivilCvCAD.activeDocument().addObject('Mesh::Feature',opname) #create a Feature for the result
...
class AddOpenSCADElement:
    def IsActive(self):
        return not CivilCvCADGui.Control.activeDialog()
...
class OpenSCADMeshBoolean:
    def IsActive(self):
        return not CivilCvCADGui.Control.activeDialog() and \
...
class Hull:
    def IsActive(self):
        return len(CivilCvCADGui.Selection.getSelection()) >= 1
    ...
    def Activated(self):
        ...
        for selobj in selection:
            ...
            selobj.Object.ViewObject.hide()
        importCSG.process_ObjectsViaOpenSCAD(CivilCvCAD.activeDocument(),objList,"hull")
        CivilCvCAD.ActiveDocument.recompute()
    def GetResources(self):
...
class Minkowski:
    ...
    def Activated(self):
        ...
        for selobj in selection:
            objList.append(selobj.Object)
            selobj.Object.ViewObject.hide()
        importCSG.process_ObjectsViaOpenSCAD(CivilCvCAD.activeDocument(),objList,"minkowski")
        CivilCvCAD.ActiveDocument.recompute()
    def GetResources(self):
...
CivilCvCADGui.addCommand('OpenSCAD_ColorCodeShape',ColorCodeShape())
CivilCvCADGui.addCommand('OpenSCAD_ExplodeGroup',ExplodeGroup())
CivilCvCADGui.addCommand('OpenSCAD_Edgestofaces',Edgestofaces())
CivilCvCADGui.addCommand('OpenSCAD_RefineShapeFeature',RefineShapeFeature())
CivilCvCADGui.addCommand('OpenSCAD_MirrorMeshFeature',MirrorMeshFeature())
CivilCvCADGui.addCommand('OpenSCAD_ScaleMeshFeature',ScaleMeshFeature())
CivilCvCADGui.addCommand('OpenSCAD_ResizeMeshFeature',ResizeMeshFeature())
CivilCvCADGui.addCommand('OpenSCAD_IncreaseToleranceFeature',IncreaseToleranceFeature())
CivilCvCADGui.addCommand('OpenSCAD_ExpandPlacements',ExpandPlacements())
CivilCvCADGui.addCommand('OpenSCAD_ReplaceObject',ReplaceObject())
CivilCvCADGui.addCommand('OpenSCAD_RemoveSubtree',RemoveSubtree())
CivilCvCADGui.addCommand('OpenSCAD_AddOpenSCADElement',AddOpenSCADElement())
CivilCvCADGui.addCommand('OpenSCAD_MeshBoolean',OpenSCADMeshBoolean())
CivilCvCADGui.addCommand('OpenSCAD_Hull',Hull())
CivilCvCADGui.addCommand('OpenSCAD_Minkowski',Minkowski())
