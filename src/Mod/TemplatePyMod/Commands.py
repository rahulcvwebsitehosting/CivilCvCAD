...
import CivilCvCAD, CivilCvCADGui,inspect
...
def addCommand(name,cmdObject):
	...
	for i in range(len(list)-1):
		source += list[i+1][pos:]
	CivilCvCADGui.addCommand(name,cmdObject,source)
...
class TemplatePyMod_Cmd2:
    ...
    def Activated(self):
        ...
        v = CivilCvCADGui.ActiveDocument.ActiveView
...
class TemplatePyMod_Cmd3:
    ...
    def Activated(self):
        ...
        CivilCvCADGui.activateWorkbench("PythonQtWorkbench")
...
class SphereCreator:
	def __init__(self):
		...
		self.pt = Part
		self.mode = False
		CivilCvCAD.Console.PrintMessage("Create instance of SphereCreator\n")

	def __del__(self):
		CivilCvCAD.Console.PrintMessage("Delete instance of SphereCreator\n")

	def enter(self):
		if (self.mode):
			return
		CivilCvCAD.Console.PrintMessage("Enter sphere creation mode\n")
		self.av = CivilCvCADGui.ActiveDocument.ActiveView
	...
	def leave(self):
		if (not self.mode):
			return
		CivilCvCAD.Console.PrintMessage("Leave sphere creation mode\n")
	...
	def create(self, info):
		...
		if (down):
			...
			CivilCvCAD.Console.PrintMessage("Clicked on position: ("+str(pos[0])+", "+str(pos[0])+")")
			msg = " -> (%f,%f,%f)\n" % (pnt.x, pnt.y, pnt.z)
...
class TemplatePyMod_Cmd4:
	...
	def __del__(self):
		CivilCvCAD.Console.PrintError('TemplatePyMod_Cmd4 was destroyed\n')

	def Activated(self):
		if CivilCvCADGui.ActiveDocument is not None:
			self.sc.enter()
		else:
			CivilCvCAD.Console.PrintWarning('A 3d view must be created\n')
...
class TemplatePyMod_Cmd5:
	...
	def Activated(self):
		...
		if myRenderArea is None:
			...
			myRenderArea = sogui.SoGuiRenderArea(CivilCvCADGui.getMainWindow())
...
class TemplatePyGrp_1:
    def Activated(self):
        import CivilCvCAD
        CivilCvCAD.Console.PrintMessage("TemplatePyGrp_1\n")
...
class TemplatePyGrp_2:
    def Activated(self):
        import CivilCvCAD
        CivilCvCAD.Console.PrintMessage("TemplatePyGrp_2\n")
...
class TemplatePyGrp_3:
    def Activated(self):
        import CivilCvCAD
        CivilCvCAD.Console.PrintMessage("TemplatePyGrp_3\n")
...
CivilCvCADGui.addCommand('TemplatePyMod_Cmd4',TemplatePyMod_Cmd4())
CivilCvCADGui.addCommand('TemplatePyMod_Cmd5',TemplatePyMod_Cmd5())
CivilCvCADGui.addCommand('TemplatePyMod_Cmd6',TemplatePyMod_Cmd6())
CivilCvCADGui.addCommand('TemplatePyGrp_1',TemplatePyGrp_1())
CivilCvCADGui.addCommand('TemplatePyGrp_2',TemplatePyGrp_2())
CivilCvCADGui.addCommand('TemplatePyGrp_3',TemplatePyGrp_3())
CivilCvCADGui.addCommand('TemplatePyGroup',TemplatePyGroup())
CivilCvCADGui.addCommand('TemplatePyCheckable',TemplatePyCheckable())
