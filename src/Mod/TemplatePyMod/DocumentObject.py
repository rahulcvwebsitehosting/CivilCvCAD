...
# CivilCvCAD module providing base classes for document objects and view provider
# (c) 2011 Werner Mayer LGPL

import CivilCvCAD

class DocumentObject(object):
    """The Document object is the base class for all CivilCvCAD objects."""
            ...
            for key in d:
                ...
                if hasattr(item, '__call__') and key!="onChanged" and key!="execute" and key!="init" and key[0]!="_":
                    ...
                    if not(hasattr(self.__object__,key)):
                        ...
                        CivilCvCAD.Console.PrintWarning('!!! The function : "'+key+'" already exist in the object, cannot override. !!!\n')
    ...
    def Content(self):
        ...
        This is the content that is saved when the file is saved by CivilCvCAD"""
    ...
    @property
    def Name(self):
        "the name ofthis object, unique in the CivilCvCAD document"
    ...
    def ViewObject(self,value):
        ...
        None if CivilCvCAD is running in console mode"""
...
class ViewProvider(object):
    ...
    the GUI space. It is only present when CivilCvCAD runs in GUI mode.
    It contains all that is needed to represent the DocumentObject in
    the 3D view and the CivilCvCAD interface"""
    def __init__(self):
...
class Box(DocumentObject):
    ...
    #-----------------------------BEHAVIOR------------------------------------
    def propertyChanged(self,prop):
        CivilCvCAD.Console.PrintMessage("Box property changed : "+ prop+ "\n")
    ...
    def _privateFunctionExample(self,attr):
        CivilCvCAD.Console.PrintMessage("The length : "+str(attr)+"\n")
...
def makeBox():
    ...
    box = CivilCvCAD.ActiveDocument.addObject(Box.type,"MyBox",Box(),None)
