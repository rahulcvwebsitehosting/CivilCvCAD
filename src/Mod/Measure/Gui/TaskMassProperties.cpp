...
void TaskMassProperties::tryUpdate()
{
    ...
    auto coordLabel = [](App::DocumentObject* obj) {
        if (auto* datum = civilcvcad_cast<App::DatumElement*>(obj)) {
            if (auto* lcs = datum->getLCS()) {
                return lcs->getFullLabel();
            }
        }
        if (auto* lcs = civilcvcad_cast<App::LocalCoordinateSystem*>(obj)) {
            return lcs->getFullLabel();
        }
        if (auto* origin = civilcvcad_cast<App::Origin*>(obj)) {
    ...
    auto isReferenceObject = [](App::DocumentObject* obj) {
        ...
        auto datum = civilcvcad_cast<App::DatumElement*>(obj);
        if (datum && datum->getLCS()) {
            return true;
        }
        if (civilcvcad_cast<App::LocalCoordinateSystem*>(obj)) {
            return true;
        }
        if (civilcvcad_cast<App::Origin*>(obj)) {
            return true;
        }
    ...
    auto getPlacementFromObject = [](App::DocumentObject* obj) {
        ...
        if (auto* prop = civilcvcad_cast<App::PropertyPlacement*>(obj->getPropertyByName("Placement"))) {
    ...
    auto collectBodies =
        ...
        if (resolved->getTypeId().getName() == std::string("PartDesign::Body")) {
            auto* tipProp = civilcvcad_cast<App::PropertyLink*>(resolved->getPropertyByName("Tip"));
            if (tipProp) {
    ...
    if (currentMode == MassPropertiesMode::Custom && referenceDatum) {
        ...
        if (!referenceDatum->isDerivedFrom<App::Line>()) {
            ...
            else if (auto datum = civilcvcad_cast<const App::DatumElement*>(referenceDatum)) {
                if (datum->getLCS()) {
                    applyOriginOffset(datum->getBasePoint());
                }
            }
            else if (auto lcs = civilcvcad_cast<const App::LocalCoordinateSystem*>(referenceDatum)) {
                applyOriginOffset(lcs->Placement.getValue().getPosition());
            }
            else if (auto origin = civilcvcad_cast<const App::Origin*>(referenceDatum)) {
...
)
{
    ...
    try {
        ...
        App::PropertyPlacement* prop = civilcvcad_cast<App::PropertyPlacement*>(baseProp);
        Base::Placement plm;
...
void TaskMassProperties::createLCS(std::string name, bool removeExisting)
{
    ...
    try {
        ...
        if (auto* lcsObj = civilcvcad_cast<App::LocalCoordinateSystem*>(LCS)) {
...
void TaskMassProperties::saveResult()
{
    ...
    auto group = civilcvcad_cast<App::DocumentObjectGroup*>(doc->getObject("Measurements"));
    ...
    auto setQuantity = [&](const char* name, const char* group, const Base::Quantity& quantity) {
        ...
        auto* prop = civilcvcad_cast<App::PropertyString*>(
    ...
    auto setVector = [&](const char* name, const char* group, Base::Vector3d& value) {
        ...
        auto* prop = civilcvcad_cast<App::PropertyVector*>(
