...
)
{
    ...
    for (const auto& object : objects) {
        ...
        if (shape.IsNull()) {
            auto* part = civilcvcad_cast<Part::Feature*>(obj);
            if (!part) {
                continue;
            }
        ...
        auto materialFeature = [](App::DocumentObject* candidate) -> Part::Feature* {
            ...
            while (candidate && visited.insert(candidate).second) {
                if (auto* feature = civilcvcad_cast<Part::Feature*>(candidate)) {
                    return feature;
                }
    ...
    if (mode == MassPropertiesMode::CenterOfGravity) {
        ...
        if (auto referenceDatumElement = civilcvcad_cast<App::DatumElement const*>(referenceDatum)) {
            lcs = referenceDatumElement->getLCS();
        }


        if (auto const* line = civilcvcad_cast<App::Line const*>(referenceDatum)) {
        ...
        if (!lcs) {
            lcs = civilcvcad_cast<const App::LocalCoordinateSystem*>(referenceDatum);
            if (!lcs) {
                return data;
            }
        }
