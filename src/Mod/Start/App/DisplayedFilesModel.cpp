...
static bool civilcvcadCanOpen(const QString& extension)
{
...
void DisplayedFilesModel::addFile(const QString& filePath)
{
    ...
    if (!civilcvcadCanOpen(qfi.suffix())) {
        return;
    }
    ...
    if (ignoredExtensions.contains(lowercaseExtension)) {
        // Don't try to generate a thumbnail for things like this: CivilCvCAD can read them, but
