...
void ThemeSelectorWidget::setupButtons(QBoxLayout* layout)
{
    ...
    std::map<Theme, QString> themeMap {
        {Theme::Classic, tr("CivilCvCAD Classic")},
        {Theme::Dark, tr("CivilCvCAD Dark")},
        {Theme::Light, tr("CivilCvCAD Light")}
    };
    ...
    for (const auto& theme : themeMap) {
        ...
        if (theme.first == Theme::Classic && styleSheetName.isEmpty()) {
            ...
            && styleSheetName.contains(QLatin1String("CivilCvCAD Light"), Qt::CaseSensitivity::CaseInsensitive)
        ) {
            button->setChecked(true);
        }
        else if (
            theme.first == Theme::Dark
            && styleSheetName.contains(QLatin1String("CivilCvCAD Dark"), Qt::CaseSensitivity::CaseInsensitive)
        ) {
            button->setChecked(true);
        }
...
{
    auto const addonManagerLink = QStringLiteral("civilcvcad:Std_AddonMgr");
...
void ThemeSelectorWidget::themeChanged(Theme newTheme)
{
    ...
    switch (newTheme) {
        case Theme::Classic:
            prefPackManager->apply("CivilCvCAD Classic");
            break;
        case Theme::Dark:
            prefPackManager->apply("CivilCvCAD Dark");
            break;
        case Theme::Light:
            prefPackManager->apply("CivilCvCAD Light");
            break;
    }
    ...
    if (Gui::Application::Instance->commandManager().getCommandByName("Std_AddonMgr")) {
        _descriptionLabel->setText(
            tr("Looking for more themes? You can obtain them using "
               "<a href=\"civilcvcad:Std_AddonMgr\">Addon Manager</a>.")
        );
    }
    else {
        _descriptionLabel->hide();
    }
    _buttons[static_cast<int>(Theme::Dark)]->setText(tr("CivilCvCAD Dark", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Light)]->setText(tr("CivilCvCAD Light", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Classic)]->setText(tr("CivilCvCAD Classic", "Visual theme name"));
}
