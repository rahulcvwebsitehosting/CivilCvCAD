...
eliminate CivilCvCAD's dependency on QtWebEngine by redesigning Start to eliminate the HTML component.

The long-term plan for this workbench is to migrate from Qt Widgets to QtQuick/QML with a C++ backend providing access
to things like CivilCvCAD's Recent Files list. This switch will happen sometime after we no longer have to support building
...
into a new CivilCvCAD `MDIView`. Inside the scroll area are three regions:
...
CivilCvCAD's preferences system, where it gets the list of recent files. That class is in`App/RecentFilesModel.*`. It is
...
case the model is `ExamplesModel` in `App/ExamplesModel.*`. It fetches a read-only list of files from the CivilCvCAD
...
display via the normal QSS mechanisms in Qt. There are three CivilCvCAD Parameters that control the spacing between the
...
panel. The parameters are intended to be used by Preference Pack authors to assist in customization of CivilCvCAD themes.
