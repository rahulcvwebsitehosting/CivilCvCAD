# Creating a Windows installer for CivilCvCAD

These are instructions for building an NSIS-based installer for CivilCvCAD. They were designed for CivilCvCAD 0.21 and later,
and presume that you have cloned a copy of CivilCvCAD's source code, and therefore have the directory *package/WindowsInstaller*.
...
```
4. Ensure the CivilCvCAD files are in place. Here you have two options:
   * If you are working from an already-compiled version of CivilCvCAD provided to you by an outside source: in this case, simply ensure that `FILES_CIVILCVCAD` is set to the directory containing those files.
   * If you compiled CivilCvCAD on your own as described [here](https://wiki.civilcvcad.org/Compile_on_Windows) (and using the Install option outlined there). Then:
...
5. Right-click on the file *CivilCvCAD-installer.nsi* and choose **Compile NSIS script**
   to compile the installer. You can also run from command line to specify some settings
```cmd
%your_nsis_path%\makensis.exe /D'FILES_CIVILCVCAD="D:\some\path\CivilCvCAD"' /D'FILES_DEPS="${__FILEDIR__}\MSVC_Redist" /D'ExeFile="my-CivilCvCAD-installer.exe"' CivilCvCAD-installer.nsi
```
...
```cmd
%your_nsis_path%\makensis.exe [OPTIONS] /DFC_TEST_BUILD CivilCvCAD-installer.nsi
```
