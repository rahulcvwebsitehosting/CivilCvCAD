...
Function InitUser
  ...
  # Get CivilCvCAD language

  ReadRegStr $FCLangName SHELL_CONTEXT "${APP_REGKEY_SETUP}" "CivilCvCAD Language"
...
Function PostMultiUserPageInit
  ...
  ${endif}
  # check if this CivilCvCAD version is already installed
  ReadRegStr $0 SHCTX "${APP_UNINST_KEY}" "UninstallString"
  ${if} $0 != ""
   # check if the uninstaller was accidentally deleted
   # if so, don't bother the user if they really want to install a new CivilCvCAD over an existing one
   ...
   # installing over an existing installation of the same CivilCvCAD release is not necessary
   # if the users does this, they most probably have a problem with CivilCvCAD that can better be solved
   # by reinstalling CivilCvCAD
  ...
  # check if there is an existing CivilCvCAD installation of the same CivilCvCAD series
...
Function .onInit
  ...
  # Check that CivilCvCAD is not currently running
  Push $R0
  Push $R1
  ${FindProc} $R0 ${BIN_CIVILCVCAD}
  ${FindProc} $R1 ${BIN_CIVILCVCADCMD}
...
Function un.onInit
  ...
  # Macro to investigate name of CivilCvCAD's preferences folders to be able remove them
  ...
  # abort when CivilCvCAD was installed with admin permissions but the user doesn't have administrator privileges
  ...
  ${andif} $MultiUser.Privileges != "Power"
   ...
   Abort
  ${endif}
  # warning when CivilCvCAD couldn't be found in the registry
  ${if} $0 == "" # check in HKCU
  ...
  # question message if the user really wants to uninstall CivilCvCAD
