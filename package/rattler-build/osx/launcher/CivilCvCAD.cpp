...
int main(int argc, char *argv[], char *const *envp) {
    ...
    std::string CivilCvCAD = realpath((std::string(cwd) + "/../Resources/bin/civilcvcad").c_str(), NULL);
    ...
    env["SSL_CERT_FILE"]        = prefix + "/ssl/cacert.pem";    // https://forum.civilcvcad.org/viewtopic.php?f=3&t=42825
    ...
    std::cout << "Running: " << CivilCvCAD << std::endl;
    i = 0;
    while(argv[i] != NULL) {
        i++;
    }
    ...
    new_argv[0] = new char[CivilCvCAD.length() + 1];
    memset(new_argv[0], 0, CivilCvCAD.length() + 1);
    strncpy(new_argv[0], CivilCvCAD.c_str(), CivilCvCAD.length());
    ...
    return execve(CivilCvCAD.c_str(), new_argv, new_env);
}
