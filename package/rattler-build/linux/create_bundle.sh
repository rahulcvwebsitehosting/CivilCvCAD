...
cp ${conda_env}/share/applications/org.civilcvcad.CivilCvCAD.desktop AppDir/
sed -i 's/Exec=CivilCvCAD/Exec=AppRun/g' AppDir/org.civilcvcad.CivilCvCAD.desktop
cp ${conda_env}/share/icons/hicolor/scalable/apps/org.civilcvcad.CivilCvCAD.svg AppDir/
...
version_name="CivilCvCAD_${BUILD_TAG}-Linux-$(uname -m)"
...
if ! "${conda_env}/bin/civilcvcadcmd" --safe-mode --version; then
    echo "CivilCvCAD command-line smoke test failed; the Linux bundle cannot start."
    exit 1
fi
...
if ! "${conda_env}/bin/civilcvcadcmd" --safe-mode --console "import pivy; from pivy import coin; print(pivy.__file__); print(coin.SoDB.getVersion())"; then
    echo "CivilCvCAD bundled Pivy smoke test failed; the Linux bundle cannot import the bundled Coin/Pivy runtime."
    exit 1
fi
...
./appimagetool-$(uname -m).AppImage \
  ...
  --mksquashfs-opt 22 \
  -u "gh-releases-zsync|CivilCvCAD|CivilCvCAD|${GH_UPDATE_TAG}|CivilCvCAD*$(uname -m)*.AppImage.zsync" \
...
if [ "${UPLOAD_RELEASE}" == "true" ]; then
    ...
    if [ "${GH_UPDATE_TAG}" == "weeklies" ]; then
        generic_name="CivilCvCAD_weekly-Linux-$(uname -m)"
