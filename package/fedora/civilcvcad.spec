...
# plugins and private shared libs in %%{_libdir}/civilcvcad/lib are private;
# prevent private capabilities being advertised in Provides/Requires
%global plugin_exclude %( for i in %{plugins}; do  echo -n "\|$i\(Gui\)\?"; done )
# prevent declaring Requires for internal CivilCvCAD libraries
...
%global __provides_exclude ^(libCivilCvCAD.*%{plugin_exclude})\.so.*
%global __requires_exclude ^(libCivilCvCAD.*%{plugin_exclude}%{lib_exclude})\.so.*



%description
CivilCvCAD is a general purpose Open Source 3D CAD/MCAD/CAx/CAE/PLM modeler, aimed
...
%package data
Summary:        Data files for CivilCvCAD
BuildArch:      noarch
Requires:       %{name} = %{epoch}:%{version}-%{release}

%description data
Data files for CivilCvCAD
...
#path that contain main CivilCvCAD sources for cmake
...
%prep
    %setup -T -a 0 -q -c -n CivilCvCAD-1.0.2
...
#         -DCMAKE_INSTALL_DATAROOTDIR=%{_datadir} \
    ...
    %cmake \
        ...
        -DCIVILCVCAD_USE_EXTERNAL_PIVY=TRUE \
        -DCIVILCVCAD_USE_PCL:BOOL=OFF \
        -DCIVILCVCAD_QT_VERSION:STRING=6 \
        -DOpenGL_GL_PREFERENCE=GLVND \
        -DUSE_OCC=TRUE \
    %if %{without bundled_pycxx}
        -DCIVILCVCAD_USE_EXTERNAL_PYCXX=TRUE \
    %endif
    %if %{without bundled_smesh}
        -DCIVILCVCAD_USE_EXTERNAL_SMESH=TRUE \
    %endif
    %if %{without bundled_zipios}
        -DCIVILCVCAD_USE_EXTERNAL_ZIPIOS=TRUE \
    %endif
    %if %{with tests}
...
%install
    ...
    ln -s ../%{_lib}/%{name}/bin/CivilCvCAD %{buildroot}%{_bindir}/CivilCvCAD
    ln -s ../%{_lib}/%{name}/bin/CivilCvCADCmd %{buildroot}%{_bindir}/CivilCvCADCmd
    ...
    mv %{buildroot}%{_libdir}/civilcvcad/share/applications %{buildroot}%{_datadir}/
    mkdir -p %{buildroot}%{_metainfodir}
    mv %{buildroot}%{_libdir}/civilcvcad/share/metainfo/* %{buildroot}%{_metainfodir}/
    mkdir -p %{buildroot}%{_datadir}
    mv %{buildroot}%{_libdir}/civilcvcad/share/icons %{buildroot}%{_datadir}/
    mv %{buildroot}%{_libdir}/civilcvcad/share/pixmaps %{buildroot}%{_datadir}/
    mv %{buildroot}%{_libdir}/civilcvcad/share/mime %{buildroot}%{_datadir}/
    mv %{buildroot}%{_libdir}/civilcvcad/share/thumbnailers %{buildroot}%{_datadir}/
    mv %{buildroot}%{_libdir}/civilcvcad/share/pkgconfig %{buildroot}%{_datadir}/
...
%endif
    ...
    desktop-file-validate %{buildroot}%{_datadir}/applications/org.civilcvcad.CivilCvCAD.desktop
    ...
    %define plugin_regexp /^\\\(libCivilCvCAD.*%(for i in %{plugins};       do echo -n "\\\|$i"; done)\\\)\\\(\\\|Gui\\\)\\.so/d
