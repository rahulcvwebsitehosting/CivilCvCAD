...
# Default values
SIGNING_KEY_ID="${CIVILCVCAD_SIGNING_KEY_ID}"
KEYCHAIN_PROFILE="CivilCvCAD"
CONTAINING_FOLDER="."
APP_NAME="CivilCvCAD.app"
VOLUME_NAME="CivilCvCAD"
DMG_NAME="CivilCvCAD-macOS-$(uname -m).dmg"
...
function usage {
    ...
    echo
    echo "This script signs and notarizes a CivilCvCAD.app bundle. It expects that the bundle is in a folder"
    ...
    echo "this script."
    echo
    echo "If <signing_key_id> is not passed it defaults to env variable CIVILCVCAD_SIGNING_KEY_ID, it should"
...
if [ -d "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns" ]; then
    ...
    if [ -f "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADThumbnailExtension.appex/Contents/MacOS/CivilCvCADThumbnailExtension" ]; then
        run_codesign "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADThumbnailExtension.appex/Contents/MacOS/CivilCvCADThumbnailExtension"
    fi
    if [ -f "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADPreviewExtension.appex/Contents/MacOS/CivilCvCADPreviewExtension" ]; then
        run_codesign "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADPreviewExtension.appex/Contents/MacOS/CivilCvCADPreviewExtension"
    fi

    # Then sign the .appex bundles themselves with extension-specific entitlements
    if [ -d "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADThumbnailExtension.appex" ] && [ -f "$THUMBNAIL_ENTITLEMENTS" ]; then
        run_codesign_extension "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADThumbnailExtension.appex" "$THUMBNAIL_ENTITLEMENTS"
    fi
    if [ -d "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADPreviewExtension.appex" ] && [ -f "$PREVIEW_ENTITLEMENTS" ]; then
        run_codesign_extension "${CONTAINING_FOLDER}/${APP_NAME}/Contents/PlugIns/CivilCvCADPreviewExtension.appex" "$PREVIEW_ENTITLEMENTS"
    fi
fi
