...
This script runs the existing CivilCvCAD unittest `TestCoinNodeSnapshots` via CivilCvCADCmd,
configuring it through environment variables (so we don't depend on CivilCvCAD forwarding
CLI args to Python).
...
Examples:
  ...
  tools/rendering/manage_coin_node_baselines.py update \
    --civilcvcadcmd build/<preset>/bin/CivilCvCADCmd
  ...
  tools/rendering/manage_coin_node_baselines.py compare \
    --out-dir /tmp/CivilCvCADTesting/CoinNodeSnapshots \
    --civilcvcadcmd build/<preset>/bin/CivilCvCADCmd
"""
...
def _default_civilcvcadcmd() -> str | None:
    # Allow explicit override from the environment.
    for env_var in ("CIVILCVCADCMD", "FC_CIVILCVCADCMD", "CIVILCVCAD_CMD"):
    ...
    candidates: list[Path] = [
        Path("build/bin/CivilCvCADCmd"),
        *sorted(Path("build").glob("*/bin/CivilCvCADCmd")),
        *sorted(Path("build").glob("*/bin/CivilCvCADCmd.exe")),
    ]
    for candidate in candidates:
        if candidate.is_file():
            return str(candidate)

    return shutil.which("CivilCvCADCmd")
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    common.add_argument(
        "--civilcvcadcmd",
        default=_default_civilcvcadcmd(),
        help=(
            "Path to CivilCvCADCmd (default: $CIVILCVCADCMD or auto-detect under build/*/bin/CivilCvCADCmd or PATH)"
        ),
    )
    common.add_argument(
        "--qt-platform",
    ...
    common.add_argument(
        "--out-dir",
        default=os.path.join("/tmp", "CivilCvCADTesting", "CoinNodeSnapshots"),
...
def main(argv: list[str]) -> int:
    ...
    if not args.civilcvcadcmd:
        print(
            "ERROR: could not auto-detect CivilCvCADCmd; pass --civilcvcadcmd "
            "build/<preset>/bin/CivilCvCADCmd (or set $CIVILCVCADCMD)",
            file=sys.stderr,
        )
        return 2

    civilcvcadcmd = Path(args.civilcvcadcmd)
    if not civilcvcadcmd.is_file():
        print(f"ERROR: CivilCvCADCmd not found: {civilcvcadcmd}", file=sys.stderr)
        return 2
    ...
    if args.mode == "update":
        ...
        cmd = [str(civilcvcadcmd), "-t", "TestCoinNodeSnapshots"]
    else:
