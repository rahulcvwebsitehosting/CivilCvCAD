...
macro(SetupPivy)
    ...
    IF (CIVILCVCAD_CHECK_PIVY) # do not make pivy a host dependency for cross compiling
        IF (NOT PIVY_VERSION)
...
macro(SetupBundledCoinPivy)
    ...
    if (EXPAT_FOUND)
        if (DEFINED EXPAT_VERSION_STRING AND NOT EXPAT_VERSION_STRING STREQUAL "")
            set(CIVILCVCAD_COIN_EXPAT_SOURCE "external (${EXPAT_VERSION_STRING})")
            set(_civilcvcad_coin_expat_message "Bundled Coin will use system Expat ${EXPAT_VERSION_STRING}")
        else ()
            set(CIVILCVCAD_COIN_EXPAT_SOURCE "external")
            set(_civilcvcad_coin_expat_message "Bundled Coin will use system Expat")
        endif ()
        message(STATUS "${_civilcvcad_coin_expat_message}")
    else ()
        set(CIVILCVCAD_COIN_EXPAT_SOURCE "bundled fallback")
        message(WARNING
    ...
    # Match external Coin usage: CivilCvCAD targets should treat Coin headers as third-party headers.
    ...
    set(PIVY_PACKAGE_INSTALL_DIR "Mod/pivy"
        CACHE STRING "Install directory for bundled Pivy inside CivilCvCAD packages" FORCE)
    if (APPLE)
        set(PIVY_MODULE_INSTALL_RPATH "@loader_path/../../${CMAKE_INSTALL_LIBDIR}"
            CACHE STRING "RUNPATH for bundled Pivy extension modules in CivilCvCAD packages" FORCE)
    elseif (UNIX)
...
macro(SetupCoinPivy)
    if (CIVILCVCAD_USE_EXTERNAL_COIN_PIVY)
        set(CIVILCVCAD_COIN3D_SOURCE "external")
        set(CIVILCVCAD_PIVY_SOURCE "external")
        SetupCoin3D()
        SetupPivy()
    else ()
        set(CIVILCVCAD_COIN3D_SOURCE "bundled")
        set(CIVILCVCAD_PIVY_SOURCE "bundled")
