...
# -------------------------------- Shiboken/PySide ------------------------
    ...
    option(CIVILCVCAD_USE_SHIBOKEN "Links to the shiboken library at build time. If OFF its Python module is imported at runtime" ON)
    option(CIVILCVCAD_USE_PYSIDE "Links to the PySide libraries at build time." ON)
    option(CIVILCVCAD_BINARY_PYTHON_RC_FILES "Put Qt resource file binaries outside of their corresponding Python files." ON)
    mark_as_advanced(CIVILCVCAD_BINARY_PYTHON_RC_FILES)
    ...
    if(CIVILCVCAD_QT_MAJOR_VERSION EQUAL 5)
        ...
        set(SHIBOKEN_MAJOR_VERSION ${CIVILCVCAD_QT_MAJOR_VERSION})
        set(PYSIDE_MAJOR_VERSION ${CIVILCVCAD_QT_MAJOR_VERSION})
    endif()
    ...
    if(NOT SHIBOKEN_INCLUDE_DIR)
        message(WARNING "Shiboken${PYSIDE_MAJOR_VERSION} include files not found, CIVILCVCAD_USE_SHIBOKEN automatically set to OFF")
        set(CIVILCVCAD_USE_SHIBOKEN OFF)
    endif()
    ...
    if(NOT PYSIDE_INCLUDE_DIR)
        message(WARNING "PySide${PYSIDE_MAJOR_VERSION} include files not found, CIVILCVCAD_USE_PYSIDE automatically set to OFF")
        set(CIVILCVCAD_USE_PYSIDE OFF)
    endif()
...
function(PYSIDE_WRAP_RC)
    ...
    foreach(it ${arg_RESOURCES})
        ...
        if(CIVILCVCAD_BINARY_PYTHON_RC_FILES)
            ...
            add_custom_command(OUTPUT "${rcpy}" "${rccbin}"
                ...
                DEPENDS "${PYSIDE_RCC_EXECUTABLE}"
                        "${CMAKE_BINARY_DIR}/cMake/CivilCvCAD_Helpers/PysideQtRccGen.cmake"
