macro(PrintFinalReport)
    ...
    value(CIVILCVCAD_USE_FREETYPE)
    value(CIVILCVCAD_USE_EXTERNAL_SMESH)
    value(CIVILCVCAD_USE_SANITIZER_ASAN)
    value(CIVILCVCAD_USE_SANITIZER_LSAN)
    value(CIVILCVCAD_USE_SANITIZER_TSAN)
    value(CIVILCVCAD_USE_SANITIZER_UBSAN)
    value(CIVILCVCAD_USE_SANITIZER_MSAN)
    value(BUILD_SMESH)
    value(BUILD_VR)
    value(CMAKE_PREFIX_PATH)
    value(CIVILCVCAD_QT_VERSION)
    value(Python3_EXECUTABLE)
    value(PYTHON_LIBRARY)
    value(CIVILCVCAD_CREATE_MAC_APP)
    value(CIVILCVCAD_USE_EXTERNAL_KDL)
    value(CIVILCVCAD_USE_PYSIDE)
    value(CIVILCVCAD_USE_SHIBOKEN)
    ...
    if(OCC_VERSION_DEVELOPMENT)
        simple(OCC "${OCC_VERSION_STRING} (${OCC_VERSION_DEVELOPMENT}) [${OCC_LIBRARY_DIR}] [${OCC_INCLUDE_DIR}]")
    else()
    ...
    if (BUILD_GUI)
        conditional(DesignerPlugin BUILD_DESIGNER_PLUGIN
                    ...
                    "[${DESIGNER_PLUGIN_LOCATION}/${libCivilCvCAD_widgets}]"
        )
    endif()
    ...
    simple(Coin3D_Source ${CIVILCVCAD_COIN3D_SOURCE})
    if(DEFINED CIVILCVCAD_COIN_EXPAT_SOURCE)
        simple(Coin3D_Expat ${CIVILCVCAD_COIN_EXPAT_SOURCE})
    endif()
    simple(pivy ${PIVY_VERSION})
    simple(pivy_Source ${CIVILCVCAD_PIVY_SOURCE})
    if(WIN32 OR APPLE)
        if(CIVILCVCAD_USE_3DCONNEXION_LEGACY AND CIVILCVCAD_USE_3DCONNEXION_NAVLIB)
            simple(3Dconnexion "Building with 3Dconnexion legacy and NavLib support")
        elseif(CIVILCVCAD_USE_3DCONNEXION_LEGACY)
            simple(3Dconnexion "Building with 3Dconnexion legacy support")
        elseif(CIVILCVCAD_USE_3DCONNEXION_NAVLIB)
