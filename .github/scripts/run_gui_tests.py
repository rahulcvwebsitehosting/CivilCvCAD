...
List registered tests via `CivilCvCAD -t`, filter for GUI tests (names containing 'Gui'), and run each
GUI test module using the specified CivilCvCAD executable.

Usage:
  run_gui_tests.py [CIVILCVCAD_EXEC]

If CIVILCVCAD_EXEC is omitted the script falls back to 'CivilCvCAD' on PATH.
If CIVILCVCAD_EXEC is a directory containing bin/CivilCvCAD, that binary is used.
If CIVILCVCAD_EXEC is an executable path, it is used directly.
...
def find_executable(arg: str | None) -> str:
    """Return the CivilCvCAD executable path to use.

    If `arg` is None or empty, returns the plain name 'CivilCvCAD' which will be looked up on PATH. If
    `arg` is a directory and contains `bin/CivilCvCAD` that binary will be returned. If `arg` is a file
    path it is returned as-is. Otherwise the original argument is returned.

    Common use cases: use the CivilCvCAD binary from a build directory or an installed CivilCvCAD prefix.
    """
    if not arg:
        return "CivilCvCAD"
    p = Path(arg)
    if p.is_dir():
        candidate = p / "bin" / "CivilCvCAD"
        if candidate.exists():
...
def validate_executable(path: str) -> tuple[bool, str]:
    ...
    This is best effort: if a bare name is given (e.g. 'CivilCvCAD') we can't stat it here, so we
    accept it but warn. If the path points to a file, we check executability. Also warn if the name
    looks like the CLI-only 'CivilCvCADCmd'.
    """
    p = Path(path)
    if p.is_file():
        if not os.access(str(p), os.X_OK):
            return False, f"File exists but is not executable: {path}"
        if p.name.endswith("CivilCvCADCmd"):
            return (
                True,
                (
                    "Warning: executable looks like 'CivilCvCADCmd' (CLI); GUI tests require the GUI "
                    "binary 'CivilCvCAD'."
                ),
            )
        return True, ""
    ...
    if p.name.endswith("CivilCvCADCmd"):
        return (
            True,
            (
                "Warning: executable name looks like 'CivilCvCADCmd' (CLI); GUI tests require the GUI "
                "binary 'CivilCvCAD'."
            ),
        )
    return True, ""
...
def parse_registered_tests(output: str) -> list[str]:
    """Parse output from `CivilCvCAD -t` and return a list of registered test unit names.
...
def main(argv: list[str]) -> int:
    """Entry point: run GUI test modules registered in the CivilCvCAD executable.

    Returns the last non-zero exit code from any GUI test module, or 0 on success.
    """
    exec_arg = argv[1] if len(argv) > 1 else None
    civilcvcad_exec = find_executable(exec_arg)

    print(f"Using CivilCvCAD executable: {civilcvcad_exec}")

    ok, msg = validate_executable(civilcvcad_exec)
    if msg:
        print(msg, file=sys.stderr)
    if not ok:
        print(f"Aborting: invalid CivilCvCAD executable: {civilcvcad_exec}", file=sys.stderr)
        return 3

    code, out = run_and_capture([civilcvcad_exec, "-t"])
    if code != 0:
        print(
    ...
    for mod in gui_tests:
        ...
        rc, out = run_and_capture([civilcvcad_exec, "-t", mod])
        print(out)
        if rc != 0:
